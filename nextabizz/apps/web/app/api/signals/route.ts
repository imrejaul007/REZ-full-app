...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error('Unexpected error in signals API:', err);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Unexpected error in signals POST:', err);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('Supabase error updating signals:', error);
      return NextResponse.json(
    ...
    console.error('Unexpected error in signals PATCH:', err);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );

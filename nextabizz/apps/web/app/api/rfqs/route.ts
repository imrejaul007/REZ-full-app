...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (error) {
      console.error('Supabase error fetching RFQs:', error);
      return NextResponse.json(
    ...
    console.error('Unexpected error in RFQs API:', err);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (rfqError) {
      console.error('Supabase error creating RFQ:', rfqError);
      return NextResponse.json(
    ...
    if (itemsError) {
      ...
      console.error('Supabase error inserting RFQ items:', itemsError);
      return NextResponse.json(
    ...
    console.error('Unexpected error in RFQs POST:', err);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );

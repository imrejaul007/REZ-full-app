...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (error) {
      console.error('Supabase error fetching catalog:', error);
    ...
    console.error('Unexpected error in catalog API:', err);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Unexpected error in catalog POST:', err);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );

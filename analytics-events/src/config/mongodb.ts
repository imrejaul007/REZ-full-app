...
export async function connectMongoDB(): Promise<void> {
  ...
  await mongoose.connect(uri, {
    ...
    // Route reads to secondary replica — keeps OLTP primary free for payment writes.
    // Falls back to primary if no secondary is available (safe on Atlas M0/M2).
    readPreference: 'secondaryPreferred',
    // Writes still go to primary with majority acknowledgement.
    w: 'majority',

...
export default function MallScreen() {
  ...
  const openCategoryForm = (category?: MallCategory) => {
    if (category) { setEditingCategory(category); setCategoryForm({ name: category.name, slug: category.slug, description: (category as any).description || '', icon: category.icon || '', image: (category as any).image || '', color: category.color || colors.navy, backgroundColor: (category as any).backgroundColor || '', maxCashback: category.maxCashback?.toString() || '0', sortOrder: category.sortOrder?.toString() || '0', isActive: category.isActive, isFeatured: category.isFeatured || false }); }
  ...
  const openOfferForm = (offer?: MallOffer) => {
    if (offer) { setEditingOffer(offer); const brandId = typeof offer.brand === 'object' ? offer.brand?._id : offer.brand; setOfferForm({ title: offer.title, subtitle: offer.subtitle || '', description: (offer as any).description || '', image: offer.image || '', store: offer.store || '', brand: brandId || '', offerType: offer.offerType, value: offer.value?.toString() || '0', valueType: offer.valueType, minPurchase: (offer as any).minPurchase?.toString() || '', maxDiscount: (offer as any).maxDiscount?.toString() || '', validFrom: offer.validFrom?.split('T')[0] || '', validUntil: offer.validUntil?.split('T')[0] || '', badge: offer.badge || '', isActive: offer.isActive, isMallExclusive: offer.isMallExclusive }); }
  ...
  const toggleOfferActive = async (offer: MallOffer) => { try { setProcessingOffer(offer._id); await mallService.updateOffer(offer._id, { isActive: !offer.isActive } as any); loadOffers(); } catch (error: any) { showAlert('Error', 'Failed to update offer'); } finally { setProcessingOffer(null); } };
  ...
  const toggleBannerActive = async (banner: MallBanner) => { try { setProcessingBanner(banner._id); await mallService.updateBanner(banner._id, { isActive: !banner.isActive } as any); loadBanners(); } catch (error: any) { showAlert('Error', 'Failed to update banner'); } finally { setProcessingBanner(null); } };
  ...
  const toggleCollectionActive = async (collection: MallCollection) => { try { setProcessingCollection(collection._id); await mallService.updateCollection(collection._id, { isActive: !collection.isActive } as any); loadCollections(); } catch (error: any) { showAlert('Error', 'Failed to update collection'); } finally { setProcessingCollection(null); } };

...
import type { AdminChallenge, ChallengeStatus, ChallengeVisibility } from '../../services/api/challenges';
...
import { s } from './challenges.styles';
...
export default function ChallengesScreen() {
  ...
  const challenges: AdminChallenge[] = (listData as any)?.challenges ?? [];
  ...
  const renderStatsRow = () => (
    <View style={s.statsRow}>
      {[
        { label: 'Total', value: (stats as any)?.total ?? 0, color: colors.text },
        { label: 'Active', value: (stats as any)?.active ?? 0, color: colors.success },
        { label: 'Participants', value: (analytics as any)?.totalParticipants ?? 0, color: colors.purple },
        { label: 'Completion', value: `${(analytics as any)?.avgCompletionRate ?? (stats as any)?.avgCompletionRate ?? 0}%`, color: colors.info },
        { label: 'Coin Liability', value: (analytics as any)?.totalCoinLiability ?? 0, color: colors.warning },
      ].map((item, idx) => (
  ...
  const renderConversionFunnel = () => {
    const funnel = (analytics as any)?.conversionFunnel;
    ...
    return (
      <View style={[s.funnelContainer, { backgroundColor: colors.card }]}>
        ...
        {steps.map((step, idx) => {
          ...
          const convRate = idx > 0 && prevUsers > 0 ? `${Math.round((data.uniqueUsers / prevUsers) * 100)}%` : '';
          return (
            <View key={step} style={s.funnelStep}>
              <View style={s.funnelLabelRow}>
                ...
                <Text style={[s.funnelStepCount, { color: colors.icon }]}>{data.uniqueUsers} users {convRate ? `(${convRate})` : ''}</Text>
              </View>

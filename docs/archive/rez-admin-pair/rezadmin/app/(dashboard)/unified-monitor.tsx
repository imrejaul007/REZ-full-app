...
export default function UnifiedMonitorScreen() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const metricsPayload = (metricsRes.status === 'fulfilled' ? (metricsRes.value as any)?.data : null) as any;
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const aggPayload = (aggRes.status === 'fulfilled' ? (aggRes.value as any)?.data : null) as any;
      ...
      setData({
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        sla: slaRes.status === 'fulfilled' ? (slaRes.value as any)?.data : null,
        businessMetrics:
          metricsSummary || metricsHealth ? { summary: metricsSummary, health: metricsHealth } : null,
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        merchantStatus: merchantRes.status === 'fulfilled' ? (merchantRes.value as any)?.data?.summary : null,
        aggregator: aggPayload
          ...
          : null,
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        bbps: bbpsRes.status === 'fulfilled' ? (bbpsRes.value as any)?.data : null,
        // eslint-disable-next-line @typescript-eslint/no-explicit-any

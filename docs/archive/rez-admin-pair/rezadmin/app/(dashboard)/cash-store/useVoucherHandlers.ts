...
export const DEFAULT_VOUCHER_FORM: VoucherFormState = {
  title: '', code: '', discount: '', expiry: '', isActive: true,
};
...
export function useVoucherHandlers(initialForm?: Partial<VoucherFormState>) {
  ...
  const deleteVoucher = async (_id: string) => { showAlert('Deleted', 'Voucher deleted'); };
  const toggleActive = async (_id: string) => {};
  const openForm = (item?: any) => { setForm(item ? { ...item } : { ...DEFAULT_VOUCHER_FORM }); };

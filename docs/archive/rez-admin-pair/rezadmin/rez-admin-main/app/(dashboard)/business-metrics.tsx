    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (normalizedData?.summary) {
          setMetrics({ success: true, data: normalizedData });
        } else {
          setError('Unexpected response format');
        }
      } else {
        const msg = (response as any).message || 'Failed to load metrics';
        setError(msg);
        if (!isBackground) {
          showAlert('Error', msg);
        }
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        <Text style={[styles.title, { color: colors.text }]}>Business Metrics</Text>
      ...
      <View style={styles.section}>
        <Text style={[styles.sectionTitle, { color: colors.icon }]}>TODAY PERFORMANCE</Text>
      ...
      <View style={styles.section}>
        <Text style={[styles.sectionTitle, { color: colors.icon }]}>PAYMENT HEALTH</Text>
        <HealthBadge
          label="Payment Success Rate"
          value={paymentSuccessRateStr === 'N/A' ? 'N/A' : `${paymentSuccessRateStr}%`}
          status={paymentStatus}
        />
        <Text style={[styles.healthDetail, { color: colors.icon }]}>
      ...
      <View style={styles.section}>
        <Text style={[styles.sectionTitle, { color: colors.icon }]}>COIN ECONOMICS</Text>
        <HealthBadge
      ...
      <View style={styles.section}>
        <Text style={[styles.sectionTitle, { color: colors.icon }]}>7-DAY TRENDS</Text>
        <Text style={[styles.chartLabel, { color: colors.text }]}>Payment Success</Text>
      ...
      {/* Orders Trend */}
      <View style={styles.section}>
        <Text style={[styles.chartLabel, { color: colors.text }]}>Orders Placed</Text>
        {events?.['order.placed'] && (
      ...
      {/* Booking Trend */}
      <View style={styles.section}>
        <Text style={[styles.chartLabel, { color: colors.text }]}>Bookings Created</Text>
      ...
      {/* New Users Trend */}
      <View style={styles.section}>
        <Text style={[styles.chartLabel, { color: colors.text }]}>New User Signups</Text>
        {events?.['user.signup'] && (
          <SimpleLineChart
      ...
      {/* Coin Economy Trend */}
      <View style={styles.section}>
        <Text style={[styles.chartLabel, { color: colors.text }]}>Coins: Earned vs Redeemed</Text>

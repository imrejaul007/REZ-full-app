    ...
    <View style={{ flex: 1 }}>
      <View style={[styles.subTabBar, { borderBottomColor: colors.border }]}>
        <TouchableOpacity
          style={[
            styles.subTab,
            subTab === 'wallet' && { borderBottomColor: colors.tint, borderBottomWidth: 2 },
          ]}
          onPress={() => setSubTab('wallet')}
        >
          <Text
            style={[styles.subTabText, { color: subTab === 'wallet' ? colors.tint : colors.icon }]}
          >
            Wallet Ops
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[
            styles.subTab,
            subTab === 'campaigns' && { borderBottomColor: colors.tint, borderBottomWidth: 2 },
          ]}
        ...
        >
          <Text
            style={[
              styles.subTabText,
              { color: subTab === 'campaigns' ? colors.tint : colors.icon },
            ]}
          >
            Campaigns
          </Text>
      ...
      {subTab === 'wallet' ? (
        <WalletOpsSection colors={colors} />
      ) : (
        <CampaignsSection colors={colors} />
      )}
    </View>
  );
}
...
function WalletOpsSection({ colors }: { colors: any }) {
  ...
  useEffect(() => {
    adminActionsService.getApprovalThreshold().then(setApprovalThreshold).catch(() => {});
  }, []);
  ...
  const handleAdjust = async () => {
    // MED-8 FIX: Provide user feedback when required fields are empty instead of silently returning
    if (!adjustUser || !adjustAmount || !adjustReason.trim()) {
    ...
    try {
      ...
      // MED-9 FIX: Check for status code 202 (Pending Approval) instead of relying on message string matching
  ...
  const handleReverseCashback = async () => {
    ...
    const confirmed = await showConfirm(
      'Reverse Cashback',
      `Reverse ${amt} NC from ${reverseUser.user.fullName || reverseUser.user.phoneNumber}?`
    );
    if (!confirmed) return;
        ...
        <View style={styles.overlay}>
          <View style={[styles.modal, { backgroundColor: colors.card }]}>
            ...
            <Text style={[styles.helperText, { color: colors.icon }]}>
              Amounts above {approvalThreshold.toLocaleString()} NC require another admin to approve.
            </Text>
            <TextInput
              style={[
                styles.input,
  ...
  const loadCampaigns = useCallback(
    async (pageNum: number = 1, append: boolean = false) => {
      ...
      try {
        ...
        setPage(pageNum);
        setHasMore(
          pageNum < ((result.pagination as any)?.totalPages || result.pagination?.pages || 1)
        );
      } catch (err: any) {
        showAlert('Error', err.message, 'error');
      } finally {
          ...
          {item.reward?.totalBudget > 0 && (
            <View style={styles.budgetRow}>
              ...
              <Text style={[styles.budgetText, { color: colors.icon }]}>
                ...
                {(item.reward?.totalBudget || 0).toFixed(0)} NC
              </Text>
            </View>
          )}
          {canToggle && (
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          <View style={{ marginTop: 8 }}>
            <Text style={[styles.userName, { color: colors.text }]}>
              ...
              {item.payload.amount?.toFixed(2) || '0'} NC
            </Text>
            ...
            {item.payload.userId && (
              <Text style={[styles.userPhone, { color: colors.icon }]}>
                Target user: {item.payload.userId}
              </Text>
            )}
            <Text style={[styles.auditDesc, { color: colors.text, marginTop: 4 }]}>
              {item.reason}
            </Text>
          </View>
  ...
  return (
    <View style={{ flex: 1 }}>
      <FlatList
        ...
        contentContainerStyle={{ padding: 12, paddingBottom: 120 }}
        onEndReached={() => {
          if (!isLoading && hasMore) loadPending(page + 1, true);
        }}
        onEndReachedThreshold={0.3}
        refreshControl={
          <RefreshControl
        ...
        <View style={styles.overlay}>
          <View style={[styles.modal, { backgroundColor: colors.card }]}>
            ...
            <Text style={[styles.modalSub, { color: colors.icon }]}>
              ...
              {rejectTarget?.payload?.amount?.toFixed(2) || '0'} NC
            </Text>
            <TextInput
              style={[
                styles.input,
...
function ActionHistoryTab({ colors }: { colors: any }) {
      ...
      return (
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          <View style={{ marginTop: 8 }}>
            ...
            <Text style={[styles.userPhone, { color: colors.icon, marginTop: 2 }]}>
              By: {initiatorName}
            </Text>
            {approverName && (
              <Text style={[styles.userPhone, { color: colors.icon }]}>
            ...
            <Text
              style={[styles.auditDesc, { color: colors.text, marginTop: 4 }]}
              numberOfLines={2}
            >
              {item.reason}
            </Text>
            {item.rejectionReason && (
              <Text
                style={[styles.auditDesc, { color: colors.error, marginTop: 2 }]}
                numberOfLines={2}
              >
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      >
        {(['all', 'pending_approval', 'executed', 'approved', 'rejected'] as HistoryFilter[]).map(
          (f) => (
            <TouchableOpacity
              key={f}
              style={[styles.filterPill, filter === f && { backgroundColor: colors.tint }]}
            ...
            >
              <Text
                style={[styles.filterText, { color: filter === f ? colors.card : colors.text }]}
              >
                {f === 'all'
                  ? 'All'
      ...
      <FlatList
        ...
        contentContainerStyle={{ padding: 12, paddingBottom: 120 }}
        onEndReached={() => {
          if (!isLoading && hasMore) loadHistory(page + 1, true);
        }}
        onEndReachedThreshold={0.3}
        ListFooterComponent={
          isLoading ? (

...
function RevenueScreenContent() {
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      showAlert('Export Failed', 'Unable to export revenue data.', 'error');
    } finally {
      setExporting(false);
    }
  }, [days]);
  ...
  if (loading && !refreshing) {
    return (
      <View style={[styles.loadingContainer, { backgroundColor: colors.background }]}>
        ...
        <Text style={[styles.loadingText, { color: colors.icon }]}>Loading revenue data...</Text>
      </View>
    );
  }
    ...
    >
      ...
      <View style={[styles.header, { backgroundColor: colors.navy }]}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.headerSub}>Platform financial overview</Text>
        </View>
        <TouchableOpacity
      ...
      <View style={[styles.rangeRow, { backgroundColor: colors.card }]}>
        {(['today', '7d', '30d'] as DateRange[]).map((r) => (
          <TouchableOpacity
            key={r}
            style={[styles.rangeBtn, dateRange === r && { backgroundColor: colors.navy }]}
            onPress={() => setDateRange(r)}
          >
            <Text style={[styles.rangeBtnText, { color: dateRange === r ? '#fff' : colors.icon }]}>
          ...
          {revenueData.length > 0 && (
            <View style={styles.revenueStats}>
              <View style={styles.revenueStat}>
                ...
                <Text style={[styles.revenueStatLabel, { color: colors.icon }]}>Total Revenue</Text>
              </View>
              <View style={[styles.revenueStatDivider, { backgroundColor: colors.border }]} />
              <View style={styles.revenueStat}>
                <Text style={[styles.revenueStatValue, { color: colors.text }]}>
                  {formatNumber(revenueData.reduce((s, d) => s + d.transactions, 0))}
                </Text>
                <Text style={[styles.revenueStatLabel, { color: colors.icon }]}>Transactions</Text>
              </View>
              <View style={[styles.revenueStatDivider, { backgroundColor: colors.border }]} />
              <View style={styles.revenueStat}>
                <Text style={[styles.revenueStatValue, { color: colors.text }]}>
                ...
                <Text style={[styles.revenueStatLabel, { color: colors.icon }]}>Daily Avg</Text>
              </View>
            </View>
          )}
        </View>
      ...
      <View style={styles.section}>
        ...
        <View style={[styles.card, { backgroundColor: colors.card }]}>
          ...
          <View style={[styles.tableHeader, { borderBottomColor: colors.border }]}>
            <Text style={[styles.tableHeaderCell, { flex: 0.5, color: colors.icon }]}>#</Text>
            <Text style={[styles.tableHeaderCell, { flex: 2.5, color: colors.icon }]}>Store</Text>
            <Text
              style={[
            ...
            <Text
              style={[styles.tableHeaderCell, { flex: 1, color: colors.icon, textAlign: 'right' }]}
            >
              Visits
            </Text>
          </View>
          {topMerchants.length === 0 ? (
            <View style={styles.emptyState}>
              ...
              <Text style={[styles.emptyText, { color: colors.icon }]}>No merchant data</Text>
            </View>
          ) : (
            topMerchants.map((m, idx) => (
              ...
              >
                <View style={[styles.rankCell, { flex: 0.5 }]}>
                  <Text style={[styles.rankText, { color: colors.tint }]}>{idx + 1}</Text>
                </View>
                <Text
                  style={[styles.tableCell, { flex: 2.5, color: colors.text }]}
                  numberOfLines={1}
                >
                  {m.storeName}
                </Text>
                ...
                <Text
                  style={[styles.tableCell, { flex: 1, color: colors.icon, textAlign: 'right' }]}
                >
        ...
        <View style={[styles.card, { backgroundColor: colors.card }]}>
          {tiers ? (
            <>
              ...
              <View style={[styles.tierTotal, { borderTopColor: colors.border }]}>
                <Text style={[styles.tierTotalText, { color: colors.icon }]}>
                  Total tiered users:{' '}
                  <Text style={[styles.tierTotalValue, { color: colors.text }]}>
            ...
            <View style={styles.emptyState}>
              ...
              <Text style={[styles.emptyText, { color: colors.icon }]}>No tier data</Text>
            </View>
          )}
        </View>
      </View>

...
export default function SupportTicketsScreen() {
  ...
  useEffect(() => {
    ...
    const handleMessageReceived = (data: any) => {
      if (__DEV__) {
        logger.info('[AdminSupport] Message received event: ' + data?.ticketId + ' selected: ' + selectedTicket?._id + ' match: ' + (data?.ticketId === selectedTicket?._id));
      }
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        Linking.openURL(url).catch((err) => {
                          logger.error('Failed to open attachment URL:', err);
                        })
                      }
                    >
                      <Image
                        ...
                        {msg.attachments.map((url, attIdx) => (
                          <TouchableOpacity
                            ...
                            onPress={() =>
                              Linking.openURL(url).catch((err) => {
                                if (__DEV__) {
                                  logger.error('Failed to open message attachment URL:', err);
                                }
                              })
                            }
                          >

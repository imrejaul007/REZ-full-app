    ...
    return (
      ...
      >
        <View style={styles.emptyContainer}>
          <Text style={styles.emptyTitle}>No cohort data yet</Text>
  ...
  return (
    ...
    >
      <View style={styles.infoCard}>
        <Text style={styles.infoTitle}>User Retention Cohorts</Text>
      ...
      <ScrollView horizontal style={styles.tableScroll}>
        <View style={styles.table}>
          ...
          <View style={styles.tableRow}>
            <Text style={[styles.cell, styles.headerCell]}>Cohort</Text>
            <Text style={[styles.cell, styles.headerCell]}>Users</Text>
            {weeks.map((w) => (
              <Text key={w} style={[styles.cell, styles.headerCell]}>
                {w}
              </Text>
            ))}
          </View>
          ...
          {cohorts.map((cohort: any, i: number) => (
            <View key={i} style={styles.tableRow}>
              ...
              <Text style={[styles.cell, styles.usersCell]}>
                {(cohort.size ?? cohort.users ?? 0).toLocaleString()}
              </Text>
              {(cohort.retention || []).map((pct: number, j: number) => (
                <View
                  key={j}
                  style={[styles.retentionCell, { backgroundColor: getRetentionColor(pct) + '30' }]}
                >
                  <Text style={[styles.retentionText, { color: getRetentionColor(pct) }]}>
                    {pct}%
                  </Text>
                </View>
              ))}
              ...
              {Array(Math.max(0, weeks.length - (cohort.retention?.length || 0)))
                .fill(null)
                .map((_, j) => (
                  <View key={`empty-${j}`} style={styles.retentionCell}>
                    <Text style={styles.retentionEmpty}>-</Text>
                  </View>
                ))}
            </View>
          ))}
      ...
      <View style={styles.insightRow}>
        {[
          ...
          <View key={m.label} style={styles.insightCard}>
            <Text style={[styles.insightValue, { color: m.color }]}>{m.value}</Text>
            <Text style={styles.insightLabel}>{m.label}</Text>
          </View>
        ))}
      </View>

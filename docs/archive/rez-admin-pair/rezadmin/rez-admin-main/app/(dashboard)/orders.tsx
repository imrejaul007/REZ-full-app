  ...
  const handleReasonConfirm = async () => {
    ...
    try {
      ...
      if (reasonAction === 'refund') {
        await ordersService.refundOrder(reasonOrderId, refundAmount, reasonText);
          ...
          <View style={styles.orderHeader}>
            <View>
              ...
              <Text style={[styles.storeName, { color: colors.icon }]}>
                {item.store?.name || (item.items?.[0] as any)?.store?.name || 'Unknown Store'}
              </Text>
              {item.items?.length > 0 && (
                ...
                <View style={[styles.totalSection, { borderTopColor: colors.border }]}>
                  ...
                  <View style={styles.totalRow}>
                    ...
                    <Text style={[styles.totalValue, { color: colors.text }]}>
                      ...
                      {formatCurrency(
                        (selectedOrder.totals as any)?.delivery ??
                          (selectedOrder.totals as any)?.deliveryFee ??
                          0
                      )}
                    </Text>
                  </View>
              ...
              <View style={{ backgroundColor: colors.border + '20', padding: 12, borderRadius: 8, marginBottom: 16 }}>
                ...
                {(selectedOrder.user?.profile?.firstName || selectedOrder.user?.profile?.lastName) && (
                  <Text style={{ fontSize: 12, color: colors.text, marginBottom: 4 }}>
                    Customer: {[selectedOrder.user.profile.firstName, selectedOrder.user.profile.lastName].filter(Boolean).join(' ')}
                  </Text>
                )}

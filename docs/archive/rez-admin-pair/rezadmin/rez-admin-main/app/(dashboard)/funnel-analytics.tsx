...
export default function FunnelAnalyticsScreen() {
  ...
  return (
    ...
    >
      {/* Funnel selector */}
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.funnelSelector}>
        {FUNNELS.map((f) => (
          <TouchableOpacity
            key={f.id}
            style={[styles.funnelTab, funnel === f.id && styles.funnelTabActive]}
            onPress={() => {
              setFunnel(f.id);
              setLoading(true);
            }}
          >
            <Text style={[styles.funnelTabText, funnel === f.id && styles.funnelTabTextActive]}>
              {f.label}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      {loading ? (
        ...
        <View style={styles.emptyContainer}>
          <Text style={styles.emptyTitle}>No funnel data yet</Text>

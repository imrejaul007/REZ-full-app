...
function MetricCard({
  ...
  return (
    <View style={[styles.card, { backgroundColor: colors.card, borderColor: cfg.color + '33' }]}>
      ...
      {detail ? <Text style={[styles.cardDetail, { color: colors.text }]}>{detail}</Text> : null}

      {metric.reason ? (
        <Text style={[styles.cardReason, { color: colors.secondaryText }]}>
          Reason: {metric.reason}
        </Text>
      ) : null}

      <Text style={[styles.cardChecked, { color: colors.secondaryText }]}>
  ...
  return (
    ...
    >
      <View style={styles.breachHeader}>
        ...
        />
        <Text style={[styles.breachMetric, { color: isError ? '#991B1B' : '#92400E' }]}>
          {item.metric.replace(/_/g, ' ').toUpperCase()}
        </Text>
        <Text style={[styles.breachTime, { color: colors.secondaryText }]}>
          {fmt(item.timestamp)}
        </Text>
      </View>
      <Text style={[styles.breachValue, { color: colors.text }]}>{item.value}</Text>
    </View>
  );
}
...
export default function SlaMonitorScreen() {
  ...
  return (
    ...
    >
      ...
      >
        <View>
          <Text style={[styles.title, { color: colors.text }]}>SLA Monitor</Text>
      ...
      {data ? (
        ...
        >
          ...
          <View style={{ flex: 1, marginLeft: 10 }}>
            <Text style={[styles.bannerTitle, { color: overallCfg.color }]}>
              System Status: {overallCfg.label}
            </Text>
            <Text style={[styles.bannerSub, { color: overallCfg.color + 'CC' }]}>
              Updated {fmt(data.generatedAt)} · Poll every 30s
            </Text>
          </View>
          {/* Socket indicator */}
          <View
            style={[styles.socketDot, { backgroundColor: connected ? '#10B981' : '#9CA3AF' }]}
          />
          <Text style={[styles.socketLabel, { color: colors.secondaryText }]}>
      ...
      {data ? (
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: colors.secondaryText }]}>METRICS</Text>
      ...
      <View style={styles.section}>
        <View style={styles.sectionRow}>
          <Text style={[styles.sectionTitle, { color: colors.secondaryText }]}>BREACH ALERTS</Text>
          {breaches.length > 0 ? (
            <TouchableOpacity onPress={() => setBreaches([])}>
              <Text style={[styles.clearBtn, { color: colors.tint }]}>Clear</Text>
            </TouchableOpacity>
          ) : null}
        </View>
      ...
      {/* ── SLA Reference ── */}
      <View style={styles.section}>
        <Text style={[styles.sectionTitle, { color: colors.secondaryText }]}>SLA THRESHOLDS</Text>
        <View
        ...
        >
          {[
            ...
            <View key={row.label} style={[styles.refRow, { borderBottomColor: colors.border }]}>
              <Text style={[styles.refLabel, { color: colors.text }]}>{row.label}</Text>
              <View style={{ alignItems: 'flex-end' }}>
                <Text style={[styles.refBreachVal, { color: '#DC2626' }]}>
                  Breach: {row.threshold}
                </Text>
                <Text style={[styles.refWarnVal, { color: '#D97706' }]}>Warn: {row.warn}</Text>
              </View>
            </View>
          ))}
        </View>

...
export default function EventsScreen() {
  ...
  const getIsFeatured = (event: AdminEvent) => event.isFeatured ?? (event as any).featured ?? false;
  const getBookingCount = (event: AdminEvent) => event.bookingCount || (event as any).analytics?.bookings || 0;
  const getIsFree = (event: AdminEvent) => {
    if (typeof event.price === 'object' && event.price !== null) return (event.price as any).isFree ?? false;
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.tabsContent}>
        {TABS.map((tab) => {
          ...
          return (
            ...
            >
              <Ionicons name={tab.icon as any} size={16} color={isActive ? colors.card : colors.icon} />
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.filterChipsRow}>
        {FILTER_CHIPS.map((chip) => {
          ...
          return (
            ...
            >
              <Ionicons name={chip.icon as any} size={14} color={isActive ? colors.tint : colors.icon} />

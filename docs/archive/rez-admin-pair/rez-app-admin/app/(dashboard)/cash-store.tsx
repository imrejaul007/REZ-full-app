    ...
    async (pageNum = 1) => {
      try {
        ...
        if (res.success) {
          ...
          setTotalPages((res as any).meta?.pagination?.totalPages || 1);
          setPage(pageNum);
        }
      } catch {
        // silent
  ...
  const openVoucherForm = (voucher?: VoucherBrand) => {
    if (voucher) {
      ...
      setVoucherForm({
        ...
        maxCoinsPerOrder:
          (voucher as any).rezCoinReward?.maximumCoinsPerOrder?.toString() || '10000',
    ...
    if (coupon) {
      ...
      const ul =
        typeof coupon.usageLimit === 'object'
          ? (coupon.usageLimit as any)?.totalUsage
          : coupon.usageLimit;

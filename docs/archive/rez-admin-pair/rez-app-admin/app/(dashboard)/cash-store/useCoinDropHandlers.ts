...
export const DEFAULT_COINDROP_FORM: CoinDropFormState = {
  title: '', amount: '', distribution: 'random', startDate: '', endDate: '', isActive: true,
};
...
export function useCoinDropHandlers(initialForm?: Partial<CoinDropFormState>) {
  ...
  const deleteCoinDrop = async (_id: string) => { showAlert('Deleted', 'Coin Drop deleted'); };
  const toggleActive = async (_id: string) => {};
  const openForm = (item?: any) => { setForm(item ? { ...item } : { ...DEFAULT_COINDROP_FORM }); };

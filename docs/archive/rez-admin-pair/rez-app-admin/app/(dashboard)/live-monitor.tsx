...
import { s } from './live-monitor.styles';
import {
...
export default function LiveMonitorScreen() {
  ...
  const fraudAlertCount = (health as any)?.fraudAlerts?.alertCount ?? 0;
  const activeAlerts = fraudAlertCount + (totalFailed > 0 ? 1 : 0);
  const ordersToday = (health as any)?.stats?.orders?.today ?? 0;
  const gmvToday = (health as any)?.stats?.revenue?.today ?? 0;

  // BUG-019: Use actual success/failure counts
  const paymentSuccessCount = (health as any)?.payments?.successCount;
  const paymentFailureCount = (health as any)?.payments?.failureCount;
    ...
    <SafeAreaView style={[s.safeArea, { backgroundColor: NAVY }]} edges={['top']}>
      ...
      >
        ...
        <KpiRow
          ...
          gmvToday={gmvToday}
          activeUsers={(health as any)?.stats?.users?.active?.toLocaleString() ?? '—'}
        ...
        {/* Financial Health */}
        <FinancialHealthSection economics={(health as any) ?? null} coinRatio={(health as any)?.coinRatio ?? 'N/A'} />

        {/* Error Rate & Alerts */}
        <ErrorRateSection totalFailed={totalFailed} fraudAlertCount={fraudAlertCount} economics={(health as any) ?? null} />

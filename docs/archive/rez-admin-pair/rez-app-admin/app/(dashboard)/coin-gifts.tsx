...
import { s } from './coin-gifts.styles';
...
export default function CoinGiftsAdminScreen() {
  ...
  const gifts = useMemo(() => {
    ...
    for (const page of (pages as any[])) {
      if ((page as any)?.gifts) allGifts.push(...(page as any).gifts);
    }
    return allGifts;
  }, [pages]);

  const total = useMemo(() => {
    const last = (pages as any[])[(pages as any[]).length - 1];
    return (last as any)?.pagination?.total ?? 0;
  }, [pages]);

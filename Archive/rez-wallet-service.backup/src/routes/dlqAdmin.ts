/**
 * DLQ Admin API — rez-wallet-service
 *
 * D12: Parity with the monolith's admin/dlqAdmin endpoints for this
 * microservice's primary queue (wallet-events). Until this router existed
 * there was no HTTP way to inspect or replay failed jobs — required
 * Redis-shell access.
 *
...
router.get('/', async (_req: Request, res: Response) => {
  ...
  try {
    ...
    return res.json({ success: true, data: { queue: QUEUE_NAME, failed, waiting, active, completed } });
  } finally {
    await q.close();
  }
});
...
router.post('/jobs/:jobId/retry', async (req: Request, res: Response) => {
  ...
  const q = openQueue();
  try {
    const job = await Job.fromId(q, jobId as string);
    if (!job) {

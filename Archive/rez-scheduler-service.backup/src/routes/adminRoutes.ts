...
import { getDlqCounts } from '../queues/dlq';
...
router.get('/health', authMiddleware, async (_req: Request, res: Response): Promise<void> => {
  try {
    ...
    const queueHealth: Record<string, object> = {};
    ...
    // SCH-03: include DLQ depths so monitoring can alert on non-zero DLQ
    const dlqCounts = await getDlqCounts();
    const dlqTotal = Object.values(dlqCounts).reduce((sum, c) => sum + c.total, 0);

    res.json({
      success: true,
      data: {
        status: recentFailures > 10 || dlqTotal > 0 ? 'degraded' : 'healthy',
        queues: queueHealth,
        dlq: dlqCounts,
        dlqTotal,

...
const logger = (0, logger_1.createServiceLogger)('gamification-worker');
exports.QUEUE_NAME = 'gamification-events';
...
const toISTDate = (d) => new Date(d.getTime() + IST_OFFSET_MS);
const istDayStart = (d) => {
...
function getAchievementQueue() {
    if (!_achievementQueue) {
        _achievementQueue = new bullmq_1.Queue('achievement-events', { connection: redis_1.bullmqRedis });
    }
    return _achievementQueue;
}
async function enqueueCoinEarnedNotification(userId, coins, source, jobId) {
...
function startGamificationWorker() {
    ...
    _worker = new bullmq_1.Worker(exports.QUEUE_NAME, async (job) => {
        ...
        const progress = rawProgress !== null && typeof rawProgress === 'object' && !Array.isArray(rawProgress)
            ? rawProgress
            : {};
        const completedHandlers = Array.isArray(progress.completedHandlers)
            ? progress.completedHandlers
            : [];
        const markHandlerDone = async (handlerName) => {
        ...
        if (!completedHandlers.includes('achievement')) {
            try {
                ...
                if (affectedMetrics && affectedMetrics.length > 0) {
                    const UserAchievementProgress = mongoose_1.default.connection.collection('userachievementprogresses');
                    for (const metric of affectedMetrics) {
                        await UserAchievementProgress.updateMany({ userId: event.userId, [`metrics.${metric}`]: { $exists: true } }, { $inc: { [`metrics.${metric}.current`]: 1 } });
                    }
                }
        ...
        if (!completedHandlers.includes('challenge')) {
            try {
                ...
                if (action) {
                    const UserChallengeProgress = mongoose_1.default.connection.collection('userchallengeprogresses');
        ...
        if (!completedHandlers.includes('streak'))
            try {
                const streakType = EVENT_TO_STREAK[event.type];
                if (streakType) {
                    const UserStreak = mongoose_1.default.connection.collection('userstreaks');
                    ...
                    const preExisting = await UserStreak.findOneAndUpdate({ userId: event.userId, type: streakType }, {
                        $setOnInsert: {
                            userId: event.userId,
                            type: streakType,
                            currentStreak: 1,
                    ...
                    if (preExisting) {
                        ...
                        if (lastDate === today) {
                            ...
                            const newStreak = (preExisting.currentStreak ?? 0) + 1;
                            const newLongest = Math.max(newStreak, preExisting.longestStreak ?? 1);
                            await UserStreak.updateOne({ _id: preExisting._id }, {
                                $set: {
        ...
        if (!completedHandlers.includes('leaderboard')) {
            try {
                if (LEADERBOARD_EVENTS.has(event.type)) {
                    await redis_1.bullmqRedis.del('leaderboard:weekly');
                    await redis_1.bullmqRedis.del('leaderboard:monthly');
                    (0, leaderboardService_1.invalidateLeaderboardCache)(); // clears httpServer.ts in-memory cache
                }
        ...
        if (!completedHandlers.includes('mission')) {
            try {
                const UserMission = mongoose_1.default.connection.collection('usermissions');
        ...
        if (!completedHandlers.includes('coin_earned_notif')) {
            try {
                ...
                if (defaultCoins !== undefined) {
                    const coins = event.data?.coins || defaultCoins;
        ...
        (0, httpServer_1.recordJobProcessed)(event.type, Date.now() - jobStartMs);
    }, {
    ...
    httpServer_1.jobMetrics.activeWorkers++;
    _worker.on('failed', async (job, err) => {
        if (job)
            (0, httpServer_1.recordJobFailed)(job.data?.type ?? 'unknown');
        ...
        if (job && job.attemptsMade >= (job.opts?.attempts ?? 1)) {
            try {
                const dlqKey = `dlq:${exports.QUEUE_NAME}`;
                ...
                logger.info(`[DLQ] Job moved to dead-letter queue`, { queue: exports.QUEUE_NAME, jobId: job.id });
            }
            catch (dlqErr) {
                ...
                logger.error('[DLQ] Failed to write to dead-letter queue', { queue: exports.QUEUE_NAME, jobId: job?.id, dlqError: dlqErr?.message });
            }
        }
    });
...
async function stopWorker() {
    if (_worker) {
        httpServer_1.jobMetrics.activeWorkers = Math.max(0, httpServer_1.jobMetrics.activeWorkers - 1);
        await _worker.close();
        _worker = null;
    }
    if (_notifQueue) {

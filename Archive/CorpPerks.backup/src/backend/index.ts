...
router.post('/makcorps/webhook', async (req: Request, res: Response) => {
  try {
    // Verify webhook signature
    const signature = req.headers['x-makcorps-signature'] as string;
    const payload = JSON.stringify(req.body);
    const secret = process.env.MAKCORPS_WEBHOOK_SECRET;

    // SECURITY FIX: Enable webhook signature verification
    if (!secret) {
      logger.error('[Makcorps Webhook] MAKCORPS_WEBHOOK_SECRET not configured');
      return res.status(500).json({ success: false, message: 'Webhook not configured' });
    }

    if (!signature || !verifyWebhookSignature(signature, payload, secret)) {
      logger.warn('[Makcorps Webhook] Invalid signature');

...
const TrialOfferSchema = new Schema<ITrialOffer>(
  {
    ...
    category: { type: String, enum: ['service', 'sample_pickup', 'experience', 'd2c_kit'], required: true },
    coinPrice: { type: Number, required: true, min: 10, max: 200 },
    commitmentFee: { type: Number, enum: [9, 19, 29], required: true },
    originalPrice: { type: Number, required: true, min: 0 },
    slotConfig: {
      dailySlots: { type: Number, required: true, min: 1 },
      qrWindowMinutes: { type: Number, required: true, min: 5 },
      windowType: { type: String, enum: ['relative', 'fixed', 'auto'], required: true },
    },
    rewardConfig: {
      rezCoins: { type: Number, required: true, min: 0 },
      brandedCoins: { type: Number, required: true, min: 0 },
      brandedCoinLabel: { type: String, trim: true },
    },
    upsellLinks: [{ title: { type: String, trim: true }, url: { type: String, trim: true } }],
...
TrialOfferSchema.index({ merchantId: 1, status: 1 });

export const TrialOffer =
  mongoose.models.TrialOffer ||
  mongoose.model<ITrialOffer>('TrialOffer', TrialOfferSchema);

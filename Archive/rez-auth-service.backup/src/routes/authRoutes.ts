...
import { otpLimiter, otpSendPhoneLimiter, otpVerifyPhoneLimiter, authLimiter, adminLoginLimiter, hasPinLimiter, profileUpdateLimiter } from '../middleware/rateLimiter';
...
function parsePhone(body: any): { phone: string; countryCode: string } | null {
  ...
  try {
    ...
    if (mfaConfig) {
      ...
      return res.json({
        ...
        backupCodesAvailable: (mfaConfig as any).backupCodes?.filter((c: any) => !c.used).length || 0,
      });
    }
...
async function completeOnboardingHandler(req: Request, res: Response) {
  try {
    ...
    const updateFields: Record<string, any> = { 'auth.isOnboarded': true, updatedAt: new Date() };
...
router.post('/auth/admin/mfa/verify', adminMfaVerifyHandler);

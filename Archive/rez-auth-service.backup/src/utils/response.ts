...
export const errors = {
  ...
  authInsufficientPermissions: (opts?: any) => ({ message: opts?.message || 'Insufficient permissions', code: 'AUTH_006', ...opts }),
  ...
  authServiceUnavailable: (msg?: string | { message: string }) => ({
    message: typeof msg === 'string' ? msg : (msg as any)?.message || 'Auth service unavailable',
    code: 'SRV_003'
  }),
};

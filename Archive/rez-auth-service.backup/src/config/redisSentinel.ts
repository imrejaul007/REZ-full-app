...
export function createSentinelClient(name: string = 'mymaster'): Redis {
  ...
  redis.on('connect', () => {
    console.log(`[Redis Sentinel] Connected to master via Sentinel`);
  });
  ...
  redis.on('failover', () => {
    console.warn(`[Redis Sentinel] Failover detected - master changed`);
  });

  redis.on('sentinelDisconnect', () => {
    console.warn(`[Redis Sentinel] Disconnected from a sentinel`);
  });
...
export function getRedisClient(): Redis {
  ...
  if (useSentinel) {
    console.log('[Redis] Using Sentinel for high availability');

...
/**
 * Optional user auth - validates JWT if present but doesn't fail if missing.
 ...
 * Use for routes that work with or without authentication (e.g., hotel search).
 */
export function optionalAuth(req: AuthenticatedRequest, _res: Response, next: NextFunction): void {
  ...
  if (!token || token.length === 0) {
    // No token provided - continue without user context (this is expected for optional auth)
    next();
    return;
  }
  ...
  if (!secret) {
    // JWT not configured - continue without user context
    next();
    return;
  }
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (redis) {
      ...
      try {
        ...
        logger.error('[RateLimit] Redis error, falling through', { err });
        next();
      }
    } else {

...
router.post('/checkin', requireAuth, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // FIX: Removed admin override - users can only check in themselves
    // This prevents karma fraud via admin impersonation
...
router.post('/checkout', requireAuth, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // FIX: Removed admin override - users can only check out themselves
    // This prevents karma fraud via admin impersonation
    if (req.userId && req.userId !== userId) {

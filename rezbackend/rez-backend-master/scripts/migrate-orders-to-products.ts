...
async function migrateOrders() {
  try {
    ...
    const store1 =
      stores.find((s: any) => s.name === 'Mukul Test Business' || s._id.toString() === '691ffad7e84b098937ac0b65') ||
      stores[0];
    const store2 =
      stores.find((s: any) => s.name === 'hhhhhhhhhhhhhf' || s._id.toString() === '692016c8ad3a6bb2af9e5e48') ||
      stores[1];
    ...
    const updatedOrders = await Order.find({}).populate('items.product', 'name').populate('items.store', 'name').lean();

    const ordersWithProducts = updatedOrders.filter(
      (o: any) => o.items && o.items.some((item: any) => item.product && item.product !== null),
    ).length;

    const ordersWithStores = updatedOrders.filter(
      (o: any) => o.items && o.items.some((item: any) => item.store && item.store !== null),
    ).length;

  ...
  try {
    ...
    for (const offerTemplate of offerTemplates) {
      ...
      const offerData: any = {
        ...
        location: {
          type: 'Point',
          coordinates: storeData.location?.coordinates || [77.209, 28.6139],
        },
        store: {
          id: storeData._id,
          name: storeData.name,
    ...
    // Display all offers
    const allOffers = await Offer.find().select('title type cashbackPercentage category isActive store');

    console.log('\n📋 All Offers:');
    allOffers.forEach((offer, i) => {
      const storeName = (offer.store as any)?.name || 'N/A';
      console.log(
        `${i + 1}. ${offer.title} (${offer.type}) - ${offer.cashbackPercentage}% | Store: ${storeName} | Category: ${offer.category}`,
      );
    });

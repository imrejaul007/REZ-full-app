  ...
  try {
    ...
    for (let i = 0; i < referralCount; i++) {
      ...
      const referral = {
        ...
        referralCode:
          referrer.referral?.referralCode || `REF${(referrer._id as any).toString().slice(-6).toUpperCase()}`,
        status,
        tier: referrerTier,
        rewards: {
    ...
    sampleReferrals.forEach((referral, index) => {
      ...
      console.log(
        `      Referrer: ${referrer?.profile?.firstName || 'Unknown'} ${referrer?.profile?.lastName || ''} (${referrer?.referral?.referralCode})`,
      );
    ...
    const topReferrers = await User.find({ 'referral.totalReferrals': { $gt: 0 } })
      ...
      .select(
        'profile.firstName profile.lastName referral.referralCode referral.totalReferrals referral.referralEarnings referralTier',
      )
      .limit(5);

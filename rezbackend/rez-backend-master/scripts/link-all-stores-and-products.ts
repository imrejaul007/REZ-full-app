...
async function linkAllStoresToMerchant() {
  try {
    ...
    const merchantUser = await User.findOne({
      $or: [{ 'profile.email': TARGET_MERCHANT_EMAIL }, { email: TARGET_MERCHANT_EMAIL }],
    }).lean();
    ...
    const updateResult = await Store.updateMany({}, { $set: { merchantId: newMerchantId } });
    ...
    for (let i = 0; i < mProducts.length; i++) {
      ...
      try {
        ...
        if (exists) {
          ...
          if (i % 10 === 0) {
            console.log(
              `   Progress: ${i + 1}/${mProducts.length} (${skippedCount} skipped, ${migratedCount} migrated)`,
            );
          }
          continue;
        }
        ...
        // Extract price info — map legacy fields to canonical pricing (see @rez/shared-types):
        //   price/sellingPrice → pricing.selling
        //   originalPrice/compareAtPrice → pricing.original (mrp)

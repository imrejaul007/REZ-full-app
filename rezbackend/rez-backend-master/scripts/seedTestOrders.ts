...
async function seedTestOrders() {
  try {
    ...
    for (const template of orderTemplates) {
      ...
      await order.save();
      console.log(
        `✅ Created order: ${order.orderNumber} (${template.status}) - ${template.itemCount} items - ₹${orderData.pricing.total}`,
      );
      seededCount++;
    }
    ...
    allOrders.forEach((order, i) => {
      ...
      console.log(
        `${i + 1}. ${order.orderNumber} | ${current.toString().toUpperCase()} | ${itemCount} items | ₹${total} | ${order.createdAt.toLocaleDateString()}`,
      );
    });

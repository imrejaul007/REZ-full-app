...
async function seedCoinTransactions(users: any[], challenges: any[]) {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const sourceData = isEarning
      ? sources.filter((s) => s.type === 'earned')[
          Math.floor(Math.random() * sources.filter((s) => s.type === 'earned').length)
        ]
      : sources.filter((s) => s.type === 'spent')[
          Math.floor(Math.random() * sources.filter((s) => s.type === 'spent').length)
        ];
...
async function seedGamification() {
  try {
    ...
    if (sampleChallenge) {
      ...
      console.log(
        `   Completion Rate: ${Math.round((sampleChallenge.completionCount / sampleChallenge.participantCount) * 100)}%`,
      );
    }

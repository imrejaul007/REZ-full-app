...
async function checkExistingProducts() {
  ...
  console.log(
    `  Cashback configured:     ${withCashback}/${totalProducts} ${withCashback === totalProducts ? '✅' : '⚠️'}`,
  );
  console.log(
    `  Delivery info set:       ${withDeliveryInfo}/${totalProducts} ${withDeliveryInfo === totalProducts ? '✅' : '⚠️'}`,
  );
  console.log(
    `  Today analytics:         ${withTodayAnalytics}/${totalProducts} ${withTodayAnalytics === totalProducts ? '✅' : '⚠️'}`,
  );
  ...
  const needsUpdate =
    withCashback < totalProducts || withDeliveryInfo < totalProducts || withTodayAnalytics < totalProducts;
...
async function seedProductFields() {
  ...
  try {
    const products = await Product.find().populate('category', 'name').populate('store', 'name location');
    ...
    for (const product of products) {
      ...
      if (product.analytics && (!product.analytics.todayPurchases || !product.analytics.lastResetDate)) {
        ...
        hasUpdates = true;
        console.log(
          `${colors.magenta}  ✓ Added today's analytics (${product.analytics.todayPurchases} sales) to: ${product.name}${colors.reset}`,
        );
      }
      ...
      if (!product.bundleProducts || product.bundleProducts.length === 0) {
        ...
        if (Math.random() < 0.3) {
          ...
          if (relatedProducts.length > 0) {
            ...
            hasUpdates = true;
            console.log(
              `${colors.cyan}  ✓ Added ${relatedProducts.length} bundle products to: ${product.name}${colors.reset}`,
            );
          }
        }
      }
      ...
      if (!product.frequentlyBoughtWith || product.frequentlyBoughtWith.length === 0) {
        ...
        if (product.ratings?.average > 3.5) {
          ...
          if (frequentProducts.length > 0) {
            ...
            hasUpdates = true;
            console.log(
              `${colors.yellow}  ✓ Added ${frequentProducts.length} frequently bought items to: ${product.name}${colors.reset}`,
            );
          }
        }
      }
...
async function createSampleProducts() {
  ...
  if (productCount > 0) {
    console.log(
      `${colors.cyan}Database already has ${productCount} products. Skipping sample creation.${colors.reset}`,
    );
    return;
  }
...
async function verifyUpdates() {
  ...
  const samples = await Product.find()
    .limit(3)
    .select(
      'name cashback deliveryInfo analytics.todayPurchases analytics.todayViews bundleProducts frequentlyBoughtWith',
    );

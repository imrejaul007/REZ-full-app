...
async function validateDiscoveryData() {
  try {
    ...
    const paymentMethodsCheck: ValidationResult = {
      ...
      status:
        storesWithPaymentMethods === totalStores
          ? 'pass'
          : storesWithPaymentMethods > totalStores * 0.9
            ? 'warning'
            : 'fail',
    ...
    console.log(
      `   ${paymentMethodsCheck.status === 'pass' ? '✅' : paymentMethodsCheck.status === 'warning' ? '⚠️' : '❌'} ${paymentMethodsCheck.message}`,
    );
    ...
    console.log(
      `   ${bnplCheck.status === 'pass' ? '✅' : bnplCheck.status === 'warning' ? '⚠️' : '❌'} ${bnplCheck.message}`,
    );
    ...
    console.log(
      `   ${searchHistoryCheck.status === 'pass' ? '✅' : searchHistoryCheck.status === 'warning' ? '⚠️' : '❌'} ${searchHistoryCheck.message}`,
    );
    ...
    console.log(
      `   ${cashbackCheck.status === 'pass' ? '✅' : cashbackCheck.status === 'warning' ? '⚠️' : '❌'} ${cashbackCheck.message}`,
    );
    ...
    const locationCheck: ValidationResult = {
      ...
      status:
        storesWithLocation >= totalStores * 0.9 ? 'pass' : storesWithLocation >= totalStores * 0.7 ? 'warning' : 'fail',
    ...
    console.log(
      `   ${locationCheck.status === 'pass' ? '✅' : locationCheck.status === 'warning' ? '⚠️' : '❌'} ${locationCheck.message}`,
    );

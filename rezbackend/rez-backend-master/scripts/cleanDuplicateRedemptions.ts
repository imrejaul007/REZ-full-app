...
async function cleanDuplicateRedemptions() {
  try {
    ...
    const activeRedemptions = await redemptionsCollection
      .find({
    ...
    for (const [key, redemptions] of userOfferMap.entries()) {
      if (redemptions.length > 1) {
        ...
        for (const duplicate of toCancel) {
          ...
          duplicatesRemoved++;
          console.log(
            `   ❌ Cancelled duplicate: ${duplicate._id} (${new Date(duplicate.redemptionDate || duplicate.createdAt).toISOString()})`,
          );
        }
      }
    }

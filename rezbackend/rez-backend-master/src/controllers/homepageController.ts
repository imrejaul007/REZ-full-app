...
import personaResolverService, { ResolvedPersona } from '../services/personaResolverService';
...
export const getHomepage = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const {
      ...
      region: regionQuery,
      segment,
      featureLevel: featureLevelQuery,
    } = req.query;
    ...
    logger.info('🏠 [Homepage Controller] Request params:', {
      ...
      // Removed full header dump — JSON.stringify(req.headers) on every homepage
      // request allocated a large string per call and created GC pressure at scale
    });
    ...
    // Fetch homepage sections (cached), user context (per-user), and persona in parallel.
    // userContext is only fetched if user is authenticated — it adds wallet, cart, voucher, etc.
    // persona is resolved for authenticated users; anonymous requests get the default 'general' persona.
    ...
    const personaPromise: Promise<ResolvedPersona> = userId
      ? personaResolverService.resolvePersona(userId).catch((err) => {
          logger.warn('[Homepage] Persona resolution failed — using default', { error: err.message });
          return personaResolverService.resolveAnonymousPersona();
        })
      : Promise.resolve(personaResolverService.resolveAnonymousPersona());

    const [cached, userContext, resolvedPersona] = await Promise.all([
      cachedPromise,
      userContextPromise,
      personaPromise,
    ]);
    ...
    if (cached) {
      ...
      // Merge per-user context and persona into cached shared data
      return sendSuccess(
        res,
        {
          ...cached,
          ...
          persona: {
            personaId: resolvedPersona.personaId,
            confidence: resolvedPersona.confidence,
            source: resolvedPersona.source,
            feedConfig: resolvedPersona.feedConfig,
          },
        },
    ...
    // Cache miss — fetch from DB (persona-aware)
    ...
    redisService
      .set(homepageCacheKey, result.data, HOMEPAGE_TTL)
      .catch((err) => logger.warn('[Homepage] Cache set for homepage data failed', { error: err.message }));
    ...
    logger.info(
      `   Total items: ${Object.values(result.data).reduce((sum: number, arr: any) => sum + (Array.isArray(arr) ? arr.length : 0), 0)}`,
    );

    // Send response with mode, region info, user context, and resolved persona
    sendSuccess(
      res,
      {
        ...result.data,
...
async function getCachedGlobalOfferCount(): Promise<number> {
  ...
  await redisService
    .set(cacheKey, count, 300)
    .catch((err) => logger.warn('[Homepage] Cache set for active offers count failed', { error: err.message }));
  return count;
}

async function fetchUserContext(
  userId: string,
  extra?: { segment?: string; featureLevel?: number },
): Promise<{
  ...
  try {
    ...
    const [wallet, activeVoucherCount, activeRedemptionCount, totalOffersCount, cart, subscription] = await Promise.all(
      [
        Wallet.findOne({ user: userId })
          .select('balance coins brandedCoins statistics')
          .lean()
          .catch(() => null),
    ...
    await redisService
      .set(userContextCacheKey, context, 30)
      .catch((err) => logger.warn('[Homepage] Cache set for user context failed', { error: err.message }));

...
class ShareController {
  ...
  trackClick = asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      const redirectUrl = this.validateRedirectUrl(result.redirectUrl);
      if (!redirectUrl) {
        return res.status(400).json({
          success: false,
  ...
  /**
   * Validate redirect URL to prevent open redirect attacks.
   * Only allows relative URLs (starting with /) or same-origin URLs.
   */
  private validateRedirectUrl(url: string | undefined): string | null {
    ...
    // Allow relative URLs (e.g., /home, /store/123)
    if (url.startsWith('/') && !url.startsWith('//')) {
      return url;
    }

    // Allow same-origin URLs (e.g., https://app.example.com/page)
    try {
  ...
  sharePurchase = asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      const result = await shareService.sharePurchase(userId, orderId, platform);

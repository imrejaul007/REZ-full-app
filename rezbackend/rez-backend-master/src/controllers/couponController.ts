...
export const getAvailableCoupons = asyncHandler(async (req: Request, res: Response) => {
  ...
  // BUG-037: Cap at 100 to prevent loading the entire coupons collection into memory.
...
export const getFeaturedCoupons = asyncHandler(async (req: Request, res: Response) => {
  ...
  // BUG-037: Cap at 100 to prevent loading all featured coupons into memory.
...
export const claimCoupon = asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await couponService.claimCoupon(new Types.ObjectId(userId), new Types.ObjectId(id));
...
export const removeCoupon = asyncHandler(async (req: Request, res: Response) => {
  ...
  await UserCoupon.findByIdAndUpdate(id, { $set: { deletedAt: new Date() } });

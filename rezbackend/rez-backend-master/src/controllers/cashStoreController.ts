...
export const getCashStoreBrands = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (filter === 'popular') {
    ...
    const result = await mallService.getBrands({ sort: sortOption }, pageNum, limitNum);
    ...
    const result = await mallService.getBrands({ minCashback: 10, sort: sortOption }, pageNum, limitNum);
    brands = result.brands;
    total = result.total;
  } else if (category && category !== 'all') {
    // Filter by category slug
    const result = await mallService.getBrandsByCategory(category as string, pageNum, limitNum);
    brands = result.brands;
    total = result.total;
  } else {
    // All brands (default)
...
export const searchCashStoreBrands = asyncHandler(async (req: Request, res: Response) => {
  ...
  return sendSuccess(res, { brands, total: brands.length }, 'Search results');
});
...
export const getCashStoreHomepage = asyncHandler(async (req: Request, res: Response) => {
  const [dbCategories, topBrandsResult, featuredBrands, highCashbackResult] = await Promise.all([
...
export const getCashStoreGiftCards = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (category && category !== 'all') {
    ...
    result = await mallService.getBrands({ sort: { 'cashback.percentage': -1 } }, pageNum, limitNum);
  }
...
export const getCashStoreTrending = asyncHandler(async (req: Request, res: Response) => {
  ...
  const [popularResult, highCashbackResult, activeOffers] = await Promise.all([
    // Popular brands sorted by clicks
    mallService.getBrands({ sort: { 'analytics.clicks': -1 } }, 1, 20),
    // High cashback brands (10%+)
    mallService.getBrands({ minCashback: 10, sort: { 'cashback.percentage': -1 } }, 1, 20),

...
export const submitPost = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const urlPatterns: Record<string, RegExp> = {
      instagram:
        /^https?:\/\/(www\.)?instagram\.com\/([\w.]+\/)?(p|reel|reels|instagramreel)\/[a-zA-Z0-9_-]+\/?(\?.*)?$/,
    ...
    if (todaySubmissions >= RATE_LIMITS.DAILY_LIMIT) {
      ...
      return sendError(
        res,
        `Maximum ${RATE_LIMITS.DAILY_LIMIT} submissions per day reached. Please try again tomorrow.`,
        429,
      );
    }
    ...
    if (weeklySubmissions >= RATE_LIMITS.WEEKLY_LIMIT) {
      ...
      return sendError(
        res,
        `Maximum ${RATE_LIMITS.WEEKLY_LIMIT} submissions per week reached. Please try again next week.`,
        429,
      );
    }
    ...
    sendSuccess(
      ...
      'Post submitted successfully! The merchant will verify your post within 24 hours.',
      201,
    );
  } catch (error) {
  ...
  if (fraudMetadata) {
    try {
      ...
      if (parsed.trustScore !== undefined && parsed.trustScore < 20) {
        logger.warn('⚠️ [FRAUD] Low trust score media submission (allowed):', {
          userId,
          trustScore: parsed.trustScore,
        });
      }
    } catch (parseError) {
      logger.warn('Failed to parse fraudMetadata:', parseError);
    }
  }
...
export const getUserPosts = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const posts = await SocialMediaPost.find(query).sort({ createdAt: -1 }).skip(skip).limit(Number(limit)).lean();
      ...
      try {
        ...
        await coinService.awardCoins(
          ...
          `Social media cashback (${post.platform}) for order ${post.order}`,
          {
            ...
            platform: post.platform,
            orderId: post.order,
            referenceId: `social_share_reward:${post._id}`,
          },
        );
      } catch (coinError) {
...
const analyzeCaption = (
  ...
  const brandMentions = INSTAGRAM_CONFIG.REQUIRED_BRAND_MENTIONS.filter((mention) =>
...
export const verifyInstagramPost = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!contentAnalysis.hasBrandMention) {
      warnings.push(
        `Post should mention our brand: ${INSTAGRAM_CONFIG.REQUIRED_BRAND_MENTIONS.slice(0, 2).join(', ')}`,
      );
    }
...
export const verifyInstagramAccount = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const simulatedAccountData = {
      ...
      username,
      account_type: 'PERSONAL',
      followers_count: randomInt(100, 600),
      follows_count: randomInt(50, 350),
      media_count: randomInt(10, 110),
...
export const checkDuplicate = asyncHandler(async (req: Request, res: Response) => {
  ...
  const existing = await SocialMediaPost.findOne(query).select('_id submittedAt').lean();
...
export const checkSharedStatus = asyncHandler(async (req: Request, res: Response) => {
  ...
  const query = isMongoId
    ...
    : { user: userId, $or: [{ 'metadata.storePaymentId': orderId }, { 'metadata.orderNumber': orderId }] };

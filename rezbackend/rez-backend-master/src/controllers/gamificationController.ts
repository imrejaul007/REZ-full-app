...
import { checkChallengeDailyCap } from '../utils/velocityLimiter';
...
export const claimChallengeReward = asyncHandler(async (req: Request, res: Response) => {
  ...
  // MIGUEL: velocity check — 1 completion per challenge per day, 10 total per day
  const capCheck = await checkChallengeDailyCap(userId, id);
  if (!capCheck.allowed) {
    return sendError(res, capCheck.reason || 'Daily challenge claim limit reached.', 429);
  }
...
export const getChallengeLeaderboard = asyncHandler(async (req: Request, res: Response) => {
  ...
  const leaderboard = await challengeService.getChallengeLeaderboard(id, parseInt(limit as string, 10));
...
export const getAchievements = asyncHandler(async (req: Request, res: Response) => {
  sendSuccess(
    res,
    ACHIEVEMENT_DEFINITIONS.filter((a) => a.isActive),
    'Achievement definitions retrieved successfully',
  );
});
...
export const getUserAchievements = asyncHandler(async (req: Request, res: Response) => {
  ...
  const achievements = await UserAchievement.find({ user: userId }).sort({ unlocked: -1, progress: -1 }).lean();
...
export const getMyAchievements = asyncHandler(async (req: Request, res: Response) => {
  ...
  let userAchievements = await UserAchievement.find({ user: userId }).sort({ unlocked: -1, progress: -1 }).lean();
  ...
  if (userAchievements.length === 0) {
    ...
    if (achievementDocs.length > 0) {
      ...
      userAchievements = await UserAchievement.find({ user: userId }).sort({ unlocked: -1, progress: -1 }).lean();
    }
  }
...
export const getLeaderboard = asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await leaderboardService.getLeaderboard(type as string, period as string, pageNum, limitNum);
...
export const awardCoins = asyncHandler(async (req: Request, res: Response) => {
  ...
  const userId = (req.user._id as Types.ObjectId).toString();
  const result = await coinService.awardCoins(userId, amount, source || 'admin', description || 'Coins awarded', {
    ...metadata,
    referenceId: metadata?.referenceId || `admin_award:${userId}:${Date.now()}`,
  });
...
export const spinWheel = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!eligibility.canSpin) {
    logger.info(
      `❌ [SPIN_WHEEL] Daily limit reached: ${eligibility.spinsUsedToday}/${eligibility.maxDailySpins} spins used today`,
    );
    return sendBadRequest(
      res,
      `Daily spin limit reached (${eligibility.maxDailySpins} spins per day). Next spin available at ${eligibility.nextResetAt}`,
    );
  }

  logger.info(
    `✅ [SPIN_WHEEL] User eligible: ${eligibility.spinsUsedToday}/${eligibility.maxDailySpins} spins used today`,
  );
  ...
  if (postSpinEligibility.spinsUsedToday > postSpinEligibility.maxDailySpins) {
    logger.warn(
      `⚠️ [SPIN_WHEEL] Race condition detected for user ${userId}: ${postSpinEligibility.spinsUsedToday} spins today, rolling back`,
    );
  ...
  if (wallet) {
    ...
    // BUG-047 FIX: balance is PII/sensitive data — log at debug, not info
    logger.debug(`💰 [SPIN_WHEEL] User balance after spin: ${actualBalance}`);
  } else {
      ...
      for (const t of activeTournaments) {
        try {
          ...
          if (!response.tournamentUpdate) {
            ...
            response.tournamentUpdate = {
              tournamentName: t.name,
              pointsAdded: spinResult.value,
              newRank: newRank || 1,
            };
          }
        } catch (err: any) {
...
export const getSpinWheelData = asyncHandler(async (req: Request, res: Response) => {
  ...
  logger.info(
    `📊 [SPIN_WHEEL] User ${userId}: ${eligibility.spinsUsedToday} used, ${eligibility.spinsRemaining} remaining`,
  );
...
export const startQuiz = asyncHandler(async (req: Request, res: Response) => {
  ...
  const quiz = await quizService.startQuiz((req.user._id as Types.ObjectId).toString(), difficulty, questionCount);
...
export const getPlayAndEarnData = asyncHandler(async (req: Request, res: Response) => {
  ...
  const spinEligibility =
    spinEligibilityResult.status === 'fulfilled'
  ...
  const _spinsToday = spinsTodayResult.status === 'fulfilled' ? spinsTodayResult.value : 0;
  ...
  const completedToday = challengesArray.filter((c: any) => c.completed && new Date(c.updatedAt) >= todayStart).length;

  // Find the app_open streak or fallback to login streak
  const streaksArray = Array.isArray(userStreaks) ? userStreaks : [];
  const appOpenStreak =
    streaksArray.find((s: any) => s.type === 'app_open') || streaksArray.find((s: any) => s.type === 'login');

  // Check if user has checked in today
  const lastActivity = appOpenStreak ? new Date(appOpenStreak.lastActivityDate) : null;
  const todayCheckedIn = lastActivity ? lastActivity.setHours(0, 0, 0, 0) === todayStart.getTime() : false;
...
export const claimSurpriseDrop = asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await withTransaction(async (_session) => {
    ...
    // Award coins to user
    const coinResult = await coinService.awardCoins(userId, claimedDrop.coins, 'surprise_drop', claimedDrop.message, {
      dropId: claimedDrop._id,
      reason: claimedDrop.reason,
      referenceId: `surprise_drop:${claimedDrop._id}:${userId}`,
    });
...
const _DAY_REWARDS = DEFAULT_DAY_REWARDS; // Fallback; streakCheckin reads from config dynamically
...
export const streakCheckin = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!previousStreak) {
    ...
    const _newStreak = await UserStreak.create({
      user: userId,
      type: 'app_open',
      currentStreak: 1,
      longestStreak: 1,
    ...
    const day1DateStr = todayStart.toISOString().split('T')[0];
    const result = await coinService.awardCoins(
      userId,
      day1Reward,
      'daily_login',
      `Day 1 streak bonus (+${day1Reward} coins)`,
      { streakDay: 1, referenceId: `daily_login:${userId}:${day1DateStr}` },
    );
  ...
  // Atomically persist milestone claims — arrayFilter targets only unclaimed milestones
  // that fall within the current streak, preventing double-claim races.
  if (milestoneReached) {
    await UserStreak.updateOne(
      { _id: updatedStreak._id },
      {
        $set: {
          'milestones.$[elem].rewardsClaimed': true,
          'milestones.$[elem].claimedAt': new Date(),
        },
      },
      { arrayFilters: [{ 'elem.day': { $lte: newCurrentStreak }, 'elem.rewardsClaimed': false }] },
    );
  }

  // Step 5: Award coins
  const streakDateStr = todayStart.toISOString().split('T')[0];
  const result = await coinService.awardCoins(
    userId,
    coinsEarned,
    'daily_login',
    ...
    { streakDay: newCurrentStreak, milestone: milestoneReached, referenceId: `daily_login:${userId}:${streakDateStr}` },
  );
  ...
  if (result.rewards.coins > 0) {
    const coinResult = await coinService.awardCoins(
      ...
      `Streak milestone Day ${day} reward: ${result.rewards.name}`,
      {
        streakDay: day,
        streakType: type,
        milestoneName: result.rewards.name,
        referenceId: `streak_milestone:${userId}:${type}:day${day}`,
      },
    );
    ...
    Referral.aggregate([
      { $match: { referrer: userObjectId } },
      {
        $group: {
          ...
          referrerCommission: {
            $sum: {
              $cond: [{ $in: ['$status', ['completed', 'qualified']] }, { $ifNull: ['$rewards.referrerAmount', 0] }, 0],
            },
          },
        },
      },
    ...
    SocialMediaPost.aggregate([
      { $match: { user: userObjectId } },
      {
        $group: {
          ...
          postCommission: {
            $sum: {
              $cond: [{ $in: ['$status', ['approved', 'credited']] }, { $ifNull: ['$cashbackAmount', 0] }, 0],
            },
          },
        },
      },
...
export const getShareSubmissions = asyncHandler(async (req: Request, res: Response) => {
  ...
  const [posts, total] = await Promise.all([
    SocialMediaPost.find({ user: userId }).sort({ submittedAt: -1 }).skip(skip).limit(limit).lean() as Promise<
      ISocialMediaPost[]
    >,
...
export const submitSharePost = asyncHandler(async (req: Request, res: Response) => {
  ...
  const newPost = new SocialMediaPost({
    ...
    submissionIp:
      typeof submissionIp === 'string' ? submissionIp : Array.isArray(submissionIp) ? submissionIp[0] : undefined,
    deviceFingerprint,
    userAgent,
    metadata: {
      postId: posterId,
...
export const getStreakBonuses = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Get user's streak — single query instead of two sequential round-trips
  const streaks = await UserStreak.find({ user: userId, type: { $in: ['app_open', 'login'] } }).lean();
  const streak = streaks.find((s) => s.type === 'app_open') || streaks.find((s) => s.type === 'login');
...
export const getBonusOpportunities = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [urgentChallenges, activeCoinDrops, activeCampaigns] = await Promise.all([

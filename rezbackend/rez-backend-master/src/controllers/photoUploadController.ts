...
export const moderatePhoto = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (action === 'approve') {
    try {
      ...
      // DP-003 FIX: must NOT use .lean() here — approve() and creditCoins() are Mongoose
      // instance methods that call this.save() internally. A plain JS object from .lean()
      // has no prototype methods, so the calls throw a silent TypeError and the coins are
      // never credited. Remove .lean() to get a live Mongoose document.
    ...
    try {
      const { PendingCoinReward } = require('../models/PendingCoinReward');
      // DP-003 FIX: same as above — remove .lean() so reject() instance method works
      const pendingReward = await PendingCoinReward.findOne({
        user: photoUpload.user,
        source: 'photo_upload',
        referenceId: photoUpload._id,

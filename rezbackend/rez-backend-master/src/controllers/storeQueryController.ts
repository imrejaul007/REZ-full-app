...
export const sendFollowerNotification = asyncHandler(async (req: Request, res: Response) => {
  const { storeId } = req.params;
  const { title, message, imageUrl, deepLink: _deepLink } = req.body;
  const userId = (req as any).userId;

  try {
    // Validate input early to avoid unnecessary DB queries
    if (!title || !message) {
      return sendBadRequest(res, 'Title and message are required');
    }

    // Verify user is store owner/merchant
    // DANIEL: Payload pruning — select only merchantId field for auth check
...
export const notifyNewOffer = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    // Verify user is store owner/merchant
    // DANIEL: Fetching store with .lean() to avoid Mongoose overhead on auth check
    const store = await Store.findById(storeId).select('merchantId').lean();
    if (!store) {
      return sendNotFound(res, 'Store not found');
    }
...
export const notifyNewProduct = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    // DANIEL: Parallel fetches — Store auth check and Product fetch run concurrently
    const [store, product] = await Promise.all([
      Store.findById(storeId).select('merchantId').lean() as any,
      Product.findById(productId).select('name description pricing images slug').lean() as any,
    ]);
...
export const getRecentEarnings = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // DANIEL: Parallel fetch + payload pruning — fetch minimal user fields only, skip heavy tx fields
    const recentTransactions = await Transaction.find({
      ...
      .populate('user', 'firstName name avatar profilePicture')
      .sort({ createdAt: -1 })
      .limit(limit)
      .lean();
...
export const getNearbyStoresForHomepage = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // DANIEL: Payload pruning — select only fields used in response (avoid rewardRules, bookingConfig overhead)
    const stores = await Store.find({
      ...
      .select(
        'name slug logo location operationalInfo offers rewardRules.baseCashbackPercent storeVisitConfig isActive',
      )
    ...
    const currentMinutes = parseInt(currentTime.split(':', 10)[0]) * 60 + parseInt(currentTime.split(':', 10)[1]);
    ...
    const queueCountMap = new Map<string, number>(queueCounts.map((q: any) => [q._id.toString(), q.count as number]));
    ...
    const processedStores = stores.map((store: any) => {
      ...
      if (todayHours && !todayHours.closed && todayHours.open && todayHours.close) {
        ...
        if (isOpen) {
          ...
          const closingMinutes = parseInt(closeTime.split(':', 10)[0]) * 60 + parseInt(closeTime.split(':', 10)[1]);
      ...
      const isLive =
        store.isActive &&
        (store.storeVisitConfig?.features?.includes('live_availability') || store.storeVisitConfig?.enabled === true);
    ...
    const validStores = processedStores
      ...
      .map((s: any) => {
        ...
        const { distanceValue: _distanceValue, ...rest } = s;
        return rest;
      });
...
export const getNewStores = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Select only the fields used by the response formatter — avoids fetching large
    // document fields (description, images array, operationalInfo, etc.) unnecessarily.
    const NEW_STORES_SELECT = 'name slug logo banner category location offers ratings analytics createdAt';

    let stores = await Store.find(query)
      .select(NEW_STORES_SELECT)
    ...
    // DANIEL: Fallback query also lean'd — ensures fast JS objects not Mongoose Documents
    ...
    const formattedStores = stores.map((store: any, index: number) => ({
      ...
      image:
        store.logo ||
        store.banner?.[0] ||
        `https://images.unsplash.com/photo-${1441984904996 + index}-e0b6ba687e04?w=400`,

...
// R13: Use centralized image helpers instead of ad-hoc fallback chains
const { parseImage, getPrimaryImageUrl, parseImages } = Product as any;
import { Category } from '../models/Category';
import { Store } from '../models/Store';
import { sendSuccess, sendNotFound, sendPaginated, sendError } from '../utils/response';
import { asyncHandler } from '../utils/asyncHandler';
import { AppError } from '../middleware/errorHandler';
import redisService from '../services/redisService';
import { CacheTTL } from '../config/redis';
import { escapeRegex } from '../utils/sanitize';
import { CacheKeys, generateQueryCacheKey } from '../utils/cacheHelper';
 ...
 *     description: Returns a paginated, filterable list of active products with support for category, store, price, rating, tags, occasion, brand, mode, and region filters.
 *     tags: [Products]
 *     security:
 *       - bearerAuth: []
 *     parameters:
...
export const getProducts = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!excludeProducts && diversityMode === 'none') {
    const filterHash = generateQueryCacheKey({
      ...
      limit,
      mode: activeMode,
      region: effectiveRegionForCache,
      tags,
      occasion,
      brand,
    });
  ...
  if (needsModeFilter || needsRegionFilter) {
    ...
    if (filteredStoreIds.length > 0) {
      if (store) {
        ...
        const storeInFiltered = filteredStoreIds.some((id: any) => id.toString() === (store as string).toString());
  ...
  if (minPrice || maxPrice) {
    ...
    if (minPrice) {
      ...
      if (!isNaN(minPriceNum) && minPriceNum >= 0) {
        query['pricing.selling'].$gte = minPriceNum;
      }
    }
    if (maxPrice) {
      const maxPriceNum = Number(maxPrice);
      if (!isNaN(maxPriceNum) && maxPriceNum >= 0) {
        query['pricing.selling'].$lte = maxPriceNum;
      }
    }
  }
  ...
  if (excludeProducts && typeof excludeProducts === 'string') {
    const excludedIds = excludeProducts
      .split(',')
      .map((id) => {
        try {
  ...
  // PACKETSENSE FIX-2: Strip heavy fields the frontend never renders on list views.
  // Excluded: analytics (internal counters), seo (server-side only), specifications
  // (only shown on PDP), reviewStats (duplicates ratings.*), frequentlyBoughtWith
  // (PDP cross-sell only), metadata (internal tagging), searchHistory (backend-only).
  // This roughly halves the per-document wire size for large catalogs.
  const LIST_SELECT = [
    '_id',
    'name',
    'slug',
  ...
  try {
    ...
    if (search) {
      productsQuery = Product.find({
        ...
        .select({ score: { $meta: 'textScore' }, ...Object.fromEntries(LIST_SELECT.split(' ').map((f) => [f, 1])) })
        .populate('category', 'name slug')
        .populate('store', 'name logo location.city')
    ...
    const [totalProducts, products] = await Promise.all([
      ...
      productsQuery.skip(skip).limit(Number(limit)).lean(),
    ]);

    // Track views for authenticated users
    if (req.user && products.length > 0) {
      // Increment view count for products (async, don't wait)
      Product.updateMany({ _id: { $in: products.map((p) => p._id) } }, { $inc: { 'analytics.views': 1 } }).catch(
        (err) => logger.error('[ProductCtrl] Product view count increment failed', { error: err.message }),
      );
    }
    ...
    if (req.user && search) {
      logProductSearch((req.user as any)._id, search as string, totalProducts, {
        ...
        logger.error('[ProductCtrl] Product search history logging failed', {
          error: err.message,
    ...
    if (!excludeProducts && diversityMode === 'none') {
      ...
      const cacheKey = CacheKeys.productList(filterHash);
      await redisService.set(cacheKey, { products: finalProducts, total: totalProducts }, CacheTTL.PRODUCT_LIST);
    }
 ...
 *     description: Returns a single product by its ID, including similar products, computed cashback, and estimated delivery.
 *     tags: [Products]
 *     security:
 *       - bearerAuth: []
 *     parameters:
...
export const getProductsByCategory = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { minPrice, maxPrice, rating, sort, sortBy: sortByParam, page = 1, limit = 20 } = req.query;
  ...
  try {
    ...
    // Cache the results
    await redisService.set(categoryCacheKey, { response, total: totalProducts }, CacheTTL.PRODUCT_LIST);
  ...
  try {
    ...
    if (subcategory) {
      ...
      products = await Product.find({
        $or: [{ subCategory: subcategory._id }, { category: subcategory._id }],
        isActive: true,
      })
        .populate('category', 'name slug')
    ...
    const transformedProducts = products.map((product: any) => {
      // R13: Use centralized helper — handles both flat strings and normalized JSON strings
      const productImage = getPrimaryImageUrl(product.images);
      ...
      return {
        ...
        image: productImage,
        // R13: Attach parsed images for consumers that need structured data
...
export const getProductsByStore = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Cache the results
    await redisService.set(storeCacheKey, { response, total: totalProducts }, CacheTTL.STORE_PRODUCTS);
  ...
  try {
    ...
    const transformedProducts = products.map((product: any) => {
      return {
        ...
        // R13: Use centralized helper for both flat strings and normalized JSON
        image: getPrimaryImageUrl(product.images) || product.image || '',
        description: product.description || '',
        // RC-2 DEPRECATED: Use Product.price virtual instead.
        price: {
  ...
  try {
    ...
    const transformedProducts = products.map((product: any) => {
      return {
        ...
        cashback:
          product.cashback?.percentage || (product.store as any)?.cashback?.percentage
            ? {
...
export const searchProducts = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Cache the results
    await redisService.set(searchCacheKey, { products, total }, CacheTTL.PRODUCT_SEARCH);
...
export const getRecommendations = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const transformedRecommendations = recommendations.map((product: any) => {
      ...
      // R13: Use centralized helper — handles flat strings, normalized JSON, and legacy objects
      const productImage = getPrimaryImageUrl(product.images) || product.image || '';
...
export const getProductAnalytics = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const product = await Product.findById(id).select('analytics cashback deliveryInfo pricing inventory store');
    ...
    try {
      ...
      /* ignore malformed location JSON */
    }
    ...
    const analytics = {
      ...
      todayViews: product.analytics.todayViews || 0,
      todayPurchases: product.analytics.todayPurchases || 0,
      peopleBoughtToday: product.analytics.todayPurchases || Math.max(1, randomInt(100, 150)), // Fallback for demo
      cashback: {
        percentage: product.cashback?.percentage || 5,
        amount: cashbackAmount,
  ...
  try {
    ...
    if (bundleProducts.length > 0) {
      const individualTotal =
        bundleProducts.reduce((sum: number, p: any) => {
...
export const getPopularSearches = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [topCategories, topBrands] = await Promise.all([
      Category.find({ isActive: true }).sort({ productCount: -1 }).limit(5).select('name').lean(),
      Product.aggregate([
...
export const getSimilarProducts = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const formattedProducts = similarProducts.map((product: any) => ({
      ...
      // R13: Use centralized helper
      image: getPrimaryImageUrl(product.images) || null,
      category: product.category,
      store: product.store,
      rating: product.ratings?.average || 0,
      reviewCount: product.ratings?.count || 0,
 ...
 *     description: Creates a new product for the authenticated merchant store. Requires merchant authentication.
 *     tags: [Products]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 ...
 *         description: Not authenticated
 *       403:
 *         description: Not authorized as merchant
 */
export const createProduct = (req: Request, res: Response) => {
  // Implemented in merchantroutes/products.ts
  res.status(501).json({ success: false, message: 'Not implemented in this controller' });
};
 ...
 *     description: Updates an existing product by ID. Requires merchant authentication and product ownership.
 *     tags: [Products]
 *     security:
 *       - bearerAuth: []
 *     parameters:

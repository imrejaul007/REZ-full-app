...
import { Subscription, SubscriptionTier, BillingCycle } from '../models/Subscription';
...
export const subscribeToPlan = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (promoCode) {
    ...
    const promoResult = await promoCodeService.validatePromoCode(promoCode, tier, billingCycle, userId);

    if (promoResult.valid && promoResult.finalPrice !== undefined) {
      appliedDiscount = promoResult.discount || 0;
      price = promoResult.finalPrice;
      logger.info(
        `[SUBSCRIBE] Promo code applied: ${promoCode}, discount: ₹${appliedDiscount}, final price: ₹${price}`,
      );
    } else {
  ...
  if (promoCode && appliedDiscount > 0) {
    try {
      await promoCodeService.applyPromoCode(promoCode, tier, billingCycle, userId, String(subscription._id));
...
export const confirmUpgrade = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Verify payment is real and successful
  if (paymentId) {
    const { razorpayService: rzpSvc } = require('../services/razorpayService');
    try {
      const payment = await rzpSvc.fetchPaymentDetails(paymentId);
      if (!payment || payment.status !== 'captured') {
        return res.status(400).json({ success: false, message: 'Payment not verified or not captured' });
      }
      if (payment.amount < (upgradeRecord as any).priceDifference * 100) {
        return res.status(400).json({ success: false, message: 'Payment amount insufficient' });
      }
    } catch (err) {
  ...
  if (subscription) {
    ...
    if (subscription.razorpaySubscriptionId) {
      try {
        ...
        await razorpaySubscriptionService.updateSubscription(subscription.razorpaySubscriptionId, {
          plan_id: newPlanId,
  ...
  await SubscriptionUpgrade.updateOne({ _id: upgradeRecord._id }, { status: 'completed', completedAt: new Date() });
  ...
  privilegeResolutionService.invalidate(userId.toString()).catch((err) =>
    logger.error('[SubscriptionCtrl] Privilege cache invalidation failed after upgrade', {
      error: err.message,
      userId,
    }),
  );
...
export const downgradeSubscription = asyncHandler(async (req: Request, res: Response) => {
  ...
  const currentTierPrice = await tierConfigService.getTierPrice(
    currentSubscription.tier,
    currentSubscription.billingCycle,
  );
  const newTierPrice =
    newTier === 'free' ? 0 : await tierConfigService.getTierPrice(newTier, currentSubscription.billingCycle);
  ...
  privilegeResolutionService.invalidate(userId.toString()).catch((err) =>
    logger.error('[SubscriptionCtrl] Privilege cache invalidation failed after downgrade', {
      error: err.message,
      userId,
    }),
  );
...
export const cancelSubscription = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Cancel in Razorpay
  if (subscription.razorpaySubscriptionId) {
    await razorpaySubscriptionService.cancelSubscription(subscription.razorpaySubscriptionId, !cancelImmediately);
  }
...
export const getValueProposition = asyncHandler(async (req: Request, res: Response) => {
  ...
  const valueProposition = await subscriptionBenefitsService.getValueProposition(userId, tier as SubscriptionTier);
...
export const handleWebhook = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // ISSUE-03/ISSUE-20 FIX: Use raw request body for HMAC verification.
    // JSON.stringify(parsed) produces different bytes than the original payload
    // (key ordering, whitespace, Unicode escaping), causing signature mismatch.
    const rawBody = (req as any).rawBody || JSON.stringify(webhookBody);
    const isValid = razorpaySubscriptionService.verifyWebhookSignature(rawBody, signature, secret);
    ...
    try {
      ...
      try {
        await ProcessedWebhookEvent.markEventFailed(eventId, processingError.message);
      } catch (recordError: any) {
...
export const validatePromoCode = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.status(200).json({
    ...
    data: {
      ...
      originalPrice: await promoCodeService.getSubscriptionPrice(
        tier as SubscriptionTier,
        billingCycle as BillingCycle,
      ),

...
import { sendSuccess, sendPaginated, sendNotFound, sendBadRequest, sendCreated } from '../utils/response';
...
export const getMallBrands = asyncHandler(async (req: Request, res: Response) => {
  const { category, tier, collection, minCashback, badges, search, page = '1', limit = '20' } = req.query;

  const parsedMinCashback = minCashback ? parseInt(minCashback as string, 10) : undefined;
  if (parsedMinCashback !== undefined && (isNaN(parsedMinCashback) || parsedMinCashback < 0)) {
    return sendBadRequest(res, 'minCashback must be a non-negative number');
  }
  ...
  const { brands, total, pages: _brandPages } = await mallService.getBrands(filters, pageNum, limitNum);
...
export const searchBrands = asyncHandler(async (req: Request, res: Response) => {
  ...
  if ((q as string).trim().length > 200) {
    return sendBadRequest(res, 'Search query must not exceed 200 characters');
  }

  const limitNum = Math.min(50, Math.max(1, parseInt(limit as string, 10) || 20));
  const brands = await mallService.searchBrands((q as string).trim(), limitNum);
...
export const createMallBrand = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (brand.collections && brand.collections.length > 0) {
    await Promise.all(
      brand.collections.map((colId: Types.ObjectId) => (MallCollection as any).updateBrandCount(colId)),
    );
  }
...
export const updateMallBrand = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (updateData.collections) {
    ...
    await Promise.all(
      Array.from(allAffected).map((colId) => (MallCollection as any).updateBrandCount(new Types.ObjectId(colId))),
    );
  }
...
export const deleteMallCollection = asyncHandler(async (req: Request, res: Response) => {
  ...
  await MallBrand.updateMany({ collections: collectionId }, { $pull: { collections: collectionId } });
...
export const getMallStores = asyncHandler(async (req: Request, res: Response) => {
  const { category, premium, minCoinReward, search, page = '1', limit = '20' } = req.query;

  const filters = {
    category: category as string,
    premium: premium === 'true',
    minCoinReward: minCoinReward ? parseInt(minCoinReward as string, 10) : undefined,
    search: search as string,
  };
  ...
  const { stores, total, pages: _storePages } = await mallService.getMallStores(filters, pageNum, limitNum);
...
export const searchMallStores = asyncHandler(async (req: Request, res: Response) => {
  ...
  const stores = await mallService.searchMallStores(q as string, parseInt(limit as string, 10));
...
export const getAdminMallStores = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (filter === 'mall') {
    ...
    query.$or = [{ 'deliveryCategories.mall': { $exists: false } }, { 'deliveryCategories.mall': false }];
  }
...
export const getAdminOffers = asyncHandler(async (req: Request, res: Response) => {
  // PERF FIX: Replaced bare find({}) (full collection scan, no pagination control) with an explicit
  // filter + paginated query.  The previous hard-coded .limit(100) with no .skip() made it impossible
  // to page through results AND still loaded up to 100 full documents in a single round-trip.
  // Now we accept page/limit query params (defaulting to page 1, 50 per page) and build a real filter
  // so MongoDB can use the existing indexes (e.g. createdAt, brand, store) instead of a COLLSCAN.
  const { page = '1', limit = '50', activeOnly } = req.query;
  const pageNum = Math.max(1, parseInt(page as string, 10) || 1);
  const limitNum = Math.min(200, parseInt(limit as string, 10) || 50);

  // When activeOnly=true return only active non-expired offers (the common read path).
  // Omit the filter for the admin overview (all offers including inactive/expired).
  ...
  return sendSuccess(res, { offers, total, page: pageNum, limit: limitNum }, 'Admin offers retrieved successfully');
});

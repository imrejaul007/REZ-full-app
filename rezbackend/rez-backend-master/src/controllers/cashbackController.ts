...
export const getPendingCashback = asyncHandler(async (req: Request, res: Response) => {
  ...
  const cashbacks = await cashbackService.getPendingReadyForCredit(new Types.ObjectId(userId));

  const totalAmount = cashbacks.reduce((sum, cb) => sum + cb.amount, 0);

  // Append estimatedCreditDate and countdown fields so the mobile app can render
  // a live "available in X hours" timer without re-deriving the hold period.
  // Uses creditableAt (precise, set at creation time from trust-tier hold hours)
  // when available; falls back to earnedDate + pendingDays for legacy records.
  const cashbacksWithDate = cashbacks.map((cb: any) => {
    const creditDate = cb.creditableAt
      ? new Date(cb.creditableAt)
      : (() => {
          const d = new Date(cb.earnedDate);
          d.setDate(d.getDate() + (cb.pendingDays ?? 7));
          return d;
        })();
    const msRemaining = Math.max(0, creditDate.getTime() - Date.now());
    const hoursRemaining = Math.round(msRemaining / (1000 * 60 * 60));
    // holdHours = total hold window derived from the credited-at timestamp vs earned date
    const holdHours = cb.creditableAt
      ? Math.round((new Date(cb.creditableAt).getTime() - new Date(cb.earnedDate).getTime()) / (1000 * 60 * 60))
      : (cb.pendingDays ?? 7) * 24;
    return {
      ...(cb.toObject?.() ?? cb),
...
export const getExpiringSoon = asyncHandler(async (req: Request, res: Response) => {
  ...
  const cashbacks = await cashbackService.getExpiringSoon(new Types.ObjectId(userId), Number(days));
...
export const redeemCashback = asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await cashbackService.redeemPendingCashback(new Types.ObjectId(userId));

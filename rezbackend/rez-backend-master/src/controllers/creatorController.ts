...
export const trackPickView = asyncHandler(async (req: Request, res: Response) => {
  const { pickId } = req.params;
  const viewerUserId = req.user?._id?.toString() || req.userId;
...
export const getCreatorPicks = asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await creatorService.getCreatorPicksByProfileId((profile._id as any).toString(), Number(limit));
...
export const getCreatorStats = asyncHandler(async (req: Request, res: Response) => {
  ...
  return sendSuccess(
    res,
    {
      videos: profile.stats?.totalPicks || 0,
      views: profile.stats?.totalViews || 0,
      likes: profile.stats?.totalLikes || 0,
      shares: 0,
      comments: 0,
      engagementRate: profile.stats?.engagementRate || 0,
...
export const adminApproveCreator = asyncHandler(async (req: Request, res: Response) => {
  const adminId = req.user?._id?.toString() || req.userId;
  const { id } = req.params;

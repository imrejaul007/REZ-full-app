...
export const cancelOrder = asyncHandler(async (req: Request, res: Response) => {
  ...
  // BUG-11: Wrap session creation in try-catch so a startSession failure rolls back
  // the 'cancelling' claim rather than leaving the order stuck in that state forever.
  ...
  try {
    ...
    logger.error('[cancelOrder] Failed to start MongoDB session — rolling back claim', sessionErr);
    await Order.findByIdAndUpdate(orderId, { $set: { status: originalStatus } }).catch(() => {});
    return sendError(res, 'Failed to start database transaction. Please try again.', 503);
  }
  ...
  try {
    ...
    for (const orderItem of order.items) {
      ...
      if (variant) {
        // Restore variant stock AND the main inventory.stock counter.
        // Also release the reservedStock reservation (set at order creation for online payments).
        ...
        if (updateResult) {
          ...
          // [DRIFT] order_cancelled_stock_not_restored — variant product not found for stock restoration
          logger.warn('[DRIFT] order_cancelled_stock_not_restored', {
        ...
        // Restore main product stock and release reservation
        ...
        if (updateResult) {
          ...
          // [DRIFT] order_cancelled_stock_not_restored — main product not found for stock restoration
          logger.warn('[DRIFT] order_cancelled_stock_not_restored', {
            orderId: String(order._id),
            productId: String(productId),
            quantity,
    ...
    // Revert coupon usage if a coupon was applied to this order
    if (order.couponCode) {
      const couponService = require('../services/couponService').default;
      await couponService.revertCouponUsage(order.user, order.couponCode, order._id, session);
      logger.info('[CANCEL ORDER] Coupon usage reverted:', order.couponCode);
    }
    ...
    if (order.payment?.coinsUsed) {
      ...
      if (rezCoins > 0) {
        ...
        await refundService.processRefund({
          ...
          session, // Run inside the active transaction for atomicity
        });
      }

      // Refund promo coins (atomic positional update — avoids read-modify-write race)
      if (promoCoins > 0) {
        await Wallet.findOneAndUpdate(
          { user: orderUserId, 'coins.type': 'promo' },
      ...
      if (storePromoCoins > 0) {
        ...
        if (storeId) {
          ...
          if (wallet) {
            // BUG-NEW-008 FIX: pass session so this write participates in the
            // surrounding MongoDB transaction and is rolled back on abort.
    ...
    if ((order as any).offerRedemption?.code) {
      ...
      try {
        ...
        // BUG-NEW-009 FIX: add order ID to filter so a redemption belonging to a
        // different order with the same code cannot be accidentally reversed.
    ...
    for (const restoration of stockRestorations) {
      ...
      CacheInvalidator.invalidateProduct(restoration.productId).catch((err) =>
        logger.error('[OrderCancelCtrl] Product cache invalidation failed after stock restoration', {
          error: err.message,
    ...
    // Reverse any credited cashback for this order (non-blocking — must not affect cancel success)
    import('../services/cashbackService')
      .then(({ default: cashbackService }) => {
        cashbackService
          .reverseCashbackForOrder(order._id as Types.ObjectId, `Order cancelled: ${reason || 'Customer request'}`)
          .catch((err: any) =>
            logger.error('[CANCEL ORDER] Cashback reversal failed (non-blocking)', {
              orderId: String(order._id),
              error: err?.message,
            }),
          );
    ...
    await activityService.order.onOrderCancelled(new Types.ObjectId(userId), order._id as Types.ObjectId, storeName);

    // Phase 8: Publish to order queue for async side effects (Strangler Fig — dual mode)
    publishOrderEvent({
      eventId: `order-cancelled:${String(order._id)}:${Date.now()}`,
      eventType: 'order.cancelled',
      userId,
      storeId: storeData?._id?.toString() || storeData?.toString(),
      payload: {
        orderId: String(order._id),
        orderNumber: order.orderNumber,
        newStatus: 'cancelled',
  ...
  try {
    ...
    if (refundType === 'partial') {
      refundAmount = refundItems.reduce((sum: number, item: any) => {
        ...
        if (orderItem) {
          const cappedQuantity = Math.min(item.quantity, orderItem.quantity);
          return sum + orderItem.price * cappedQuantity;
        }
        return sum;
      }, 0);
    }

    // BUG-NEW-003 FIX: Cap refundAmount so we never issue more than what was actually paid
    // minus any amount already refunded. Without this cap a partial-refund payload with
    // inflated quantities could produce a refundAmount larger than paidAmount.
    const maxRefundable = (order.totals?.paidAmount || order.totals?.total || 0) - (order.totals?.refundAmount || 0);
    ...
    try {
      ...
      if (saveErr.code === 11000) {
        return sendBadRequest(res, 'A refund request for this order is already pending');
      }
      throw saveErr;
    }
    ...
    try {
      ...
      if (storeIds.length > 0) {
        ...
        for (const store of stores) {
          ...
          if (merchantEmail) {
            try {
              await EmailService.sendRefundRequestNotification(merchantEmail, storeName, {
                orderNumber: order.orderNumber,
                refundAmount,
                refundType,
                refundReason: reason,

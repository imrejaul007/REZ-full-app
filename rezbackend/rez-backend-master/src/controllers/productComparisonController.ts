...
export const createProductComparison = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const comparison = new ProductComparison({
      user: userId,
      products: productIds,
      // D18: IST locale for display names visible to Indian users.
      name: name || `Comparison ${new Date().toLocaleDateString('en-IN', { timeZone: 'Asia/Kolkata' })}`,
    });
    ...
    await comparison.populate({
      path: 'products',
      select:
        'name description images pricing ratings inventory cashback store category brand weight specifications deliveryInfo',
      populate: [
        {
          path: 'store',
          select: 'name logo',
...
export const addProductToComparison = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    // BUG FIX: removed .lean() — this document needs .save() and .populate() calls below
    const comparison = await ProductComparison.findOne({
      _id: comparisonId,
      user: userId,
    });
    ...
    // BUG FIX: compare by string to handle ObjectId vs string mismatch
    if (comparison.products.some((p) => p.toString() === productId.toString())) {
...
export const removeProductFromComparison = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    comparison.products = comparison.products.filter((id) => id.toString() !== productId);

...
export const getConsolidatedEarningsSummary = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const pendingEarnings =
      Math.round((pendingProjects + pendingCashback + pendingRewards + pendingConversions) * 100) / 100;
    ...
    const thisMonthAgg = await CoinTransaction.aggregate([
      {
        $match: {
          user: userObjectId,
          type: { $in: earningTypes },
          $or: [{ 'metadata.partnerEarning': true }, { source: 'referral' }],
    ...
    try {
      ...
      if (partner) {
        partnerLevel = {
          ...
          daysRemaining:
            typeof (partner as any).getDaysRemaining === 'function' ? (partner as any).getDaysRemaining() : 0,
          ordersToNextLevel:
            typeof (partner as any).getOrdersNeededForNextLevel === 'function'
              ? (partner as any).getOrdersNeededForNextLevel()
              : 0,
        };
      }
    } catch {
...
export const getProjectStats = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    allActiveProjects.forEach((project) => {
      ...
      const hasUserSubmission = project.submissions?.some((sub: any) => sub.user && sub.user.toString() === userId);
...
export const getNotifications = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const limitedNotifications = limit
      ? filteredNotifications.slice(0, parseInt(limit as string, 10))
...
export const withdrawEarnings = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // AHMED: atomic debit + ledger entry — no read-modify-write race condition
    const withdrawalId = new Types.ObjectId();

    // 1. Atomically deduct from wallet (fails if balance insufficient — $gte guard)
    const updatedWallet = await Wallet.findOneAndUpdate(
      { user: userId, 'coins.type': 'rez', 'coins.amount': { $gte: amount } },
      {
    ...
    if (!updatedWallet) {
      return sendBadRequest(res, 'Insufficient balance or wallet not found');
    }

    // 2. Create CoinTransaction debit record (audit trail)
    await CoinTransaction.create({
      ...
      amount,
      source: 'withdrawal',
      description: `Withdrawal via ${method || 'bank'} — ref ${withdrawalId.toString().slice(-8).toUpperCase()}`,
      metadata: {
        method,
        accountDetails: accountDetails ? '[REDACTED]' : undefined,
        withdrawalId: withdrawalId.toString(),
      },
      status: 'completed',
      idempotencyKey: `withdrawal:${userId}:${withdrawalId.toString()}`,
    });

    logger.info(
      '[EARNINGS] Withdrawal deducted and logged. withdrawalId:',
      withdrawalId,
      'userId:',
      userId,
      'amount:',
    ...
    try {
      earningsSocketService.emitNotification(userId, {
        ...
        description: `₹${amount} has been deducted and your withdrawal is being processed`,
        data: {
          withdrawalId: withdrawalId.toString(),
          amount,
          method,
    ...
    logger.info('[EARNINGS] Withdrawal request created and wallet debited:', withdrawalId);

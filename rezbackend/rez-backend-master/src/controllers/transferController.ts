...
import { sendSuccess, sendError, sendBadRequest, sendTooManyRequests, sendNotFound } from '../utils/response';
...
export const initiateTransfer = asyncHandler(async (req: Request, res: Response) => {
  ...
  // SECURITY: Amount validation — Prevent NaN, Infinity, negative, and overflow attacks
  ...
  if (idempotencyKey) {
    ...
    if (existing) {
      ...
      if (existing.status === 'failed') {
        await Transfer.findOneAndUpdate({ _id: existing._id }, { $unset: { idempotencyKey: 1 } });
        // Fall through to create a new transfer
      }
    }
  }
  if (!coinType || !['rez', 'promo', 'branded'].includes(coinType)) {
  ...
  try {
    ...
    if (validatedAmount > config.transferLimits.perTransactionMax) {
      return sendBadRequest(
        res,
        `Maximum transfer amount is ${config.transferLimits.perTransactionMax} NC per transaction`,
      );
    }
    ...
    if (!recipient) return sendNotFound(res, 'Recipient not found');
    ...
    if (!recipientCheck.allowed) {
      ...
      return sendTooManyRequests(res, 'Too many unique recipients today. Try again tomorrow.');
    }
    ...
    if (!velocityCheck.allowed) {
      ...
      return sendTooManyRequests(
        res,
        `Transfer rate limit exceeded. Try again in ${Math.ceil(velocityCheck.resetInSeconds / 60)} minutes.`,
      );
    }
    ...
    if (coinType === 'rez') {
      ...
      const brandedCoin = senderWallet.brandedCoins.find((bc: any) => bc.merchantId.toString() === merchantId);
...
export const confirmTransfer = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!transfer) return sendNotFound(res, 'Transfer not found or already processed');
  ...
  if (otpHash !== transfer.otpHash) {
    ...
    const _remaining = updated ? 3 - updated.otpAttempts : 0;
...
async function executeTransfer(transferId: string): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    ...
    try {
      // NOTE: No .lean() here — branded-coin paths call instance methods
      // (useBrandedCoins / addBrandedCoins) which only exist on hydrated documents.
      ...
      if (transfer.coinType === 'rez') {
        ...
        await recipientWallet.addBrandedCoins(
          transfer.merchantId,
          senderWallet.brandedCoins.find((bc) => bc.merchantId.toString() === transfer.merchantId?.toString())
      ...
      const senderName = senderUser?.fullName || senderUser?.phoneNumber || 'User';
      const recipientName = recipientUser?.fullName || recipientUser?.phoneNumber || 'User';
      ...
      const senderBalanceAfter = updatedSenderWallet?.balance?.available ?? 0;
      const recipientBalanceAfter = updatedRecipientWallet?.balance?.available ?? 0;
      ...
      try {
        await ledgerService.recordEntry({
          ...
          amount,
          coinType: transfer.coinType || 'rez',
      ...
      await Promise.all([
        ...
        logger.error('[TransferCtrl] Wallet cache invalidation failed after transfer', {
          error: err.message,
      ...
      try {
        const recipientPhone = recipientUser?.phoneNumber;
        if (recipientPhone) {

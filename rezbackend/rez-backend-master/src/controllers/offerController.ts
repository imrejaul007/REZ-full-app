...
import { UserOfferUsage } from '../models/UserOfferUsage';
...
export const getOffers = asyncHandler(async (req: Request, res: Response) => {
  ...
  const filter: any = {
    ...
    // Only show offers that have NOT been explicitly rejected by admin.
    // Offers with adminApproved: true or adminApproved absent (legacy) are shown;
    // offers with adminApproved: false (rejected) are hidden.
...
export const searchOffers = asyncHandler(async (req: Request, res: Response) => {
  ...
  const [offers, total] = await Promise.all([
    Offer.find(filter, { score: { $meta: 'textScore' } })
      ...
      .skip(skip)
      .limit(limitNum)
      .select('-adminNotes -approvedBy -rejectedBy -adminApproved -eligibility.requiredZones')
      .lean(),
    Offer.countDocuments(filter),
  ]);
...
export const getOffersByStore = asyncHandler(async (req: Request, res: Response) => {
  ...
  // H28: Only show admin-approved offers on consumer-facing route.
  // adminApproved: true means explicitly approved; absent means legacy (show by default);
  // adminApproved: false means rejected — hide these.
  filter.adminApproved = { $ne: false };
  ...
  // C14: Post-query zone filter — if offer.zone is set and user has a known city,
  // exclude offers whose zone does not match the user's city. If user city is unknown,
  // be inclusive and show all offers.
  const userCity: string | undefined = (req.user as any)?.city || (req.user as any)?.profile?.location?.city;
  const zoneFiltered = userCity ? offers.filter((o: any) => !o.zone || o.zone === userCity) : offers;

  sendPaginated(res, zoneFiltered, pageNum, limitNum, total, 'Offers fetched successfully');
});
...
export const redeemOffer = asyncHandler(async (req: Request, res: Response) => {
  ...
  // C15: Enforce Offer.usageLimit via UserOfferUsage collection (per-user counter).
  // This is separate from the global OfferRedemption count check below — it tracks
  // per-user usage across sessions and is the authoritative per-user gate.
  if (offer.restrictions.usageLimit && offer.restrictions.usageLimit > 0) {
    const userUsage = await UserOfferUsage.findOne({ userId: new mongoose.Types.ObjectId(userId), offerId: offer._id });
    if (userUsage && userUsage.count >= offer.restrictions.usageLimit) {
      return sendError(res, 'Offer usage limit reached', 400, { code: 'OFFER_LIMIT_REACHED' });
    }
  }
  ...
  // C15: Increment per-user usage counter (upsert so first use also creates the record).
  if (offer.restrictions.usageLimit && offer.restrictions.usageLimit > 0) {
    await UserOfferUsage.findOneAndUpdate(
      { userId: new mongoose.Types.ObjectId(userId), offerId: offer._id },
      { $inc: { count: 1 }, $set: { lastUsedAt: new Date() } },
      { upsert: true },
    ).catch((err) =>
      logger.warn('[OfferCtrl] Failed to increment UserOfferUsage counter', {
        userId,
        offerId: id,
        error: err.message,
      }),
  ...
  // Send merchant notification for voucher redemption (non-blocking)
  setImmediate(async () => {
    try {
      const storeId = offer.store?.id?.toString() || offer.store?.toString();
      if (storeId) {
        ...
        if (store && store.merchantId) {
          ...
          const customerName = user?.profile?.firstName
            ? `${user.profile.firstName} ${user.profile.lastName || ''}`.trim()
            : 'Customer';
          const discountValue = offer.cashbackPercentage || 0;
          ...
          await merchantNotificationService.notify({
            ...
            message: `🎟️ ${customerName} used ${offer.title} — ₹${discountValue} off`,
          });
        }
      }
    } catch (err) {
      logger.warn('[OFFER] Merchant notification failed (non-blocking)', { error: err });
    }
  });
...
export const getNearbyOffers = asyncHandler(async (req: Request, res: Response) => {
  ...
  let offers = await Offer.findNearbyOffers(userLocation, Number(maxDistance));

  // Filter by time window if configured on the offer
  const currentHour = new Date().getHours();
  const currentDay = new Date().getDay();
  offers = offers.filter((o) => {
    if (!o.timeWindow) return true; // no time restriction
    if (o.timeWindow.activeDays?.length && !o.timeWindow.activeDays.includes(currentDay)) return false;
    return currentHour >= (o.timeWindow.startHour ?? 0) && currentHour < (o.timeWindow.endHour ?? 24);
  });
...
export const getOffersPageData = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (userId) {
    ...
    if (wallet) {
      ...
      // Fallback: user.wallet.balance (walletBalance field does not exist on IUser)
      const user = await User.findById(userId).select('wallet phoneNumber').lean();
      userWalletBalance = (user?.wallet as any)?.balance ?? 0;
    }
  }
...
export const getHeroBanners = asyncHandler(async (req: Request, res: Response) => {
  const { page = 'offers', position: _position = 'top' } = req.query;

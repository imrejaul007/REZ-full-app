...
export const getHotspots = asyncHandler(async (req: Request, res: Response) => {
  ...
  const allHotspots = await HotspotArea.find(query).sort({ priority: -1 }).lean();
  ...
  const hotspots = allHotspots.slice(0, parseInt(limit as string, 10));
  ...
  if (!userId) {
    ...
    const zones = await withCache('offers:exclusiveZones:anon', 300, () =>
      ExclusiveZone.find({ isActive: true })
        .select(
          'name slug description shortDescription icon iconColor backgroundColor image eligibilityType verificationRequired priority isActive cashbackBonusPercent offersCount',
        )
        .sort({ priority: -1 })
        .lean(),
    );
  ...
  const zonesWithEligibility = await withCache(`offers:exclusiveZones:${userId}`, 60, async () => {
    ...
    const [zones, user] = await Promise.all([
      ExclusiveZone.find({ isActive: true })
        ...
        .sort({ priority: -1 })
        .lean(),
      User.findById(userId).select('verifications profile.dateOfBirth profile.gender').lean(),
    ]);
...
export const getDoubleCashbackCampaigns = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (region) {
    filter.$or = [{ region: region }, { region: 'all' }, { region: { $exists: false } }];
  }
...
export const getLoyaltyProgress = asyncHandler(async (req: Request, res: Response) => {
  ...
  const milestones = await LoyaltyMilestone.find({ isActive: true }).sort({ order: 1 }).lean();
...
export const getLoyaltyMilestones = asyncHandler(async (req: Request, res: Response) => {
  const milestones = await withCache('offers:loyaltyMilestones', 3600, () =>
    LoyaltyMilestone.find({ isActive: true }).sort({ order: 1 }).lean(),
  );
  ...
  const result = await Offer.aggregate([
    { $match: baseFilter },
    {
      $facet: {
        ...
        '25off': [{ $match: { cashbackPercentage: { $gte: 25, $lt: 50 } } }, { $count: 'count' }],
        // 50% OFF: cashbackPercentage >= 50 and < 80
        '50off': [{ $match: { cashbackPercentage: { $gte: 50, $lt: 80 } } }, { $count: 'count' }],
        // 80% OFF: cashbackPercentage >= 80
        '80off': [{ $match: { cashbackPercentage: { $gte: 80 } } }, { $count: 'count' }],
        // Free Delivery
        freeDelivery: [{ $match: { isFreeDelivery: true } }, { $count: 'count' }],
      },
    },
  ]);
...
export const getSuperCashbackStores = asyncHandler(async (req: Request, res: Response) => {
  ...
  const storeFilter: any = {
    isActive: true,
    $or: [
      { 'paymentInfo.cashback': { $gte: parseInt(minCashback as string, 10) } },
      { 'paymentInfo.baseCashbackPercent': { $gte: parseInt(minCashback as string, 10) } },
    ],
  };
  ...
  const transformedOffers = offers.map((offer: any) => ({
    ...
    discountPercentage:
      ...
      (offer.metadata?.flashSale?.originalPrice && offer.metadata?.flashSale?.salePrice
        ? Math.round(
            ((offer.metadata.flashSale.originalPrice - offer.metadata.flashSale.salePrice) /
              offer.metadata.flashSale.originalPrice) *
              100,
          )
        : 0),
  }));

...
export const getDiscounts = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (cardType) {
    if (cardType === 'credit' || cardType === 'debit') {
      filter.$or = [{ cardType: cardType }, { cardType: 'all' }, { cardType: { $exists: false } }];
    }
  }
  ...
  if (storeId && mongoose.Types.ObjectId.isValid(storeId as string)) {
    ...
    filter.$and = [{ $or: usageLimitConditions }, { $or: scopeConditions }];
  } else {
    // No storeId: include global discounts and legacy discounts
    filter.$and = [{ $or: usageLimitConditions }, { $or: [{ scope: 'global' }, { scope: { $exists: false } }] }];
  }
  ...
  const [discounts, total] = await Promise.all([
    Discount.find(filter).sort(sortOptions).skip(skip).limit(limitNum).lean(),
...
export const getDiscountById = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!mongoose.isValidObjectId(id)) {
    return sendError(res, 'Invalid discount ID', 400);
  }
...
export const getDiscountsForProduct = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (isNaN(parsedOrderValue) || parsedOrderValue < 0) {
    return sendError(res, 'orderValue must be a non-negative number', 400);
  }
  ...
  const filter: any = {
    ...
    $and: [
      {
        $or: [{ applicableOn: 'all' }, { applicableOn: 'specific_products', applicableProducts: productObjId }],
      },
    ],
  };

  let discountDocs = await Discount.find(filter).sort({ priority: -1, value: -1 });
...
export const validateDiscount = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (orderValue < discount.minOrderValue) {
    return sendError(res, `Minimum order value of ₹${discount.minOrderValue} required`, 400);
  }
  ...
  if (discount.applicableOn === 'specific_categories') {
    ...
    const hasApplicableCategory = categoryIds.some((id: string) => applicableCategoryIds.includes(id));
...
export const applyDiscount = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Find discount (not lean - need to call save later)
  ...
  // Check expiry and usage limit atomically
  const now = new Date();
  if (discount.validFrom > now) {
    return sendError(res, 'This discount is not yet active', 400);
  }
  ...
  // Atomically increment usage count with concurrent safety check
  const updateResult = await Discount.findByIdAndUpdate(
    discountId,
    {
      $inc: { usedCount: 1 },
  ...
  // Verify the update didn't push us over the limit
  if (updateResult && discount.usageLimit && updateResult.usedCount > discount.usageLimit) {
    // Rollback: remove the usage record we just created
    await DiscountUsage.deleteOne({ _id: discountUsage._id });
    return sendError(res, 'This discount has reached its usage limit', 400);
  }
...
export const getBillPaymentDiscounts = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (storeId && mongoose.Types.ObjectId.isValid(storeId as string)) {
    ...
    storeFilter.$or = [{ scope: 'global' }, { scope: { $exists: false } }];
  }
  ...
  let discounts = await Discount.find(filter).sort({ priority: -1, value: -1 }).lean();
...
export const validateCardForOffers = asyncHandler(async (req: Request, res: Response) => {
  ...
  const filter: any = {
    ...
    $or: [{ cardType: 'all' }, { cardType: cardType }],
    $and: [
      {
        $or: [
  ...
  let offers = await Discount.find(filter).sort({ priority: -1, value: -1 }).lean();
...
export const applyCardOffer = asyncHandler(async (req: Request, res: Response) => {
  const { discountId, orderId: _orderId, cardLast4: _cardLast4 } = req.body;
  const userId = req.user?.id;

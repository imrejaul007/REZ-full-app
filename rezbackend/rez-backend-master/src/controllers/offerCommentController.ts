...
export const moderateComment = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // instance methods; a plain object from .lean() has no prototype and the calls
    // throw silently, leaving coins never credited/rejected.
...
export const getCommentableOffers = asyncHandler(async (req: Request, res: Response) => {
  ...
  const [offers, total] = await Promise.all([
    Offer.find(filter).populate('store', 'name logo').sort({ createdAt: -1 }).skip(skip).limit(limitNum).lean(),

...
export const listCashbackRequests = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [cashbackRequests, total] = await Promise.all([
      CashbackMongoModel.find(query).sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
    ...
    return sendInternalError(res, 'An error occurred processing your request');
  }
});
...
export const getCashbackRequest = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  const _merchantId = (req as any).merchantId;
  ...
  try {
    ...
    return sendSuccess(
      res,
      {
        ...
        auditTrail: (cashbackRequest as any).timeline || [],
      },
...
export const createCashbackRequest = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const orderStoreIds = order.items
      .map((item: any) => {
    ...
    const hasMerchantStore = orderStoreIds.some((storeId: string) => merchantStoreIds.includes(storeId));
    ...
    // Calculate cashback based on admin-configurable rate (default 5%).
    // Rate is read from RewardConfig so it can be adjusted without a deploy.
    // Falls back to 5.0 if the config key is absent or Redis is unavailable.
    const cashbackRate = await getRewardConfig('merchant_cashback_rate_pct', 5.0);
    ...
    const customerData = {
      ...
      verificationStatus: (customer.auth?.isVerified ? 'verified' : 'unverified') as
        | 'verified'
        | 'pending'
        | 'unverified',
    };
  ...
  try {
    ...
    if (cashbackRequest.status !== 'approved') {
      return sendBadRequest(
        res,
        `Cannot mark as paid. Current status is '${cashbackRequest.status}'. Must be 'approved'.`,
      );
    }
    ...
    try {
      for (const cashbackId of cashbackIds) {
        try {
          ...
          if (action === 'approve') {
            updatedCashback = await CashbackModel.approve(cashbackId, cashback.requestedAmount, notes, merchantId);
          } else if (action === 'reject') {
            updatedCashback = await CashbackModel.reject(cashbackId, reason || 'Rejected by merchant', merchantId);
          }
          ...
          if (updatedCashback) {
            ...
            try {
              ...
              if (customer && customer.email) {
                const subject = action === 'approve' ? 'Cashback Request Approved' : 'Cashback Request Update';
...
export const getCashbackMetrics = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const currentPaid = currentPeriodRequests
      .filter((r) => r.status === 'paid')
    ...
    const previousPaid = previousPeriodRequests
      .filter((r) => r.status === 'paid')
      .reduce((sum, r) => sum + (r.approvedAmount || r.requestedAmount), 0);
    ...
    const amountGrowth = previousTotal > 0 ? ((currentTotal - previousTotal) / previousTotal) * 100 : 0;

    const approvalGrowth = previousApproved > 0 ? ((currentApproved - previousApproved) / previousApproved) * 100 : 0;

    const paidGrowth = previousPaid > 0 ? ((currentPaid - previousPaid) / previousPaid) * 100 : 0;

    // Processing time metrics
    const completedRequests = requests.filter((r) => r.reviewedAt && r.createdAt);
    const processingTimes = completedRequests.map(
      (r) => (r.reviewedAt!.getTime() - r.createdAt.getTime()) / (1000 * 60 * 60), // hours
    );
    ...
    const lowRiskApprovalRate =
      lowRiskRequests.length > 0
        ? (lowRiskRequests.filter((r) => r.status === 'approved' || r.status === 'paid').length /
            lowRiskRequests.length) *
          100
        : 0;

    const mediumRiskRequests = requests.filter((r) => r.riskScore >= 40 && r.riskScore < 70);
    const mediumRiskApprovalRate =
      mediumRiskRequests.length > 0
        ? (mediumRiskRequests.filter((r) => r.status === 'approved' || r.status === 'paid').length /
            mediumRiskRequests.length) *
          100
        : 0;

    const highRiskRequests = requests.filter((r) => r.riskScore >= 70);
    const highRiskApprovalRate =
      highRiskRequests.length > 0
        ? (highRiskRequests.filter((r) => r.status === 'approved' || r.status === 'paid').length /

...
import { validateSortField, escapeRegex } from '../../utils/sanitize';
import { publishNotificationEvent } from '../../events/notificationQueue';
import pushNotificationService from '../../services/pushNotificationService';
import { isValidMerchantTransition, MERCHANT_TRANSITIONS } from '../../config/orderStateMachine';
import { walletService } from '../../services/walletService';
// BUG-011 FIX: Import at top level instead of dynamic require()
import coinService from '../../services/coinService';
import adminWalletService from '../../services/adminWalletService';
import SystemConfig from '../../models/SystemConfig';

// BED-021 FIX: IOrder does not explicitly declare _id, so TypeScript infers it as
// 'unknown' from the Document base type. This local interface adds the _id override
// so that all downstream property accesses (order._id, order.user) are properly typed.
type MerchantOrderDocument = IOrder & { _id: Types.ObjectId };

// ---------------------------------------------------------------------------
// getCashbackRate — dynamic lookup from SystemConfig 'cashback_rules'
// ROOT CAUSE: coin award on delivery used hardcoded 0.05. Admin-configured rate
// (POST /api/admin/cashback-rules → SystemConfig key 'cashback_rules') was ignored.
// FIX: Read from SystemConfig at runtime with a 5-minute in-process cache.
// ---------------------------------------------------------------------------
interface _CashbackRulesConfig {
  globalSettings: { defaultCashbackRate: number; maxCashbackPerTransaction: number; minOrderForCashback: number };
  categoryMultipliers: Array<{ id: string; categoryName: string; multiplier: number }>;
}
const _cashbackRateCache: { data: { rate: number; expiresAt: number } | null } = { data: null };
const CASHBACK_RATE_CACHE_TTL_MS = 5 * 60 * 1000;
async function getCashbackRate(): Promise<number> {
  const now = Date.now();
  if (_cashbackRateCache.data && _cashbackRateCache.data.expiresAt > now) return _cashbackRateCache.data.rate;
  try {
    const doc = await SystemConfig.findOne({ key: 'cashback_rules' }).lean();
    const config = doc?.value as unknown as _CashbackRulesConfig | undefined;
    const pct = config?.globalSettings?.defaultCashbackRate;
    const rate = typeof pct === 'number' && pct >= 0 && pct <= 100 ? pct / 100 : 0.05;
    _cashbackRateCache.data = { rate, expiresAt: now + CASHBACK_RATE_CACHE_TTL_MS };
    return rate;
  } catch (err: any) {
    logger.warn('[ORDER STATUS] getCashbackRate lookup failed, using fallback 5%', { error: err?.message });
    return 0.05;
  }
}
...
function transformOrderForMerchant(order: any): any {
  ...
  return {
    ...
    items: (order.items || []).map((item: any) => ({
      ...
      productName: item.name || (item.product as unknown as { name?: string })?.name || 'Unknown Item',
      sku: item.sku || '',
      price: item.price || 0,
    ...
    deliveredAt:
      order.delivery?.deliveredAt || order.timeline?.find((t: any) => t.status === 'delivered')?.timestamp || null,
  };
}
...
async function sendOrderStatusNotification(order: any, action: string): Promise<void> {
  try {
    ...
    // DM-H2 fix: user.profile.phoneNumber does not exist on UserSchema.
    // Phone is stored at top-level phoneNumber. user.phone is a virtual, unsafe on lean().
    ...
    const _orderTotal = order.totals?.total || 0;
    ...
    if (userPhone && message.sms) {
      try {
        ...
        logger.info(`✅ [ORDER NOTIFICATION] SMS sent to customer: ***${userPhone.slice(-4)}`);
      } catch (smsError) {
    ...
    if (userEmail && userName) {
      try {
        ...
        await EmailService.sendOrderStatusUpdate(userEmail, userName, {
          orderId,
          orderNumber,
        ...
        logger.info(`✅ [ORDER NOTIFICATION] Email sent to customer (orderId: ${orderId})`);
      } catch (emailError) {
  ...
  try {
    ...
    if (order.items && order.items.length > 0) {
      try {
        ...
        const firstStore = firstItem.store as
          | Types.ObjectId
          | { _id?: Types.ObjectId; name?: string; location?: unknown };
        if (firstStore) {
          if (typeof firstStore === 'object' && 'name' in firstStore) {
            storeInfo = {
              _id: firstStore._id ? String(firstStore._id) : String(firstItem.store),
              name: firstStore.name,
              location: firstStore.location,
            };
          } else {
            storeInfo = { _id: String(firstItem.store), name: undefined, location: undefined };
          }
        } else if (firstItem.product) {
          // Try to get store from product.
          // NOTE: the Product schema field is `store` (not `storeId`).
          const product = (await Product.findById(firstItem.product).select('store').lean()) as {
            store?: Types.ObjectId;
          } | null;
          if (product?.store) {
            // BUG-011 FIX: Use top-level import instead of dynamic import
            const store = await Store.findById(product.store).select('name location').lean();
            if (store) {
              storeInfo = {
...
export const getMerchantOrders = asyncHandler(async (req: Request, res: Response) => {
  ...
  logger.info('🔍 [ORDERS] req.merchantId:', req.merchantId);
  logger.info('🔍 [ORDERS] req.merchant:', req.merchant);
  ...
  try {
    ...
    if (search) {
      ...
      // NOTE: email is a top-level field on User (not profile.email), phoneNumber is also top-level.
      ...
      query.$or = [{ orderNumber: searchRegex }, { user: { $in: userIds } }];
    }
    ...
    const safeSortBy = (ALLOWED_SORT_FIELDS as readonly string[]).includes(rawSortBy) ? rawSortBy : 'createdAt';
    ...
    // Compute summary fields for source-filtered views (e.g. RendezBookingsCard)
    const totalRevenuePaise = source
      ? orders.reduce((sum: number, o: any) => sum + Math.round((o.totals?.total ?? 0) * 100), 0)
      : undefined;
...
export const refundOrder = asyncHandler(async (req: Request, res: Response) => {
  ...
  // BUG-004 FIX: Validate merchantId before proceeding
  if (!merchantId) {
  ...
  try {
    ...
    // BUG-003 FIX: Verify merchant owns this order
    const merchantStores = await Store.find({ merchantId: new Types.ObjectId(merchantId) })
      .select('_id')
      .session(session)
      .lean();
    ...
    const orderBelongsToMerchant = merchantStoreIds.some((storeId) => order.store?.toString() === storeId.toString());
    ...
    try {
      switch (paymentMethod) {
        case 'razorpay': {
          ...
          const razorpayRefund = await createRazorpayRefund(order.payment.transactionId, amount, {
            notes: {
        ...
        case 'wallet': {
          ...
          // Get user wallet — order.user may be an ObjectId or a populated user doc
          const user = order.user as Types.ObjectId | { _id?: Types.ObjectId };
          const userId = typeof user === 'string' ? user : '_id' in user ? user._id?.toString() : String(user);
    ...
    if (refundItems && refundItems.length > 0) {
      ...
      for (const refundItem of refundItems) {
        const orderItem = order.items.find((item: any) => item._id.toString() === refundItem.itemId);
      ...
      // Full refund - restore all items only when goods were never delivered/dispatched.
      // BUG-29 FIX: Restocking a delivered or dispatched order would inflate inventory
      // with units the merchant already handed to the customer.
      if (['delivered', 'dispatched'].includes(order.status as string)) {
        logger.info('📦 [REFUND] Full refund on delivered/dispatched order — skipping inventory restock');
      } else {
      ...
      } // end else (not delivered/dispatched)
    }
    ...
    const refundRecord = new Refund({
      ...
      status:
        gatewayRefundStatus === 'completed' || gatewayRefundStatus === 'processed'
          ? 'completed'
          : gatewayRefundStatus === 'pending_manual_processing'
            ? 'pending'
            : 'processing',
    ...
    // BUG-28 FIX: Atomic guard prevents concurrent over-refund.
    // The filter condition ensures that adding `amount` to the current
    // refundAmount will not exceed paidAmount — if another request already
    // incremented refundAmount between our snapshot read and this write, the
    // filter will not match and the update will return null, letting us
    // abort instead of persisting an over-refund.
    const orderUpdateResult = await Order.findOneAndUpdate(
      {
        _id: order._id,
        $expr: {
          $lte: [{ $add: [{ $ifNull: ['$totals.refundAmount', 0] }, amount] }, '$totals.paidAmount'],
        },
      },
      {
        $set: {
          'totals.refundAmount': (order.totals.refundAmount || 0) + amount,
          'payment.status': order.payment.status,
          'payment.refundId': gatewayRefundId,
          'payment.refundedAt': order.payment.refundedAt,
    ...
    if (!orderUpdateResult) {
      ...
      return sendError(
        res,
        `Refund rejected: concurrent refund would exceed paid amount (₹${order.totals.paidAmount}). Please retry.`,
        409,
      );
    }
    ...
    if (paymentMethod === 'cod') {
      try {
        ...
        if (adminEmail) {
          await EmailService.sendAdminRefundRequestNotification(adminEmail, {
            orderNumber: order.orderNumber || (order._id as any)?.toString(),
            refundAmount: amount,
            refundType: 'full',
            refundReason: 'COD refund - requires manual processing',
            customerName: 'Customer',
            refundId: refundRecord._id ? String(refundRecord._id) : '',
          });
    ...
    if (notifyCustomer) {
      try {
        ...
        let userForNotification = order.user as
          | Types.ObjectId
          | {
              phoneNumber?: string;
              email?: string;
        ...
        if (typeof userForNotification === 'string' || userForNotification instanceof mongoose.Types.ObjectId) {
          userForNotification =
            ((await User.findById(userForNotification).lean()) as unknown as typeof userForNotification) ??
            userForNotification;
        }
        const user = userForNotification as any;
        ...
        if (userPhone) {
          logger.info(`📱 [REFUND] Sending SMS to customer: ***${userPhone.slice(-4)}`);
        ...
        if (userEmail && userName) {
          logger.info(`📧 [REFUND] Sending email to customer (userId: ${userId})`);

          await EmailService.sendRefundConfirmation(userEmail, userName, {
            orderNumber: order.orderNumber,
            refundAmount: amount,
            refundType: isPartialRefund ? 'partial' : 'full',
...
export const updateMerchantOrderStatus = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const merchantStores = await Store.find({ merchantId: new Types.ObjectId(merchantId) })
      .select('_id')
      .lean();
    const _merchantStoreIds = merchantStores.map((s) => s._id.toString());

    // BED-021 FIX: Explicit type annotation needed because the Mongoose query chain
    // with .session() makes TypeScript infer order._id as 'unknown'. Using
    // MerchantOrderDocument (IOrder + _id override) properly types both _id and user.
    const order = (await Order.findOne({
      _id: new Types.ObjectId(id),
      'items.store': { $in: merchantStores.map((s) => s._id) },
    }).session(session)) as MerchantOrderDocument;
    ...
    const allowedNextStatuses = MERCHANT_TRANSITIONS[currentStatus] || [];

    if (!isValidMerchantTransition(currentStatus, status)) {
      await session.abortTransaction();
      session.endSession();
      return sendBadRequest(
        res,
        `Cannot change status from "${currentStatus}" to "${status}". Allowed: ${allowedNextStatuses.join(', ') || 'none'}`,
      );
    }
    ...
    // ── BullMQ push notification for tracked status transitions ──────────────
    const TRACKED_TRANSITIONS: Record<string, boolean> = {
      confirmed: true,
      preparing: true,
      delivered: true,
      cancelled: true,
    ...
    if (TRACKED_TRANSITIONS[status]) {
      // BED-021 FIX: Replace `as any` with proper TypeScript types.
      // order.user is Types.ObjectId (populated or not); order._id is always Types.ObjectId.
      const orderUserId = order.user?.toString() ?? String(order.user);
      const orderId = order._id.toString();

      publishNotificationEvent({
        eventId: `order_update:${orderId}:${status}`,
        ...
        logger.warn('[ORDER STATUS] BullMQ notification enqueue failed (non-fatal)', {
          orderId,
          status,
          error: err?.message,
        });
    ...
    // Send customer push notifications for key status transitions (fire-and-forget)
    {
      ...
      const orderNumber = order.orderNumber || String(order._id);
      const orderId = order._id.toString();
      const storeName = order.items?.[0]?.storeName || 'your store';

      const pushMessages: Record<string, { title: string; body: string }> = {
        confirmed: {
          title: 'Order Confirmed',
          body: `${storeName} is preparing your order #${orderNumber}`,
        },
        ready: {
          title: 'Order Ready',
      ...
      if (pushMsg) {
        pushNotificationService
          ...
          .catch((err) => logger.warn('[ORDER STATUS] Customer push notification failed (non-fatal):', err));
      }
    }
    ...
    if (status === 'delivered') {
      ...
      if (populatedOrder) {
        const populatedUser = populatedOrder.user as Types.ObjectId | { _id: Types.ObjectId };
        const userIdObj =
          typeof populatedUser === 'object' && '_id' in populatedUser ? populatedUser._id : populatedUser;

        // 1. Award purchase reward coins — rate from admin-configured SystemConfig 'cashback_rules'
        try {
          // BUG-011 FIX: Use top-level import instead of require()
          const cashbackRate = await getCashbackRate();
          const cashbackRatePct = Math.round(cashbackRate * 100);
          const coinsToAward = Math.floor((populatedOrder.totals.subtotal || 0) * cashbackRate);
          if (coinsToAward > 0) {
            await coinService.awardCoins(
              userIdObj.toString(),
              coinsToAward,
              'purchase_reward',
              `${cashbackRatePct}% purchase reward for order ${populatedOrder.orderNumber}`,
              { orderId: populatedOrder._id, referenceId: `purchase_reward:${populatedOrder._id}` },
            );
            logger.info(`✅ [ORDER STATUS] Purchase reward: ${coinsToAward} coins (rate: ${cashbackRatePct}%)`);
          }
        } catch (err) {
        ...
        try {
          ...
          if (firstItem && firstItem.store) {
            const storeField = firstItem.store as Types.ObjectId | { _id: Types.ObjectId };
            const storeId = typeof storeField === 'object' && '_id' in storeField ? storeField._id : storeField;
    ...
    const transformed = updatedOrder
      ? transformOrderForMerchant(updatedOrder)
      : {
          orderId: order._id,
          orderNumber: order.orderNumber,

...
export const listSocialMediaPosts = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    logger.info(
      '📱 [MERCHANT SOCIAL] Found stores for this merchant:',
      stores.map((s) => ({ id: s._id, name: s.name })),
    );
...
export const getSocialMediaStats = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const approvalRate =
      totalReviewed > 0 ? Math.round(((result.approved + result.credited) / totalReviewed) * 100) : 0;
  ...
  try {
    ...
    if (post.status !== 'pending') {
      ...
      return sendBadRequest(
        res,
        `Cannot approve post with status '${post.status}'. Only pending posts can be approved.`,
      );
    }
    ...
    try {
      await AuditLog.log({
        ...
        merchantUserId: merchantUserId
          ? new Types.ObjectId(merchantUserId)
  ...
  try {
    ...
    const result = await coinService.awardCoins(
      ...
      `Social media cashback (${postPlatform}) for order ${postOrderId}`,
      {
        ...
        orderId: postOrderId,
        referenceId: `social_share_reward:${savedPostId}`,
      },
    );
    newBalance = result.newBalance || 0;
    logger.info(
      `✅ [MERCHANT SOCIAL] Post approved and ${cashbackAmount} REZ coins credited to user via CoinTransaction`,
    );
  } catch (coinError) {
...
export const rejectSocialMediaPost = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (post.status !== 'pending') {
      return sendBadRequest(
        res,
        `Cannot reject post with status '${post.status}'. Only pending posts can be rejected.`,
      );
    }

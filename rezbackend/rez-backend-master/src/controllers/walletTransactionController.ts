...
export const getTransactions = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { page = 1, limit = 20, type, category, status, startDate, endDate, minAmount, maxAmount } = req.query;
  ...
  const transactions = await Transaction.getUserTransactions(userId, filters, Number(limit), skip);

  // Build a Mongo-native query for countDocuments — dateRange and amountRange are
  // app-level objects consumed by getUserTransactions, not real Mongo fields.
  const countQuery: any = { user: userId };
  ...
  if (startDate || endDate) {
    ...
    if (startDate) countQuery.createdAt.$gte = new Date(startDate as string);
    if (endDate) countQuery.createdAt.$lte = new Date(endDate as string);
  }
  if (minAmount || maxAmount) {
    countQuery.amount = {};
    if (minAmount) countQuery.amount.$gte = Number(minAmount);
    if (maxAmount) countQuery.amount.$lte = Number(maxAmount);
  }
...
export const getTransactionById = asyncHandler(async (req: Request, res: Response) => {
  ...
  // SECURITY: IDOR Guard — Verify transaction belongs to authenticated user
  // Transaction queries must always filter by user ID to prevent cross-tenant data access
...
export const getTransactionSummary = asyncHandler(async (req: Request, res: Response) => {
  ...
  const summary = await Transaction.getUserTransactionSummary(userId, period as 'day' | 'week' | 'month' | 'year');

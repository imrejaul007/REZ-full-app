...
import PriveVoucher, { VoucherType } from '../models/PriveVoucher';
...
const aggregateWeeklyEarnings = async (userObjectId: mongoose.Types.ObjectId) => {
  ...
  const [result] = await withCache(cacheKey, 300, () =>
...
const buildHabitLoops = async (
  ...
  const loopDefs = (habitConfig?.loops || DEFAULT_LOOPS).filter(
    (l: any) => (l as { enabled?: boolean }).enabled !== false,
  );
...
export const refreshEligibility = asyncHandler(async (req: Request, res: Response) => {
  ...
  await reputationService.recalculateReputation(userId, 'user_refresh');
...
export const getImprovementTips = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { pillars, factors: _factors } = await reputationService.getPillarBreakdown(userId);
...
export const getPriveDashboard = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Run access check in parallel with all user-independent queries to eliminate
  // the sequential bottleneck that previously blocked every dashboard load.
  // featuredOffers depends on userTier, so it is fetched in a second micro-batch
  // once accessCheck resolves — still saves one full round-trip vs. sequential.
  const [
    accessCheck,
    user,
    wallet,
    ...
    (async () => {
      try {
        ...
        const result = await priveNotificationService.getNotifications(userId, 'none');
        return { counts: result.counts, topUrgent: result.notifications.slice(0, 3) };
      } catch {
        return { counts: { critical: 0, warning: 0, info: 0 }, topUrgent: [] };
      }
    })(),
  ...
  // Fetch featured offers now that we know userTier (depends on accessCheck result)
  const featuredOffers = await PriveOffer.findFeaturedOffers(userTier, 3);
  ...
  const dashboard = {
    ...
    highlights: await (async () => {
      ...
      if (!isNaN(lat) && !isNaN(lng)) {
        const stores = await Store.find({
          ...
          .select('name logo slug location tags')
          .limit(1)
          .lean();
        if (stores.length) {
          nearbyStore = {
      ...
      const opportunity = campaign
        ? {
            ...
            subtitle: campaign.description || 'Limited time opportunity',
            badge: 'Live',
            badgeColor: '#FF9800',
          }
        : null;
    ...
    stats: {
      activeCampaigns,
      completedCampaigns,
      avgRating: avgRatingResult?.[0]?.avgRating ? parseFloat(avgRatingResult[0].avgRating.toFixed(1)) : null,
    },
    user: {
      name:
        user?.fullName ||
...
export const getPriveOffers = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { page = 1, limit = 10, category, tier: _tierFilter } = req.query;
  ...
  const accessibleTiers = Object.keys(tierHierarchy).filter((t) => tierHierarchy[t] <= userTierLevel);
  ...
  const filteredOffers = allActiveOffers.filter((offer) => {
...
export const getPriveOfferById = asyncHandler(async (req: Request, res: Response) => {
  ...
  PriveOffer.updateOne({ _id: id }, { $inc: { views: 1 } })
    .exec()
    .catch((err) => logger.error('[PriveCtrl] Offer view increment failed', { error: err.message, offerId: id }));
...
export const getEarnings = asyncHandler(async (req: Request, res: Response) => {
  ...
  const resultEarnings = cursor && earnings.length > limitNum ? earnings.slice(0, limitNum) : earnings;
...
export const getTransactions = asyncHandler(async (req: Request, res: Response) => {
  ...
  const resultTransactions = cursor && transactions.length > limitNum ? transactions.slice(0, limitNum) : transactions;
...
export const redeemCoins = asyncHandler(async (req: Request, res: Response) => {
  ...
  const minCoinsMap: Record<string, number> = rc?.minCoinsPerCategory || {
    gift_card: 500,
    bill_pay: 100,
    experience: 1000,
    charity: 100,
  };
  const minCoins = minCoinsMap[type] || 100;
  ...
  if (existingTx) {
    ...
    if (existingVoucher) {
      return res.status(200).json({
        ...
        data: {
          voucher: {
            id: existingVoucher._id.toString(),
            code: existingVoucher.code,
            type: existingVoucher.type,
            value: existingVoucher.value,
            currency: existingVoucher.currency,
            coinAmount: existingVoucher.coinAmount,
            expiresAt: existingVoucher.expiresAt,
            expiresIn: calculateExpiresIn(existingVoucher.expiresAt),
            status: existingVoucher.status,
            partnerName: existingVoucher.partnerName,
            partnerLogo: existingVoucher.partnerLogo,
            category: existingVoucher.category,
            terms: existingVoucher.terms,
            howToUse: existingVoucher.howToUse,
          },
          wallet: {
  ...
  const conversionRates: Record<string, number> = rc?.conversionRates || {
    gift_card: 0.1,
    bill_pay: 0.1,
    experience: 0.12,
    charity: 0.15,
  };
  const rate = conversionRates[type] || 0.1;
  const voucherValue = Math.round(coinAmount * rate * 100) / 100;

  // Calculate expiry from WalletConfig (not hardcoded helper)
  const expiryDaysMap: Record<string, number> = rc?.expiryDays || {
    gift_card: 365,
    bill_pay: 30,
    experience: 90,
    charity: 7,
  };
  const expiryDays = expiryDaysMap[type] || 30;
  ...
  invalidateWalletCache(userId).catch((err) =>
    logger.error('[PriveCtrl] Wallet cache invalidation failed after coin redemption', { error: err.message, userId }),
  );
...
export const getRedeemConfig = asyncHandler(async (req: Request, res: Response) => {
  ...
  return res.status(200).json({
    success: true,
    data: {
      conversionRates: rc?.conversionRates || { gift_card: 0.1, bill_pay: 0.1, experience: 0.12, charity: 0.15 },
...
export const getCatalog = asyncHandler(async (_req: Request, res: Response) => {
  ...
  const config = await getCachedWalletConfig();
  const enabledCategories = config?.redemptionConfig?.enabledCategories || [
    'gift_card',
    'bill_pay',
    'experience',
    'charity',
...
export const getTierComparison = asyncHandler(async (req: Request, res: Response) => {
  ...
  const [config, accessCheck] = await Promise.all([getCachedWalletConfig(), priveAccessService.checkAccess(userId)]);
  ...
  const tiers = (pc?.tiers || []).map((t: any) => ({
    ...
    threshold:
      t.tier === 'entry'
        ? pc?.tierThresholds.entryTier
        : t.tier === 'signature'
          ? pc?.tierThresholds.signatureTier

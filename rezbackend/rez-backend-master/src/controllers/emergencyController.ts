...
export const getEmergencyContacts = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (city) {
      query.$or = [{ city: new RegExp(escapeRegex(city as string), 'i') }, { isNational: true }];
    }

    if (state) {
      query.$or = query.$or || [];
      query.$or.push({ state: new RegExp(escapeRegex(state as string), 'i') }, { isNational: true });
    }
    ...
    const contacts = await EmergencyContact.find(query).sort({ priority: 1, name: 1 }).lean();
    ...
    return sendError(res, 'An error occurred processing your request', 500);
  }
});
...
export const getNearbyContacts = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const nearbyContacts = contacts
      .map((contact: any) => {
        const distance = calculateDistance(lat, lng, contact.coordinates.latitude, contact.coordinates.longitude);
...
export const bookEmergencyService = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!serviceType || !emergencyType || !patientName || !patientPhone || !pickupAddress?.address) {
    return sendError(
      res,
      'Missing required fields: serviceType, emergencyType, patientName, patientPhone, pickupAddress',
      400,
    );
  }

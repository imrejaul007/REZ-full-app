...
export const getActivityById = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!mongoose.isValidObjectId(id)) {
    return sendBadRequest(res, 'Invalid activity ID');
  }
...
export const createActivity = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!title || typeof title !== 'string' || title.trim().length === 0) {
    return sendBadRequest(res, 'Activity title is required');
  }
  if (title.trim().length > 200) {
    return sendBadRequest(res, 'Activity title must not exceed 200 characters');
  }
  if (amount !== undefined && (isNaN(Number(amount)) || Number(amount) < 0)) {
    return sendBadRequest(res, 'Activity amount must be a non-negative number');
  }

  // Get default icon and color if not provided
  const defaults = getActivityTypeDefaults(type as ActivityType);

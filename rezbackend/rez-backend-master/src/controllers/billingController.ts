...
export const getBillingHistory = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Get subscription billing records (count is derived from combined billingHistory below)
  ...
  for (const sub of subscriptions) {
    ...
    billingHistory.push({
      ...
      tier: sub.tier,
      type: 'subscription',
      invoiceUrl: sub.razorpaySubscriptionId ? `/api/billing/invoice/${sub._id}` : undefined,
  ...
  if (Types.ObjectId.isValid(transactionId)) {
    ...
    if (subscription) {
      invoice = {
        ...
        tax: 0,
        discount: subscription.metadata?.promoCode ? Math.round(subscription.price * 0.1 * 100) / 100 : 0, // Assume 10% promo discount, rounded to 2 decimals
        ...
        // Additional info
        notes: subscription.metadata?.promoCode ? `Promo code applied: ${subscription.metadata.promoCode}` : undefined,
      };
    }
  }
...
export const getBillingSummary = asyncHandler(async (req: Request, res: Response) => {
  ...
  const totalTransactions =
    subscriptions.filter((sub) => sub.status === 'active' || sub.status === 'expired').length + payments.length;
  ...
  res.status(200).json({
    ...
    data: {
      ...
      netSavings: Math.round((totalSavings - totalSpent) * 100) / 100,
      currentTier: currentSubscription?.tier || 'free',
      memberSince: subscriptions.length > 0 ? subscriptions[subscriptions.length - 1].createdAt : null,

...
export const createVideo = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    CacheInvalidator.invalidateVideo(String(video._id)).catch((err) =>
      logger.warn('[Video] Cache invalidation after video upload failed', { error: err.message }),
    );
...
export const getVideos = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const cacheKey = search
      ? null
      : `video:list:${category || 'all'}:${contentType || 'all'}:${sortBy}:${page}:${limit}`;
    ...
    if (cacheKey) {
      redisService
        .set(cacheKey, result, VIDEO_CACHE_TTL.LIST)
        .catch((err) => logger.warn('[Video] Cache set for video list failed', { error: err.message }));
    }
...
export const getVideoById = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!mongoose.isValidObjectId(videoId)) {
    return sendBadRequest(res, 'Invalid video ID');
  }
...
export const getVideosByCategory = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    redisService
      .set(cacheKey, result, VIDEO_CACHE_TTL.CATEGORY)
      .catch((err) => logger.warn('[Video] Cache set for category videos failed', { error: err.message }));
...
export const getTrendingVideos = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!videos) {
      ...
      redisService
        .set(cacheKey, videos, VIDEO_CACHE_TTL.TRENDING)
        .catch((err) => logger.warn('[Video] Cache set for trending videos failed', { error: err.message }));
    }

    // Transform videos to include isLiked status and flatten data (per-user, not cached)
    const transformedVideos = videos.map((video: any) => {
      const isLiked =
        userId && video.engagement?.likes
...
export const getVideosByCreator = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const creator = await User.findById(creatorId)
      .select('profile.firstName profile.lastName profile.avatar profile.bio')
      .lean();
...
export const toggleVideoLike = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const existing = await Video.findOne({ _id: videoId, likedBy: userObjectId }, { _id: 1 }).lean();
    ...
    logger.info(
      `✅ [toggleVideoLike] User ${userId} ${isLiked ? 'liked' : 'unliked'} video ${videoId}. Total likes: ${totalLikes}`,
    );
...
export const searchVideos = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    // SECURITY: escapeRegex() is already imported — must be applied here.
    // Passing req.query.q directly as a $regex value allows ReDoS and regex-operator injection.
    // An attacker could submit `(a+)+$` or similar to cause catastrophic backtracking.
    const safeSearchText = escapeRegex((searchText as string).trim());
    const query: any = {
      isPublished: true,
      isApproved: true,
      $or: [
        ...
        { tags: { $in: [new RegExp(safeSearchText, 'i')] } },
      ],
    };
...
export const getVideosByStore = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const query: any = {
      ...
      $or: [{ contentType: 'ugc', isApproved: true, moderationStatus: 'approved' }, { contentType: 'merchant' }],
    };
    ...
    const content = videos.map((video: any) => {
      ...
      const isBookmarked = userId ? video.bookmarkedBy?.some((id: any) => id.toString() === userId) || false : false;
    ...
    redisService
      .set(cacheKey, result, VIDEO_CACHE_TTL.STORE)
      .catch((err) => logger.warn('[Video] Cache set for store videos failed', { error: err.message }));
...
export const toggleCommentLike = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const comment = (updated.comments as any[])?.find((c: any) => c._id.toString() === commentId);

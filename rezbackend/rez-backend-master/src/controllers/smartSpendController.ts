...
export const getSmartSpendCatalog = asyncHandler(async (req: Request, res: Response) => {
  ...
  const accessibleTiers = Object.keys(TIER_HIERARCHY).filter((t) => TIER_HIERARCHY[t] <= userTierLevel);
...
export const getSmartSpendItem = asyncHandler(async (req: Request, res: Response) => {
  ...
  const item = await SmartSpendItem.findByIdAndUpdate(id, { $inc: { views: 1 } }, { new: true })
...
export const trackSmartSpendClick = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Fire-and-forget click increment — intentionally not awaited (analytics are non-critical).
  // Added .catch() to surface errors in logs instead of generating an unhandled rejection.
  SmartSpendItem.findByIdAndUpdate(id, { $inc: { clicks: 1 } })
    .exec()
    .catch((err: any) => logger.warn('[SMART SPEND] Click increment failed (non-critical):', err.message));

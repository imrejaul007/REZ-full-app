...
export const getPointBalance = asyncHandler(async (req: Request, res: Response) => {
  ...
  const [loyalty, currentBalance, _subscription] = await Promise.all([
...
export const getRewardsCatalog = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (minPoints) {
    filter.rewardCoins = { ...filter.rewardCoins, $gte: parseInt(minPoints as string, 10) || 0 };
  }
  if (maxPoints) {
    filter.rewardCoins = { ...filter.rewardCoins, $lte: parseInt(maxPoints as string, 10) || 999999 };
  }
  ...
  sendSuccess(
    res,
    {
      ...
      filters: {
        ...
        minPoints: minPoints ? parseInt(minPoints as string, 10) : null,
        maxPoints: maxPoints ? parseInt(maxPoints as string, 10) : null,
        search: search || null,
      },
    },
...
export const getChallenges = asyncHandler(async (req: Request, res: Response) => {
  ...
  const progressMap = new Map(progressRecords.map((p) => [p.challenge.toString(), p]));
...
export const dailyCheckInGame = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const bonusInfo =
      coinsEarned > 10 ? { points: coinsEarned - 10, message: `Streak bonus! ${newStreak} days in a row` } : undefined;
...
export const getCheckInStatus = asyncHandler(async (req: Request, res: Response) => {
  ...
  const streak = {
    ...
    longest: loyalty?.streak?.history ? Math.max(loyalty.streak.current || 0, loyalty.streak.history.length) : 0,
...
export const claimChallengeReward = asyncHandler(async (req: Request, res: Response) => {
  ...
  await coinService.awardCoins(
    ...
    { challengeId, idempotencyKey, referenceId: idempotencyKey },
    null,
  );
...
export const redeemReward = asyncHandler(async (req: Request, res: Response) => {
  ...
  const idempotencyKey = `loyalty_redeem_${userId}_${rewardId}_${randomUUID()}`;
  await coinService.deductCoins(userId, totalCost, 'redemption', `Redeemed: ${milestone.title} x${quantity}`, {
    rewardId,
    quantity,
    idempotencyKey,
  });

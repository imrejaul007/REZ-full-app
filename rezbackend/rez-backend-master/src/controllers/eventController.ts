...
// @access  Public
//
// SECURITY NOTE — ObjectId Enumeration Risk:
// This endpoint returns raw MongoDB ObjectIds (_id) for every event in the listing.
// Because ObjectIds are sequential by timestamp (first 4 bytes = Unix epoch), an attacker
// can harvest an _id from one response and increment/decrement it to enumerate adjacent
// document IDs across the entire collection — including non-published events if any other
// endpoint accepts an _id without a `status: 'published'` guard.
//
// Recommended mitigations (apply when feasible):
//   1. Add a `status: 'published'` check to EVERY endpoint that looks up by _id (already done here).
//   2. Expose a separate, opaque `slug` or `publicId` field (e.g. nanoid) for URLs, and accept
//      only that field on public routes — never the raw _id.
//   3. If raw _ids must be returned, consider response-shaping to omit _id and use slug only.
...
//
// SECURITY NOTE — ObjectId Enumeration Risk:
// This endpoint accepts a raw MongoDB ObjectId from the URL. While the `status: 'published'`
// guard below prevents access to draft/private events, the publicly returned `_id` in listing
// responses allows sequential enumeration of all ObjectIds near any visible event.
// See mitigation notes on getAllEvents above. Ensure all branch paths enforce status check.
...
export const getEventsByCategory = asyncHandler(async (req: Request, res: Response) => {
  ...
  const events = await Event.find(query).sort({ featured: -1, priority: -1, date: 1 }).limit(limit).skip(offset).lean();
...
export const searchEvents = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (category) {
    query.category = safeRegexQuery(category as string);
  }
...
export const getFeaturedEvents = asyncHandler(async (req: Request, res: Response) => {
  ...
  const events = await Event.find(query).sort({ priority: -1, date: 1 }).limit(limit).lean();
...
export const bookEventSlot = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (
    attendeeInfo.age !== undefined &&
    (typeof attendeeInfo.age !== 'number' || attendeeInfo.age < 0 || attendeeInfo.age > 150)
  ) {
  ...
  if (attendeeInfo.specialRequirements)
    attendeeInfo.specialRequirements = attendeeInfo.specialRequirements.trim().substring(0, 500);
  ...
  if (event.availableSlots && event.availableSlots.length > 0) {
    ...
    if (!slotUpdate) {
      ...
      const existingEvent = await Event.findOne({
        _id: id,
        'availableSlots.id': slotId,
        'availableSlots.available': true,
      }).lean();
      if (!existingEvent) {
  ...
  if (ticketTypeId && event.ticketTypes && event.ticketTypes.length > 0) {
    const ticketType = event.ticketTypes.find(
      (t: any) => t._id?.toString() === ticketTypeId || t.name === ticketTypeId,
    );
    if (ticketType) {
  ...
  if (!isFree && bookingAmount > 0) {
    try {
      ...
      const sessionId =
        paymentResponse.paymentUrl && typeof paymentResponse.paymentUrl === 'string'
  ...
  if (paymentIntentId) {
    try {
      ...
      if (payment) {
        ...
        logger.warn(
          '⚠️ [EVENT BOOKING] Payment not found yet — proceeding with confirmation. Webhook will validate:',
          paymentIntentId,
        );
      }
    } catch (error) {
    ...
    res.json({
      ...
      data: {
        ...
        reward: rewardResult?.success
          ? { coinsAwarded: rewardResult.coinsAwarded, message: rewardResult.message }
          : null,
      },
    });
  } catch (error: any) {
  ...
  try {
    ...
    const results = await Promise.allSettled(
      events.map(async (eventData: any) => {
        const { eventId, eventType, metadata: _metadata } = eventData;
...
export const checkInToEvent = asyncHandler(async (req: Request, res: Response) => {
  ...
  let rewardResult = null;
  try {
    rewardResult = await eventRewardService.grantEventReward(userId.toString(), id, bookingId, 'checkin_reward', {
      eventName: event.title,
...
export const getMyEvents = asyncHandler(async (req: Request, res: Response) => {
  ...
  const bookings = await EventBooking.find(bookingFilter).populate('eventId').sort({ createdAt: -1 }).limit(50).lean();

  // For past bookings, enrich with reward info (batch query instead of N+1)
  if (tab === 'past' && bookings.length > 0) {
    const eventIds = bookings.map((b) => (b as any).eventId?._id).filter(Boolean);
    const attendances = await EventAttendance.find({
      eventId: { $in: eventIds },
      userId,
    }).lean();
    const attendanceMap = new Map(attendances.map((a) => [String(a.eventId), a]));

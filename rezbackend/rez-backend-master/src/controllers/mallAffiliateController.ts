...
import { sendSuccess, sendPaginated, sendNotFound, sendBadRequest, sendUnauthorized } from '../utils/response';
 ...
 */
const checkIdempotency = async (idempotencyKey: string): Promise<{ isDuplicate: boolean; cachedResponse?: any }> => {
 ...
 */
const storeIdempotency = async (idempotencyKey: string, response: any): Promise<void> => {
  await redisService.set(`webhook_idem:${idempotencyKey}`, response, IDEMPOTENCY_TTL);
};

// Helper function to log webhook request
const logWebhookRequest = async (req: Request, webhookType: AffiliateWebhookType): Promise<Types.ObjectId | null> => {
  try {
...
export const getUserClicks = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { clicks, total, pages: _pages1 } = await mallAffiliateService.getUserClicks(userId, page, limit);
...
export const getUserPurchases = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { purchases, total, pages: _pages2 } = await mallAffiliateService.getUserPurchases(userId, status, page, limit);
...
export const processConversionWebhook = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { isDuplicate: isConvDuplicate, cachedResponse: cachedConvResponse } =
    await checkIdempotency(conversionIdempKey);
...
export const confirmPurchaseWebhook = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (isConfirmDup && cachedConfirmResp) {
    await updateWebhookLog(logId, {
      status: 'duplicate',
      responseStatus: 200,
      responseBody: cachedConfirmResp,
      processingTime: Date.now() - startTime,
...
export const fastTrackCredit = asyncHandler(async (req: Request, res: Response) => {
  ...
  // BUG FIX: Fetch without .lean() to allow instance methods

...
export const getExperiences = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (region) {
      const regionFilter = [{ regions: { $exists: false } }, { regions: { $size: 0 } }, { regions: region }];
      if (query.$or) {
    ...
    const experiences = await StoreExperience.find(query).sort({ sortOrder: 1 }).limit(Number(limit)).lean();
...
export const getStoresByExperience = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const assignedStoreIds = experience.assignedStores || [];
    const hasAssignedStores = assignedStoreIds.length > 0;
    const hasFilterCriteria =
      experience.filterCriteria &&
      ((experience.filterCriteria.tags && experience.filterCriteria.tags.length > 0) ||
        experience.filterCriteria.minRating ||
        experience.filterCriteria.isPremium ||
        experience.filterCriteria.isOrganic ||
        experience.filterCriteria.isMall ||
        experience.filterCriteria.isFastDelivery ||
        experience.filterCriteria.isBudgetFriendly ||
        experience.filterCriteria.isPartner ||
        experience.filterCriteria.isVerified ||
    ...
    if (filterCriteria) {
      ...
      // Verified filter
      if (filterCriteria.isVerified === true) {
      ...
      if (filterCriteria.categories && filterCriteria.categories.length > 0) {
        ...
        const categoryIds = filterCriteria.categories.filter((c: any) => mongoose.Types.ObjectId.isValid(c));
        const categoryNames = filterCriteria.categories.filter((c: any) => !mongoose.Types.ObjectId.isValid(c));
    ...
    if (q) {
      ...
      storeQuery.$or = [{ name: searchRegex }, { description: searchRegex }, { tags: { $in: [searchRegex] } }];
    }
    ...
    if (hasAssignedStores && hasFilterCriteria && !storeQuery._id) {
      ...
      finalQuery = {
        $or: [{ _id: { $in: assignedStoreIds }, isActive: true }, filterBasedQuery],
      };
    }
    ...
    const stores = storesRaw.map((store: any) => {
      ...
      if (userLng && userLat && store.location?.coordinates) {
        ...
        const dist = Math.sqrt(Math.pow(storeLng - userLng, 2) + Math.pow(storeLat - userLat, 2)) * 111; // Rough estimation in km
...
export const getHomepageExperiences = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Filter by region - show experiences that have no regions set (global) or include user's region
    if (region) {
      query.$or = [{ regions: { $exists: false } }, { regions: { $size: 0 } }, { regions: region }];
    }
...
export const getUniqueFinds = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (q) {
      const escapedQ = (q as string).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const searchRegex = new RegExp(escapedQ, 'i');
      query.$or = [{ name: searchRegex }, { description: searchRegex }, { tags: { $in: [searchRegex] } }];
    }
    ...
    const formattedProducts = products.map((p: any) => ({
      id: p._id,
      title: p.name,
      category: (p.category as Types.ObjectId & { name?: string })?.name || 'General',

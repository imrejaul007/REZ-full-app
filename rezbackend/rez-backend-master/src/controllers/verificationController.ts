...
const VERIFICATION_CONFIGS: Record<
  string,
  {
    methods: string[];
...
export const submitVerification = asyncHandler(async (req: Request, res: Response) => {
  const userId = (req as any).userId;
  const { zone } = req.params;
  const { method, documentNumber: _documentNumber, documentImage, email, additionalInfo } = req.body;
    ...
    case 'corporate':
      if (method === 'corporate_email' && email) {
        ...
        if (domain && !personalDomains.includes(domain)) {
          ...
          (verificationData as any).companyName =
  ...
  logger.info(
    `✅ [VERIFICATION] ${zone} verification ${isAutoApproved ? 'auto-approved' : 'submitted'} for user ${userId}`,
  );
...
export const reviewVerification = asyncHandler(async (req: Request, res: Response) => {
  ...
  logger.info(
    `✅ [VERIFICATION] Admin ${isApproved ? 'approved' : 'rejected'} ${zone} verification for user ${userId}`,
  );

...
export const verifyDevice = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    let existingTrust = await getTrustData(deviceId);
    let verificationCount = (existingTrust?.verificationCount || 0) + 1;
    let trustScore = calculateTrustScore(isNewDevice, suspiciousFlags, deviceUserCount, verificationCount);

    // Update trust score (Redis)
    // Note: This is still subject to race conditions in high-concurrency scenarios.
    // A more robust solution would use Redis Lua scripts for atomic read-modify-write.
    // Current implementation: if concurrent requests happen, later write may overwrite earlier one
    // with a potentially stale verificationCount. Consider implementing Redis Lua script:
    // SCRIPT LOAD "local current = redis.call('GET', KEYS[1]); local count = (current and cjson.decode(current).verificationCount or 0) + 1; ..."
...
export const reportSuspicious = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Use Redis pipeline to atomically increment and set expiry together
    // This prevents race condition where TTL might not be set if concurrent requests occur
    const reportCount = (await redisService.incr(redisKey)) ?? 1;

    // Only set expiry on first increment to maintain 24h window from first report
    if (reportCount === 1) {

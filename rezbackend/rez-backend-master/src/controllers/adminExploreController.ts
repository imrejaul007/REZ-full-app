...
export const getFeaturedReviews = asyncHandler(async (req: Request, res: Response) => {
  ...
  // BUG-015 FIX: Cap limit to prevent unbounded queries
...
export const bulkToggleReviewsFeatured = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const result = await Review.updateMany({ _id: { $in: reviewIds } }, { isFeaturedOnExplore: featured });
...
export const getExploreVideoStats = asyncHandler(async (req: Request, res: Response) => {
  try {
    const [totalVideos, publishedVideos, featuredVideos, trendingVideos, pendingVideos, totalViews] = await Promise.all(
      [
...
export const getAdminVideos = asyncHandler(async (req: Request, res: Response) => {
  const { page = 1, limit = 20, status, contentType, featured, trending, search } = req.query;
  // BUG-015 FIX: Cap limit to prevent unbounded queries
  const safeLimit = Math.min(Number(limit) || 20, 100);
...
export const createAdminVideo = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const video = new Video({
      ...
      description,
      videoUrl,
      thumbnail: thumbnail || 'https://placehold.co/400x600?text=Video',
      contentType,
      category,
      tags,
...
export const bulkUpdateVideos = asyncHandler(async (req: Request, res: Response) => {
  const { videoIds, action, value: _value } = req.body;
  ...
  try {
    ...
    const result = await Video.updateMany({ _id: { $in: videoIds } }, updateQuery);

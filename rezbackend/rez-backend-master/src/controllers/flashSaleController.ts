...
class FlashSaleController {
  ...
  getExpiringSoonFlashSales = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    try {
      const minutes = parseInt(req.query.minutes as string, 10) || 5;
  ...
  /**
   * Initiate flash sale purchase - creates Razorpay order
  ...
  initiateFlashSalePurchase = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Defense-in-depth validation — Joi schema on the route covers the happy path
      // but controller validation catches direct API callers that bypass the middleware.
      if (!flashSaleId || typeof flashSaleId !== 'string' || !/^[0-9a-fA-F]{24}$/.test(flashSaleId)) {
        res.status(400).json({
          success: false,
      ...
      const result = await flashSaleService.initiateFlashSalePurchase(userId, flashSaleId, parsedQuantity, metadata);
  ...
  verifyFlashSalePayment = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!purchaseId || !razorpayOrderId || !razorpayPaymentId || !razorpaySignature) {
        res.status(400).json({
          success: false,
          message: 'Missing required fields: purchaseId, razorpayOrderId, razorpayPaymentId, razorpaySignature',
        });
        return;
      }

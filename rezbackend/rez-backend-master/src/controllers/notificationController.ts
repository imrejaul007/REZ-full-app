...
export const getUnreadCount = asyncHandler(async (req: Request, res: Response) => {
  ...
  const [total, byTypeAgg, byPriorityAgg] = await Promise.all([
    ...
    Notification.aggregate([{ $match: baseQuery }, { $group: { _id: '$type', count: { $sum: 1 } } }]),
    Notification.aggregate([{ $match: baseQuery }, { $group: { _id: '$priority', count: { $sum: 1 } } }]),
  ]);
  ...
  redisService
    .set(cacheKey, result, 30)
    .catch((err) => logger.warn('[Notification] Cache set for unread count failed', { error: err.message })); // 30s cache
...
export const getUserNotifications = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Run find + both countDocuments in parallel — was 3 sequential round-trips
...
export const markAsRead = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Invalidate unread count cache — await so subsequent countDocuments
    // and any concurrent getUnreadCount calls see a cache miss, not stale data
    await redisService
      .del(`notification:unread:${userId}`)
      .catch((err) =>
        logger.warn('[Notification] Cache invalidation for unread count on markAsRead failed', { error: err.message }),
      );
...
export const deleteNotification = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Invalidate unread count cache — await so concurrent getUnreadCount
    // calls see a cache miss and re-fetch fresh data rather than stale counts
    await redisService
      .del(`notification:unread:${userId}`)
      .catch((err) =>
        logger.warn('[Notification] Cache invalidation for unread count on delete failed', { error: err.message }),
      );

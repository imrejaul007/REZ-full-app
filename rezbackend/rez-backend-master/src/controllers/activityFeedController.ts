...
export const getUserActivities = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    if (String(userId) !== String((req as any).user?._id || (req as any).user?.id || (req as any).userId)) {
      ...
      try {
        ...
        if (partner) {
          ...
          if (socialTask && socialTask.progress.current < socialTask.progress.target) {
            ...
            await partner.save();
            logger.info(
              '✅ [SOCIAL] Partner social task updated:',
              socialTask.progress.current,
              '/',
              socialTask.progress.target,
            );
          }
        }
      } catch (error) {

...
export const getConversations = asyncHandler(async (req: Request, res: Response) => {
  ...
  const [conversations, total] = await Promise.all([
    Conversation.find(query).sort({ lastActivityAt: -1 }).skip(skip).limit(limitNum).lean(),
    Conversation.countDocuments(query),
  ]);

  // Get summary
  const summary = await Conversation.getConversationsSummary(new Types.ObjectId(userId), status as ConversationStatus);
...
export const getMessages = asyncHandler(async (req: Request, res: Response) => {
  ...
  const [messages, total] = await Promise.all([
    Message.find(query).sort({ sentAt: -1 }).skip(skip).limit(limitNum).lean(),
...
export const sendMessage = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    socketIO.to(SocketRoom.store(storeId)).emit('message:received', {
      conversationId: id,
      message,
      timestamp: new Date(),
    });
    // Also notify the customer room (for multi-device sync)
    socketIO.to(SocketRoom.user(userId)).emit('message:received', {
      conversationId: id,
      message,
      timestamp: new Date(),
    });
...
export const markConversationAsRead = asyncHandler(async (req: Request, res: Response) => {
  ...
  await Message.markConversationAsRead(new Types.ObjectId(id), new Types.ObjectId(userId));
...
export const deleteConversation = asyncHandler(async (req: Request, res: Response) => {
  ...
  await Message.updateMany({ conversationId: new Types.ObjectId(id) }, { $set: { deletedAt: new Date() } });

  // Soft delete conversation
  await Conversation.findByIdAndUpdate(id, { $set: { deletedAt: new Date(), isDeleted: true } });
...
export const getUnreadCount = asyncHandler(async (req: Request, res: Response) => {
  ...
  const totalUnread = await Conversation.getTotalUnreadCount(new Types.ObjectId(userId));
...
export const getStoreAvailability = asyncHandler(async (req: Request, res: Response) => {
  ...
  const store = await Store.findById(id).select('name operationalInfo').lean();
  ...
  const isCurrentlyOpen = todayHours.isOpen && currentTime >= todayHours.open && currentTime <= todayHours.close;

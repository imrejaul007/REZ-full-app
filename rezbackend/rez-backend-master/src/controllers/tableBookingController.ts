...
export const createTableBooking = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { storeId, bookingDate, bookingTime, partySize, customerName, customerPhone, customerEmail, specialRequests } =
    req.body;
  ...
  // ATOMIC: Get current capacity state as of this exact moment
  const capacityDoc = await (TableBooking.findOneAndUpdate(
    {
      // Unique compound doc tracking capacity for this slot
      storeId: new Types.ObjectId(storeId),
      bookingDate: { $gte: startOfBookingDay, $lte: endOfBookingDay },
      bookingTime,
      // Guard: only proceed if incrementing used capacity won't exceed max
      $expr: { $lte: [{ $add: ['$totalSeatsUsed', partySize] }, maxCapacity] },
    },
    { $inc: { totalSeatsUsed: partySize } },
    { upsert: true, new: true },
  ) as any);

  if (!capacityDoc || (capacityDoc as any).totalSeatsUsed > maxCapacity) {
    // Capacity exceeded or guard failed — revert the increment
    if (capacityDoc) {
      await TableBooking.findByIdAndUpdate(capacityDoc._id, { $inc: { totalSeatsUsed: -partySize } });
    }
    // Fallback: get actual booked amount for error message
  ...
  // Create booking (now safe — capacity was atomically reserved)
...
export const getMerchantTableBookings = asyncHandler(async (req: Request, res: Response) => {
  ...
  const merchantStores = await Store.find({ merchantId: new Types.ObjectId(userId) })
    .select('_id name')
    .lean();
...
export const updateTableBookingStatus = asyncHandler(async (req: Request, res: Response) => {
  ...
  // BUG FIX #3: Booking status state machine enforcement
  // Define valid transitions: pending → confirmed → completed | cancelled | no_show
  ...
  if (!VALID_TRANSITIONS[currentStatus]?.includes(status)) {
    return sendBadRequest(res, `Cannot transition from ${currentStatus} to ${status}`);
  }
...
export const cancelTableBooking = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (store && (store as any).merchantId) {
    merchantNotificationService
      ...
      .catch((err: any) =>
        logger.error('[TABLE BOOKING] Failed to send cancel notification to merchant: ' + err.message),
      );
  }
...
export const checkAvailability = asyncHandler(async (req: Request, res: Response) => {
  ...
  const bookings = await TableBooking.find({
    ...
    .select('bookingTime partySize status')
    .lean();
  ...
  const startHour = parseInt(workingStart.split(':', 10)[0]);
  const endHour = parseInt(workingEnd.split(':', 10)[0]);

...
export const getStoreVouchers = asyncHandler(async (req: Request, res: Response) => {
  ...
  const [vouchers, total] = await Promise.all([
    StoreVoucher.find(filter).sort({ discountValue: -1 }).skip(skip).limit(limitNum).lean(),
  ...
  if (userObjId) {
    ...
    return sendPaginated(res, vouchersWithEligibility, pageNum, limitNum, total, 'Store vouchers fetched successfully');
  }
...
export const getStoreVoucherById = asyncHandler(async (req: Request, res: Response) => {
  ...
  const voucher = await StoreVoucher.findById(id).populate('store', 'name logo location contact').lean();
...
export const claimStoreVoucher = asyncHandler(async (req: Request, res: Response) => {
  ...
  sendSuccess(res, userVoucher, 'Voucher claimed successfully', 201);
});
...
export const redeemStoreVoucher = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Find user's voucher (not lean - need to work with the full document)
  const userVoucher = await UserStoreVoucher.findOne({
    _id: id,
    user: userId,
  }).populate('voucher');
  ...
  const voucher = userVoucher.voucher as any;
  ...
  if (billAmount < voucher.minBillAmount) {
    return sendError(res, `Minimum bill amount of ₹${voucher.minBillAmount} required`, 400);
  }
  ...
  // Atomically mark user voucher as used (only if currently 'assigned')
  const updateUserVoucherResult = await UserStoreVoucher.updateOne(
    {
      _id: id,
      user: userId,
      status: 'assigned', // Only update if still assigned
    },
    {
      $set: {
        status: 'used',
  ...
  if (updateUserVoucherResult.matchedCount === 0) {
    return sendError(res, 'This voucher has already been redeemed', 400);
  }

  // Atomically increment voucher usage count
  const updateVoucherResult = await StoreVoucher.findByIdAndUpdate(
    voucher._id,
    {
      $inc: { usedCount: 1 },
  ...
  if (updateVoucherResult && updateVoucherResult.usedCount > voucher.usageLimit) {
    // Rollback the user voucher status
    await UserStoreVoucher.updateOne({ _id: id, user: userId }, { $set: { status: 'assigned' } });
    return sendError(res, 'This voucher has reached its usage limit', 400);
  }

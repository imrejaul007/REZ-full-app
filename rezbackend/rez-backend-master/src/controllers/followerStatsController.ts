...
import { sendSuccess, sendNotFound, sendForbidden } from '../utils/response';
...
export const getTopFollowers = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // DB-AUDIT-4/5 FIX: added .limit(500) to prevent unbounded query against large stores
    ...
    // DB-AUDIT-4/5 FIX: added .limit(500) to prevent unbounded query
    ...
    const followerEngagement = wishlists.map((wishlist: any) => {
      ...
      // Count orders
      const userOrders = orders.filter((order: any) => order.user.toString() === followerId);
      const orderCount = userOrders.length;
      const totalSpent = userOrders.reduce((sum: number, order: any) => sum + (order.totalAmount || 0), 0);

      // Count reviews
      const reviewCount = reviews.filter((review: any) => review.user.toString() === followerId).length;

...
export const getActiveCampaigns = asyncHandler(async (req: Request, res: Response) => {
  ...
  const campaignsWithState = await bonusCampaignService.getActiveCampaigns(userId, region as string | undefined);
  ...
  const campaigns = campaignsWithState.map(({ campaign, userState, userClaimCount, userTotalReward }) => ({
    ...
    // Convenience field for flash deal countdowns — ISO string of campaign end time
    end_time: campaign.endTime instanceof Date ? campaign.endTime.toISOString() : campaign.endTime,
    ...
    globalClaimsRemaining:
      campaign.limits.totalGlobalClaims > 0
        ? Math.max(0, campaign.limits.totalGlobalClaims - (campaign.limits.currentGlobalClaims || 0))
        : null,
    terms: campaign.terms,
    fundingSource:
      campaign.fundingSource.type !== 'platform'
        ? {
...
export const getCampaignDetail = asyncHandler(async (req: Request, res: Response) => {
  ...
  const eligibility = await bonusCampaignService.checkEligibility(campaign._id.toString(), userId);
  ...
  let computedUserState: 'eligible' | 'claimed' | 'limit_reached' | 'not_eligible' | 'budget_exhausted' | 'expired' =
    'eligible';
  ...
  if (campaign.endTime < now) {
    ...
    campaign.limits.totalGlobalClaims > 0 &&
    (campaign.limits.currentGlobalClaims || 0) >= campaign.limits.totalGlobalClaims
  ) {
    computedUserState = 'budget_exhausted';
...
export const claimCampaignReward = asyncHandler(async (req: Request, res: Response) => {
  const userId = (req as any).userId;
  const { slug } = req.params;
  const { transactionRef, paymentMethod, bankCode, cardBin, transactionAmount } = req.body;
  ...
  if (!VALID_TRANSACTION_REF_TYPES.includes(transactionRef.type)) {
    return sendBadRequest(
      res,
      `Invalid transactionRef.type. Must be one of: ${VALID_TRANSACTION_REF_TYPES.join(', ')}`,
    );
  }
  ...
  try {
    const result = await bonusCampaignService.claimReward(campaign._id.toString(), userId, {
      transactionRef,
      paymentMethod,
      bankCode,
      cardBin,
...
export const checkEligibility = asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await bonusCampaignService.checkEligibility(campaign._id.toString(), userId);
...
export const adminGetCampaigns = asyncHandler(async (req: Request, res: Response) => {
  const { status, campaignType, region, search, page = '1', limit = '20' } = req.query;
  ...
  if (region) {
    andConditions.push({
      $or: [{ 'eligibility.regions': region }, { 'eligibility.regions': 'all' }],
    });
  }
  if (search) {
  ...
  const [campaigns, total] = await Promise.all([
    BonusCampaign.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limitNum).lean(),
...
export const adminCreateCampaign = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!slug || !title || !subtitle || !campaignType || !reward || !deepLink || !startTime || !endTime) {
    return sendBadRequest(
      res,
      'Missing required fields: slug, title, subtitle, campaignType, reward, deepLink, startTime, endTime',
    );
  }
...
export const adminUpdateCampaign = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (['active', 'exhausted'].includes(campaign.status)) {
    ...
    if (
      updates.limits?.maxClaimsPerUser !== undefined &&
      updates.limits.maxClaimsPerUser < campaign.limits.maxClaimsPerUser
    ) {
...
export const adminDeleteCampaign = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!['draft', 'cancelled'].includes(campaign.status)) {
    return sendBadRequest(
      res,
      `Cannot delete campaign with status "${campaign.status}". Only draft or cancelled campaigns can be deleted.`,
    );
  }
...
export const adminUpdateStatus = asyncHandler(async (req: Request, res: Response) => {
  ...
  // FT-D003 FIX: Use findById (Mongoose document) NOT findById(...).lean().
  // .lean() returns a plain JS object — calling .save() on it throws
  // "campaign.save is not a function" at runtime, meaning the status flip
  // silently fails (or crashes) and the campaign stays active in the DB.
  // The cache IS invalidated (line below), so the consumer app briefly
  // fetches a stale "active" list from DB while thinking it was paused.
  const campaign = await BonusCampaign.findById(id);
  if (!campaign) {
    return sendNotFound(res, 'Campaign not found');
  }
  ...
  // FT-D004 FIX: Await cache invalidation and surface errors in the response log.
  //
  // ROOT CAUSE: invalidateBonusZoneCache() is fire-and-forget (no await, no error
  // handling). If Redis is momentarily slow or disconnected, delPattern() returns
  // silently, the stale `bonus-zone:active:*` entry survives for up to 60 more
  // seconds, and the consumer app continues showing a paused campaign as active.
  // During that window, users can still see (and attempt to claim) the paused
  // campaign — claims are rejected by the DB-level status check in claimReward(),
  // but the user experience is confusing ("it says active but I can't claim").
  //
  // FIX: Await the cache invalidation so the response to the admin only succeeds
  // after the cache is definitively cleared.  Swallow failures with a warning so a
  // Redis outage doesn't block the admin action (the DB is already updated; the
  // cache will expire naturally within CACHE_TTL=60 s).
  try {
    await bonusCampaignService.invalidateBonusZoneCacheAsync();
  } catch (cacheErr: any) {
    logger.warn('[BonusZone] Cache invalidation failed after status update (DB already updated)', {
      campaignId: id,
      newStatus: status,
...
export const adminDuplicateCampaign = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { _id: _dupId, createdAt: _dupCreatedAt, updatedAt: _dupUpdatedAt, ...rest } = source;
...
export const adminRejectClaim = asyncHandler(async (req: Request, res: Response) => {
  ...
  await BonusClaim.findByIdAndUpdate(claimId, {
    status: 'rejected',
    rejectionReason: reason || 'Rejected by admin',
    verifiedAt: new Date(),
    verifiedBy: (req as any).user?._id?.toString() || (req as any).userId || 'admin',
  });

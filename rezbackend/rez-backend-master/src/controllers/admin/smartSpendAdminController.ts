...
export const createSmartSpendItem = asyncHandler(async (req: Request, res: Response) => {
  const { itemType, store, product, coinDisplayText } = req.body;
...
export const deleteSmartSpendItem = asyncHandler(async (req: Request, res: Response) => {
  ...
  const item = await SmartSpendItem.findByIdAndUpdate(req.params.id, { $set: { isActive: false } }, { new: true });
...
export const getSmartSpendAnalytics = asyncHandler(async (req: Request, res: Response) => {
  ...
  const topItems = await SmartSpendItem.find({ isActive: true })
    .select(
      'itemType store product displayTitle coinDisplayText coinRewardRate sectionLabel badgeText views clicks purchases isFeatured sortOrder isActive',
    )

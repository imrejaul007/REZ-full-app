...
export const getOffers = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    const [offers, total] = await Promise.all([
      PriveOffer.find(filter).sort({ priority: -1, createdAt: -1 }).skip(skip).limit(limit).lean(),
    ...
    const enrichedOffers = offers.map((offer) => ({
      ...offer,
      ctr: offer.views > 0 ? Math.round((offer.clicks / offer.views) * 10000) / 100 : 0,
      redemptionRate:
...
export const createOffer = asyncHandler(async (req: Request, res: Response) => {
  try {
    const { title, subtitle, description, brand, reward, startsAt, expiresAt } = req.body;
    ...
    if (
      ...
      return sendError(
        res,
        'Missing required fields: title, subtitle, description, brand.name, reward.value, reward.displayText, startsAt, expiresAt',
        400,
      );
    }
...
export const deleteOffer = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    const offer = await PriveOffer.findByIdAndUpdate(req.params.id, { $set: { isActive: false } }, { new: true });
...
export const getVouchers = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    if (req.query.search) {
      const escaped = escapeRegex(String(req.query.search).substring(0, 200));
      filter.$or = [{ code: { $regex: escaped, $options: 'i' } }, { partnerName: { $regex: escaped, $options: 'i' } }];
    }
...
export const recalculateUserReputation = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    const reputation = await reputationService.recalculateReputation(userId, `admin_recalculation by ${adminId}`);
...
export const getAnalytics = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    offerPerformance.overallCTR =
      offerPerformance.totalViews > 0
...
/**
 * GET /api/admin/prive/submissions
 * Paginated list of post submissions with user & campaign info
 */
export const getSubmissions = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    if (req.query.campaignId) {
      if (!Types.ObjectId.isValid(req.query.campaignId as string)) {
        return sendError(res, 'Invalid campaign ID', 400);
      }
      filter.campaign = new Types.ObjectId(req.query.campaignId as string);
    }
    if (req.query.userId) {
      if (!Types.ObjectId.isValid(req.query.userId as string)) {
        return sendError(res, 'Invalid user ID', 400);
      }
      filter.user = new Types.ObjectId(req.query.userId as string);
    }
    ...
    if (req.query.search) {
      const escaped = escapeRegex(String(req.query.search).substring(0, 200));
      filter.caption = { $regex: escaped, $options: 'i' };
    }

    const [submissions, total] = await Promise.all([
      PrivePostSubmission.find(filter)
        ...
        .limit(limit)
        .lean(),
      PrivePostSubmission.countDocuments(filter),
    ]);
    ...
    return sendSuccess(
      ...
      'Privé submissions fetched',
    );
  } catch (error) {
    logger.error('[Admin Privé] Error fetching submissions:', error);
    return sendError(res, 'Failed to fetch Privé submissions', 500);
  }
});

/**
 * PATCH /api/admin/prive/submissions/:submissionId/review
 * Approve or reject a submission
 *
 * Body: { status: 'approved' | 'rejected', rejectionReason?: string, moderatorNotes?: string }
 */
export const reviewSubmission = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    if (status === 'rejected' && !rejectionReason) {
      return sendError(res, 'rejectionReason is required when rejecting a submission', 400);
    }

    const submission = await PrivePostSubmission.findById(submissionId);
    if (!submission) {
      return sendError(res, 'Submission not found', 404);
    }

    if (submission.isDeleted) {
      return sendError(res, 'Cannot review a deleted submission', 400);
    }
    ...
    return sendSuccess(res, submission, `Submission ${status}`);
  } catch (error) {
    logger.error('[Admin Privé] Error reviewing submission:', error);
    return sendError(res, 'Failed to review submission', 500);
  }
});

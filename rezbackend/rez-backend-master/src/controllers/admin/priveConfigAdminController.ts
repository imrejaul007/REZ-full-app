...
export const updateProgramConfig = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { tierThresholds, pillarWeights, tiers, featureFlags, dashboardCacheTtlSeconds, notificationConfig } =
    priveProgramConfig;
...
export const getAuditLog = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Map to frontend-expected format: adminName, action, targetType, targetId, detail, createdAt
  const entries = logs.map((log: any) => ({
    adminId: String(log.performedBy?._id || log.userId || ''),
    adminName: log.performedBy?.fullName || 'Unknown',
    action: log.action || '',
    targetType: log.entityType || '',
    targetId: String(log.entityId || ''),
    detail: log.changes || log.description || '',
    createdAt: log.createdAt || new Date(),
  }));

  return sendSuccess(res, entries, 'Audit log fetched');
});

...
export const createAdminMission = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!title || !targetPillar || !actionType || !targetCount || !startDate || !endDate) {
    throw new AppError(
      'Missing required fields: title, targetPillar, actionType, targetCount, startDate, endDate',
      400,
    );
  }
...
export const updateAdminMission = asyncHandler(async (req: Request, res: Response) => {
  ...
  const updatedMission = await PriveMission.findByIdAndUpdate(id, updateData, { new: true });
  sendSuccess(res, { mission: updatedMission }, 'Mission updated');
});

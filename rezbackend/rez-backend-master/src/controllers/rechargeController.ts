...
import { sendSuccess, sendBadRequest, sendNotFound, sendPaginated } from '../utils/response';
...
// Recharge Aggregator warning
logger.warn('[RECHARGE] No live aggregator connected. Using stub mode.');
logger.warn('[RECHARGE] Connect one of: Eko API, PaySprint, Fino Payments Bank, or IRCTC Prepaid API');
...
export const getOperators = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Cache the result
  await redisService.set<OperatorsCacheData>(cacheKey, { operators, page, limit, total }, CACHE_TTL_OPERATORS);
...
export const getPlans = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (cached) {
    ...
    return sendPaginated(res, cached.plans, cached.page, cached.limit, cached.total, 'Plans fetched successfully');
  }
  ...
  // Cache the result
  await redisService.set<PlansCacheData>(cacheKey, { plans: paginatedPlans, page, limit, total }, CACHE_TTL_OPERATORS);
...
export const initiateRecharge = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (numAmount > 100000) {
    return sendBadRequest(res, 'Amount exceeds maximum limit of 100000');
  }
  ...
  const transactionId = `RCH-${Date.now()}-${randomInt(100000, 999999)}`;
  ...
  logger.info(
    `[RECHARGE] Initiated recharge: txn=${transactionId}, user=${userId}, operator=${operatorCode}, amount=${amount}`,
  );
...
/**
 * POST /api/recharge/webhook/razorpay
 * Razorpay payment webhook callback handler
 * Verifies signature and processes successful payment
 */
export const handleRazorpayWebhook = asyncHandler(async (req: Request, res: Response) => {
  const { event, payload } = req.body;
  const signature = req.headers['x-razorpay-signature'] as string;

  // Verify Razorpay signature (assuming webhook middleware has done this)
  logger.info(`[RECHARGE] Webhook received: event=${event}`);

  if (event !== 'payment.authorized') {
    return sendSuccess(res, { status: 'ignored' }, 'Event not relevant to recharge', 200);
  }
  ...
  try {
    // Verify order metadata contains recharge info
    if (!order.notes || !order.notes.type || order.notes.type !== 'recharge') {
      return sendSuccess(res, { status: 'skipped' }, 'Not a recharge order', 200);
    }

    const { userId, mobile, operator, circle, amount } = order.notes;

    if (!userId || !mobile || !operator || !circle || !amount) {
      logger.error('[RECHARGE] Incomplete order metadata', { orderId, notes: order.notes });
      return sendBadRequest(res, 'Incomplete order metadata');
    }
    ...
    logger.info('[RECHARGE] Webhook processed', {
      paymentId,
      success: result.success,
      message: result.message,
    });

    return sendSuccess(res, { success: result.success, message: result.message }, 'Webhook processed', 200);
  } catch (err: any) {
    logger.error('[RECHARGE] Webhook processing failed: ' + err.message);
    return sendBadRequest(res, 'Webhook processing failed');
  }
});

/**
 * GET /api/recharge/history?page=1&limit=20
 * Get recharge transaction history for the authenticated user
 */
export const getRechargeHistory = asyncHandler(async (req: Request, res: Response) => {
  const userId = (req as any).userId;
  const page = parseInt(req.query.page as string, 10) || 1;
  const limit = parseInt(req.query.limit as string, 10) || 20;

  if (page < 1 || limit < 1 || limit > 100) {
    return sendBadRequest(res, 'Invalid page or limit');
  }

  const result = await rechargeService.getRechargeHistory(userId, page, limit);

  logger.info(`[RECHARGE] Fetched history for user=${userId} (page=${page})`);

  return sendPaginated(res, result.transactions, page, limit, result.total, 'Recharge history fetched successfully');
});
 ...
 */
export const getRechargeDetails = asyncHandler(async (req: Request, res: Response) => {
  const userId = (req as any).userId;
  ...
  if (!transaction) {
    return sendNotFound(res, 'Recharge transaction not found');
  }

  logger.info(`[RECHARGE] Fetched details for transaction=${transactionId}`);

  return sendSuccess(res, transaction, 'Recharge details fetched successfully', 200);
});

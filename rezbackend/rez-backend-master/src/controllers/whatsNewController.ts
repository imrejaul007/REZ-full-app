...
import { sendSuccess, sendNotFound, sendCreated, sendBadRequest } from '../utils/response';
...
export const trackView = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!mongoose.isValidObjectId(id)) {
    return sendBadRequest(res, 'Invalid story ID');
  }
...
export const createStory = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { title, subtitle, icon, slides, ctaButton, validity, targeting, priority } = req.body;

  if (!title || typeof title !== 'string' || title.trim().length === 0) {
    return sendBadRequest(res, 'Story title is required');
  }
  if (title.trim().length > 200) {
    return sendBadRequest(res, 'Story title must not exceed 200 characters');
  }
  if (!slides || !Array.isArray(slides) || slides.length === 0) {
    return sendBadRequest(res, 'Story must have at least one slide');
  }

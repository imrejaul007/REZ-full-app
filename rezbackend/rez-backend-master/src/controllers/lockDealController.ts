...
import { sendSuccess, sendCreated, sendBadRequest, sendNotFound, sendConflict, sendPaginated } from '../utils/response';
...
export const getLockDeals = asyncHandler(async (req: Request, res: Response) => {
  const { region, category, storeId, featured, tag, search, page = '1', limit = '20' } = req.query;
...
export const getLockDealById = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const deal = await LockPriceDeal.findById(id).populate('store', 'name logo address ratings').lean();
 ...
 * 3. Create Razorpay order for deposit amount
 * 4. Return razorpayOrderId + razorpayKeyId for frontend checkout
 */
export const initiateLock = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 5. Create Razorpay order for deposit
    const razorpayOrder = await createRazorpayOrder(deal.depositAmount, `lock_dep_${id}_${userId!.slice(-6)}`, {
...
export const confirmLock = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const pickupCode = UserLockDeal.generatePickupCode();
      ...
      [
        {
          ...
          depositRazorpayOrderId: razorpayOrderId,
    ...
    await LockPriceDeal.findByIdAndUpdate(id, { $inc: { currentLocks: 1 } }, { session });
...
export const initiateBalancePayment = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const razorpayOrder = await createRazorpayOrder(userLock.balanceAmount, `lock_bal_${lockId}_${userId!.slice(-6)}`, {
 ...
 * Confirm balance payment after Razorpay succeeds
 */
export const confirmBalancePayment = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const userLock = await UserLockDeal.findOneAndUpdate(
      ...
      },
      {
        $set: {
          ...
          balanceRazorpayOrderId: razorpayOrderId,
    ...
    if (!userLock) {
      return sendNotFound(
        res,
        'Lock not found, already paid, or expired. If you were charged, a refund will be processed.',
      );
    }
...
export const verifyPickup = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    await LockPriceDeal.findByIdAndUpdate(userLock.lockDeal, { $inc: { totalPickedUp: 1 } }, { session });
...
export const getMyLockDetail = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const lock = await UserLockDeal.findOne({ _id: lockId, user: userId }).populate('lockDeal').lean();
...
export const cancelLock = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    await LockPriceDeal.findByIdAndUpdate(userLock.lockDeal, { $inc: { currentLocks: -1 } }, { session });
    ...
    // 9. Initiate Razorpay refund for the deposit (best-effort, outside the DB session).
    // razorpayCreateRefund takes amount in rupees (not paise).
    if (userLock.depositPaymentId && refundAmount > 0) {
      try {
        logger.info(
          `[LOCK DEALS] Initiating Razorpay refund of ₹${refundAmount} ` + `for payment ${userLock.depositPaymentId}`,
        );
        const refund = await razorpayCreateRefund(userLock.depositPaymentId, refundAmount, {
          lockDealId: String(userLock._id),
          userId: String(userLock.user),
          cancelReason: 'user_initiated_cancellation',
        });
        logger.info(`✅ [LOCK DEALS] Razorpay refund created: ${refund.id} (status: ${refund.status})`);
      } catch (refundError: any) {
        // Refund failed — log as critical so ops team can manually process it.
        // The DB transaction (wallet credit) is already committed above.
        logger.error(
          `🚨 [LOCK DEALS] Razorpay refund FAILED — MANUAL REFUND REQUIRED. ` +
            `LockDealId: ${userLock._id}, PaymentId: ${userLock.depositPaymentId}, ` +
            `Amount: ₹${refundAmount}. Error: ${refundError.message}`,
        );
        // Mark the lock deal record so the reconciliation job can find it
        await UserLockDeal.findByIdAndUpdate(userLock._id, {
          $set: { 'metadata.razorpayRefundFailed': true, 'metadata.razorpayRefundError': refundError.message },
        }).catch(() => {}); // best-effort
      }
    }
  ...
  try {
    ...
    for (const lock of expiredLocks) {
      try {
        ...
        await LockPriceDeal.findByIdAndUpdate(lock.lockDeal, { $inc: { currentLocks: -1 } });

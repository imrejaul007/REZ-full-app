...
export const getUserBoot = asyncHandler(async (req: Request, res: Response) => {
  ...
  const [profile, wallet, cart, unreadCount] = await Promise.all([
    User.findById(userId)
      ...
      .lean(),
    Wallet.findOne({ user: userObjectId }).select('coins balance brandedCoinsTotal').lean(),
    Cart.findOne({ user: userObjectId, isActive: true }).select('items').lean(),
  ...
  try {
    await redisService.set(cacheKey, response, 30);
  } catch (err) {
    logger.warn('[Boot] Cache write failed:', { error: (err as Error).message });
  }

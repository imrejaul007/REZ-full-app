...
import { StoreVisit, VisitType, VisitStatus } from '../models/StoreVisit';
...
// Fixed: Import gamificationEventBus to connect store visits to streak system - Phase 1.1
import gamificationEventBus from '../events/gamificationEventBus';

// Fire-and-forget: send AdBazaar visit attribution webhook when user checks into a store
async function fireAdBazaarVisitAttribution(
  rezUserId: string,
  merchantId: string,
  visitTimestamp: Date,
): Promise<void> {
  const webhookUrl = process.env.ADBAZAAR_WEBHOOK_URL;
  const webhookSecret = process.env.ADBAZAAR_WEBHOOK_SECRET;
  if (!webhookUrl || !webhookSecret) {
    logger.warn('[ADBAZAAR] ADBAZAAR_WEBHOOK_URL or ADBAZAAR_WEBHOOK_SECRET not set — visit attribution skipped', {
      rezUserId,
      merchantId,
    });
    return;
  ...
  try {
    const scan = await AdBazaarScan.findOne({ rezUserId, merchantId, visitAttributed: false }).sort({ scannedAt: -1 });
    if (!scan) return;
    scan.visitAttributed = true;
    scan.visitAttributedAt = new Date();
    await scan.save();
    fetch(`${webhookUrl}/api/webhooks/rez-visit`, {
      method: 'POST',
      ...
      body: JSON.stringify({
        ...
        visitTimestamp: visitTimestamp.toISOString(),
      }),
    }).catch((err) => logger.error('❌ [ADBAZAAR] Visit webhook failed:', err));
  } catch (err) {
    logger.error('❌ [ADBAZAAR] Visit attribution error:', err);
  }
}

// Fire-and-forget: send AdBazaar purchase attribution webhook when visit is completed
async function fireAdBazaarPurchaseAttribution(
  rezUserId: string,
  merchantId: string,
  completedAt: Date,
): Promise<void> {
  ...
  try {
    const scan = await AdBazaarScan.findOne({
      rezUserId,
      merchantId,
      visitAttributed: true,
      purchaseAttributed: false,
    }).sort({ scannedAt: -1 });
    if (!scan) return;
    scan.purchaseAttributed = true;
    scan.purchaseAttributedAt = new Date();
    await scan.save();
    fetch(`${webhookUrl}/api/webhooks/rez-purchase`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-webhook-secret': webhookSecret },
      body: JSON.stringify({
        rezUserId,
        ...
        scanEventId: scan.scanEventId,
        purchaseAmount: 0,
        purchaseTimestamp: completedAt.toISOString(),
      }),
    }).catch((err) => logger.error('❌ [ADBAZAAR] Purchase webhook failed:', err));
  } catch (err) {
    logger.error('❌ [ADBAZAAR] Purchase attribution error:', err);
  }
}
  ...
  if (store.merchantId) {
    merchantNotificationService
      .notifyNewVisit({
        ...
        visitId: (visit._id as Types.ObjectId).toString(),
        visitNumber: visit.visitNumber,
        customerName,
...
export const getQueueNumber = asyncHandler(async (req: Request, res: Response) => {
  ...
  // MA-L3 fix: If the user is not authenticated (no req.userId), attempt to find a registered
  // REZ user by phone number so walk-in visits can be attributed to loyalty accounts.
  // Without this, walk-in visits created by merchants have no userId and gamification events
  // are skipped (the "if (visit.userId)" guard in updateVisitStatusByMerchant never fires).
  let resolvedUserId = req.userId || undefined;
  if (!resolvedUserId && customerPhone) {
    try {
      const matchedUser = await User.findOne({ phoneNumber: customerPhone }).select('_id').lean();
      if (matchedUser) {
        resolvedUserId = matchedUser._id.toString();
        logger.info('[QUEUE] Walk-in visit attributed to registered user via phone lookup', {
          phone: customerPhone,
          userId: resolvedUserId,
        });
      }
    } catch (lookupErr) {
      // Non-fatal: if lookup fails, visit is created without userId (degraded attribution)
      logger.warn('[QUEUE] Phone-based user lookup failed for walk-in attribution', {
        phone: customerPhone,
  ...
  try {
    ...
    const estimatedWaitTime = currentQueueSize > 0 ? `${currentQueueSize * 15} minutes` : 'Less than 15 minutes';
...
export const getStoreVisits = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (store.merchantId && store.merchantId.toString() !== userId) {
    // Also check if this is the store owner (legacy field not in IStore; keep cast)
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const storeOwner = (store as any).owner as Types.ObjectId | undefined;
    if (storeOwner && storeOwner.toString() !== userId) {
...
export const cancelStoreVisit = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (cancelStore && cancelStore.merchantId) {
    merchantNotificationService
      .notifyVisitCancelled({
        merchantId: cancelStore.merchantId.toString(),
        visitId: (visit._id as Types.ObjectId).toString(),
        visitNumber: visit.visitNumber,
  ...
  try {
    await pushNotificationService.sendVisitCancelled(cancelStore?.name || '', visit.visitNumber, visit.customerPhone);
  } catch (smsError) {
...
export const getAvailableSlotsHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  const storeOperationalHours = store.operationalInfo?.hours;
  const dayHours = storeOperationalHours?.[dayName as keyof typeof storeOperationalHours];
...
export const updateVisitStatusByMerchant = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Fixed: Emit gamification events when visit status changes to CHECKED_IN or COMPLETED - Phase 1.1
  // This connects store visits to the savings streak system
  if (visit.userId) {
    const userId = visit.userId.toString();
    if (status === VisitStatus.CHECKED_IN) {
      gamificationEventBus.emit('visit_checked_in', {
        userId,
        entityId: (visit._id as Types.ObjectId).toString(),
        entityType: 'StoreVisit',
        storeId: visit.storeId?.toString(),
        source: { controller: 'storeVisitController', action: 'updateVisitStatusByMerchant' },
      });
    } else if (status === VisitStatus.COMPLETED) {
      gamificationEventBus.emit('visit_completed', {
        userId,
        entityId: (visit._id as Types.ObjectId).toString(),
        entityType: 'StoreVisit',
        storeId: visit.storeId?.toString(),
  ...
  // AdBazaar closed-loop attribution (fire-and-forget — never blocks response)
  if (visit.userId && store?.merchantId) {
    const rezUserId = visit.userId.toString();
    const storeMerchantId = store.merchantId.toString();
    if (status === VisitStatus.CHECKED_IN) {
      fireAdBazaarVisitAttribution(rezUserId, storeMerchantId, new Date()).catch(() => {});
    } else if (status === VisitStatus.COMPLETED) {
      fireAdBazaarPurchaseAttribution(rezUserId, storeMerchantId, new Date()).catch(() => {});
    }
  }
  ...
  try {
    if (status === VisitStatus.CHECKED_IN) {
      await pushNotificationService.sendVisitCheckedIn(store.name, visit.visitNumber, visit.customerPhone);
    } else if (status === VisitStatus.COMPLETED) {
      await pushNotificationService.sendVisitCompleted(store.name, visit.visitNumber, visit.customerPhone);
    } else if (status === VisitStatus.CANCELLED) {
      await pushNotificationService.sendVisitCancelled(store.name, visit.visitNumber, visit.customerPhone);
    }
  } catch (smsError) {

...
const DEFAULT_CONFIGS = [
  { action: 'share_store', isEnabled: true, baseCoins: 10, bonusCoins: 0, dailyLimit: 5, requiresModeration: false },
  { action: 'share_offer', isEnabled: true, baseCoins: 5, bonusCoins: 0, dailyLimit: 10, requiresModeration: false },
...
export const updateConfig = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (updates.baseCoins !== undefined && (typeof updates.baseCoins !== 'number' || updates.baseCoins < 0 || updates.baseCoins > 1000)) {
      return res.status(400).json({ success: false, error: 'baseCoins must be between 0 and 1000' });
    }
    if (updates.bonusCoins !== undefined && (typeof updates.bonusCoins !== 'number' || updates.bonusCoins < 0 || updates.bonusCoins > 1000)) {
      return res.status(400).json({ success: false, error: 'bonusCoins must be between 0 and 1000' });
    }
    if (updates.dailyLimit !== undefined && (typeof updates.dailyLimit !== 'number' || updates.dailyLimit < 1 || updates.dailyLimit > 100)) {
      return res.status(400).json({ success: false, error: 'dailyLimit must be between 1 and 100' });
    }
    if (updates.multiplier !== undefined && (typeof updates.multiplier !== 'number' || updates.multiplier < 1 || updates.multiplier > 10)) {

...
export const addToWishlist = asyncHandler(async (req: Request, res: Response) => {
  ...
  const {
    ...
    tags,
    discountSnapshot: clientSnapshot,
  } = req.body;
  ...
  try {
    ...
    redisService
      .delPattern(`wishlist:status:${userId}:*`)
      .catch((err) =>
        logger.warn('[Wishlist] Cache invalidation for wishlist status after add failed', { error: err.message }),
      );
...
export const removeFromWishlist = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    redisService
      .delPattern(`wishlist:status:${userId}:*`)
      .catch((err) =>
        logger.warn('[Wishlist] Cache invalidation for wishlist status after remove failed', { error: err.message }),
      );
...
export const checkWishlistStatus = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const normalizedItemType =
      (itemType as string).charAt(0).toUpperCase() + (itemType as string).slice(1).toLowerCase();
    ...
    let result: any;
    if (wishlist) {
      const item = (wishlist as any).items.find(
        (i: any) => i.itemType === normalizedItemType && i.itemId.toString() === itemId,
      );
    ...
    redisService
      ...
      .catch((err) => logger.warn('[Wishlist] Cache set for wishlist status failed', { error: err.message })); // 60s cache

...
import { ReferralAbuseDetector } from '../services/ReferralAbuseDetector';
...
export const getRewards = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const referrals = await Referral.find({ referrer: userId }).limit(500).lean();
...
export const claimReward = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!isValidObjectId(referralId)) {
    return sendBadRequest(res, 'Invalid referral ID');
  }
...
export const getLeaderboard = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const leaderboard = await referralAnalyticsService.getLeaderboard(limitNum);
...
export const generateQR = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const user = await User.findById(userId).select('_id referralCode phoneNumber').lean();
    ...
    if (!referralCode) {
      ...
      // Update the document with new referral code
...
export const validateCode = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const user = await User.findOne({ referralCode: code.trim().toUpperCase() }).lean();
...
export const applyCode = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const referrer = await User.findOne({ referralCode: code.trim().toUpperCase() }).lean();
    ...
    // Device/cluster-based abuse detection (ISSUE #14)
    const abuseCheck = await ReferralAbuseDetector.check({
      referrerId: referrer._id.toString(),
      refereeId: userId,
      refereePhone: referee?.phone || '',
      refereeDevice: req.headers['x-device-id'] as string,
      refereeIp: req.ip,
      // signupDevice/signupIp are legacy fields not in the IUser interface
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      referrerDevice: (referrer as any)?.signupDevice,
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      referrerIp: (referrer as any)?.signupIp,
    });

    if (abuseCheck.action === 'block') {
      await ReferralAbuseDetector.storeFraudFlag(referrer._id.toString(), userId, abuseCheck);
      // Don't reveal we blocked — just silently skip the reward
      return sendSuccess(
        res,
        {
          success: true,
    ...
    if (abuseCheck.action === 'flag') {
      await ReferralAbuseDetector.storeFraudFlag(referrer._id.toString(), userId, abuseCheck);
      // Issue reward but flag for review
    }
    ...
    const bonusAmount = abuseCheck.action === 'flag' ? 15 : 30; // Reduced bonus if flagged
    // NOTE: referee is a lean document — using findByIdAndUpdate for atomic wallet credit
    await User.findByIdAndUpdate(referee._id, {
    ...
    sendSuccess(
      res,
      {
        ...
        message: `Welcome! You've received ₹${bonusAmount} bonus for using ${referrer.fullName || 'a friend'}'s referral code`,
      },
      'Referral code applied successfully',
    );
  } catch (error: any) {

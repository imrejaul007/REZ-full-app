...
import { sendSuccess, sendError, sendBadRequest, sendCreated } from '../utils/response';
...
import personaResolverService, { PersonaId, PriceBucket } from '../services/personaResolverService';
...
// ─────────────────────────────────────────────────────────────────────────────
// Persona-aware seller comparator
// Sorts the sellers array within a grouped-product result according to the
// active persona's ranking profile.
//
// Student  : price ASC → distance ASC → popularityWeight proxy (reviewCount DESC)
// Employee : rating DESC → distance ASC → booking-availability (hasQuickBooking flag)
// General  : current best-value score (unchanged legacy behaviour)
// ─────────────────────────────────────────────────────────────────────────────

function applyPersonaRanking(sellers: any[], personaId: PersonaId): any[] {
  const sorted = [...sellers];

  if (personaId === 'student') {
    sorted.sort((a, b) => {
      // Primary: price ASC
      const priceDiff = (a.price?.current ?? 0) - (b.price?.current ?? 0);
      if (priceDiff !== 0) return priceDiff;
      // Secondary: distance ASC
      const distDiff = (a.distance ?? 999) - (b.distance ?? 999);
      if (distDiff !== 0) return distDiff;
      // Tertiary: popularity proxy — reviewCount DESC
    ...
    sorted.sort((a, b) => {
      ...
      const distDiff = (a.distance ?? 999) - (b.distance ?? 999);
      if (distDiff !== 0) return distDiff;
      // Tertiary: booking availability — sellers with hasQuickBooking first
      const aBook = a.hasQuickBooking ? 0 : 1;
      const bBook = b.hasQuickBooking ? 0 : 1;
      return aBook - bBook;
    });
  } else {
    // General: relevance / best-value composite (legacy)
    sorted.sort((a, b) => {
      const scoreA =
...
/**
 * Check whether a seller matches student persona boost criteria:
 *   avgTicket < 300  AND  cashbackPercentage >= 10
 */
function isStudentBoosted(seller: any): boolean {
  return (seller.price?.current ?? Infinity) < 300 && (seller.cashback?.percentage ?? 0) >= 10;
}

/**
 * Check whether a seller matches employee persona boost criteria:
 *   rating >= 4.0  AND  hasQuickBooking flag
 */
function isEmployeeBoosted(seller: any): boolean {
  return (seller.rating ?? 0) >= 4.0 && !!seller.hasQuickBooking;
}
...
const searchProducts = async (query: string, limit: number, mode?: ModeId, region?: RegionId): Promise<any> => {
  try {
    ...
    // DANIEL: lean() on Store query — returns plain JS objects avoiding Mongoose overhead
    if (needsStoreFilter) {
      // DB-AUDIT-4 FIX: added .limit(10000) to prevent unbounded collection scan (BUG-008)
      const filteredStores = await Store.find(storeFilter).select('_id').limit(10000).lean();
    ...
    const sortedProducts = sortByRelevance(regexProducts, query, ['name', 'brand', 'description']).slice(0, limit);
...
const searchProductsGroupedInternal = async (
  ...
  try {
    ...
    // BUG-040 FIX: Run the two store queries in parallel and cache the resulting
    // IDs so they are never re-fetched downstream.  Previously the queries were
    // sequential (two round-trips) and the IDs were not reused — later code in
    // this function re-queried the same store collection.
    //
    // Query 1: stores that match the search term (for store-name result cards)
    // Query 2: ALL stores in region (for product filtering) — only if region set
    const [matchingStores, regionStores] = await Promise.all([
      // DANIEL: lean() on matchingStores — returns plain JS objects, not Mongoose Documents
      Store.find({
        ...baseStoreQuery,
        $or: [
          { name: { $regex: escapeRegex(query), $options: 'i' } },
        ...
        .limit(10)
        .lean(),
      // DB-AUDIT-5 FIX: added .limit(10000) to prevent unbounded collection scan (BUG-008)
      // Only run this query when a region filter is active
      region ? Store.find(baseStoreQuery).select('_id').limit(10000).lean() : Promise.resolve([] as any[]),
    ]);

    // Cache the IDs so they are never re-queried further down this function
    const matchingStoreIds = matchingStores.map((s) => s._id);
    const regionStoreIds: any[] = regionStores.map((s: any) => s._id);
    ...
    for (const [_normalizedName, productList] of productGroups.entries()) {
      ...
      for (const product of productList) {
        ...
        if (userLocation && store.location?.coordinates) {
          ...
          distance = calculateDistance(userLocation.latitude, userLocation.longitude, storeLat, storeLon);
        }

        // Get delivery info
        const deliveryInfo = product.deliveryInfo || store.operationalInfo;
        const deliveryTime =
          deliveryInfo?.estimatedDays || deliveryInfo?.deliveryTime || deliveryInfo?.standardDeliveryTime || '2-3 days';
      ...
      if (sellers.length > 0) {
        // Sort sellers using default general best-value ranking within this internal function.
        // The persona-aware re-sort is applied by the calling controller (searchProductsGrouped)
        // after the full grouped result is assembled.
        sellers.sort((a, b) => {
    ...
    // Calculate summary
    const prices = groupedProducts.flatMap((gp) => gp.sellers.map((s: any) => s.price.current));
    const priceRange = {
    ...
    const storeResults = sortByRelevance(matchingStores, query, ['name', 'description'])
      .slice(0, 5)
      .map((store: any) => {
        let distance: number | undefined;
        if (userLocation && store.location?.coordinates) {
...
export const globalSearch = asyncHandler(async (req: Request, res: Response) => {
  ...
  // ── Persona resolution ──
  // If the request is authenticated, resolve the user's persona to enable
  // persona-aware ranking and analytics enrichment.
  const userId: string | undefined = (req as any).user?._id?.toString() || (req as any).userId;
  let resolvedPersona = personaResolverService.resolveAnonymousPersona();
  if (userId) {
    try {
      resolvedPersona = await personaResolverService.resolvePersona(userId);
    } catch (personaErr) {
      logger.warn('[GLOBAL SEARCH] Persona resolution failed — using default', {
        userId,
        error: (personaErr as Error).message,
      });
    }
  }
  const personaId: PersonaId = resolvedPersona.personaId;
  const priceBucket: PriceBucket = resolvedPersona.feedConfig.priceBucket;
  ...
  // Generate cache key (include mode, region AND persona so each segment gets its own ranked cache)
  const cacheKey = `search:global:${query}:${validTypes.sort().join(',')}:${limit}:${activeMode}:${region || 'all'}:${personaId}`;
  ...
  try {
    ...
    const totalResults = Object.values(results).reduce((sum: number, r: any) => sum + r.total, 0);
    ...
    const responseData = {
      ...
      // Persona-aware analytics metadata
      persona: personaId,
      priceBucket,
    };
    ...
    const executionTime = Date.now() - startTime;
    logger.info(
      `[GLOBAL SEARCH] Completed in ${executionTime}ms. Total results: ${totalResults}. Persona: ${personaId}`,
    );
...
export const searchProductsGrouped = asyncHandler(async (req: Request, res: Response) => {
  ...
  // ── Persona resolution ──
  const groupedUserId: string | undefined = (req as any).user?._id?.toString() || (req as any).userId;
  let groupedPersona = personaResolverService.resolveAnonymousPersona();
  if (groupedUserId) {
    try {
      groupedPersona = await personaResolverService.resolvePersona(groupedUserId);
    } catch (personaErr) {
      logger.warn('[GROUPED SEARCH] Persona resolution failed — using default', {
        userId: groupedUserId,
        error: (personaErr as Error).message,
      });
    }
  }
  const groupedPersonaId: PersonaId = groupedPersona.personaId;
  const groupedPriceBucket: PriceBucket = groupedPersona.feedConfig.priceBucket;
  ...
  // Generate cache key (include region, filters, AND persona for per-segment ranked caches)
  const filterKey = hasFilters ? JSON.stringify(filters) : 'nofilter';
  const cacheKey = `search:grouped:${query}:${limit}:${userLocation ? `${userLocation.latitude},${userLocation.longitude}` : 'noloc'}:${region || 'all'}:${filterKey}:${groupedPersonaId}`;
  ...
  try {
    ...
    const result = await searchProductsGroupedInternal(
      query,
      limit,
      userLocation,
      region,
    ...
    // ── Persona-aware post-processing ──
    // Re-sort each product group's sellers using the persona ranking profile.
    // For student: boost items with avgTicket < 300 and cashback >= 10%.
    // For employee: boost items with rating >= 4.0 and hasQuickBooking.
    if (result.groupedProducts && result.groupedProducts.length > 0) {
      result.groupedProducts = result.groupedProducts.map((group: any) => {
        let sellers: any[] = group.sellers || [];

        // Apply persona-specific boosts: move boosted items to the front,
        // then apply the persona ranking within each partition.
        if (groupedPersonaId === 'student') {
          const boosted = sellers.filter(isStudentBoosted);
          const rest = sellers.filter((s: any) => !isStudentBoosted(s));
          sellers = [...applyPersonaRanking(boosted, groupedPersonaId), ...applyPersonaRanking(rest, groupedPersonaId)];
        } else if (groupedPersonaId === 'employee') {
          const boosted = sellers.filter(isEmployeeBoosted);
          const rest = sellers.filter((s: any) => !isEmployeeBoosted(s));
          sellers = [...applyPersonaRanking(boosted, groupedPersonaId), ...applyPersonaRanking(rest, groupedPersonaId)];
        } else {
          sellers = applyPersonaRanking(sellers, groupedPersonaId);
        }

        return { ...group, sellers };
      });
    }

    // Attach persona analytics metadata
    const enrichedResult = {
      ...result,
      persona: groupedPersonaId,
      priceBucket: groupedPriceBucket,
    };

    // Cache the results for 10 minutes (600 seconds)
    const CACHE_TTL = 600;
    await redisService.set(cacheKey, enrichedResult, CACHE_TTL);

    const executionTime = Date.now() - startTime;
    logger.info(
      `[GROUPED SEARCH] Completed in ${executionTime}ms. Products: ${result.total}, Sellers: ${result.summary.sellerCount}. Persona: ${groupedPersonaId}`,
    );
...
export const aiSearch = asyncHandler(async (req: Request, res: Response) => {
  ...
  const underMatch = queryLower.match(
    /(?:under|below|less than|max|upto|up to|within)\s*(?:₹|aed|rs\.?|inr|dhs?)?\s*(\d+[\d,]*)/i,
  );
  if (underMatch) {
    maxPrice = parseInt(underMatch[1].replace(/,/g, ''));
  }
  const aboveMatch = queryLower.match(
    /(?:above|over|more than|min|at least)\s*(?:₹|aed|rs\.?|inr|dhs?)?\s*(\d+[\d,]*)/i,
  );
  if (aboveMatch) {
  ...
  let keywords = rawQuery
    .replace(
      /(?:under|below|less than|max|upto|up to|within|above|over|more than|min|at least)\s*(?:₹|aed|rs\.?|inr|dhs?)?\s*\d+[\d,]*/gi,
      '',
    )
  ...
  const productResult = await searchProductsGroupedInternal(
    keywords,
    10,
    undefined,
    region,
...
export const saveSearchHistory = asyncHandler(async (req: Request, res: Response) => {
  ...
  const isDuplicate = await (SearchHistory as any).isDuplicate(userId, trimmedQuery, type, 5);
  ...
  // Fire search signal to marketing service for keyword targeting (non-blocking)
  MarketingSignalService.searchSignal(userId.toString(), trimmedQuery);
...
export const getSearchHistory = asyncHandler(async (req: Request, res: Response) => {
  ...
  const [searches, total] = await Promise.all([
    SearchHistory.find(query).sort({ createdAt: -1 }).skip(skip).limit(limitNum).select('-__v').lean(),
...
export const getPopularSearches = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (userId) {
    ...
    popularSearches = await (SearchHistory as any).getPopularSearches(userId, limitNum);
  }
...
export const getAutocomplete = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // BUG-040 FIX (global search): Cache region store IDs from a single query
    // and reuse them for both product and store filtering.  Previously the region
    // filter was re-applied twice: once to get store IDs for product filtering and
    // again by merging regionFilter into storeQuery — causing two store collection
    // scans for the same data.
    let regionStoreIds: any[] = [];
    let cachedRegionFilter: any = null;
    if (region) {
      cachedRegionFilter = regionService.getStoreFilter(region);
      const regionStores = await Store.find({ isActive: true, ...cachedRegionFilter })
        .select('_id')
        .lean();
      regionStoreIds = regionStores.map((s) => s._id);
    }

    // Build product query with region filter (reuse cached IDs)
    const productQuery: any = {
      isActive: true,
      'inventory.isAvailable': true,
      $or: [{ name: searchRegex }, { title: searchRegex }, { brand: searchRegex }, { description: searchRegex }],
    };
    ...
    // Build store query — reuse the cached region filter (no second getStoreFilter call)
    const storeQuery: any = {
      isActive: true,
      $or: [{ name: searchRegex }, { description: searchRegex }],
    };

    if (region && cachedRegionFilter) {
      Object.assign(storeQuery, cachedRegionFilter);
    }
...
export const getDidYouMean = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (q.length > 200) return sendBadRequest(res, 'Search query too long (max 200 characters)');

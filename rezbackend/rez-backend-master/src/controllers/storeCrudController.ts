...
import { getCategoryRootMap } from './orderCreateController';
  ...
  try {
    ...
    if (isFeatured !== undefined) {
      ...
      const isFeaturedValue =
        typeof isFeatured === 'string' ? isFeatured.toLowerCase() === 'true' : Boolean(isFeatured);
    ...
    const [stores, total] = await Promise.all([
      Store.find(query)
        .select(
          'name slug logo banner description category tags ratings location isActive isFeatured offers operationalInfo serviceCapabilities bookingConfig bookingType hasStorePickup',
        )
        .populate({
          path: 'category',
          select: 'name slug',
...
export const getStoreById = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Cache store core data for 5 minutes — called on every store visit (hot path).
    // View count increment is intentionally excluded from the cache to avoid stale counters.
    const storeCacheKey = `store:detail:${storeId}`;
    const STORE_DETAIL_TTL = 300; // 5 minutes

    // Single aggregation: fetch store with category $lookup (replaces populate)
    const store = await withCache(storeCacheKey, STORE_DETAIL_TTL, async () => {
    ...
    // Build category resolution promise — uses cached root map (1 query for ALL categories)
    // instead of a sequential per-ancestor findById loop (was up to 5 sequential queries).
    const resolveCategorySlug = async (): Promise<string | null> => {
      try {
        const storeCat = store.category;
        const catId =
          storeCat && typeof storeCat === 'object' && '_id' in storeCat
            ? storeCat._id?.toString()
            : storeCat?.toString();
        if (!catId) return null;
        const rootMap = await getCategoryRootMap();
        return rootMap.get(catId) || null;
      } catch {
        /* Non-critical */
      }
      return null;
    ...
    if (deviceHash) {
      import('../services/deviceFingerprintService')
        .then((svc) => svc.trackMerchantAccess(deviceHash, String(store._id), store.name))
        .catch((err) =>
          logger.error('[StoreCrudCtrl] Device fingerprint merchant access tracking failed', {
            error: err.message,
            storeId: store._id,
          }),
        );
...
/**
 * PACKETSENSE OPTIMIZATION: Store detail aggregation endpoint
 * Single API call returns store info + services + reviews + campaigns + wishlisted status
 * Reduces 4-5 separate API calls to 1, saving significant bandwidth
 */
export const getStoreDetailFull = asyncHandler(async (req: Request, res: Response) => {
  const { storeId } = req.params;
  ...
  try {
    ...
    // Fetch store in parallel with services, reviews, campaigns
    const [storeResults, servicesResults, reviewsResults, campaignsResults] = await Promise.all([
      // Store detail with minimal fields
      Store.aggregate([
        { $match: matchStage },
        { $lookup: { from: 'categories', localField: 'category', foreignField: '_id', as: 'category' } },
        { $unwind: { path: '$category', preserveNullAndEmptyArrays: true } },
      ...
      // Services - limited to summary fields only
      (async () => {
        try {
          // @ts-ignore
          const { default: Service } = await import('../models/Service');
          return await Service.find({ storeId, isActive: true })
            .select('name price discountedPrice duration category images')
            .limit(20)
            .lean();
        } catch {
          return [];
      ...
      // Reviews - only top 5, minimal fields
      (async () => {
        try {
          const { default: Review } = await import('../models/Review');
          return await Review.find({ storeId })
            .select('rating comment createdAt userId userName')
            .sort({ createdAt: -1 })
            .limit(5)
            .lean();
        } catch {
      ...
      (async () => {
        try {
          ...
          const now = new Date();
          return await BonusCampaign.find({
            storeId,
            isActive: true,
            startDate: { $lte: now },
            endDate: { $gte: now },
          })
            .select('title description coinReward expiresAt')
            .lean();
        } catch {
          return [];
        }
    ...
    // Check wishlist status if authenticated (parallelizable after store ID obtained)
    let isWishlisted = false;
    if (userId) {
      try {
        const { Wishlist } = await import('../models/Wishlist');
        isWishlisted = !!(await Wishlist.findOne({ userId, storeId: store._id }).lean());
      } catch {
        isWishlisted = false;
      }
    }
    ...
    Store.updateOne({ _id: store._id }, { $inc: { 'analytics.views': 1 } }).catch(() => {});
    ...
    const etag = require('crypto')
      ...
      .update(
        JSON.stringify({
          ...
          updated: store.updatedAt?.getTime() || Date.now(),
        }),
      )
      .digest('hex');
    ...
    logger.error('[GET STORE DETAIL FULL] Error:', error);
    throw new AppError(`Failed to fetch store detail: ${error.message}`, 500);
  }
});
...
export const getStoreProducts = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // PERF: Explicit .select() keeps payload to fields the client actually renders.
    // Without it, the full Product document (~60+ fields including inventory details,
    // merchant metadata, allergens, variants, etc.) is transferred on every store-products
    // page load. Trimming to display-only fields cuts per-item payload by ~70%.
    const [products, total] = await Promise.all([
      Product.find(query)
        .populate('category', 'name slug')
        .select(
          'name slug images pricing ratings category tags inventory.isAvailable inventory.stock inventory.unlimited isFeatured analytics.views createdAt',
        )
        .sort(sortOptions)
        .skip(skip)
        .limit(Number(limit))
...
export const getStoreOperatingStatus = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const isOpen =
      todayHours && !todayHours.closed && currentTime >= todayHours.open && currentTime <= todayHours.close;
...
export const getStoresByCategory = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [stores, total] = await Promise.all([
      Store.find(query)
        .select(
          'name slug logo banner category tags ratings location isActive isFeatured offers operationalInfo serviceCapabilities bookingConfig bookingType hasStorePickup',
        )
        .populate('category', 'name slug')
        .sort(sortOptions)
        .skip(skip)
...
export const getStoreCategories = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    const configs = await StoreCollectionConfig.find({ isEnabled: true }).sort({ sortOrder: 1 }).limit(50).lean();
  ...
  try {
    ...
    if (isSubcategory) {
      ...
      query = {
        ...
        $or: [{ subcategory: category._id }, { subCategories: category._id }, { subcategorySlug: slug }],
      };
    } else {
      ...
      logger.info(
        `[GET STORES BY SLUG] Searching as MAIN CATEGORY: ${slug}, including ${categoryIds.length} category IDs`,
      );
      query = {
        isActive: true,
        isSuspended: { $ne: true },
    ...
    const [stores, total] = await Promise.all([
      Store.find(query)
        .select(
          'name slug logo banner category tags ratings location isActive isFeatured offers operationalInfo serviceCapabilities bookingConfig bookingType hasStorePickup deliveryCategories type isOpen rewardRules priceForTwo storeVisitConfig',
        )
    ...
    const storesWithProducts = stores.map((store: any) => {
      ...
      const transformedProducts = products.map((product: any) => ({
        ...
        imageUrl: product.images?.[0] || 'https://placehold.co/150',

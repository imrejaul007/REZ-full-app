...
/** Shape of a populated Store field on VoucherBrand */
            ...
            metadata: {
              ...
              storeInfo: brand.store
                ? {
                    name: brand.name,
                    id:
                      typeof brand.store === 'object'
                        ? (brand.store as unknown as PopulatedStore)._id?.toString()
                        : String(brand.store),
                  }
                : undefined,
            },
...
export const confirmCardPurchase = asyncHandler(async (req: Request, res: Response) => {
  ...
  const transaction = new Transaction({
    ...
    source: {
      type: 'order',
      reference: userVoucher._id as Types.ObjectId,
      description: `Voucher purchase - ${brand.name}`,
      metadata: {
        orderNumber: `VOUCHR-${String(userVoucher._id).substring(0, 8)}`,
        paymentIntentId,
  ...
  await VoucherBrand.findByIdAndUpdate(brandId, { $inc: { purchaseCount: 1 } });
...
export const getHeroCarousel = asyncHandler(async (req: Request, res: Response) => {
  ...
  const cachedCarousel = await withCache(cacheKey, CacheTTL.VOUCHER_LIST, async () => {
    ...
    const carouselItems = limitedBrands.map((brand, _index) => {
      ...
      return {
        ...
        store:
          brand.store && typeof brand.store === 'object' && '_id' in brand.store
            ? {
                id: (brand.store as unknown as PopulatedStore)._id?.toString(),
                name: (brand.store as unknown as PopulatedStore).name,
                slug: (brand.store as unknown as PopulatedStore).slug,
                address: (brand.store as unknown as PopulatedStore).location?.address,
              }
            : null,
        action: {
          type: 'brand' as const,

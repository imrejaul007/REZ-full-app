...
import { dispatchGiftRedeemed, dispatchGiftExpired } from '../services/rendezWebhookDispatch';
...
export const sendGift = asyncHandler(async (req: Request, res: Response) => {
  ...
  const senderId = (req as any).userId;
  const { recipientPhone, recipientId, amount, coinType, theme, message, deliveryType, scheduledAt, idempotencyKey } =
    req.body;
  ...
  if (idempotencyKey) {
    ...
    if (existing) {
      ...
      // Failed/cancelled/expired — atomically clear key so user can retry fresh
      if (['failed', 'cancelled', 'expired'].includes(existing.status)) {
        await CoinGift.findOneAndUpdate(
          { _id: existing._id, status: { $in: ['failed', 'cancelled', 'expired'] } },
  ...
  const effectiveCoinType = coinType || 'rez';
  ...
  try {
    ...
    // ISSUE-63 FIX: pass session so this write is part of the same MongoDB
    // transaction — without it the CoinTransaction document is committed even
    // when the session is later aborted, leaving an orphaned debit record with
    // no matching wallet mutation.
    ...
    await ledgerService.recordEntry({
      ...
      amount: validatedAmount,
      coinType: (effectiveCoinType as any) || 'rez',
      operationType: 'gift',
  ...
  logTransaction({
    ...
    balanceAfter: {
      total: debitResult.balance.total,
      available: debitResult.balance.available,
      pending: 0,
      cashback: 0,
    },
    reference: {
      type: 'other',
      id: String(gift._id),
      description: `Gift to ${recipient.fullName || recipient.phoneNumber}`,
    },
  });
...
export const claimGift = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!gift) {
      ...
      if (!existing) return sendNotFound(res, 'Gift not found');
      ...
      if (existing.expiresAt < new Date()) {
        if (existing.status !== 'expired') {
          ...
          // Fire-and-forget: notify Rendez that a gift expired
          dispatchGiftExpired({
            giftId: String(existing._id),
            senderRezUserId: String(existing.sender),
            recipientRezUserId: userId,
            amount: existing.amount,
    ...
    await CoinGift.updateOne({ _id: gift._id }, { $set: { recipientTxId: recipientTx[0]._id } }, { session });
  ...
  logTransaction({
    ...
    amount: gift.amount,
    balanceBefore: {
      total: recipientWalletBefore.balance.total,
      available: recipientWalletBefore.balance.available,
      pending: 0,
      cashback: 0,
    },
    balanceAfter: {
  ...
  try {
    ...
    await ledgerService.recordEntry({
      ...
      amount: gift.amount,
      coinType: (gift.coinType as any) || 'rez',
      operationType: 'gift',
      referenceId: String(gift._id),
      referenceModel: 'CoinGift',
      metadata: {
  ...
  // Fire-and-forget: notify Rendez that a gift was redeemed
  dispatchGiftRedeemed({
    giftId: String(gift._id),
    senderRezUserId: String(gift.sender),
    recipientRezUserId: userId,
    amount: gift.amount,
    claimedAt: new Date(),
  });

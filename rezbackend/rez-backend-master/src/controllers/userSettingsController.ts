  ...
  if (sessionManagement) {
    ...
    for (const field of smFields) {
      if (sessionManagement[field] !== undefined)
        updates[`security.sessionManagement.${field}`] = sessionManagement[field];
    }
  }
...
export const enableTwoFactorAuth = asyncHandler(async (req: Request, res: Response) => {
  ...
  // SECURITY: Block 2FA enrollment in production until verification is implemented.
  // Without a working verifyTwoFactorCode endpoint, users who enable 2FA will be
  // unable to verify codes and effectively lock themselves into a broken state.
  if (process.env.NODE_ENV === 'production') {
    throw new AppError(
      'Two-factor authentication is coming soon. This feature is not yet available.',
      503,
    );
  }
...
export const verifyTwoFactorCode = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { code, method: _method } = req.body;
  ...
  if (isBackupCode) {
    // Use atomic findOneAndUpdate with $pull to remove backup code in single operation
    // This prevents TOCTOU race condition where multiple concurrent requests use same code
    const result = await UserSettings.findOneAndUpdate(
      { user: req.user._id, 'security.twoFactorAuth.backupCodes': code },
      { $pull: { 'security.twoFactorAuth.backupCodes': code } },
    );

    if (!result) {
      // Code was already used by another concurrent request
      throw new AppError('Backup code already used', 400);
    }
...
export const updatePaymentPreferences = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { defaultPaymentMethodId, autoPayEnabled, paymentPinEnabled, biometricPaymentEnabled, transactionLimits } =
    req.body;
  const updates: Record<string, any> = {};
  ...
  if (transactionLimits) {
    ...
    for (const field of tlFields) {
      if (transactionLimits[field] !== undefined)
        updates[`payment.transactionLimits.${field}`] = transactionLimits[field];
    }
  }
...
export const updateAppPreferences = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (modeSettings) {
    if (modeSettings.nearU) {
      if (modeSettings.nearU.radius !== undefined)
        updates['preferences.modeSettings.nearU.radius'] = modeSettings.nearU.radius;
      if (modeSettings.nearU.showNotifications !== undefined)
        updates['preferences.modeSettings.nearU.showNotifications'] = modeSettings.nearU.showNotifications;
    }
    if (modeSettings.mall) {
      if (modeSettings.mall.preferredCategories !== undefined)
        updates['preferences.modeSettings.mall.preferredCategories'] = modeSettings.mall.preferredCategories;
    }
    if (modeSettings.cash) {
      if (modeSettings.cash.minCashbackPercent !== undefined)
        updates['preferences.modeSettings.cash.minCashbackPercent'] = modeSettings.cash.minCashbackPercent;
    }
    if (modeSettings.prive) {
      if (modeSettings.prive.tier !== undefined)
        updates['preferences.modeSettings.prive.tier'] = modeSettings.prive.tier;
    }
  }
...
export const updateCourierPreferences = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { preferredCourier, deliveryTimePreference, deliveryInstructions, alternateContact, courierNotifications } =
    req.body;
  const updates: Record<string, any> = {};

  if (preferredCourier !== undefined) updates['courier.preferredCourier'] = preferredCourier;
  if (deliveryTimePreference) {
    if (deliveryTimePreference.weekdays !== undefined)
      updates['courier.deliveryTimePreference.weekdays'] = deliveryTimePreference.weekdays;
    if (deliveryTimePreference.preferredTimeSlot) {
      if (deliveryTimePreference.preferredTimeSlot.start !== undefined)
        updates['courier.deliveryTimePreference.preferredTimeSlot.start'] =
          deliveryTimePreference.preferredTimeSlot.start;
      if (deliveryTimePreference.preferredTimeSlot.end !== undefined)
        updates['courier.deliveryTimePreference.preferredTimeSlot.end'] = deliveryTimePreference.preferredTimeSlot.end;
    }
    if (deliveryTimePreference.avoidWeekends !== undefined)
      updates['courier.deliveryTimePreference.avoidWeekends'] = deliveryTimePreference.avoidWeekends;
  }
  if (deliveryInstructions) {
    ...
    for (const field of diFields) {
      if (deliveryInstructions[field] !== undefined)
        updates[`courier.deliveryInstructions.${field}`] = deliveryInstructions[field];
    }
  }
  if (alternateContact) {
  ...
  if (courierNotifications) {
    ...
    for (const field of cnFields) {
      if (courierNotifications[field] !== undefined)
        updates[`courier.courierNotifications.${field}`] = courierNotifications[field];
    }
  }

...
export const getStoreMenu = asyncHandler(async (req: Request, res: Response) => {
  ...
  // ETag + cache headers for menu responses
  const menuHash = crypto.createHash('md5').update(JSON.stringify(response)).digest('hex');
  const etag = `"${menuHash}"`;
...
export const addMenuItem = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Get menu — must be a Mongoose document to call instance methods
...
export const createPreOrder = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { storeId, items, scheduledTime, deliveryType, tableNumber, deliveryAddress, contactPhone, notes } = req.body;
...
export const getPreOrder = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Ownership check is always enforced — userId is required above
  if (preOrder.userId._id.toString() !== userId) {
...
export const cancelPreOrder = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Must NOT use .lean() — updateStatus is a Mongoose instance method
  const preOrder = await PreOrder.findById(preOrderId);
  if (!preOrder) {
    return sendError(res, 'Pre-order not found', 404);
  }

  // Check authorization — userId is always set (route is under authenticate middleware)

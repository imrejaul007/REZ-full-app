...
export const getNearbyStores = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!finalLng || !finalLat) {
    return sendBadRequest(
      res,
      'Longitude and latitude are required. Provide lng/lat, longitude/latitude, or location as "lng,lat"',
    );
  }
  ...
  try {
    ...
    const stores = await Store.find(query)
      .select(
        'name slug logo category tags ratings.average location isActive isFeatured offers.cashback operationalInfo.deliveryTime serviceCapabilities.serviceTypes bookingType hasStorePickup',
      )
...
export const getFeaturedStores = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const storeListSelect =
      'name slug logo banner category tags ratings location isActive isFeatured offers operationalInfo';
    ...
    const transformedStores = await withCache(cacheKey, CacheTTL.STORE_LIST, async () => {
      ...
      let stores = await Store.find({
        ...
        .select(
          'name slug logo category tags ratings.average location isActive isFeatured offers.cashback operationalInfo.deliveryTime',
        )
        .populate('category', 'name slug')
...
export const searchStores = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [stores, total] = await Promise.all([
      Store.find(query)
        .select(
          'name slug logo category tags ratings.average location isActive isFeatured offers.cashback operationalInfo.deliveryTime bookingType hasStorePickup',
        )
        .populate('category', 'name slug')
        .sort({ 'ratings.average': -1, createdAt: -1 })
        .skip(skip)
...
export const searchStoresByCategory = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { location, radius = 10, page = 1, limit = 20, sortBy = 'rating', rezPay } = req.query;
    ...
    if (total > 0) {
      ...
      for (const store of stores) {
        ...
        const transformedProducts = products.map((product: any) => {
          ...
          return {
            ...
            imageUrl: product.images?.[0] || product.image || 'https://placehold.co/300',
            ...
            subcategory:
              (product.subcategory && typeof product.subcategory === 'object' ? product.subcategory.name : null) || '',
            brand: product.brand || '',
      ...
      if (stores.length > 0 && stores[0].products) {
        ...
        if (stores[0].products.length > 0) {
          ...
          logger.debug(
            `  First product: "${p.name}", price: ${p.price} (type: ${typeof p.price}), rating: ${p.rating}`,
          );
        }
      }
    }
...
export const searchStoresByDeliveryTime = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const filteredStores = stores.filter((store: any) => {
      ...
      if (timeMatch) {
        const minDeliveryTime = parseInt(timeMatch[1], 10);
        const maxDeliveryTime = parseInt(timeMatch[2], 10);
      ...
      if (singleTimeMatch) {
        const deliveryTime = parseInt(singleTimeMatch[1], 10);
...
export const advancedStoreSearch = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [stores, total] = await Promise.all([
      Store.find(query)
        .select(
          'name slug logo banner category tags ratings location isActive isFeatured offers operationalInfo serviceCapabilities bookingConfig bookingType hasStorePickup paymentSettings.acceptRezCoins paymentSettings.acceptPromoCoins paymentSettings.maxCoinRedemptionPercent rewardRules.baseCashbackPercent',
        )
        .sort(sort)
        .skip(skip)
        .limit(Number(limit))
    ...
    if (userLng !== undefined && userLat !== undefined && stores.length > 0) {
      stores.forEach((store: any) => {
        if (
          ...
          try {
            store.distance = calculateDistance([userLng, userLat], store.location.coordinates);
          } catch (_e) {
            store.distance = null;
          }
        }
          ...
          pipeline: [
            {
              $match: {
                $expr: {
                  $and: [{ $eq: ['$_id', '$$categoryId'] }, { $eq: ['$isActive', true] }],
                },
              },
            },
            {
  ...
  try {
    ...
    const formattedStores = stores.map((store: any) => {
      ...
      if (
        ...
        paymentMethods.includes('pay-later') ||
...
export const getStoresByTag = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [stores, total] = await Promise.all([
      Store.find(query).populate('category', 'name slug icon').sort(sortOptions).skip(skip).limit(Number(limit)).lean(),
      Store.countDocuments(query),
    ]);
...
export const getCuisineCounts = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    const [facetResult] = await Store.aggregate([{ $match: { isActive: true } }, { $facet: facetStages }]);
    ...
    const sortedCounts = cuisineCounts.filter((c) => c.count > 0).sort((a, b) => b.count - a.count);
  ...
  try {
    ...
    switch (sort) {
      ...
      case 'newest':
        ...
        break;
      case 'popularity':
        sortOptions['analytics.totalOrders'] = -1;
        break;
      case 'name':
        sortOptions.name = 1;
        break;
    ...
    const [stores, total] = await Promise.all([
      Store.find(query)
        .select(
          'name slug logo banner ratings tags offers operationalInfo location serviceCapabilities deliveryCategories rewardRules isFeatured priceForTwo bookingType bookingConfig',
        )
        .populate('category', 'name slug')
        .sort(sortOptions)
        .skip(skip)

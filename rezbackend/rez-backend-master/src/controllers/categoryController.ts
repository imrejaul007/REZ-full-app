...
export const getFeaturedCategories = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Cache featured categories for 1 hour — called on every app open (hot path).
    const cacheKey = `category:featured:${type || 'all'}:${limitNum}`;
    const categories = await withCache(cacheKey, CacheTTL.CATEGORY_LIST, () =>
      Category.find(query)
...
export const getBestSellerCategories = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    // Cache best-seller categories for 1 hour — involves multiple aggregation pipelines.
    const bestSellerCacheKey = `category:bestSeller:${Number(limit)}`;
    const categories = await withCache(bestSellerCacheKey, CacheTTL.CATEGORY_LIST, async () => {
      ...
      if (cats.length === 0) {
        ...
        if (categoryStats.length > 0) {
          ...
          const fallbackCategoryIds = cats.map((c: any) => c._id);
          ...
          cats.sort((a: any, b: any) => (b.productCount || 0) - (a.productCount || 0));
        }
      } else {
        // Batch compute stats for all marked categories at once
        const markedIds = cats.map((c: any) => c._id);
...
export const getCategoryVibes = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const category = await Category.findOne({ slug, isActive: true }).select('vibes').lean();
...
export const getCategoryOccasions = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const category = await Category.findOne({ slug, isActive: true }).select('occasions').lean();
...
export const getCategoryHashtags = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const category = await Category.findOne({ slug, isActive: true }).select('trendingHashtags').lean();
...
export const getCategoryAISuggestions = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const category = await Category.findOne({ slug, isActive: true }).select('aiSuggestions aiPlaceholders').lean();
...
export const getRecentOrders = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    // Cache recent-orders ticker for 2 minutes — social proof data, slight staleness is acceptable.
    const recentOrdersCacheKey = `category:recentOrders:${slug}:${limit}`;
    const formattedOrders = await withCache(recentOrdersCacheKey, 120, async () => {
      const category = await Category.findOne({ slug, isActive: true }).select('_id').lean();

      if (!category) return [];

      // Parallel: resolve category product IDs (needed to scope the orders query)
      const categoryProductIds = await Product.find({ category: category._id, isActive: true }, { _id: 1 })
        .limit(500)
        .lean(); // cap at 500 product IDs to keep $in query fast
      const productIdSet = categoryProductIds.map((p) => p._id);

      if (productIdSet.length === 0) return [];
    ...
    if (formattedOrders === null || formattedOrders.length === 0) {
      // Category might not exist or have no orders — check if category exists
      const exists = await Category.exists({ slug, isActive: true });
      if (!exists) return sendNotFound(res, 'Category not found');
    }
...
export const getCategoryPageConfig = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const category = await Category.findOne({ slug, isActive: true })
      .select(
        'name slug icon image bannerImage type pageConfig metadata vibes occasions trendingHashtags aiSuggestions aiPlaceholders promotions maxCashback childCategories sortOrder',
      )
      .populate(
        'childCategories',
        'name slug icon image sortOrder metadata isActive maxCashback productCount storeCount',
      )
      .lean();
    ...
    const childCategories = (category.childCategories || []).sort(

...
export const completeContent = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (timeSpentSeconds === undefined || timeSpentSeconds === null || isNaN(parsedTime) || parsedTime < 0) {
    return sendError(res, 'timeSpentSeconds is required and must be a non-negative number', 400);
  }
  if (parsedTime > 86400) {
    return sendError(res, 'timeSpentSeconds cannot exceed 86400 (24 hours)', 400);
  }

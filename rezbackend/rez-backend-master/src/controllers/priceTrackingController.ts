...
export const getPriceStats = asyncHandler(async (req: Request, res: Response) => {
  ...
  const [latest, lowest, highest, average, trend] = await Promise.all([
    ...
    PriceHistory.getLowestPrice(productId, variantId as string, parseInt(days as string, 10)),
    PriceHistory.getHighestPrice(productId, variantId as string, parseInt(days as string, 10)),
    PriceHistory.getAveragePrice(productId, variantId as string, parseInt(days as string, 10)),
    PriceHistory.getPriceTrend(productId, variantId as string, parseInt(days as string, 10)),
  ]);
...
export const createPriceAlert = asyncHandler(async (req: Request, res: Response) => {
  ...
  const hasActive = await PriceAlert.hasActiveAlert(userId as string, productId, variantId);
  if (hasActive) {
  ...
  // Get current price — IProduct uses pricing.selling / pricing.original
  // IProductVariant uses variantId (string) and price (number)
  let currentPrice = 0;
  if (variantId && product.inventory?.variants) {
    const variant = product.inventory.variants.find((v: any) => v.variantId === variantId);
    if (variant) {
      currentPrice = variant.price || 0;
    }
  } else {
    currentPrice = product.pricing?.selling || product.pricing?.original || 0;
  }
  ...
  const alert = new PriceAlert({
    ...
    metadata: {
      ...
      variantAttributes: variantId
        ? product.inventory?.variants?.find((v: any) => v.variantId === variantId)?.attributes
        : null,
      ipAddress: req.ip,
...
export const getMyAlerts = asyncHandler(async (req: Request, res: Response) => {
  ...
  const alerts = await PriceAlert.getUserAlerts(userId as string, {
  ...
  const countQuery: Record<string, unknown> = { userId };
  if (status) countQuery.status = status;
  const total = await PriceAlert.countDocuments(countQuery);
...
export const checkAlert = asyncHandler(async (req: Request, res: Response) => {
  ...
  const hasActive = await PriceAlert.hasActiveAlert(userId as string, productId, (variantId as string) || null);
...
export const cancelAlert = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Do NOT use .lean() — we need the hydrated document to call .cancel()
...
export const recordPriceChange = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (history.changeType === 'decrease') {
    const triggered = await PriceAlert.checkAndTriggerAlerts(productId, variantId, price.salePrice);

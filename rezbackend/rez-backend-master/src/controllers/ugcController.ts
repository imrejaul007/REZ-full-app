...
async function deindexRejectedUgcVideo(video: any): Promise<void> {
  try {
    let searchSyncService: any = null;
    try {
      // Optional integration: only present if a search sync service has been added.
      const searchSyncServiceModule = require('../services/searchSyncService');
      searchSyncService =
        searchSyncServiceModule?.default || searchSyncServiceModule?.searchSyncService || null;
    } catch {
      searchSyncService = null;
    }

    if (searchSyncService && typeof searchSyncService.removeVideo === 'function') {
      await searchSyncService.removeVideo(String(video._id));
      logger.info('[UGC MODERATION] Removed rejected reel from search index', {
        videoId: String(video._id),
      });
      return;
    }

    logger.debug('[UGC MODERATION] No UGC search deindexer available; skipping search removal', {
      videoId: String(video._id),
    });
  } catch (searchError: any) {
    logger.warn('[UGC MODERATION] Failed to deindex rejected reel from search', {
      videoId: String(video._id),
...
export const createUgcReel = asyncHandler(async (req: Request, res: Response) => {
  ...
  const video = await Video.create({
    ...
    videoUrl: videoUrl,
    thumbnail: thumbnailUrl || 'https://placehold.co/720x1280?text=Processing',
...
export const moderateUgcReel = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { id } = req.params;
  const { action, notes, qualityScore: _qualityScore } = req.body;
  ...
  if (action === 'approve') {
    ...
    const cloudinaryPublicId =
      (video as any).cloudinaryPublicId ||
      (video as any).processing?.cloudinaryPublicId ||
      undefined;

    if (cloudinaryPublicId) {
      await publishMediaEvent({
        eventId: `ugc-reel-delete-asset:${video._id}:${Date.now()}`,
        operation: 'delete-asset',
        publicId: cloudinaryPublicId,
        resourceType: 'video',
        source: 'ugc-moderation',
        createdAt: new Date().toISOString(),
      });
    } else {
      logger.debug('[UGC MODERATION] Rejected reel has no Cloudinary public ID; skipping CDN deletion', {
        videoId: String(video._id),
      });
    }
  }
  ...
  if (action === 'reject') {
    await deindexRejectedUgcVideo(video);
  }

  // Credit or reject the pending coin reward
  try {
    const { PendingCoinReward } = require('../models/PendingCoinReward');
    // DP-003 FIX: must NOT use .lean() — approve/reject/creditCoins are Mongoose
    // instance methods that will throw on a plain lean object.

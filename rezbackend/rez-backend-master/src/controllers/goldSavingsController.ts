...
// Gold provider configuration check
const GOLD_PROVIDER_CONFIGURED = !!process.env.GOLD_PROVIDER_API_KEY;
if (!GOLD_PROVIDER_CONFIGURED) {
...
const GOLD_PRICE_CACHE_TTL = 600; // 10 minutes (metals.live updates ~15min intervals)
...
/**
 * @desc    Get current gold price (FEAT-1: now fetches live from metals.live API)
...
export const getCurrentPrice = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    // Check Redis cache FIRST (hot path optimization: ~200ms faster than live fetch)
    const cachedPrice = await redisService.get<any>(regionCacheKey);
    if (cachedPrice) {
      return sendSuccess(res, cachedPrice, 'Gold price fetched (cached)');
    }

    // Try to fetch live price (FEAT-1)
    const livePrice = await getLiveGoldPriceInr();

    const priceData = {
      pricePerGram: livePrice,
      currency: 'INR',
      region: region || 'all',
    ...
    return sendSuccess(res, priceData, 'Gold price fetched (live)');
  } catch (liveErr: any) {
    logger.warn('[GOLD] Live price fetch failed, falling back to database:', liveErr.message);

    // Fallback to database prices
    const latestPrice = await getRegionPrice(region);

    if (!latestPrice) {
      return sendError(res, 'Gold price not available. Please try again later.', 503);
    }
    ...
    return sendSuccess(res, priceData, 'Gold price fetched (db fallback)');
  }
});
...
export const buyGold = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!amount || typeof amount !== 'number' || !Number.isFinite(amount) || amount <= 0 || amount > 5_000_000) {
    return sendBadRequest(res, 'Amount must be a finite number between 1 and 5,000,000');
  }
  ...
  if (existingTx) {
    const matchedTx = existingTx.transactions.find((t) => t.idempotencyKey === idempotencyKey);
    if (matchedTx) {
  ...
  try {
    ...
    // Invalidate cache
    await redisService
      .delPattern(`gold:holding:${userId}:*`)
      .catch((err) =>
        logger.warn('[Gold] Cache invalidation for gold holding after buy failed', { error: err.message }),
      );
...
export const sellGold = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    await redisService
      .delPattern(`gold:holding:${userId}:*`)
      .catch((err) =>
        logger.warn('[Gold] Cache invalidation for gold holding after sell failed', { error: err.message }),
      );
...
export const getTransactions = asyncHandler(async (req: Request, res: Response) => {
  ...
  const allTxs = [...holding.transactions].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
...
export const setGoldPrice = asyncHandler(async (req: Request, res: Response) => {
  ...
  await redisService
    .delPattern(`${GOLD_PRICE_CACHE_KEY}:*`)
    .catch((err) => logger.warn('[Gold] Cache invalidation for gold price failed', { error: err.message }));

...
class ProgramController {
  ...
  getSocialImpactLeaderboard = asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      const leaderboard = await socialImpactService.getLeaderboard(metric as any, parseInt(limit as string, 10) || 10);
  ...
  completeSocialImpact = asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      const enrollment = await socialImpactService.completeParticipation(userId, id, adminId, impactValue);

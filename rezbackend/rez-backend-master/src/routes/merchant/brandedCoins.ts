...
async function verifyStoreOwnership(storeId: string, merchantId: string): Promise<any> {
  ...
  if (String((store as any).merchantId) !== merchantId && String((store as any).merchant) !== merchantId) {
    return null;
  }
  return store;
}
 ...
 */
router.get(
  '/stores/:storeId/branded-campaigns',
 ...
 */
router.get(
  '/stores/:storeId/branded-campaigns/customers',
  asyncHandler(async (req: Request, res: Response) => {
    const { storeId } = req.params;
    const merchantId = req.merchantId!;
 ...
 */
router.post(
  '/stores/:storeId/branded-campaigns/award',
  asyncHandler(async (req: Request, res: Response) => {
    const { storeId } = req.params;
    const merchantId = req.merchantId!;
    ...
    await wallet.addBrandedCoins(new mongoose.Types.ObjectId(storeId), storeName, amount, storeLogo);
    ...
    try {
      ...
      try {
        ...
        /* fallback handled by backfill job */
      }
    ...
    const brandedCoin = updatedWallet?.brandedCoins?.find((bc: any) => bc.merchantId.toString() === storeId);

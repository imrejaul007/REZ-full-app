...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [coinDrops, total] = await Promise.all([
      CoinDrop.find({ storeId }).sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { multiplier, normalCashback, category, startTime, endTime, minOrderValue, maxCashback, isActive, priority } =
      req.body;
 ...
 */
router.get(
  '/stores/:storeId/coin-drops/:id/stats',
  asyncHandler(async (req: Request, res: Response) => {
    const { storeId, id } = req.params;
    const merchantId = req.merchantId!;

...
router.post(
  ...
  validate(
    Joi.object({
      notificationIds: Joi.array().items(commonSchemas.objectId()).min(1).max(100).required(),
    }),
  ),
  markMultipleAsRead,
);
 ...
 * @returns { updated: number, unreadCount: number }
 */
router.post('/mark-all-read', allowEmptyBody, markAllAsRead);
 ...
 */
router.post('/subscribe-email', subscribeToEmail);
 ...
 */
router.post('/subscribe-sms', subscribeToSMS);
 ...
 * @returns Updated SMS subscription status
 */
router.post('/unsubscribe-sms', unsubscribeFromSMS);

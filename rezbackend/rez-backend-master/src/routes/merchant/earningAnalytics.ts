 ...
 */
router.get(
  '/stores/:storeId/earning-analytics',
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [coinsAwardedResult, coinDropStats, brandedCoinsResult, customerRepeatResult, topEarnersResult] =
      await Promise.all([
        // 1. Coins Awarded — aggregate CoinTransaction where metadata.storeId matches
        CoinTransaction.aggregate([
          {
            $match: {
              $or: [{ 'metadata.storeId': storeId }, { 'metadata.storeId': storeObjectId }],
            ...
            $group: {
              ...
              active: {
                $sum: {
                  $cond: [
                    {
                      $and: [{ $eq: ['$isActive', true] }, { $gte: ['$endTime', now] }],
                    },
                    1,
                    0,
                  ],
        ...
        Order.aggregate([
          {
            $match: {
              $or: [{ store: storeObjectId }, { 'items.store': storeObjectId }],
      ...
      CoinTransaction.aggregate([
        ...
        },
        {
          $group: {
            ...
            totalAwarded: {
              $sum: {
                $cond: [{ $in: ['$type', ['branded_award', 'earned', 'bonus']] }, '$amount', 0],
              },
            },
            totalRedeemed: {
              $sum: {

...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    // DANIEL: Added .limit(100) — prevents unbounded result sets if milestones grow large
    const milestones = await LoyaltyMilestone.find({ isActive: true }).sort({ order: 1 }).limit(100).lean();

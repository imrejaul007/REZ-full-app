  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    switch (zone.eligibilityType) {
      case 'gender':
        ...
        message = isEligible ? 'You are eligible for this zone' : 'This zone is exclusive to women';
        break;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const verifications = await UserZoneVerification.find({ userId }).sort({ createdAt: -1 }).lean();
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const institutions = await VerifiedInstitution.find({
      isActive: true,
      $or: [{ name: { $regex: escapedSearch, $options: 'i' } }, { aliases: { $regex: escapedSearch, $options: 'i' } }],
    })

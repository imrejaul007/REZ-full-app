...
router.post('/upload', authenticate, uploadMiddleware.single('file'), uploadProjectFile);

// Upload multiple project files (images/videos) to Cloudinary
router.post(
  '/upload-multiple',
  authenticate,
  (uploadMiddleware as any).array('files', 10), // Max 10 files
  ...
  validate(
    Joi.object({
      ...
      content: Joi.alternatives()
        .try(Joi.string().trim().min(1).max(5000), Joi.array().items(Joi.string().uri()))
        .required(),
...
router.get(
  ...
  validateQuery(
    Joi.object({
      ...
      sortBy: Joi.string()
        .valid('newest', 'popular', 'trending', 'difficulty_easy', 'difficulty_hard')
        .default('newest'),
      page: Joi.number().integer().min(1).default(1),
      limit: Joi.number().integer().min(1).max(50).default(20),
      excludeUserSubmissions: Joi.alternatives().try(Joi.boolean(), Joi.string().valid('true', 'false')).optional(),
    }),
  ),
  getProjects,
);

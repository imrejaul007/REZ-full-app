...
import { authenticate, requireStoreOwnerOrAdmin } from '../middleware/auth';
import { createRateLimiter } from '../middleware/rateLimiter';

// 20 bookings/hour — prevents slot-flooding attacks
const bookingCreateLimiter = createRateLimiter({ windowMs: 60 * 60 * 1000, max: 20 });
import {
  createBooking,
  getUserBookings,
  getBookingById,
...
router.get(
  ...
  asyncHandler(async (req: any, res: any) => {
    ...
    const [total, upcoming, completed, cancelled] = await Promise.all([
      ServiceBooking.countDocuments({ user: userId }),
      ServiceBooking.countDocuments({
        user: userId,
        status: { $in: ['pending', 'confirmed', 'assigned', 'in_progress'] },
      }),
      ServiceBooking.countDocuments({ user: userId, status: 'completed' }),
      ServiceBooking.countDocuments({ user: userId, status: { $in: ['cancelled', 'no_show'] } }),
    ]);
    res.json({
      success: true,
      data: { totalBookings: total, upcomingCount: upcoming, completedCount: completed, cancelledCount: cancelled },
    });
  }),
);
 ...
 */
router.post('/', authenticate, bookingCreateLimiter, asHandler(createBooking));
 ...
 *         description: Booking not found
 */
router.put('/:id/complete', authenticate, asHandler(completeBooking));
...
/**
 * @swagger
 * /api/service-bookings/{id}/charge-no-show:
 *   post:
 *     summary: Charge no-show fee from customer's saved payment method
 *     tags: [Service Bookings]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *     responses:
 *       200:
 *         description: No-show fee charged successfully
 *       400:
 *         description: Cannot charge fee (invalid status or no payment method)
 *       401:
 *         description: Not authenticated
 *       403:
 *         description: Not authorized to charge fee
 *       404:
 *         description: Booking not found
 *       500:
 *         description: Failed to process charge
 */
router.post('/:id/charge-no-show', authenticate, requireStoreOwnerOrAdmin, asHandler(chargeNoShowFee));

/**
 * @swagger
 * /api/service-bookings/{id}/treatment-notes:
 *   patch:
 *     summary: Save treatment notes for a completed booking
 *     tags: [Service Bookings]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *               colourFormula:
 *                 type: string
 *                 description: Colour formula used
 *               productsUsed:
 *                 type: array
 *                 items:
 *                   type: string
 ...
 *               stylistNotes:
 *                 type: string
 *                 description: Internal notes for stylist
 *               clientVisibleNotes:
 *                 type: string
 *                 description: Notes visible to client
 *     responses:
 *       200:
 *         description: Treatment notes saved successfully
 *       400:
 ...
 */
router.patch('/:id/treatment-notes', authenticate, asHandler(addTreatmentNotes));

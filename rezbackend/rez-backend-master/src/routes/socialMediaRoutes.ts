...
const orderIdSchema = Joi.alternatives().try(
  ...
  Joi.string()
    .pattern(/^SP-[A-Z0-9]+-[A-Z0-9]+$/)
    .message('Invalid StorePayment ID format'),
);
...
router.post(
  '/submit-media',
  (uploadSocialMediaProof as any).array('files', 5),
  validateBody(
    Joi.object({
      platform: Joi.string().valid('instagram', 'facebook', 'twitter', 'tiktok').required(),
      orderId: orderIdSchema,
      fraudMetadata: Joi.alternatives()
        .try(
          Joi.object({
            deviceId: Joi.string().optional(),
            trustScore: Joi.number().optional(),
      ...
      try {
        await coinService.awardCoins(
          ...
          `Social media cashback (${post.platform}) for order ${post.order} [fix]`,
          {
            ...
            orderId: post.order,
            referenceId: `social_share_reward:${post._id}:fix`,
          },
        );
        fixed++;
        results.push({

...
const giftWriteLimiter = createRateLimiter({
  windowMs: 60 * 60 * 1000,
  max: 10,
  message: 'Too many gift requests. Please try again later.',
});
...
router.post(
  ...
  requireReAuthAbove('gift'),
  validate(sendGiftSchema),
  sendGift,
);
router.get('/received', giftReadLimiter, getReceivedGifts);
router.post('/:id/claim', giftWriteLimiter, idempotencyMiddleware(), validateParams(claimGiftSchema), claimGift);

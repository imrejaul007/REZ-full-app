...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const validTypes: AdminActionType[] = [
      'manual_adjustment',
      'bulk_credit',
      'freeze_override',
      'config_change',
    ...
    const typeFilter =
      actionType && validTypes.includes(actionType as AdminActionType) ? (actionType as AdminActionType) : undefined;
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      const status = error.message.includes('not found')
        ? 404
        : error.message.includes('same as initiator')
          ? 403
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      const status = error.message.includes('not found') ? 404 : error.message.includes('not pending') ? 409 : 400;
      res.status(status).json({ success: false, message: error.message });
    }
  }),
);

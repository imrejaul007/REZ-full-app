...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (storeId) {
      if (!Types.ObjectId.isValid(storeId as string)) {
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (status === 'no_show' && !(appointment as any).noShowMarkedAt) (appointment as any).noShowMarkedAt = new Date();

    // Append to audit trail so admin changes are visible in status history
    (appointment as any).statusHistory = (appointment as any).statusHistory || [];
    (appointment as any).statusHistory.push({
      status,
      timestamp: new Date(),

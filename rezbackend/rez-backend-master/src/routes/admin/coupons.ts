...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!couponCode || !title || !description || !discountType || !discountValue || !validFrom || !validTo) {
      return sendError(
        res,
        'couponCode, title, description, discountType, discountValue, validFrom, and validTo are required',
        400,
      );
    }
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    // D15: Whitelist-based update — never spread raw req.body into findByIdAndUpdate.
    // A blacklist ("delete viewCount, claimCount, usageCount") can't defend against
    // fields the admin API doesn't know about yet (e.g., future schema additions)
    // or Mongo operators. Build the update from an explicit allowlist.

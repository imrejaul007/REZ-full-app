...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const categories = await Category.find({
      ...
      .select(
        'name slug icon image type sortOrder metadata pageConfig isActive maxCashback storeCount productCount updatedAt',
      )
      .sort({ sortOrder: 1 })
      .lean();
    ...
    parentWithChildren.forEach((cat: any) => {
      const ids = [
        cat._id,
        ...(cat.childCategories || []).map((id: any) => new mongoose.Types.ObjectId(id.toString())),
      ];
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const category = await Category.findByIdAndUpdate(req.params.id, { $set: value }, { new: true }).lean();

    if (!category) {
      return res.status(404).json({ success: false, message: 'Category not found' });
    }

    // RC-04: Invalidate category-root-map cache across all pods via Redis pub/sub
    try {
      await redisService.del('cache:category-root-map');
      await redisService.publish(
        'cache:invalidate',
        JSON.stringify({ key: 'category-root-map', timestamp: Date.now() }),
      );
    } catch (cacheErr) {
      logger.warn('[categories] Redis cache invalidation failed (non-fatal):', cacheErr);
    }
...
const pageConfigSchema = Joi.object({
  ...
  curatedCollections: Joi.array().items(
    Joi.object({
      id: Joi.string().required(),
  ...
  valuePropItems: Joi.array().items(
    Joi.object({
      icon: Joi.string().required(),
  ...
  experienceBenefits: Joi.array().items(
    Joi.object({
      icon: Joi.string().required(),
      title: Joi.string().required(),
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    res.json({
      success: true,
      data: { quickActions: (category.pageConfig as any).quickActions },
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    res.json({
      success: true,
      data: { serviceTypes: (category.pageConfig as any).serviceTypes },
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    res.json({
      success: true,
      data: { dietaryOptions: (category.pageConfig as any).dietaryOptions },
      message: 'Dietary options updated',
    });
  }),
);
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    res.json({
      success: true,
      data: { curatedCollections: (category.pageConfig as any).curatedCollections },
      message: 'Curated collections updated',
    });
  }),
);
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    res.json({
      success: true,
      data: { searchPlaceholders: (category.pageConfig as any).searchPlaceholders },
      message: 'Search placeholders updated',
    });
  }),
);
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    res.json({
      success: true,
      data: { valuePropItems: (category.pageConfig as any).valuePropItems },
      message: 'Value prop items updated',
    });
  }),
);
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!value.slug) {
      return res.status(400).json({
        success: false,
        message:
          'Could not generate a valid slug. Please provide a slug manually using only letters, numbers, and hyphens.',
      });
    }
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const updated = await Category.findByIdAndUpdate(req.params.subId, { $set: value }, { new: true }).lean();
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    res.json({
      success: true,
      data: { pageConfig: category.pageConfig },
      message: 'Sort/filter options updated successfully',
    });
  }),
);

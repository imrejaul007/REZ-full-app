...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const travelCategories = await ServiceCategory.find({
      ...
      .select('_id name slug cashbackPercentage')
      .lean();
...
router.get(
  '/bookings',
  asyncHandler(async (req: Request, res: Response) => {
    const { page = 1, limit = 20, status, cashbackStatus, category, search, dateFrom, dateTo } = req.query;
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (status === 'cancelled') {
      ...
      // H19 FIX: Admin cancellation must trigger a full payment refund.
      // Admin override always issues 100% refund regardless of tiered refund policy.
      if ((booking as any).paymentStatus === 'paid' && (booking as any).paymentId) {
        try {
          const paymentId = (booking as any).paymentId as string;
          const refundAmount = (booking as any).pricing?.total || 0;
          if (refundAmount > 0) {
                    ...
                    [
                      {
                        ...
                        orderNumber: (booking as any).bookingNumber,
                        ...
                        metadata: { bookingType: 'travel_booking', cancelledBy: 'admin' },
                      },
                    ],
          ...
          // Log but do not block the cancellation — admin will review
          logger.error('[AdminTravel] Refund failed after admin cancel (manual review needed):', {
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const booking = await ServiceBooking.findById(id).populate('serviceCategory', 'slug');

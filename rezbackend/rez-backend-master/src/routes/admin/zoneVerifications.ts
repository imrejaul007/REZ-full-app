...
const SEGMENT_BY_TYPE: Record<(typeof SEGMENT_PRIORITY)[number], string> = {
  student: 'verified_student',
  corporate: 'verified_employee',
  defence: 'verified_defence',
  healthcare: 'verified_healthcare',
  senior: 'verified_senior',
  teacher: 'verified_teacher',
  government: 'verified_government',
  differentlyAbled: 'verified_differentlyAbled',
};

async function syncUserVerificationState(userId: string | Types.ObjectId): Promise<void> {
  const userObjectId = typeof userId === 'string' ? new Types.ObjectId(userId) : userId;

  const [userDoc, approvedVerifications] = await Promise.all([
    User.findById(userObjectId).select('verifications').lean(),
    UserZoneVerification.find({
      userId: userObjectId,
      status: 'approved',
    })
      .select('zoneSlug verificationType')
      .lean(),
  ]);

  const verifications = ((userDoc as any)?.verifications || {}) as Record<string, any>;
  const activeType = SEGMENT_PRIORITY.find((type) => verifications?.[type]?.verified === true);
  const activeZones = Array.from(
    new Set(approvedVerifications.map((verification: any) => verification.zoneSlug).filter(Boolean)),
  );

  await User.findByIdAndUpdate(userObjectId, {
    $set: {
      verificationSegment: activeType ? 'verified' : 'none',
      featureLevel: activeType ? 2 : 1,
      segment: activeType ? SEGMENT_BY_TYPE[activeType] : 'normal',
      activeZones,
    },
  });

  await privilegeResolutionService.invalidate(userObjectId.toString()).catch((err) =>
    logger.error('[ZoneVerifications] Privilege cache invalidation failed', {
      error: err.message,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const { status = 'pending', verificationType, search = '', page = 1, limit = 20 } = req.query;
    ...
    if (searchTerm) {
      ...
      const matchingUserIds = matchingUsers.map((user: any) => user._id);
      query.$or = [
        { userId: { $in: matchingUserIds } },
        { 'submittedData.instituteName': regex },
        { 'submittedData.companyName': regex },
        { 'submittedData.email': regex },
        { 'submittedData.documentType': regex },
      ];
    }
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    verification.rejectionReason = status === 'approved' ? undefined : rejectionReason || undefined;
    if (expiresAt) {
    ...
    if (status === 'approved') {
      ...
      await syncUserVerificationState(verification.userId);
      ...
      if (!isRevoke) {
        ...
        const userDoc = await User.findById(verification.userId)
          .select('verificationSegment segment activeZones')
          .lean();
    ...
    for (const v of pending) {
      ...
      await User.findByIdAndUpdate(v.userId, {
        $set: {
          [`${updatePath}.verified`]: true,
          [`${updatePath}.verifiedAt`]: new Date(),
          ...(v.submittedData.instituteName && {
            [`${updatePath}.instituteName`]: v.submittedData.instituteName,
          }),
          ...(v.submittedData.companyName && {
            [`${updatePath}.companyName`]: v.submittedData.companyName,
          }),
        },
        $addToSet: { activeZones: v.zoneSlug },
      });

      await syncUserVerificationState(v.userId);
      await NotificationService.createNotification({
        userId: v.userId,
        title: 'Verification Approved! ðŸŽ‰',
        message: `Your ${v.verificationType} verification has been approved. You now have access to exclusive ${v.verificationType} deals!`,
        type: 'success',
        category: 'general',
        priority: 'high',
        data: {

...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (target.type === 'user') {
      if (!Types.ObjectId.isValid(target.userId)) {
        return sendBadRequest(res, 'target.userId is not a valid ObjectId');
      }
      userIds = [target.userId];
    } else if (target.type === 'segment' || target.type === 'all') {
      const filter: Record<string, any> = { isActive: true };
      if (target.type === 'segment' && target.segment) {
        filter.$or = [{ tier: target.segment }, { role: target.segment }];
      }
      const users = await User.find(filter).select('_id').limit(50_000).lean();
      userIds = users.map((u: any) => String(u._id));
    }

    if (userIds.length === 0) {
      return sendSuccess(res, { queued: 0, status: 'no_audience' }, 'No matching users found');
    }

    // ── Enqueue notification jobs per channel ────────────────────────────────
    // push → push_notification.ts  (Expo push API via PushNotificationService)
    // email → email_notification.ts (SendGrid via EmailService)
    // sms   → sms_notification.ts   (MSG91/Twilio via SMSService)
    let queued = 0;

    const delay = isImmediate ? 0 : scheduledAt.getTime() - Date.now();
    const jobOpts = {
      attempts: 3,
    ...
    const baseJobData = {
      ...
      body: template.body,
      data: { templateId, category: template.category, source: 'admin_send' },
      source: 'admin_template_send',
    };
        ...
        notificationQueue
          .add(
            jobName,
            {
              ...
              ...(template.channel === 'push' ? { channelId: 'promotions' } : {}),
            },
            jobOpts,
          )
          .then(() => {
          ...
          .catch((err: Error) => {
            logger.warn(`[Admin] Failed to enqueue ${jobName} job`, { userId: uid, error: err.message });
          }),
      ),
    );

    logger.info('[Admin] Notification send complete', {
      templateId,
    ...
    return sendSuccess(
      ...
      isImmediate
        ? `${queued} ${template.channel} notification(s) queued for immediate delivery`
        : `${queued} ${template.channel} notification(s) scheduled for ${scheduledAt.toISOString()}`,
    );
  }),
);
...
router.get(
  ...
  asyncHandler(async (_req: Request, res: Response) => {
    const [byChannel, byStatus, total] = await Promise.all([
      NotificationTemplate.aggregate([{ $group: { _id: '$channel', count: { $sum: 1 } } }]),
      NotificationTemplate.aggregate([{ $group: { _id: '$isActive', count: { $sum: 1 } } }]),

  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [brands, total] = await Promise.all([
      MallBrand.find(query)
        .select(
          'name slug logo cashback rezCoinReward isActive isFeatured tier tags analytics externalUrl affiliateConfig createdAt',
        )
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const brand = await MallBrand.findByIdAndUpdate(req.params.id, { $set: value }, { new: true }).lean();

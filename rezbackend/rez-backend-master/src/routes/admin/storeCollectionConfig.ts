...
const DEFAULT_CATEGORIES = [
  {
    categoryKey: 'fastDelivery',
    displayName: '30 min delivery',
  ...
  {
    categoryKey: 'budgetFriendly',
    displayName: '1 rupees store',
  ...
  {
    categoryKey: 'alliance',
    displayName: 'Alliance Store',
  ...
  {
    categoryKey: 'cashStore',
    displayName: 'Cash Store',
...
router.get(
  ...
  asyncHandler(async (_req: Request, res: Response) => {
    const configs = await StoreCollectionConfig.find().sort({ sortOrder: 1 }).lean();
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { isEnabled, sortOrder, displayName, description, icon, color, badgeText, imageUrl, regions, tags } =
      req.body;
    ...
    const config = await StoreCollectionConfig.findOneAndUpdate({ categoryKey }, { $set: update }, { new: true });

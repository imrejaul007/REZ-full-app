...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const tiers = await SubscriptionTier.find(filter).sort({ sortOrder: 1, createdAt: -1 }).lean();
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      const { tier, name, description, pricing, benefits, features, isActive, sortOrder, trialDays } = req.body;
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      const { name, description, pricing, benefits, features, isActive, sortOrder, trialDays } = req.body;
...
router.get(
  '/subscribers',
  asyncHandler(async (req: Request, res: Response) => {
    const { tier, status, search: _search, page = '1', limit = '20' } = req.query;
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    privilegeResolutionService
      .invalidate(userId)
      .catch((err) =>
        logger.error('[MembershipAdmin] Privilege cache invalidation failed after tier override', {
          error: err.message,
          userId,
        }),
      );

...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const { status = 'pending', storeId, rating, search, page = '1', limit = '20' } = req.query;
    ...
    if (search) {
      ...
      query.$or = [{ comment: { $regex: escaped, $options: 'i' } }, { title: { $regex: escaped, $options: 'i' } }];
    }
    ...
    const [reviews, total] = await Promise.all([
      Review.find(query)
        ...
        .select(
          'rating title comment moderationStatus moderationReason moderatedAt createdAt user store isActive images',
        )
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limitNum)
...
router.get(
  ...
  asyncHandler(async (_req: Request, res: Response) => {
    const stats = await Review.aggregate([{ $group: { _id: '$moderationStatus', count: { $sum: 1 } } }]);

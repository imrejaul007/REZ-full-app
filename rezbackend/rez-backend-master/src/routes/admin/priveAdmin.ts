...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      const logs = await (await import('../../models/PriveAuditLog')).PriveAuditLog.find(filter)
        .sort({ createdAt: -1 })
        .limit(10000)
      ...
      // Generate CSV
      const headers = ['Admin', 'Action', 'Entity Type', 'Entity ID', 'Changes', 'Timestamp'];
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { UserReputation: _UserReputation } = await import('../../models/UserReputation');
    const { UserMission } = await import('../../models/UserMission');
    const { PriveAuditLog: _PriveAuditLog } = await import('../../models/PriveAuditLog');
    const { SupportTicket } = await import('../../models/SupportTicket');
    const { CoinTransaction } = await import('../../models/CoinTransaction');

    const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);

    const [totalMembers, tierDistribution, activeLast30d, missionEngagement, conciergeMetrics, coinFlow] =
      await Promise.all([
        ...
        Promise.all([
          ...
          .then(([active, completed, expired]) => ({
            ...
            completionRate:
              active + completed + expired > 0 ? Math.round((completed / (active + completed + expired)) * 100) : 0,
          }))
    ...
    // Format coin flow
    const coinSummary = coinFlow.reduce(
      (acc: any, c: any) => {
    ...
    const activationRate = totalMembers > 0 ? Math.round((activeLast30d / totalMembers) * 100) : 0;
...
router.patch('/submissions/:submissionId/review', requireOperator, reviewSubmission);

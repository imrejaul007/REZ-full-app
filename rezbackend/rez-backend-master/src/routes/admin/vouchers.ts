...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [vouchers, total] = await Promise.all([
      VoucherBrand.find(filter).populate('store', 'name logo').sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const voucher = await VoucherBrand.findById(req.params.id).populate('store', 'name logo').lean();

...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (req.query.search) {
      ...
      filter.$or = [{ title: { $regex: search, $options: 'i' } }, { subtitle: { $regex: search, $options: 'i' } }];
    }
    ...
    const [cards, total] = await Promise.all([
      ValueCard.find(filter).sort({ sortOrder: 1, createdAt: -1 }).skip(skip).limit(limit).lean(),
...
router.post(
  '/',
  asyncHandler(async (req: Request, res: Response) => {
    const { title, subtitle, emoji, deepLinkPath, sortOrder, isActive, regions } = req.body;

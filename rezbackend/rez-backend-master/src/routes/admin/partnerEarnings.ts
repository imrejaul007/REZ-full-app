...
router.get(
  '/users',
  asyncHandler(async (req: Request, res: Response) => {
    const { search, level, page = '1', limit = '20', sortBy = 'earnings.total', sortDir = 'desc' } = req.query;
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    // Use a 30-second idempotency window — prevents double-clicks on the admin UI
    // while still allowing the same adjustment to be legitimately resubmitted later.
    // Callers can also supply an explicit idempotencyKey to get stronger deduplication.
    const idempotencyKey =
      (req.body.idempotencyKey as string | undefined) ||
      `partner-adj:${userId}:${type}:${amount}:${Math.floor(Date.now() / 30000)}`;
    ...
    if (type === 'credit') {
      await walletService.credit({
        ...
        metadata: { partnerEarning: true, adjustmentType: type, reason, adminUserId, idempotencyKey },
      });
    } else {

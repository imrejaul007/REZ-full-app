...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [configs, total] = await Promise.all([
      LeaderboardConfig.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (existingDistribution) {
      return sendBadRequest(
        res,
        `Prizes already distributed for cycle ${cycleStartDate.toISOString()} - ${cycleEndDate.toISOString()} (status: ${existingDistribution.status})`,
      );
    }
    ...
    for (const prizeSlot of config.prizePool) {
      const slotEntries = entries.filter((e: any) => e.rank >= prizeSlot.rankStart && e.rank <= prizeSlot.rankEnd);
    ...
    distribution.status =
      totalFlagged > 0 && totalDistributed > 0 ? 'partial' : totalDistributed > 0 ? 'completed' : 'partial';
    await distribution.save();

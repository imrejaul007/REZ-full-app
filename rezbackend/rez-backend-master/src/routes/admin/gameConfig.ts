...
async function publishGameConfigUpdate(payload: {
  gameType?: string;
  configId?: string;
  scope?: 'single' | 'all';
  action: 'created' | 'seeded' | 'updated' | 'toggled' | 'featured' | 'reordered' | 'deleted' | 'invalidated';
}): Promise<void> {
  try {
    await redisService.publish(
      GAME_CONFIG_UPDATE_CHANNEL,
      JSON.stringify({
        ...payload,
    ...
    // Fail open: admin updates must still succeed if pub/sub is unavailable.
  }
}
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const gameConfigs = await GameConfig.find(filter).sort({ sortOrder: 1, createdAt: 1 }).lean();
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      const {
        ...
        dailyLimit: gcDailyLimit,
        cooldownMinutes: gcCooldown,
        rewards: gcRewards,
        difficulty: gcDifficulty,
        config: gcConfig,
        schedule: gcSchedule,
        sortOrder: gcSortOrder,
        featured: gcFeatured,
      } = req.body;
      ...
      await publishGameConfigUpdate({
        gameType: gameConfig.gameType,
        configId: String(gameConfig._id),
        scope: 'single',
        action: 'created',
      });
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const {
      displayName: gcUpDN,
      description: gcUpDesc,
      icon: gcUpIcon,
      isEnabled: gcUpEnabled,
      dailyLimit: gcUpDailyLimit,
      cooldownMinutes: gcUpCooldown,
      rewards: gcUpRewards,
      difficulty: gcUpDifficulty,
      config: gcUpConfig,
      schedule: gcUpSchedule,
      sortOrder: gcUpSortOrder,
      featured: gcUpFeatured,
    } = req.body;
    const gcUpFields: Record<string, any> = {};
    if (gcUpDN !== undefined) gcUpFields.displayName = gcUpDN;
    if (gcUpDesc !== undefined) gcUpFields.description = gcUpDesc;
    if (gcUpIcon !== undefined) gcUpFields.icon = gcUpIcon;
    if (gcUpEnabled !== undefined) gcUpFields.isEnabled = gcUpEnabled;
    if (gcUpDailyLimit !== undefined) gcUpFields.dailyLimit = gcUpDailyLimit;
    if (gcUpCooldown !== undefined) gcUpFields.cooldownMinutes = gcUpCooldown;
    if (gcUpRewards !== undefined) gcUpFields.rewards = gcUpRewards;
    if (gcUpDifficulty !== undefined) gcUpFields.difficulty = gcUpDifficulty;
    if (gcUpConfig !== undefined) gcUpFields.config = gcUpConfig;
    if (gcUpSchedule !== undefined) gcUpFields.schedule = gcUpSchedule;
    if (gcUpSortOrder !== undefined) gcUpFields.sortOrder = gcUpSortOrder;
    if (gcUpFeatured !== undefined) gcUpFields.featured = gcUpFeatured;
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    return sendSuccess(
      res,
      gameConfig,
      `Game "${gameConfig.displayName}" ${gameConfig.isEnabled ? 'enabled' : 'disabled'}`,
    );
  }),
);
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    return sendSuccess(
      res,
      gameConfig,
      `Game "${gameConfig.displayName}" ${gameConfig.featured ? 'featured' : 'unfeatured'}`,
    );
  }),
);
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    await publishGameConfigUpdate({ scope: 'all', action: 'reordered' });
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [sessions, user] = await Promise.all([
      GameSession.find(query)
        ...
        .lean(),
      // SANA: Response sanitization — exclude phoneNumber from API responses
      User.findById(userId).select('_id fullName username gameBanned gameBanReason gameBannedAt').lean(),
    ]);
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const user = await User.findByIdAndUpdate(
      ...
      { new: true },
    ).select('_id fullName username gameBanned gameBanReason gameBannedAt');
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const user = await User.findByIdAndUpdate(
      ...
      { new: true },
    ).select('_id fullName username gameBanned');
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await coinService.awardCoins(userId, amount, 'admin', `Admin credit: ${reason}`, {
      adminAction: true,
      reason,
      adminId: (req.user as any)?.id,
      referenceId: `admin_credit:${userId}:${Date.now()}`,
    });
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await coinService.awardCoins(userId, -amount, 'admin', `Admin revoke: ${reason}`, {
      adminAction: true,
      reason,
      adminId: (req.user as any)?.id,
      revoke: true,
      referenceId: `admin_revoke:${userId}:${Date.now()}`,
    });
...
router.post(
  ...
  asyncHandler(async (_req: Request, res: Response) => {
    invalidateGameConfigCache();
    await publishGameConfigUpdate({ scope: 'all', action: 'invalidated' });
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [statusBreakdown, prizeDistribution, dailyActivity, suspiciousActivity] = await Promise.all([
      // Cards by status
      ...
      GameSession.aggregate([
        {
          $match: {
            ...
            'metadata.ip': { $exists: true, $ne: null },
          },
        },
        {
          $group: {

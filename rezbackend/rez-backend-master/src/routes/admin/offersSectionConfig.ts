...
const DEFAULT_SECTIONS = [
  ...
  {
    sectionKey: 'superCashbackStores',
    displayName: 'Super Cashback Stores',
    tab: 'cashback',
    sortOrder: 3,
    maxItems: 20,
  },
...
router.get(
  ...
  asyncHandler(async (_req: Request, res: Response) => {
    const configs = await OffersSectionConfig.find().sort({ tab: 1, sortOrder: 1 }).lean();
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const config = await OffersSectionConfig.findOneAndUpdate({ sectionKey }, { $set: update }, { new: true });
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    for (const section of DEFAULT_SECTIONS) {
      ...
      if (!existing) {
        const _config = await OffersSectionConfig.create({
          ...section,
          isEnabled: true,
          regions: [],
        });

...
const offerSchema = Joi.object({
  ...
  restrictions: Joi.object({
    ...
    // R35 FIX: added applicableOn and excludedProducts — previously silently dropped
    applicableOn: Joi.array().items(Joi.string()),
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const { page = 1, limit = 20, exclusiveZone, category, type, isActive, search } = req.query;
    ...
    if (exclusiveZone) {
      if (exclusiveZone === 'all-exclusive') {
        ...
        query.$or = [{ exclusiveZone: { $exists: false } }, { exclusiveZone: null }];
      } else {
    ...
    const [offers, total] = await Promise.all([
      Offer.find(query).sort({ 'metadata.priority': -1, createdAt: -1 }).skip(skip).limit(Number(limit)).lean(),
      ...
      Offer.aggregate([
        { $match: { exclusiveZone: { $exists: true, $ne: null } } },
        {
          $group: {
            ...
            active: {
              $sum: {
                $cond: [{ $and: [{ $eq: ['$validity.isActive', true] }, { $gte: ['$validity.endDate', now] }] }, 1, 0],
              },
            },
          },
        },
...
router.get(
  '/stores',
  asyncHandler(async (req: Request, res: Response) => {
    const stores = await Store.find({ isActive: true }).select('_id name logo').sort({ name: 1 }).limit(100).lean();
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [offers, total] = await Promise.all([
      Offer.find(filter).sort({ createdAt: -1 }).skip(skip).limit(Number(limit)).lean(),

...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    return sendSuccess(
      res,
      { created, skipped, total: achievementEntries.length },
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const {
      title: achUpTitle,
      description: achUpDesc,
      icon: achUpIcon,
      category: achUpCategory,
      trigger: achUpTrigger,
      threshold: achUpThreshold,
      rewardCoins: achUpCoins,
      rewardBadge: achUpBadge,
      isActive: achUpIsActive,
      isHidden: achUpIsHidden,
      priority: achUpPriority,
      expiresAt: achUpExpiresAt,
      maxEarnsPerUser: achUpMaxEarns,
    } = req.body;
    const achUpFields: Record<string, any> = {};
    if (achUpTitle !== undefined) achUpFields.title = achUpTitle;
    if (achUpDesc !== undefined) achUpFields.description = achUpDesc;
    if (achUpIcon !== undefined) achUpFields.icon = achUpIcon;
    if (achUpCategory !== undefined) achUpFields.category = achUpCategory;
    if (achUpTrigger !== undefined) achUpFields.trigger = achUpTrigger;
    if (achUpThreshold !== undefined) achUpFields.threshold = achUpThreshold;
    if (achUpCoins !== undefined) achUpFields.rewardCoins = achUpCoins;
    if (achUpBadge !== undefined) achUpFields.rewardBadge = achUpBadge;
    if (achUpIsActive !== undefined) achUpFields.isActive = achUpIsActive;
    if (achUpIsHidden !== undefined) achUpFields.isHidden = achUpIsHidden;
    if (achUpPriority !== undefined) achUpFields.priority = achUpPriority;
    if (achUpExpiresAt !== undefined) achUpFields.expiresAt = achUpExpiresAt;
    if (achUpMaxEarns !== undefined) achUpFields.maxEarnsPerUser = achUpMaxEarns;

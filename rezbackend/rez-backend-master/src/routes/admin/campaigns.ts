...
import Campaign, { ICampaignDeal } from '../../models/Campaign';
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (req.query.search) {
      ...
      filter.$or = [{ title: searchRegex }, { subtitle: searchRegex }, { campaignId: searchRegex }];
    }
    ...
    const [campaigns, total] = await Promise.all([
      Campaign.find(filter).sort({ priority: -1, createdAt: -1 }).skip(skip).limit(limit).lean(),
    ...
    const stats = await Campaign.aggregate([
      {
        $facet: {
          ...
          totalDeals: [{ $unwind: { path: '$deals', preserveNullAndEmptyArrays: true } }, { $count: 'count' }],
        },
      },
    ]);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const stores = await Store.find(filter).select('_id name logo category location').limit(limit).lean();
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      const _dealsCount = Array.isArray(deals) ? deals.length : 0;
      ...
      const campaign = new Campaign({
        ...
        bannerImage,
        region: region || 'all',
        exclusiveToProgramSlug: exclusiveToProgramSlug || undefined,
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    // If force delete, refund paid redemptions then cancel all active/pending
    let refundedCount = 0;
    let refundFailures = 0;
    if (activeRedemptionsCount > 0 && force === 'true') {
      // Find paid redemptions that need refund before cancellation
      const paidRedemptions = await DealRedemption.find({
        campaign: campaign._id,
        status: { $in: ['active', 'pending'] },
        isPaid: true,
        purchaseAmount: { $gt: 0 },
      ...
      for (const redemption of paidRedemptions) {
        try {
          ...
          refundFailures++;
          logger.error(`[ADMIN CAMPAIGNS] Failed to refund redemption ${redemption._id} for user ${redemption.user}`, {
      ...
      // Cancel all active/pending redemptions regardless of refund outcome
      await DealRedemption.updateMany(
        { campaign: campaign._id, status: { $in: ['active', 'pending'] } },
        { $set: { status: 'cancelled' } },
      );
      logger.info(
        `[ADMIN CAMPAIGNS] Force delete: cancelled ${activeRedemptionsCount} redemptions, refunded ${refundedCount} paid redemptions (${refundFailures} refund failures)`,
        { campaignId: campaign.campaignId },
      );
    }
    ...
    res.json({
      ...
      data: {
        ...
        refundedRedemptions: refundedCount,
        refundFailures,
      },
    });
  }),
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    logger.info(
      `✅ [ADMIN CAMPAIGNS] Campaign ${campaign.isActive ? 'activated' : 'deactivated'}: ${campaign.campaignId} by admin ${req.userId}`,
    );
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    logger.info(
      `✅ [ADMIN CAMPAIGNS] ${isPaidDeal ? 'Paid' : 'Free'} deal added to campaign: ${campaign.campaignId} by admin ${req.userId}`,
      {
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { store, storeId, image, cashback, coins, bonus, drop, discount, endsIn, price, currency, purchaseLimit } =
      req.body;
    ...
    logger.info(
      `✅ [ADMIN CAMPAIGNS] Deal ${index} updated in campaign: ${campaign.campaignId} by admin ${req.userId}`,
      {
        isPaid: isPaidDeal,
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    logger.info(
      `✅ [ADMIN CAMPAIGNS] Campaign duplicated: ${originalCampaign.campaignId} -> ${newCampaignId} by admin ${req.userId}`,
    );
 ...
 * @access  Admin (activate/deactivate); Senior Admin level 80+ (delete)
 */
router.post(
  '/bulk-action',
  asyncHandler(async (req: Request, res: Response) => {
    ...
    // Safety cap: prevent unbounded bulk operations
    ...
    // Bulk delete requires Senior Admin (level 80+) — support agents must not delete campaigns
    if (action === 'delete') {
      const adminLevel = (req as any).adminLevel ?? (req as any).user?.adminLevel ?? 0;
      if (adminLevel < 80) {
        return res.status(403).json({
          success: false,
          message: 'Bulk delete requires Senior Admin privileges (level 80+)',
        });
      }
    }
    ...
    switch (action) {
      case 'activate':
        result = await Campaign.updateMany({ _id: { $in: objectIds } }, { $set: { isActive: true } });
        break;
      case 'deactivate':
        result = await Campaign.updateMany({ _id: { $in: objectIds } }, { $set: { isActive: false } });
        break;
      case 'delete':
    ...
    logger.info(
      `✅ [ADMIN CAMPAIGNS] Bulk action '${action}' on ${campaignIds.length} campaigns by admin ${(req as any).userId}`,
    );

    // Write audit log for bulk delete (fire-and-forget, non-blocking)
    if (action === 'delete') {
      ...
      setImmediate(() => {
        AdminAuditLog.create({
          ...
          requestBody: { campaignIds, count: campaignIds.length },
          responseSuccess: true,
          responseStatus: 200,
          timestamp: new Date(),
          changes: {
            afterValues: { deletedCount },

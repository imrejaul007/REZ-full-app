...
import { requireAuth, requireAdmin, requireSeniorAdmin, requireSuperAdmin } from '../../middleware/auth';
import { sendSuccess, sendError } from '../../utils/response';
import { asyncHandler } from '../../utils/asyncHandler';

const router = Router();

// All routes require authenticated admin (platform-level — defense in depth)
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const adminUsers = await Promise.all(
      admins.map(async (admin: any) => {
        ...
        return {
          id: String(admin._id),
          name: `${admin.profile?.firstName || ''} ${admin.profile?.lastName || ''}`.trim() || 'Admin',
          email: admin.email || '',
          role: 'admin',
 ...
 * Requires senior admin for profile fields (firstName, lastName, phoneNumber).
 * Requires super admin for sensitive fields (email, password, isActive) —
 * these fields affect authentication and account access, so they need the
 * highest privilege level to prevent a compromised admin account from
 * locking out or hijacking other admin accounts.
 */
router.put(
  '/:id',
  requireSeniorAdmin,
  asyncHandler(async (req: Request, res: Response) => {
    ...
    // SECURITY: email, password, and isActive changes require super_admin.
    // A senior admin (level 80) must not be able to change another admin's
    // email or password — that would allow account takeover.
    const requestingUserRole = (req as any).user?.role;
    const sensitiveFieldRequested = email !== undefined || password !== undefined || isActive !== undefined;
    if (sensitiveFieldRequested && requestingUserRole !== 'super_admin') {
      return sendError(res, 'Changing email, password, or active status requires super_admin role', 403);
    }

    // Update profile fields (allowed for senior admin+)
    ...
    // Update sensitive fields (super_admin only — guard above already checked)

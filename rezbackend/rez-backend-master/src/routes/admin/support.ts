...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const { page = 1, limit = 20, status, category, priority, assignedTo, search, dateFrom, dateTo } = req.query;
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (userId) {
      ...
      const agentName = admin
        ? `${(admin as any).profile?.firstName || ''} ${(admin as any).profile?.lastName || ''}`.trim() ||
          'Support Agent'
        : 'Support Agent';
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    // BAK-GATEWAY-009/BAK-GATEWAY-018 FIX: Check balance before debit, and do not
    // mark the ticket resolved if the wallet debit fails.
    ...
    if (status === 'resolved' && walletAdjustment) {
      ...
      if (parsedAmount > 0 && parsedAmount <= 100000 && ['credit', 'debit'].includes(type) && reason?.trim()) {
        const userId = (ticket as any).user?._id?.toString();
        if (userId) {
          try {
            const { walletService } = await import('../../services/walletService');
            // BAK-GATEWAY-009: For debit, check user has sufficient balance before attempting
            if (type === 'debit') {
              const Wallet = require('../../models/Wallet').default;
              const wallet = await Wallet.findOne({ userId: new Types.ObjectId(userId) });
              const availableBalance = wallet?.balance.available ?? wallet?.balance.total ?? 0;
              if (availableBalance < parsedAmount) {
                walletResult = {
                  success: false,
                  error: `Insufficient balance. Available: ${availableBalance}, Required: ${parsedAmount}`,
                };
                debitFailed = true;
                logger.warn('[Admin Support] Debit skipped — insufficient balance', {
                  ticketId: id,
            ...
            if (!debitFailed) {
              ...
              logger.info(
                `[Admin Support] Wallet ${type} of ${parsedAmount} for ticket ${(ticket as any).ticketNumber}`,
              );
            }
          } catch (walletErr: any) {
    ...
    // BAK-GATEWAY-018: If debit failed, do not mark ticket as resolved.
    // Downgrade to 'in_progress' so the admin can fix and retry.
    if (debitFailed) {
      await SupportTicket.findByIdAndUpdate(id, { $set: { status: 'in_progress' } });
      return sendSuccess(res, {
        ticket: { ...ticket, status: 'in_progress' },
        walletResult,
        warning: 'Ticket left in_progress because wallet debit failed',
      });
    }
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const macros = await SupportMacro.find(query).sort({ category: 1, sortOrder: 1 }).lean();

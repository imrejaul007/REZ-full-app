...
router.get(
  '/',
  validateQuery(adminStoreSearchSchema),
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const category = await Category.findById(req.params.categoryId).select('_id childCategories name').lean();
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (missingIds.length > 0) {
      return sendBadRequest(
        res,
        `${missingIds.length} store(s) not found: ${missingIds.slice(0, 5).join(', ')}${missingIds.length > 5 ? '...' : ''}`,
      );
    }

    const result = await Store.updateMany({ _id: { $in: storeIds } }, { $set: { category: categoryId } });
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const store = await Store.findByIdAndUpdate(req.params.storeId, { $set: updateFields }, { new: true })
...
// ============================================
// PATCH /admin/stores/:storeId/settings — Update operational settings (prep time, etc.)
// Called by admin merchantsService.updateStoreSettings()
// ============================================
router.patch(
  '/:storeId/settings',
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (req.body.estimatedPrepMinutes !== undefined) {
      const mins = parseInt(req.body.estimatedPrepMinutes, 10);
      if (!isNaN(mins) && mins >= 0) {
        updateFields.estimatedPrepMinutes = mins;
      }
    }

    if (Object.keys(updateFields).length === 0) {
      return sendBadRequest(res, 'No valid settings fields provided');
    }

    const store = await Store.findByIdAndUpdate(req.params.storeId, { $set: updateFields }, { new: true })
      .select('name slug estimatedPrepMinutes')
      .lean();
    ...
    CacheInvalidator.invalidateStore(req.params.storeId).catch((err) => {
      logger.error('[ADMIN STORES] Cache invalidation error (settings):', err);
    });
...
// ============================================
// PATCH /admin/stores/:storeId/program — Toggle REZ Program membership
// Called by admin merchantsService.toggleStoreProgram()
// ============================================
router.patch(
  '/:storeId/program',
  asyncHandler(async (req: Request, res: Response) => {
    const { isProgramMerchant, baseCashbackPercent } = req.body;

    if (typeof isProgramMerchant !== 'boolean') {
      return sendBadRequest(res, '`isProgramMerchant` must be a boolean');
    }
    ...
    if (isProgramMerchant && baseCashbackPercent !== undefined) {
      const pct = parseFloat(baseCashbackPercent);
      if (!isNaN(pct) && pct >= 0 && pct <= 100) {
        updateFields['rewardRules.baseCashbackPercent'] = pct;
      }
    }

    const store = await Store.findByIdAndUpdate(req.params.storeId, { $set: updateFields }, { new: true })
      .select('name slug isProgramMerchant rewardRules.baseCashbackPercent')
      .lean();
    ...
    CacheInvalidator.invalidateStore(req.params.storeId).catch((err) => {
      logger.error('[ADMIN STORES] Cache invalidation error (program):', err);
    });

    const message = isProgramMerchant ? 'Store enrolled in REZ Program' : 'Store removed from REZ Program';

    return sendSuccess(res, { store }, message);
  }),
);

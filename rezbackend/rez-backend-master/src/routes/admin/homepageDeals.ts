...
const updateConfigSchema = Joi.object({
  ...
  regions: Joi.array()
    .items(Joi.string().valid('bangalore', 'dubai', 'china', 'all'))
    .optional(),
  tabs: Joi.object({
    offers: Joi.object({
...
const createItemSchema = Joi.object({
  ...
  verificationType: Joi.string()
    .valid('student', 'corporate', 'defence', 'senior', 'birthday', 'women', 'none')
    .optional(),
...
const updateItemSchema = createItemSchema.fork(['tabType', 'itemType', 'title', 'icon', 'navigationPath'], (schema) =>
  schema.optional(),
);
    ...
    const stats = await HomepageDealsItem.aggregate([
      {
        $facet: {
          ...
          byTab: [{ $group: { _id: '$tabType', count: { $sum: 1 } } }],
          totalImpressions: [{ $group: { _id: null, total: { $sum: '$impressions' } } }],
          totalClicks: [{ $group: { _id: null, total: { $sum: '$clicks' } } }],
        },
      },
    ]);
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const item = await HomepageDealsItem.findByIdAndUpdate(id, { $set: value }, { new: true, runValidators: true });
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const item = await HomepageDealsItem.findByIdAndUpdate(id, { $set: { cachedCount: count } }, { new: true });

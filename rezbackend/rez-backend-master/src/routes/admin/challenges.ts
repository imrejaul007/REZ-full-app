...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [total, active, byType, byDifficulty, completionStats] = await Promise.all([
      ...
      Challenge.aggregate([{ $group: { _id: '$type', count: { $sum: 1 } } }]),
      Challenge.aggregate([{ $group: { _id: '$difficulty', count: { $sum: 1 } } }]),
      Challenge.aggregate([
        {
          $group: {
            _id: null,
    ...
    const avgCompletionRate =
      stats.totalParticipants > 0 ? ((stats.totalCompletions / stats.totalParticipants) * 100).toFixed(1) : '0';
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const avgCompletionRate = totalParticipants > 0 ? Math.round((totalCompletions / totalParticipants) * 100) : 0;
    ...
    const challengeBreakdown = activeChallenges.map((c) => ({
      ...
      completionRate:
        (c.participantCount || 0) > 0 ? Math.round(((c.completionCount || 0) / c.participantCount) * 100) : 0,
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      if (!type || !title || !description || !icon || !requirements || !rewards || !startDate || !endDate) {
        return sendError(
          res,
          'type, title, description, icon, requirements, rewards, startDate, and endDate are required',
          400,
        );
      }
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const challenge = await Challenge.findByIdAndUpdate(req.params.id, { $set: { visibility } }, { new: true });
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const challenge = await Challenge.findByIdAndUpdate(req.params.id, { $set: { priority } }, { new: true });

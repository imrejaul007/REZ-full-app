...
// Admin coin award cap — must match the UI limit in the admin panel (loyalty.tsx).
// Set via ADMIN_COIN_AWARD_MAX env var; defaults to 100,000 NC per transaction.
const ADMIN_COIN_AWARD_MAX = parseInt(process.env.ADMIN_COIN_AWARD_MAX || '100000', 10);
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (search) {
      ...
      const matchingUsers = await User.find({
        $or: [{ 'profile.firstName': searchRegex }, { 'profile.lastName': searchRegex }, { phoneNumber: searchRegex }],
      })
        .select('_id')
        .lean();
    ...
    switch (sortBy) {
      case 'coins':
        ...
        break;
      case 'missions':
        // For sorting by missions count, use aggregation to calculate array size
        break;
      case 'streak':
      default:
    ...
    // Use aggregation pipeline for mission sorting to properly count array length
    const usersData =
      sortBy === 'missions'
        ? await UserLoyalty.aggregate([
            { $match: filter },
            { $addFields: { missionCount: { $size: '$missions' } } },
            { $sort: { missionCount: -1 } },
            { $skip: skip },
            { $limit: limit },
            {
    ...
    const [users, total] = await Promise.all([Promise.resolve(usersData), UserLoyalty.countDocuments(filter)]);
    ...
    const stats = await UserLoyalty.aggregate([
      {
        $facet: {
          ...
          activeStreaks: [{ $match: { 'streak.current': { $gt: 0 } } }, { $count: 'count' }],
          totalCoinsEarned: [{ $group: { _id: null, total: { $sum: '$coins.available' } } }],
          completedMissions: [
            { $unwind: '$missions' },
            { $match: { 'missions.completedAt': { $exists: true, $ne: null } } },
            { $count: 'count' },
          ],
          avgStreak: [{ $group: { _id: null, avg: { $avg: '$streak.current' } } }],
...
router.get(
  '/:userId',
  asyncHandler(async (req: Request, res: Response) => {
    const loyalty = await UserLoyalty.findOne({ userId: req.params.userId }).populate(
      'userId',
      'profile.firstName profile.lastName phoneNumber email',
    );
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (amount > ADMIN_COIN_AWARD_MAX) {
      return res.status(400).json({
        success: false,
        message: `Admin coin award exceeds maximum of ${ADMIN_COIN_AWARD_MAX} coins per transaction`,
      });
    }

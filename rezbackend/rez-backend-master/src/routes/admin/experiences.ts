...
const createExperienceSchema = Joi.object({
  ...
  regions: Joi.array()
    .items(Joi.string().valid('bangalore', 'dubai', 'china'))
    .optional(),
...
const updateExperienceSchema = createExperienceSchema.fork(['slug', 'title', 'icon', 'type'], (schema) =>
  schema.optional(),
);
...
router.get(
  '/categories/list',
  asyncHandler(async (req: Request, res: Response) => {
    const categories = await Category.find({ isActive: true }).select('_id name slug icon').sort({ name: 1 }).lean();
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const experience = await StoreExperience.findByIdAndUpdate(id, { $set: value }, { new: true, runValidators: true });
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const isAlreadyAssigned = (experience as any).assignedStores.some((sid: any) => sid.toString() === storeId);

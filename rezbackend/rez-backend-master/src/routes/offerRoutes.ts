...
router.post(
  ...
  validate(
    Joi.object({
      redemptionType: Joi.string().valid('online', 'instore').default('instore').required(),
      location: Joi.object({
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    // Get section config
    // DANIEL: .lean() on sectionConfig query — avoids Mongoose Document overhead for simple read
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const { itemIds, tabType: _tabType } = req.body;
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const { itemId, tabType: _tabType } = req.body;
...
// Get exclusive zones
// BR-H2 note: getExclusiveZones is a live endpoint returning real data from ExclusiveZone collection.
// Returns empty array if no zones are configured (not a stub). Authenticated users get eligibility info.
...
// BR-H2 note: getSpecialProfiles is a live endpoint. Returns empty array if no SpecialProfile
// documents exist in DB. Authenticated users get eligibility info from User.verifications.
...
// BR-H2 note: getLoyaltyMilestones is a live endpoint returning milestones from LoyaltyMilestone
// collection. Returns empty array if no milestones are configured (cached for 1 hour).

...
router.get(
  '/products/similar/:productId', // generalLimiter,, // Disabled for development
  optionalAuth,
  validateParams(
    Joi.object({
...
router.get(
  '/products/frequently-bought/:productId', // generalLimiter,, // Disabled for development
  optionalAuth,
  validateParams(
    Joi.object({
      productId: Joi.string().pattern(/^[0-9a-fA-F]{24}$/), // MongoDB ObjectId
...
router.get(
  '/products/bundle/:productId', // generalLimiter,, // Disabled for development
  optionalAuth,
  validateParams(
    Joi.object({
      productId: Joi.string().pattern(/^[0-9a-fA-F]{24}$/), // MongoDB ObjectId
...
router.get(
  '/products/personalized', // recommendationLimiter,, // Disabled for development
  requireAuth,
  validateQuery(
    Joi.object({
      limit: Joi.number().integer().min(1).max(50).default(10),
...
// Track product view
router.post(
  '/products/:productId/view', // generalLimiter,, // Disabled for development
  optionalAuth,
  validateParams(
    Joi.object({
      productId: Joi.string().pattern(/^[0-9a-fA-F]{24}$/), // MongoDB ObjectId
 ...
 */
router.post(
  '/diverse', // recommendationLimiter,, // Disabled for development
  optionalAuth,
  validateBody(
    Joi.object({
      excludeProducts: Joi.array()
        ...
        .optional(),
      excludeStores: Joi.array()
        .items(Joi.string().pattern(/^[0-9a-fA-F]{24}$/))
        .optional(),
      shownProducts: Joi.array()
        .items(Joi.string().pattern(/^[0-9a-fA-F]{24}$/))
        .optional(),
      limit: Joi.number().integer().min(1).max(50).default(10),

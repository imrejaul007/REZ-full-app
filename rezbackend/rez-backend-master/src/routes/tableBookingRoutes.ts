...
// NOTE: must be defined BEFORE /:bookingId so Express does not treat
// the literal segment "store" as a bookingId parameter value.
router.get('/store/:storeId', getStoreTableBookings);

// ==================== ADMIN ROUTES ====================

// Admin: view all bookings across stores
// NOTE: must be defined BEFORE /:bookingId — otherwise Express matches
// the literal path segment "admin" as a bookingId parameter and this
...
// These MUST remain after all literal-segment routes (/user, /merchant,
// /store/:storeId, /admin) so Express does not shadow those routes.

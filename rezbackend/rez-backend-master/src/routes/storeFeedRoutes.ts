...
// REZ Mind Intelligence Hub URL
const INTELLIGENCE_HUB_URL = process.env.INTELLIGENCE_HUB_URL || 'https://rez-intelligence-hub.onrender.com';

/**
 * Fetch user intents from REZ Mind Intelligence Hub
 */
async function fetchUserIntentsFromRezMind(userId: string): Promise<string[]> {
  try {
    ...
    if (response.data?.success && response.data?.data?.affinities) {
      // Extract categories from affinities
      const affinities = response.data.data.affinities;
      const categories = affinities
        .map((a: any) => a.category?.toLowerCase())
        .filter(Boolean);
      return categories;
    }
  } catch (err: any) {
    logger.warn('[StoreFeed] REZ Mind unavailable:', err.message);
  }
  return [];
}

/**
 * Fetch personalized store scores from REZ Mind
 * Returns map of storeId -> REZ Mind score (0-100)
 */
async function fetchRezMindStoreScores(userId: string, storeIds: string[]): Promise<Map<string, number>> {
  const scores = new Map<string, number>();

  try {
    // Call recommendation engine for similar user preferences
    const response = await axios.post(
      `${INTELLIGENCE_HUB_URL}/api/recommendations/stores`,
      {
        userId,
        storeIds,
        limit: storeIds.length,
      },
      { timeout: 3000 }
    );

    if (response.data?.success && response.data?.data?.recommendations) {
      for (const rec of response.data.data.recommendations) {
        scores.set(rec.storeId, rec.score * 100); // Convert 0-1 to 0-100
      }
    }
  } catch (err: any) {
    logger.debug('[StoreFeed] REZ Mind scores unavailable:', err.message);
  }
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (interestsParam) {
      ...
      // ENHANCEMENT: Also fetch from REZ Mind Intelligence Hub for better personalization
      try {
        const rezMindInterests = await fetchUserIntentsFromRezMind(userId);
        if (rezMindInterests.length > 0) {
          // Merge with existing interests, deduplicate
          userInterests = [...new Set([...userInterests, ...rezMindInterests])];
          logger.info('[StoreFeed] Enhanced with REZ Mind interests', {
            userId,
            interests: rezMindInterests
          });
        }
      } catch (err: any) {
        logger.warn('[StoreFeed] REZ Mind interests unavailable', { userId, error: err.message });
      }
    }
    ...
    // ENHANCEMENT: Fetch REZ Mind personalization scores for all stores
    let rezMindScores = new Map<string, number>();
    if (userId && stores.length > 0) {
      try {
        const storeIds = stores.map((s: any) => String(s._id));
        rezMindScores = await fetchRezMindStoreScores(userId, storeIds);
        logger.info('[StoreFeed] Got REZ Mind scores', { userId, scoredStores: rezMindScores.size });
      } catch (err: any) {
        logger.warn('[StoreFeed] REZ Mind scoring failed', { error: err.message });
      }
    }
    ...
    const scored = stores
      .map((store: any) => {
        ...
        // --- REZ Mind personalization score (+30 pts max) ---
        const rezMindScore = rezMindScores.get(storeId) || 0;
        total += rezMindScore;

...
router.post('/process-expired', requireAuth, requireAdmin, processExpiredLocks);
...
// Confirm lock after deposit payment (Razorpay signature verification)
router.post(
  '/:id/confirm-lock',
  requireAuth,
  validateParams(
...
// Confirm balance payment (Razorpay signature verification)
router.post(
  '/:lockId/confirm-balance',
  requireAuth,
  validateParams(
    Joi.object({
      lockId: Joi.string().required(),

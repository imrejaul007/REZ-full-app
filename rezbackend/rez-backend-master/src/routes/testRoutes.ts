...
// Require authentication AND admin role — financial verification data must not be
// accessible to regular authenticated users even in non-production environments.
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    return res.json({
      success: true,
      message:
        verification.feeCorrect && verification.payoutCorrect
 ...
 */
router.get(
  '/verify-merchant-wallet/:merchantId',
  asyncHandler(async (req: Request, res: Response) => {
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    return res.json({
      success: true,
      message: purchaseRewards.length > 0 ? '✅ Purchase reward coins found' : '❌ No purchase reward coins found',
      data: {
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    return res.json({
      success: true,
      message: purchaseShares.length > 0 ? '✅ Purchase shares found' : '❌ No purchase shares found',
      data: {
        userId: req.params.userId,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const merchantId = storeItem?.store
      ? (storeItem.store as any).owner?.toString() || (storeItem.store as any).merchantId?.toString()
      : null;
    ...
    const feeCheck = {
      ...
      feePercentage:
        order.totals.subtotal > 0 ? ((order.totals.platformFee / order.totals.subtotal) * 100).toFixed(2) + '%' : 'N/A',
    ...
    if (merchantId) {
      ...
      if (wallet) {
        const orderTransaction = wallet.transactions.find((t) => t.orderId?.toString() === orderId);
        walletCheck = {
    ...
    return res.json({
      success: true,
      message: allPassed ? '✅ All demo flow features verified!' : '⚠️ Some features need attention',
      data: {
        orderId: order._id,
        orderNumber: order.orderNumber,

...
router.get('/', validateQuery(listEventsQuerySchema), async (req: Request, res: Response) => {
  try {
    ...
    const [events, totalCount] = await Promise.all([
      Event.find(query).sort(sortOptions).skip(skip).limit(limitNum).lean(),
...
router.get('/:id/analytics', validateParams(eventIdSchema), async (req: Request, res: Response) => {
  try {
    ...
    const analytics = {
      ...
      capacityUtilization,
      conversionRate:
        event.analytics?.views > 0 ? Math.round((event.analytics.bookings / event.analytics.views) * 100) : 0,
    };

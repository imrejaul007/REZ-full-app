...
router.get('/history', async (req, res) => {
  try {
    ...
    const history = await SyncService.getSyncHistory(merchantId, parseInt(limit as string, 10));
...
// @desc    Get sync statistics scoped to the authenticated merchant
// @access  Private
router.get('/statistics', async (req, res) => {
  try {
    ...
    const stats = SyncService.getSyncStatistics(merchantId);
...
// @access  Private
// NOTE: Orders are managed by the user backend directly. The merchant-side sync
// pipeline reads orders but the database write is intentionally skipped (orders
// are read-only from the merchant side). Return 501 so callers are not misled.
router.post('/orders', async (_req, res) => {
  return res.status(501).json({
    success: false,
    error: 'Order sync not implemented. Orders are managed by the backend directly.',
...
// @access  Private
// NOTE: Cashback records are managed by the user backend directly. The merchant-
// side sync pipeline reads cashback data but the database write is intentionally
// skipped. Return 501 so callers are not misled.
router.post('/cashback', async (_req, res) => {
  return res.status(501).json({
    success: false,

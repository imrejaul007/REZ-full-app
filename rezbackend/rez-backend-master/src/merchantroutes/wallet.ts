...
import { financialRateLimit } from '../middleware/financialRateLimiter';

// 5 withdrawal requests per hour, fail-closed (money movement)
const withdrawLimiter = financialRateLimit({ windowMs: 60 * 60 * 1000, maxOps: 5, keyBy: 'userId', failClosed: true });
// 10 bank detail updates per hour
const bankDetailsLimiter = financialRateLimit({ windowMs: 60 * 60 * 1000, maxOps: 10, keyBy: 'userId' });
...
router.get('/transactions', async (req: Request, res: Response) => {
  try {
    ...
    const result = await merchantWalletService.getTransactionHistory(merchantId, page, limit, type);
 ...
 * @access  Private (Merchant)
 */
router.post('/withdraw', withdrawLimiter, async (req: Request, res: Response) => {
  try {
    const merchantId = req.merchantId;
    if (!merchantId) {
      return res.status(400).json({
 ...
 * @access  Private (Merchant)
 */
router.put('/bank-details', bankDetailsLimiter, async (req: Request, res: Response) => {
  try {
    const merchantId = req.merchantId;
    if (!merchantId) {
      return res.status(400).json({
...
router.get('/stats', async (req: Request, res: Response) => {
  try {
    ...
    const stats = {
      ...
      // BR-M1 fix: Read platformFeeRate from env var (admin-configurable without deploy).
      // Fallback to 0.15 (15%) if not set. Long-term: read from SystemConfig model.
      ...
      highlights: {
        netEarningsRate:
          summary.statistics.totalSales > 0

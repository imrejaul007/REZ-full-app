...
// Stream to disk before uploading to Cloudinary (prevents memory exhaustion)
const storage = multer.diskStorage({
  destination: '/tmp/gallery-uploads/',
  filename: (req, file, cb) => {
    cb(null, `${Date.now()}-${crypto.randomUUID().replace('-', '').substring(0, 9)}-${file.originalname}`);
  },
});
...
const upload = multer({
  ...
  fileFilter,
  limits: {
    fileSize: 10 * 1024 * 1024, // 10MB limit (not 50MB — more reasonable for cloud)
  },
});
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const folder = isVideo
        ? `merchants/${merchantId}/stores/${storeId}/gallery/videos`
        : `merchants/${merchantId}/stores/${storeId}/gallery/images`;

      if (isVideo) {
        result = await CloudinaryService.uploadBuffer(req.file.buffer, {
          folder,
          resource_type: 'video',
          quality: 'auto',
        });
      ...
      const itemOrder = order !== undefined ? parseInt(order, 10) : (maxOrderItem?.order || 0) + 1;

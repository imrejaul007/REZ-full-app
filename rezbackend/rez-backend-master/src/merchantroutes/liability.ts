...
/**
 * POST /api/merchant/liability/instant
 * Request an instant/on-demand settlement payout.
 * Marks all pending_settlement liabilities for this merchant as settled
 * and queues a payout trigger (async — actual transfer is handled by finance).
 */
router.post('/instant', async (req: Request, res: Response) => {
  try {
    ...
    const pending = await MerchantLiability.find({
      merchant: new Types.ObjectId(String(merchantId)),
      status: 'pending_settlement',
    }).lean();

    if (pending.length === 0) {
      return res.status(400).json({ success: false, message: 'No pending settlement balance to process' });
    }

    const totalAmount = pending.reduce((sum, r) => sum + (r.pendingAmount || 0), 0);

    // Mark all pending records as settled
    await MerchantLiability.updateMany(
      { merchant: new Types.ObjectId(String(merchantId)), status: 'pending_settlement' },
      {
        $set: {
          status: 'settled',
          settlementDate: new Date().toISOString(),
          settlementTransactionId: `INSTANT-${Date.now()}`,
        },
      },
    );

    logger.info('Instant settlement requested', { merchantId: String(merchantId), totalAmount, count: pending.length });

    return res.json({
      success: true,
      message: `Instant settlement of ₹${totalAmount.toFixed(2)} across ${pending.length} record(s) has been queued.`,
      data: { totalAmount, count: pending.length },
    });
  } catch (error: any) {
    logger.error('Instant settlement failed', error);
    return res.status(500).json({ success: false, message: error.message || 'Instant settlement failed' });
  }
});

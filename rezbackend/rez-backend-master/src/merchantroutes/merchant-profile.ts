...
// @desc    Get merchant's own profile
// @access  Private
router.get('/', async (req, res) => {
  try {
    const merchantId = req.merchantId;
    const merchant = await MerchantModel.findById(merchantId).select('-password');
    if (!merchant) {
      return res.status(404).json({ success: false, message: 'Merchant not found' });
    }
    const stores = await Store.find({ merchant: merchantId }).select('name address isActive');
    res.json({
      success: true,
      data: {
        merchant,

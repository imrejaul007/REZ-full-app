...
const createDiscountSchema = Joi.object({
  ...
  cardBins: Joi.array()
    .items(Joi.string().regex(/^\d{6}$/))
    .optional(),
});

const updateDiscountSchema = createDiscountSchema.fork(['name', 'type', 'value', 'validFrom', 'validUntil'], (schema) =>
  schema.optional(),
);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (req.query.paymentMethod) {
        query.paymentMethod =
          req.query.paymentMethod === 'all' ? { $in: ['all', 'card', 'upi'] } : req.query.paymentMethod;
      }
    ...
    try {
      ...
      const analytics = {
        ...
        usage: {
          ...
          remainingUses: discount.usageLimit ? Math.max(0, discount.usageLimit - (discount.usedCount || 0)) : null,
        },
        validity: {

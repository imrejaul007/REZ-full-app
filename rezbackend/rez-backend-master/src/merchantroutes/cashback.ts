...
router.get('/', async (req, res) => {
  try {
    ...
    if (page) searchParams.page = parseInt(page as string, 10);
    if (limit) searchParams.limit = parseInt(limit as string, 10);
...
router.post('/:id/process-payment', async (req, res) => {
  try {
    ...
    const payoutResult = await PaymentService.processCashbackPayout(request, customerBankDetails);
...
// @desc    Export cashback data as CSV
// @access  Private
router.get('/export', async (req, res) => {
  try {
    ...
    const start = startDate ? new Date(startDate as string) : new Date(Date.now() - 90 * 86400000); // Default: last 90 days
    const end = endDate ? new Date(endDate as string) : new Date();

    // Query Cashback requests for this merchant in date range
    const requests = await CashbackModel.search({
      merchantId,
      dateRange: { start, end },
      page: 1,
      limit: 1000,
    ...
    const transactions = Array.isArray(requests?.requests) ? requests.requests : [];
    ...
    const rows = transactions.map((t: any) => ({
      ...
      ApprovedAmount: (t.approvedAmount || 0).toFixed(2),
    }));
    ...
    const csvContent = [
      ...
      ...rows.map((row: any) => headers.map((h) => escapeCsvValue(row[h])).join(',')),
    ].join('\n');

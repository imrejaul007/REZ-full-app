...
router.post('/step/:stepNumber', authenticateMerchant, async (req: Request, res: Response) => {
  try {
    ...
    const statusCode =
      error.message?.includes('required') || error.message?.includes('invalid') || error.message?.includes('Invalid')
        ? 400
        : 500;
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const uploadResult = await DocumentVerificationService.uploadDocument(req.file, merchantId, documentType);
...
router.delete('/documents/:documentIndex', authenticateMerchant, async (req: Request, res: Response) => {
  try {
    const merchantId = (req as any).merchantId;
    const documentIndex = parseInt(req.params.documentIndex, 10);
...
router.post('/:merchantId/approve', authenticate, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    // authenticate sets req.userId (not req.admin) — use that as the adminId
 ...
 */
router.post(
  '/:merchantId/documents/:documentIndex/verify',
  authenticate,
  requireAdmin,
  async (req: Request, res: Response) => {
    try {
...
router.post('/:merchantId/documents/verify-all', authenticate, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const result = await DocumentVerificationService.verifyAllDocuments(merchantId, adminId, approved, rejectionReason);
...
router.post('/:merchantId/request-documents', authenticate, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const result = await DocumentVerificationService.requestAdditionalDocuments(merchantId, documentTypes, message);
 ...
 * @access  Private (Admin)
 */
router.get('/documents/statistics', authenticate, requireAdmin, async (req: Request, res: Response) => {
  try {

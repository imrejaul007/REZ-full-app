 ...
 */
router.post('/bulk-import', authMiddleware, upload.single('file'), async (req: Request, res: Response) => {
  try {
...
async function processImportJob(
  ...
  try {
    ...
    // Process the import
    const result = await bulkImportService.processBulkImport(filePath, fileType, storeId, merchantId);
 ...
 * @access  Private (Merchant)
 */
router.get('/import-status/:jobId', authMiddleware, async (req: Request, res: Response) => {
  try {
    const merchantId = req.merchantId;
    ...
    const progressPercent =
      importJob.progress.total > 0 ? Math.round((importJob.progress.processed / importJob.progress.total) * 100) : 0;
 ...
 * @access  Private (Merchant)
 */
router.get('/import-jobs', authMiddleware, async (req: Request, res: Response) => {
  try {
    const merchantId = req.merchantId;
    ...
    const [jobs, total] = await Promise.all([
      ImportJob.find(query).populate('storeId', 'name').sort({ createdAt: -1 }).limit(Number(limit)).skip(skip),
 ...
 * @access  Private (Merchant)
 */
router.get('/import-instructions', authMiddleware, async (req: Request, res: Response) => {
  try {
 ...
 * @access  Private (Merchant)
 */
router.delete('/import-job/:jobId', authMiddleware, async (req: Request, res: Response) => {
  try {
    const merchantId = req.merchantId;
    const { jobId } = req.params;

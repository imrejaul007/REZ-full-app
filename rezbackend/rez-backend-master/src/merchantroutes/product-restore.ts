...
router.get('/deleted', async (req, res) => {
  try {
    ...
    const [products, totalCount] = await Promise.all([
      Product.find(query).sort({ deletedAt: -1 }).skip(skip).limit(limit),
...
// @route   POST /api/products/bulk-restore
// @desc    Restore multiple soft-deleted products
// @access  Private
router.post('/bulk-restore', async (req, res) => {
  try {
    ...
    logger.info('🔄 [BULK RESTORE] Request received:');
    ...
    // Find deleted products that belong to this merchant's stores
    ...
    for (const product of products) {
      try {
        ...
        if (product.deletedAt) {
          ...
          if (daysSinceDeletion > 30) {
            failed.push({
              productId: product._id,
              name: product.name,
              error: `Product was deleted ${daysSinceDeletion} days ago and cannot be restored (30-day limit exceeded)`,
            });
            continue;
          }
        }
        ...
        logger.error(`❌ Failed to restore product "${product.name}":`, error);
      }
    }

    // Send real-time notification
    if (global.io && restored.length > 0) {
      global.io.to(`merchant-${req.merchantId}`).emit('products_restored', {
    ...
    return res.json({
      success: true,
      message: `${restored.length} product${restored.length !== 1 ? 's' : ''} restored successfully`,
      data: {
        restored,
        failed,
      },

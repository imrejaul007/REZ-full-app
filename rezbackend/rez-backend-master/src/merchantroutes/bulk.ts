...
// Stream to disk before parsing (prevents memory exhaustion on large CSV/Excel uploads)
const storage = multer.diskStorage({
  destination: '/tmp/bulk-imports/',
...
router.post('/products/import', upload.single('file'), async (req: Request, res: Response) => {
  try {
    ...
    if (global.io && result.success) {
      global.io.to(`merchant-${merchantId}`).emit('bulk-operation-completed', {
...
router.post('/products/export/advanced', async (req: Request, res: Response) => {
  try {
    ...
    const _jobId = `export-${Date.now()}-${crypto.randomUUID().replace('-', '').substring(0, 9)}`;

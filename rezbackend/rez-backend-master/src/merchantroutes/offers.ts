...
const createOfferSchema = Joi.object({
  ...
  category: Joi.string()
    .valid('mega', 'student', 'new_arrival', 'trending', 'food', 'fashion', 'electronics', 'general', 'entertainment', 'beauty', 'wellness')
    .default('general'),
  ...
  timeWindow: Joi.object({
    ...
    endHour: Joi.number().integer().min(1).max(23).required(),
    activeDays: Joi.array().items(Joi.number().integer().min(0).max(6)).optional(),
  }).optional(),
});

const updateOfferSchema = createOfferSchema.fork(['title', 'image', 'storeId', 'validity'], (schema) =>
  schema.optional(),
);
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      // SPEEDCORE: Add .lean() for read-only query
      const stores = await Store.find({ merchantId: new mongoose.Types.ObjectId(merchantId) }).lean();
      ...
      if (req.query.store) {
        if (!mongoose.isValidObjectId(req.query.store as string)) {
...
router.post('/', validateRequest(createOfferSchema), async (req: Request, res: Response) => {
  try {
    ...
    const offer = new Offer({
      ...
      ...(offerData.timeWindow && { timeWindow: offerData.timeWindow }),

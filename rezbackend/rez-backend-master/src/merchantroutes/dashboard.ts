...
// SPEEDCORE: Helper to cache merchant dashboard stats (2-min TTL)
async function withMerchantCache<T>(key: string, ttlSecs: number, fn: () => Promise<T>): Promise<T> {
  try {
    const cached = await redisService.get<T>(key);
    if (cached) return cached;
  } catch (err) {
    logger.warn('[CACHE] Redis get failed, continuing:', (err as Error).message);
  }

  const result = await fn();

  try {
    await redisService.set(key, JSON.stringify(result), ttlSecs);
  } catch (err) {
    logger.warn('[CACHE] Redis set failed:', (err as Error).message);
  }
...
router.get('/activity', async (req, res) => {
  try {
    ...
    const activity = await getRecentActivity(merchantId, parseInt(limit as string, 10));
...
router.get('/top-products', async (req, res) => {
  try {
    ...
    const topProducts = await getTopProductsByPeriod(merchantId, days, sortBy as string, parseInt(limit as string, 10));
...
router.get('/low-stock', async (req, res) => {
  try {
    ...
    const lowStockProducts = await getLowStockProducts(merchantId, parseInt(threshold as string, 10));
...
router.get('/performance', async (req, res) => {
  try {
    ...
    const revenueTrend =
      firstHalfAvgRevenue > 0 ? ((secondHalfAvgRevenue - firstHalfAvgRevenue) / firstHalfAvgRevenue) * 100 : 0;
    ...
    const ordersTrend =
      firstHalfAvgOrders > 0 ? ((secondHalfAvgOrders - firstHalfAvgOrders) / firstHalfAvgOrders) * 100 : 0;
...
router.get('/export/scheduled/:type', async (req, res) => {
  try {
    ...
    const reportData = await ExportService.generateScheduledReport(merchantId, type as 'daily' | 'weekly' | 'monthly');
...
router.get('/reports/history', async (req, res) => {
  try {
    ...
    const history = ReportService.getHistoryByMerchant(merchantId, parseInt(limit as string, 10));
...
router.get('/reports/upcoming', async (req, res) => {
  try {
    ...
    const upcoming = ReportService.getUpcomingReports(merchantId, parseInt(days as string, 10));
...
router.get('/customer-payments', async (req, res) => {
  try {
    ...
    const _halfLimit = Math.ceil(limit / 2) + 2; // fetch extra to ensure enough after merge
...
router.get('/action-items', async (req, res) => {
  try {
    ...
    if (storeIds.length === 0) {
      return res.json({
        success: true,
        data: { actionItems: [], summary: { total: 0, urgent: 0, high: 0, medium: 0 } },
      });
    }
    ...
    const pendingOrders = await Order.find({
      ...
      .select('store orderNumber createdAt')
      .sort({ createdAt: 1 })
      .limit(50)
      .lean();
    ...
    const lowStock = await Product.find({
      ...
      .select('store name inventory.stock')
      .limit(50)
      .lean();
...
async function getRecentActivity(merchantId: string, limit: number = 20, storeId?: string): Promise<any[]> {
  try {
    ...
    const filterStoreIds = storeId ? allStoreIds.filter((id) => id.toString() === storeId) : allStoreIds;
    ...
    recentOrders.forEach((order: any) => {
      const customerName =
        typeof order.user === 'object' && order.user?.fullName ? order.user.fullName : 'Unknown Customer';
 ...
 */
async function getTopProductsByPeriod(merchantId: string, days: number, sortBy: string, limit: number): Promise<any[]> {
  try {
    const endDate = new Date();
    const startDate = new Date();
    startDate.setDate(endDate.getDate() - days);
 ...
 */
async function getSalesChartData(merchantId: string, days: number, granularity: string): Promise<any[]> {
  try {
    const timeSeriesData = await BusinessMetricsService.getTimeSeriesData(merchantId, days);
    ...
    let _periodStart: Date | null = null;

    timeSeriesData.forEach((day, _index) => {
...
// @route   GET /api/merchant/dashboard/today-revenue
// @desc    Get today's revenue and order count
// @access  Private
router.get('/today-revenue', async (req: any, res) => {
  try {
    const merchantId = req.merchantId;
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    ...
    const result = await Order.aggregate([
      { $match: { store: { $in: storeIds }, createdAt: { $gte: today }, status: { $nin: ['cancelled', 'refunded'] } } },
      { $group: { _id: null, totalRevenue: { $sum: '$totals.total' }, orderCount: { $sum: 1 } } },
    ]);
    res.json({ success: true, data: { revenue: result[0]?.totalRevenue || 0, orders: result[0]?.orderCount || 0 } });
  } catch (error: any) {
...
// @route   GET /api/merchant/dashboard/top-items-today
// @desc    Get top selling items for today
// @access  Private
router.get('/top-items-today', async (req: any, res) => {
  try {
    const merchantId = req.merchantId;
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    ...
    const storeIds = stores.map((s: any) => s._id);
    const result = await Order.aggregate([
      { $match: { store: { $in: storeIds }, createdAt: { $gte: today }, status: { $nin: ['cancelled'] } } },
      { $unwind: '$items' },
      {
        $group: {
          _id: '$items.name',
...
// @route   GET /api/merchant/dashboard/campaign-performance
// @desc    Get campaign performance summary
// @access  Private
router.get('/campaign-performance', async (req: any, res) => {
  try {
    res.json({
      success: true,
      data: { campaigns: [], summary: { totalCampaigns: 0, activeCampaigns: 0, totalReach: 0 } },
    });
  } catch (error: any) {
    logger.error('[ROUTE ERROR]', error.message);
    res.status(500).json({ success: false, message: 'Operation failed. Please try again.' });
...
// @route   GET /api/merchant/dashboard/customer-retention
// @desc    Get customer retention metrics
// @access  Private
router.get('/customer-retention', async (req: any, res) => {
  try {
    res.json({ success: true, data: { retentionRate: 0, repeatCustomers: 0, newCustomers: 0, churned: 0 } });
  } catch (error: any) {
    logger.error('[ROUTE ERROR]', error.message);
    res.status(500).json({ success: false, message: 'Operation failed. Please try again.' });
  }
...
// @route   GET /api/merchant/dashboard/basket-size-trend
// @desc    Get average basket size trend over last 30 days
// @access  Private
router.get('/basket-size-trend', async (req: any, res) => {
  try {
    const merchantId = req.merchantId;
    const stores = await Store.find({ merchantId }).select('_id').lean();
    const storeIds = stores.map((s: any) => s._id);
    ...
    const result = await Order.aggregate([
      { $match: { store: { $in: storeIds }, createdAt: { $gte: thirtyDaysAgo }, status: 'delivered' } },
      {
        $group: {

...
// 100 coin award ops per hour per merchant (prevents bulk coin minting abuse)
const coinAwardLimiter = createRateLimiter({ windowMs: 60 * 60 * 1000, max: 100, prefix: 'merchant-coins-award' });
 ...
 * @access  Merchant
 */
router.post('/award', coinAwardLimiter, async (req: Request, res: Response) => {
  try {
    ...
    return res.json({
      ...
      data: {
        ...
        newBrandedBalance:
          wallet.brandedCoins.find((c: any) => c.merchantId.toString() === storeId)?.amount || coinAmount,
...
router.get('/stats', async (req: Request, res: Response) => {
  try {
    ...
    // --- GAP FIX #5: merchant campaign ROI — coins redeemed at this merchant's stores ---
    // We look for 'spent' CoinTransactions where the order metadata references the merchant's store(s),
    // enabling merchants to see: (coins issued) vs (coins that drove purchases back to their stores).
    const redemptionMatchQuery: any = {
      type: 'spent',
      source: 'purchase',
    };
    if (storeId) {
      redemptionMatchQuery['metadata.storeId'] = storeId;
    } else {
      redemptionMatchQuery['metadata.merchantId'] = merchantId;
    }

    const [redemptionStats, todayRedemptions] = await Promise.all([
      CoinTransaction.aggregate([
        { $match: redemptionMatchQuery },
        {
          $group: {
            _id: null,
            totalCoinsRedeemed: { $sum: '$amount' },
      ...
      CoinTransaction.aggregate([
        { $match: { ...redemptionMatchQuery, createdAt: { $gte: today } } },
        {
          $group: {
            _id: null,
    ...
    // Compute a simple ROI ratio: coins redeemed / coins issued (higher = better engagement)
    const totalIssued = stats[0]?.totalCoinsAwarded ?? 0;
    const totalRedeemed = redemptionStats[0]?.totalCoinsRedeemed ?? 0;
    const roiRatio = totalIssued > 0 ? Math.round((totalRedeemed / totalIssued) * 100) / 100 : 0;

    logger.info('merchant_campaign_roi_viewed', {
      event: 'merchant_campaign_roi_viewed',
      merchantId,
    ...
    return res.json({
      ...
      data: {
        ...
        // --- GAP FIX #5: redemption & ROI fields exposed to merchant dashboard ---
        redemption: redemptionStats[0] || {
        ...
        roi: {
          // roiRatio = how many coins came back per coin issued (e.g. 0.8 = 80 % returned)
          roiRatio,
          totalCoinsIssued: totalIssued,
          totalCoinsRedeemed: totalRedeemed,
        },

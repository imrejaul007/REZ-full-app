...
import { validateQuery } from '../middleware/merchantvalidation';
...
const _updateCategorySchema = createCategorySchema.fork(['name'], (schema) => schema.optional());

const _categoryIdSchema = Joi.object({
...
const searchCategoriesSchema = Joi.object({
  ...
  isActive: Joi.alternatives()
    ...
    .optional(),
  sortBy: Joi.string().valid('name', 'sortOrder', 'created', 'productCount').default('sortOrder'),
  sortOrder: Joi.string().valid('asc', 'desc').default('asc'),
  includeEmpty: Joi.alternatives()
    .try(
      Joi.boolean(),
      Joi.string()
        .valid('true', 'false')
...
router.get('/', validateQuery(searchCategoriesSchema), async (req, res) => {
  try {
    const { query, parentId, storeId, isActive, sortBy, sortOrder, includeEmpty } = req.query as any;
...
router.post('/auto-categorize', async (req, res) => {
  try {
    ...
    for (const product of products) {
      ...
      // Electronics
      if (
        /\b(phone|laptop|computer|tablet|headphone|speaker|tv|monitor|camera|gaming|electronic|tech|digital|smart|wireless|bluetooth|usb|charger|cable|battery)\b/.test(
          productText,
        )
      ) {
      ...
      // Clothing
        ...
        /\b(shirt|pants|dress|shoes|clothing|apparel|fashion|wear|jacket|coat|jeans|sweater|t-shirt|blouse|skirt|shorts|socks|underwear|hat|cap|belt|scarf)\b/.test(
          productText,
        )
      ) {
      ...
      // Home & Garden
        ...
        /\b(furniture|home|kitchen|garden|decor|decoration|lamp|chair|table|bed|sofa|plant|outdoor|indoor|living|bedroom|bathroom|dining)\b/.test(
          productText,
        )
      ) {
      ...
      // Books & Media
        ...
        /\b(book|magazine|cd|dvd|blu-ray|media|novel|textbook|guide|manual|literature|reading|author)\b/.test(
          productText,
        )
      ) {
      ...
      // Sports & Outdoors
        ...
        /\b(sport|sports|fitness|exercise|outdoor|camping|hiking|fishing|cycling|running|gym|workout|athletic|recreation)\b/.test(
          productText,
        )
      ) {

...
router.get('/verify/:code', async (req: Request, res: Response) => {
  try {
    ...
    let redemption = await OfferRedemption.findOne({
      $or: [{ redemptionCode: code }, { verificationCode: code }],
    })
    ...
    if (offerStoreId) {
      ...
      const storeMatch = merchantStoreIds.some((storeId) => storeId.toString() === offerStoreId);
    ...
    if (new Date() > redemption.expiryDate) {
      ...
      await OfferRedemption.updateOne({ _id: redemption._id }, { status: 'expired' });
...
router.post('/:code/use', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { storeId, orderAmount, notes: _notes } = value;
    ...
    // Notify merchant about voucher redemption (fire-and-forget)
    try {
      const { User } = await import('../models/User');
      const user = (await User.findById(userId).select('name profile').lean()) as any;
      ...
      await merchantNotificationService
        .notify({
          ...
          message: `${userName} redeemed voucher ${code} — ₹${cashbackAmount} off`,
          data: { voucherId: (redemption as any)._id?.toString(), userId: userId },
        })
        .catch((err: any) => logger.error('[VOUCHER REDEMPTION] Failed to send merchant notification: ' + err.message));
    } catch (notifErr: any) {
      logger.error('[VOUCHER REDEMPTION] Notification error: ' + notifErr.message);
    }

    logger.info(
      `✅ [MERCHANT VOUCHER USE] Code ${code} marked as used by merchant ${merchantId} at store ${storeId}. Cashback: ₹${cashbackAmount}`,
    );
      ...
      data: {
        redemptions: redemptions.map((r) => ({
          ...
          user: r.user
            ? {
                name:
                  `${(r.user as any).profile?.firstName || ''} ${(r.user as any).profile?.lastName || ''}`.trim() ||
                  'Customer',
              }
            : null,
        })),

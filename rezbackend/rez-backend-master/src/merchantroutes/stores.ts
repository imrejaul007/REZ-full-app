...
const createStoreSchema = Joi.object({
  ...
  operationalInfo: Joi.object({
    hours: Joi.object({
      ...
      .custom((hours, helpers) => {
        // Validate that close time is strictly after open time for each open day
        ...
        for (const day of days) {
          ...
          if (closeH * 60 + closeM <= openH * 60 + openM) {
            return helpers.error('any.invalid', { message: `${day}: close time must be after open time` });
          }
        }
        return hours;
      })
    ...
    const store = new Store({
      ...
      operationalInfo: {
        ...
        acceptsWalletPayment:
          storeData.operationalInfo?.acceptsWalletPayment !== undefined
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const stores = await Store.find(query).populate('category', 'name slug').sort({ createdAt: -1 }).lean();
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (updates.banner !== undefined && mongoose.connection.db) {
        ...
        if (updates.serviceCapabilities !== undefined)
          fieldsToUpdate.serviceCapabilities = (store as any).serviceCapabilities;
        if (store.category) {
          // Ensure category is an ObjectId
          fieldsToUpdate.category =
            store.category instanceof mongoose.Types.ObjectId
              ? store.category
...
/**
 * @route   PATCH /api/merchant/stores/:id
 * @desc    Partially update store (e.g. promotions array)
 * @access  Private (Merchant)
 */
router.patch('/:id', validateParams(storeIdSchema), async (req: Request, res: Response) => {
  try {
    const merchantId = (req as any).merchantId;
    const storeId = req.params.id;
    const updates = req.body;
    ...
    // Only update explicitly allowed fields to prevent mass-assignment.

    if (updates.promotions !== undefined) {
      if (!Array.isArray(updates.promotions)) {
        return res.status(400).json({ success: false, message: 'promotions must be an array' });
      }
      // Validate each promotion entry
      for (const p of updates.promotions) {
        if (!p.title || typeof p.title !== 'string') {
          return res.status(400).json({ success: false, message: 'Each promotion must have a title' });
        }
        if (p.title.length > 100) {
          return res.status(400).json({ success: false, message: 'Promotion title must be 100 chars or less' });
        }
        if (p.subtitle && p.subtitle.length > 200) {
          return res.status(400).json({ success: false, message: 'Promotion subtitle must be 200 chars or less' });
        }
      }
      (store as any).promotions = updates.promotions.map((p: any) => ({
        title: p.title.trim(),
        subtitle: p.subtitle?.trim() || '',
        image: p.image || null,
        backgroundColor: p.backgroundColor || '#1e3a5f',
        actionText: p.actionText?.trim() || '',
        actionUrl: p.actionUrl?.trim() || '',
      }));
      store.markModified('promotions');
    }
    ...
    if (updates.gstSettings !== undefined) {
      (store as any).gstSettings = { ...(store as any).gstSettings, ...updates.gstSettings };
    }

    await store.save();

    // Invalidate web menu cache so next customer fetch shows updated promotions
    try {
      const redisService = require('../services/redisService').default || require('../services/redisService');
    ...
    logger.error('Patch store error:', error);
    return res.status(500).json({ success: false, message: error.message || 'Failed to update store' });
  }
});
 ...
 */
router.post(
  '/:id/request-mall-listing',
  validateParams(storeIdSchema),
  validateRequest(requestMallListingSchema),
  async (req: Request, res: Response) => {
    try {
...
router.get('/:id/mall-status', validateParams(storeIdSchema), async (req: Request, res: Response) => {
  try {
    ...
    const store = await Store.findOne({ _id: storeId, merchant: merchantId }).select('name deliveryCategories').lean();
    if (!store) {
      return sendNotFound(res, 'Store not found or unauthorized');
    }
    ...
    // Get latest request
    const latestRequest = await MallListingRequest.findOne({ storeId }).sort({ createdAt: -1 }).lean();

...
export const initSentry = (app: Express) => {
  ...
  try {
    Sentry.init({
      ...
      // Also stamps every event with the current correlation ID so failures
      // that span multiple microservices can be correlated in Sentry by
      // filtering on the `correlation_id` tag.
...
export const sentryTracingHandler = createSafeMiddleware(Sentry.Handlers.tracingHandler());
 ...
 *
 * OBS-6: exported so the worker-process Sentry.init() in server.ts can
 * share the same scrubber. Previously the worker init ran without any
 * beforeSend hook, so BullMQ job errors with OTP / phone / aadhaar in
 * the payload were uploaded to Sentry unscrubbed.
 */
export function sanitizeEvent(event: Sentry.Event, _hint: Sentry.EventHint): Sentry.Event | null {
  ...
  const sensitiveFields = [
    ...
    'ssn',
    'businessNumber',
    // OBS-6 additions — India-specific PII that D20 added to sanitizeLog.
    // Kept in sync so logger-redacted values don't leak through the
    // Sentry channel instead.
    'otp',
    'phoneNumber',
    'phone',
    'mobile',
 ...
 * OBS-6: exported for worker-process Sentry.init() reuse.
 */
export function sanitizeBreadcrumb(breadcrumb: Sentry.Breadcrumb): Sentry.Breadcrumb | null {
  ...
  if (sensitiveCategories.includes(breadcrumb.category || '')) {
    if (breadcrumb.data) {
      breadcrumb.data = sanitizeObject(breadcrumb.data, ['password', 'token', 'authorization', 'apiKey', 'secret']);
    }
  }

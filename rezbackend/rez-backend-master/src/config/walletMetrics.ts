...
// DP-D006: Persistence success/failure metrics — tracks whether critical
// wallet writes were majority-acknowledged or had to fall back / retry.
// Alerts fire when wallet_write_failures_total > 0 over a 5-minute window.
export const walletWriteTotal = new Counter({
  name: 'wallet_write_total',
  help: 'Total wallet DB write operations (credit/debit/update)',
  labelNames: ['operation', 'outcome'], // outcome: success | retry_success | failure
});

// Transaction commit retry counter — fires when a primary stepdown forces a
// commitTransaction() retry (UnknownTransactionCommitResult).
export const walletCommitRetryTotal = new Counter({
  name: 'wallet_commit_retry_total',
  help: 'Total wallet transaction commit retries due to UnknownTransactionCommitResult',
  labelNames: ['operation'],
});

// Stale cache detection counter — fires when a cached wallet entry is found to
// be stale via version-stamp comparison (DP-B001 fix).
export const walletCacheStaleTotal = new Counter({
  name: 'wallet_cache_stale_total',
  help: 'Total stale wallet cache entries detected and discarded via version stamp',
  labelNames: ['userId'],
});
 ...
 */
export async function timeWalletOperation<T>(operation: string, coinType: string, fn: () => Promise<T>): Promise<T> {

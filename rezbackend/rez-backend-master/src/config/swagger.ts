...
- Some endpoints also support cursor-based pagination via \`cursor\` parameter
      ...
      schemas: {
        ...
        Order: {
          ...
          properties: {
            ...
            status: {
              type: 'string',
              // Source of truth: src/models/Order.ts (IOrder.status schema enum)
              enum: [
                'placed',
                'confirmed',
                'preparing',
            ...
            paymentStatus: {
              type: 'string',
              // Source of truth: src/models/Order.ts (IOrderPayment.status schema enum)
              enum: [
                'pending',
                'awaiting_payment',
                'processing',
        ...
        AnalyticsSummary: {
          ...
          properties: {
            revenue: {
              type: 'object',
              properties: {
                total: { type: 'number', example: 12500.5 },
        ...
        WalletBalance: {
          type: 'object',
          properties: {
            totalValue: { type: 'number', example: 1250.5 },
            breakdown: {
              type: 'object',
              properties: {
                rezCoins: {
                ...
                cashbackBalance: { type: 'number', example: 150.5 },
            ...
            balance: {
              ...
              properties: {
                ...
                cashback: { type: 'number', example: 150.5 },
  ...
  apis: ['./src/controllers/*.ts', './src/routes/*.ts', './src/merchantroutes/*.ts', './src/models/*.ts'],
};

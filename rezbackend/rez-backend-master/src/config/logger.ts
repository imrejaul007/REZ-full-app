...
export const logger = winston.createLogger({
  ...
  exceptionHandlers: [
    // Console MUST be first so Render/cloud log aggregators capture the crash
...
export const sanitizeLog = (data: any): any => {
  ...
  const sensitiveFields = [
    ...
    'secretKey',
    'passphrase',
    // D20: additional India-specific PII that was logged in the clear previously
    'otp',
    'phoneNumber',
    'phone',
    'mobile',
...
// Correlation ID middleware — also emits W3C traceparent for distributed tracing.
// Wraps next() inside runWithCorrelation() so every async operation downstream
// (including serviceClient inter-service calls) automatically inherits the
// X-Correlation-ID from the originating HTTP request via AsyncLocalStorage.
export const correlationIdMiddleware = (req: Request, res: Response, next: NextFunction) => {
  const correlationId = (req.headers['x-correlation-id'] ||
    req.headers['x-request-id'] ||
    req.headers['x-trace-id'] ||
    `${Date.now()}-${crypto.randomUUID().replace('-', '').substring(0, 9)}`) as string;
  ...
  // W3C traceparent propagation — extract from incoming header or generate new trace ID
  const incoming = req.headers['traceparent'] as string | undefined;
  let traceId: string;
  if (incoming) {
    const parts = incoming.split('-');
    traceId = parts.length >= 2 && parts[1].length === 32 ? parts[1] : require('crypto').randomUUID().replace(/-/g, '');
  } else {
    traceId = require('crypto').randomUUID().replace(/-/g, '');
  }
  ...
  // Bind correlation context to the async execution chain so serviceClient
  // can forward X-Correlation-ID on all outbound inter-service HTTP calls
  // without requiring manual header passing through every function.

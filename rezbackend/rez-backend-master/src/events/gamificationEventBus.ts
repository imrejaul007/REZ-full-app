...
import { publishGamificationEvent } from './gamificationQueue';
...
export type ActivityEventType =
  ...
  | 'pos_bill_paid' // P2-C6: in-store POS bill payment — wires POS sales into streaks/challenges
  | 'deal_locked'
  | 'cashback_earned'
  // Fixed: Add visit events to connect store visits to streak system - Phase 0 / Phase 1.1
  | 'visit_completed'
  | 'visit_checked_in';
...
class GamificationEventBus {
  ...
  // Track wrapped listener references so we can remove them later and avoid leaks
  ...
  emit(
    ...
    setImmediate(() => {
      ...
      // Durability: also persist to BullMQ so the event survives a process crash.
      // publishGamificationEvent is fail-open — never throws.
      publishGamificationEvent(event).catch((err: any) => {
        logger.warn('[GamificationEventBus] Event publish failed — job may be lost', {
          eventType: event.type,
          userId: event.userId,
          eventId: event.eventId,
          error: err?.message,
        });
        // Do NOT throw — event publishing is non-blocking for the HTTP response.
      });
    });
  }
   ...
   * Each unique handler function is registered only once — calling onAll() with the
   * same handler reference again is a no-op, preventing listener accumulation.
   */
  onAll(handler: (event: ActivityEvent) => Promise<void> | void): void {
    const mapKey = `all::${handler.toString().slice(0, 64)}`;
    if (this.listenerMap.has(mapKey)) return; // already registered, skip

    const wrapped = async (event: ActivityEvent) => {
      try {
        await handler(event);
      } catch (error) {
    ...
    this.listenerMap.set(mapKey, wrapped);
    this.emitter.on('gamification_event', wrapped);
  }

  /**
   * Register a handler for a specific event type.
   * Each unique (type, handler) pair is registered only once — re-registering the
   * same handler is a no-op, preventing listener accumulation across hot-reloads or
   * repeated initialize() calls.
   */
  on(type: ActivityEventType, handler: (event: ActivityEvent) => Promise<void> | void): void {
    const mapKey = `${type}::${handler.toString().slice(0, 64)}`;
    if (this.listenerMap.has(mapKey)) return; // already registered, skip
    ...
    this.listenerMap.set(mapKey, wrapped);
    this.emitter.on(`event:${type}`, wrapped);
  }

  /**
   * Remove a previously registered handler.
   * Call during graceful shutdown or test teardown to release listener memory.
   */
  off(type: ActivityEventType | 'all', handler: (event: ActivityEvent) => Promise<void> | void): void {
    const mapKey = `${type === 'all' ? 'all' : type}::${handler.toString().slice(0, 64)}`;
    const wrapped = this.listenerMap.get(mapKey);
    if (!wrapped) return;
    const eventName = type === 'all' ? 'gamification_event' : `event:${type}`;
    this.emitter.off(eventName, wrapped);
    this.listenerMap.delete(mapKey);
  }
    ...
    try {
      ...
      this.on('review_submitted', async (event) => {
        try {
          ...
          const store = await Store.findById(storeId).select('rewardRules merchantId name').lean();
          if (!store) return;
          ...
          logger.info(
            `[REVIEW] Awarded ${bonusCoins} ${coinType} coins to user ${event.userId} for reviewing store ${storeId}`,
          );
        } catch (err) {
  ...
  private inferCategory(type: ActivityEventType): EventCategory {
    ...
    if (type.startsWith('visit')) return 'event'; // Fixed: visit events map to 'event' category - Phase 1.1
    return 'event';
  }
}

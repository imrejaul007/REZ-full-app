 ...
 */
priceAlertSchema.statics.hasActiveAlert = async function (
  userId: string,
  productId: string,
 ...
 */
priceAlertSchema.statics.checkAndTriggerAlerts = async function (
  productId: string,
    ...
    if (alert.shouldTrigger(newPrice)) {
      ...
      (async () => {
        try {
          ...
          const product = await (Product as any).findById(alert.productId).select('name').lean();
          ...
          if (alert.notificationMethod.includes('push')) {
            ...
            if (user?.phoneNumber) {
              await pushService.sendOrderUpdate(String(alert._id), user.phoneNumber, 'Price Drop Alert', message);
            }
          }

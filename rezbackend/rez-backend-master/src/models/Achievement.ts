...
// Indexes
// { user, type } covers achievementType query pattern (unique per user+achievementType)
UserAchievementSchema.index({ user: 1, type: 1 }, { unique: true });
// { user, achievement } covers achievementId query pattern (unique per user+achievementId ref)

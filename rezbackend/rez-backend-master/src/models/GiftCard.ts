...
export interface IUserGiftCard extends Document {
  ...
  deletedAt?: Date; // Soft-delete timestamp; set instead of hard-deleting
  createdAt: Date;
  updatedAt: Date;
}

// Simple encryption for gift card codes (use proper key management in production)
const _giftCardEnvKey = process.env.GIFT_CARD_ENCRYPTION_KEY;
if (!_giftCardEnvKey) {
  // Non-fatal: log an error in production but do not crash the process.
  // Without a stable key, gift card codes cannot be decrypted after restart —
  // add GIFT_CARD_ENCRYPTION_KEY to your Render environment variables ASAP.
  const msg =
    process.env.NODE_ENV === 'production'
      ? '[GiftCard] GIFT_CARD_ENCRYPTION_KEY is not set — gift card encrypt/decrypt will fail until the key is configured in Render environment variables'
      : '[GiftCard] GIFT_CARD_ENCRYPTION_KEY not set — using temporary key (dev only)';
  logger.error(msg);
}
const ENCRYPTION_KEY = _giftCardEnvKey ?? crypto.randomBytes(32).toString('hex').slice(0, 32);
const IV_LENGTH = 16;

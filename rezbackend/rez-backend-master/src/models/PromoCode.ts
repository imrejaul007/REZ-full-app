...
// Support descending sort by popularity (admin dashboards, analytics queries).
PromoCodeSchema.index({ usedCount: -1 });
// Compound index for the most common validation query: active codes that haven't expired.
...
// Instance method to increment usage — fully atomic with maxUses AND per-user guard.
//
// SECURITY FIX: Previously only the global maxUses cap was inside the atomic
// findOneAndUpdate filter. The per-user limit was checked earlier via canBeUsedBy()
// on a potentially stale in-memory document. Two concurrent requests could both pass
// canBeUsedBy(), then both proceed to incrementUsage(), each adding an entry to
// usedBy — effectively doubling the discount.
//
// Fix: embed the per-user check directly in the atomic filter using $expr so that
// the DB refuses the update if the user has already reached maxUsesPerUser.
PromoCodeSchema.methods.incrementUsage = async function (
  ...
  const result = await (this.constructor as any).findOneAndUpdate(
    {
      ...
      // SECURITY FIX: Per-user cap enforced atomically.
      // Count how many times this specific user appears in usedBy and ensure it
      // is strictly less than maxUsesPerUser before allowing the write.
      $expr: {
        $lt: [
          {
            $size: {
  ...
  if (!result) {
    // The update was rejected: either the global cap or the per-user cap was hit.
    // Throw a clear error so the caller (applyPromoCode) surfaces the right message.
    throw new Error('Promo code usage limit exceeded — global cap or per-user limit reached');
  }
};

// Instance method to calculate discount
PromoCodeSchema.methods.calculateDiscount = function (this: IPromoCode, originalPrice: number): number {
  if (this.discountType === 'percentage') {
...
PromoCodeSchema.statics.validateCode = async function (
  ...
  try {
    ...
    if (
      promoCode.applicableBillingCycles &&
...
PromoCodeSchema.statics.getActivePromoCodes = async function (
  ...
  const promoCodes = await this.find(query).sort({ discountValue: -1 }).limit(20);

...
export interface ICampaign extends Document {
  ...
  maxBenefit?: number;
  totalBenefitIssued?: number; // Running total of benefit issued; enforced against maxBenefit at redemption (MA-M1)
  icon?: string;
  bannerImage?: string;
  region?: 'bangalore' | 'dubai' | 'china' | 'all'; // Region restriction - 'all' means available everywhere
  exclusiveToProgramSlug?: 'student_zone' | 'corporate_perks' | 'rez_prive'; // If set, only visible to active program members
  targetSegment: 'all' | 'new_users' | 'lapsed_users' | 'high_value'; // Audience segment this campaign targets
  createdAt: Date;
  updatedAt: Date;
...
const CampaignSchema = new Schema<ICampaign>(
  {
    ...
    // Running total of benefit (coins) issued against this campaign.
    // Used with maxBenefit to enforce an atomic cap at redemption time (MA-M1).
    totalBenefitIssued: {
      type: Number,
      default: 0,
      min: 0,
    ...
    exclusiveToProgramSlug: {
      type: String,
      enum: ['student_zone', 'corporate_perks', 'rez_prive'],
      index: true,
      sparse: true,
    },
    targetSegment: {
      type: String,
      enum: ['all', 'new_users', 'lapsed_users', 'high_value'],
      default: 'all',
      index: true,
    },
  },
...
// KAVITA: Compound index for getActiveCampaigns() and getCampaignsByType() — prevents collection scan on time-range queries
CampaignSchema.index({ isActive: 1, startTime: -1, endTime: -1 }); // Sort order for chronological queries
CampaignSchema.index({ type: 1, isActive: 1, startTime: -1, endTime: -1 }); // Type-specific campaign queries

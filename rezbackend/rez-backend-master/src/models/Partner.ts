...
export const LEVEL_OFFERS = {
  PARTNER: [
    ...
    },
    {
      ...
      minPurchase: 500,
      maxDiscount: 300,
      termsAndConditions: ['Valid for 30 days from activation', 'Minimum purchase of ₹500', 'Maximum discount ₹300'],
    },
  ],
  INFLUENCER: [
    {
      ...
      minPurchase: 300,
      maxDiscount: 200,
      termsAndConditions: ['Valid for 30 days from activation', 'Minimum purchase of ₹300', 'Maximum discount ₹200'],
    },
    {
  ...
  AMBASSADOR: [
    ...
    },
    {
      ...
      minPurchase: 0,
      maxDiscount: 1000,
      termsAndConditions: ['Valid only during birthday month', 'No minimum purchase required', 'One-time use only'],
    },
  ],
};
...
PartnerSchema.methods.getOrdersNeededForNextLevel = function (): number {
  ...
  const nextLevelConfig = Object.values(PARTNER_LEVELS).find((level) => level.level === nextLevel);
...
PartnerSchema.methods.canUpgradeLevel = function (): boolean {
  ...
  const nextLevelConfig = Object.values(PARTNER_LEVELS).find((level: any) => level.level === nextLevel);
  ...
  logger.info(
    `📊 [LEVEL CHECK] User ${this.userId}: Orders ${this.ordersThisLevel}/${(nextLevelConfig as any).requirements.orders}, Days ${daysSinceStart}/${this.currentLevel.requirements.timeframe}, Can upgrade: ${hasEnoughOrders && withinTimeframe}`,
  );
...
PartnerSchema.methods.upgradeLevel = function (): void {
  ...
  // Add current level to history (explicitly copy fields — spread on Mongoose subdoc loses nested values)
  this.levelHistory.push({
    level: this.currentLevel.level,
    name: this.currentLevel.name,
    requirements: {
      orders: this.currentLevel.requirements.orders,
      timeframe: this.currentLevel.requirements.timeframe,
    },
    achievedAt: this.currentLevel.achievedAt || new Date(),
  });
...
PartnerSchema.methods.handleLevelExpiry = function (): void {
  ...
  if (nextLevelConfig && this.ordersThisLevel >= (nextLevelConfig as any).requirements.orders) {
    ...
    logger.info(
      `🔄 [LEVEL EXPIRY] Progress reset (lost ${ordersLost} orders), new deadline: ${this.validUntil.toISOString().split('T')[0]}`,
    );
  }
};

...
MallPurchaseSchema.statics.getReadyForCredit = async function (batchSize: number = 200): Promise<IMallPurchase[]> {
  const now = new Date();
  ...
  return this.find({
    ...
    $expr: {
      $lte: [{ $add: ['$purchasedAt', { $multiply: ['$verificationDays', 86400000] }] }, now],
    },
  }).limit(batchSize);
};
...
MallPurchaseSchema.statics.getUserPurchases = async function (
  ...
  const [purchases, total] = await Promise.all([
    this.find(query).sort({ purchasedAt: -1 }).skip(skip).limit(limit).populate('brand', 'name slug logo').lean(),
...
MallPurchaseSchema.statics.getUserCashbackSummary = async function (userId: Types.ObjectId): Promise<{
  totalEarned: number;
  pending: number;
  credited: number;
  rejected: number;
...
MallPurchaseSchema.statics.getBrandPurchaseAnalytics = async function (
  ...
  result.averageOrderValue = result.totalPurchases > 0 ? result.totalAmount / result.totalPurchases : 0;
...
export const MallPurchase = mongoose.model<IMallPurchase>('MallPurchase', MallPurchaseSchema);

...
export interface IUserLockDeal extends Document {
  ...
  // Deposit payment
  depositPaymentId?: string; // Generic: holds Razorpay payment ID
  depositRazorpayOrderId?: string; // Razorpay order ID for deposit
  depositStripePaymentIntentId?: string; // Legacy Stripe field (kept for backward compat)
  depositPaidAt?: Date;
  depositAmount: number;

  // Balance payment
  balancePaymentId?: string; // Generic: holds Razorpay payment ID
  balanceRazorpayOrderId?: string; // Razorpay order ID for balance
  balanceStripePaymentIntentId?: string; // Legacy Stripe field (kept for backward compat)
  balancePaidAt?: Date;
...
const UserLockDealSchema = new Schema<IUserLockDeal>(
  {
    ...
    // Deposit
    depositPaymentId: { type: String, trim: true }, // Razorpay payment ID
    depositRazorpayOrderId: { type: String, trim: true }, // Razorpay order ID
    depositStripePaymentIntentId: { type: String, trim: true }, // Legacy
    depositPaidAt: { type: Date },
    depositAmount: { type: Number, required: true, min: 0 },

    // Balance
    balancePaymentId: { type: String, trim: true }, // Razorpay payment ID
    balanceRazorpayOrderId: { type: String, trim: true }, // Razorpay order ID
    balanceStripePaymentIntentId: { type: String, trim: true }, // Legacy
...
UserLockDealSchema.statics.getUserActiveLocks = async function (
  userId: string,
  status?: string,
): Promise<IUserLockDeal[]> {
...
export const UserLockDeal = mongoose.model<IUserLockDeal, IUserLockDealModel>('UserLockDeal', UserLockDealSchema);

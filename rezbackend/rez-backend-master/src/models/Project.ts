...
ProjectSchema.methods.canUserParticipate = async function (userId: string): Promise<boolean> {
  ...
  const userSubmissions = this.submissions.filter((sub: IProjectSubmission) => sub.user.toString() === userId);
  ...
  if (this.requirements.demographics) {
    ...
    // Gender check
    if (
      this.requirements.demographics.gender &&
...
ProjectSchema.methods.getUserSubmission = function (userId: string): IProjectSubmission | null {
  const submission = this.submissions.find((sub: IProjectSubmission) => sub.user.toString() === userId);
  return submission || null;
};

// Method to submit work
ProjectSchema.methods.submitWork = async function (userId: string, content: any): Promise<IProjectSubmission> {
...
ProjectSchema.methods.reviewSubmission = async function (
  ...
  const submission = this.submissions.find((sub: IProjectSubmission) => sub._id?.toString() === submissionId);
  ...
  if (submission.user) {
    try {
      await redisService.delPattern(`earnings:consolidated:${submission.user.toString()}:*`);
    } catch (_e) {
      logger.warn('[PROJECT] Failed to invalidate earnings cache on submission review', {
...
ProjectSchema.methods.updateAnalytics = async function (): Promise<void> {
  ...
  if (totalSubmissions > 0) {
    ...
    const qualityScores = this.submissions.filter((s: any) => s.qualityScore).map((s: any) => s.qualityScore!);
...
ProjectSchema.statics.getActiveProjects = function (filters: any = {}, limit: number = 50) {
  const query: any = {
    status: 'active',
    $and: [
      {
        $or: [{ 'limits.expiryDate': { $exists: false } }, { 'limits.expiryDate': { $gte: new Date() } }],
      },
      {
        $or: [{ 'limits.startDate': { $exists: false } }, { 'limits.startDate': { $lte: new Date() } }],
      },
    ],
  };

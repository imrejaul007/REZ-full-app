...
export interface IEventBooking extends Document {
  ...
  bookingDate: Date;
  status: 'pending' | 'confirmed' | 'cancelled' | 'completed' | 'refunded' | 'in_progress' | 'no_show' | 'expired';
...
const EventBookingSchema = new Schema<IEventBooking>(
  {
    ...
    status: {
      type: String,
      enum: ['pending', 'confirmed', 'cancelled', 'completed', 'refunded', 'in_progress', 'no_show', 'expired'],
      default: 'pending',
    },
    paymentId: {
      type: Schema.Types.ObjectId,
    ...
    eventDayReminderSent: { type: Boolean, default: false },
  },
  {
    timestamps: true,

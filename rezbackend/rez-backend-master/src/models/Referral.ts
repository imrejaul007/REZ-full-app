...
// Enforce one referral per referee — prevents duplicate referral relationships
// under concurrent signups using the same code.
ReferralSchema.index({ referee: 1 }, { unique: true, sparse: true });

// SECURITY: Deduplication index on referrer + code to prevent multiple active referrals from same referrer
ReferralSchema.index(
  { referrer: 1, referralCode: 1, status: 1 },
  {
    sparse: true,
    partialFilterExpression: {
      status: { $in: ['pending', 'registered', 'active', 'qualified'] },
    },
    name: 'referrer_code_dedup_idx',
  },
);

// SECURITY: Device-based referral farming prevention — tracks referrals by device ID
ReferralSchema.index(
  { 'metadata.deviceId': 1, createdAt: -1 },
  {
    ...
    name: 'device_referral_idx',
  },
);

// SECURITY: IP-based referral farming prevention
ReferralSchema.index(
...
// KAVITA: compound index for referral program reporting and analytics — prevents collection scan on status-based queries

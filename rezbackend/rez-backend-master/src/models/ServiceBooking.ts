...
const ServiceBookingSchema = new Schema<IServiceBooking>(
  {
    ...
    noShowFeeCharged: { type: Boolean, default: false },
    noShowFeeAmount: { type: Number, default: 0 },
...
ServiceBookingSchema.statics.generateBookingNumber = async function (prefix: string = 'SB'): Promise<string> {
  ...
  const random = crypto.randomUUID().replace('-', '').substring(0, 4).toString().padStart(4, '0');
...
ServiceBookingSchema.statics.checkSlotAvailability = async function (
  ...
  const query: any = {
    store: storeId,
    service: serviceId, // Scope to specific service — different services in same store should not block each other
...
ServiceBookingSchema.methods.confirm = async function (assignedStaff?: string): Promise<IServiceBooking> {
...
ServiceBookingSchema.methods.complete = async function (): Promise<IServiceBooking> {
  // M18 FIX: Enforce allowed pre-completion states so merchants cannot bypass the
  // state machine by jumping directly from 'pending' to 'completed'.
  const allowedStatuses = ['confirmed', 'assigned', 'in_progress'];
  if (!allowedStatuses.includes(this.status)) {
    throw new Error(
      `Cannot complete booking with status: ${this.status}. Booking must be in one of: ${allowedStatuses.join(', ')}`,
    );
  }
  this.status = 'completed';
  this.completedAt = new Date();

  // Only default to cash/paid when no payment was taken at all — preserve partial/deposit payments
  if (!this.requiresPaymentUpfront && this.paymentStatus === 'pending') {
    this.paymentStatus = 'paid';
    this.paymentMethod = this.paymentMethod || 'cash'; // preserve existing method (e.g. wallet)
  }
...
ServiceBookingSchema.methods.reschedule = async function (
  ...
  // 'rescheduled' is not a top-level status enum value — log against current status
  this.statusHistory.push({
    status: this.status,
    timestamp: new Date(),
...
ServiceBookingSchema.methods.addRating = async function (score: number, review?: string): Promise<IServiceBooking> {
...
export const ServiceBooking = mongoose.model<IServiceBooking, IServiceBookingModel>(
  'ServiceBooking',

...
export interface ITableBooking extends Document {
  ...
  status: 'pending' | 'confirmed' | 'completed' | 'cancelled' | 'no_show' | 'in_progress' | 'refunded' | 'expired';
  ...
  // Methods
  updateStatus(
    newStatus: 'pending' | 'confirmed' | 'completed' | 'cancelled' | 'no_show' | 'in_progress' | 'refunded' | 'expired',
  ): Promise<void>;
}
...
const TableBookingSchema = new Schema<ITableBooking>(
  {
    ...
    status: {
      type: String,
      enum: ['pending', 'confirmed', 'completed', 'cancelled', 'no_show', 'in_progress', 'refunded', 'expired'],
      default: 'pending',
      index: true,
    },
    cancellationReason: {
...
TableBookingSchema.statics.findStoreBookings = function (storeId: Types.ObjectId | string, date?: Date) {
...
TableBookingSchema.statics.markNoShows = async function (
  ...
  // Get candidates
  const candidates = await this.find(query).select('_id bookingDate bookingTime').lean();
  ...
  const result = await this.updateMany({ _id: { $in: expiredIds } }, { $set: { status: 'no_show' } });
...
export const TableBooking = mongoose.model<ITableBooking, ITableBookingModel>('TableBooking', TableBookingSchema);

...
// MenuItem variant — e.g. Half / Full, Small / Medium / Large
export interface IMenuItemVariant {
  name: string; // "Half", "Full", "Small", "Regular", "Large"
  price: number; // Absolute price for this variant (overrides base price)
  isDefault?: boolean;
}

// MenuItem modifier (add-on) — e.g. Extra Cheese +₹30, Add Paneer +₹50
export interface IMenuItemModifier {
  name: string; // "Extra Cheese", "Add Paneer", "Extra Sauce"
  price: number; // Additional price
  isVeg?: boolean; // Vegetarian indicator for the add-on
  max?: number; // Max selectable quantity of this modifier (default 1)
}
...
export interface IMenuItem {
  ...
  allergens?: string[];
  tags?: string[];
  variants?: IMenuItemVariant[]; // Size/portion variants (pick one)
  modifiers?: IMenuItemModifier[]; // Add-ons / extras (pick multiple)
  is86d?: boolean; // Temporarily 86'd / out of stock
  cashbackPercentage?: number; // Item-level cashback override (%)
}
...
const MenuItemSchema = new Schema<IMenuItem>(
  {
    ...
    modifiers: [MenuItemModifierSchema],
    is86d: { type: Boolean, default: false },

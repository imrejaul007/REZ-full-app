...
DealRedemptionSchema.index({ redemptionCode: 1 }, { unique: true });
DealRedemptionSchema.index({ expiresAt: 1, status: 1 }); // For expiry cleanup job
DealRedemptionSchema.index({ status: 1, expiresAt: 1 }); // Selective expiry: equality on status first, then range

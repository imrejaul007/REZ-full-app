...
SurveySessionSchema.methods.submitAnswer = async function (questionId: string, answer: any): Promise<ISurveySession> {
  if (this.status !== 'in_progress') {
    throw new Error('Survey session is not in progress');
  }

  // Check if already answered
  const existingIndex = this.answers.findIndex((a: ISurveyAnswer) => a.questionId === questionId);
...
SurveySessionSchema.statics.getUserStats = async function (userId: string) {
  ...
  const totalEarned = completedSessions.reduce((sum: number, session: ISurveySession) => sum + session.coinsEarned, 0);

  const totalTimeSpent = completedSessions.reduce((sum: number, session: ISurveySession) => sum + session.timeSpent, 0);
  ...
  const averageTime =
    surveysCompleted > 0
      ...
      : 0;
  const completionRate = totalSessions > 0 ? Math.round((surveysCompleted / totalSessions) * 100) : 0;
...
SurveySessionSchema.statics.getUserHistory = async function (userId: string, limit: number = 20) {
...
export const SurveySession = mongoose.model<ISurveySession, ISurveySessionModel>('SurveySession', SurveySessionSchema);

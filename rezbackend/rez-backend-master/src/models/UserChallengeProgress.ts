...
// SECURITY: Daily cap enforcement — track claims per calendar day to prevent same-day re-completion
UserChallengeProgressSchema.index(
  { user: 1, challenge: 1, claimedAt: 1 },
  {
    sparse: true,
    ...
    name: 'challenge_daily_claim_idx',
  },
);
...
UserChallengeProgressSchema.methods.addProgress = async function (amount: number, source?: string) {
  ...
  if (this.progress >= this.target && !this.completed) {
    ...
    await mongoose.model('Challenge').findByIdAndUpdate(this.challenge, { $inc: { completionCount: 1 } });
  }

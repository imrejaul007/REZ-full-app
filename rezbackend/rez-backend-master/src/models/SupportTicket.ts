...
// SLA monitor job: { status: $in, slaDeadline: $lte, slaBreached: false }
SupportTicketSchema.index({ status: 1, slaDeadline: 1, slaBreached: 1 });
...
SupportTicketSchema.methods.markMessagesAsRead = async function (userType: 'user' | 'agent') {
...
SupportTicketSchema.methods.resolveTicket = async function (resolution: string, resolvedBy: Types.ObjectId) {
...
SupportTicketSchema.methods.reopenTicket = async function (userId: Types.ObjectId, reason: string) {
  this.status = 'open';
...
SupportTicketSchema.methods.rateTicket = async function (score: number, comment: string) {
  this.rating = {
    score,
    comment,
    ratedAt: new Date(),
...
SupportTicketSchema.statics.getUserActiveTickets = async function (userId: Types.ObjectId) {
  return this.find({
    user: userId,
...
SupportTicketSchema.statics.getTicketsByStatus = async function (userId: Types.ObjectId, status: string) {
  return this.find({
    user: userId,
    status,
  })
...
// Static method to auto-assign ticket (least-loaded round-robin)
SupportTicketSchema.statics.autoAssignTicket = async function (ticketId: Types.ObjectId): Promise<void> {
  try {
    // Lazy require to avoid circular dependency (User model imports nothing from here)
    const { User } = require('./User');

    // Find all active support-tier agents (role: 'support') and admins
    ...
    if (agents.length === 0) {
      logger.info(`[SUPPORT_TICKET] No active agents for auto-assignment of ticket ${ticketId}`);
      return;
    }
    ...
    // Assign to the agent with the fewest open tickets (stable sort: preserves order on tie)
    agentLoads.sort((a, b) => a.count - b.count);
    const { agent: selectedAgent } = agentLoads[0];
    const agentId = selectedAgent._id;

    // Atomic CAS: only assign if still unassigned to avoid overwriting manual assignment
    ...
    if (updated) {
      const agentName =
        `${selectedAgent.profile?.firstName || ''} ${selectedAgent.profile?.lastName || ''}`.trim() || 'Support Agent';
      logger.info(`[SUPPORT_TICKET] Auto-assigned ticket ${ticketId} to ${agentName} (${agentId})`);

      // Emit socket event via supportSocketService (lazy require — avoids circular dep)
      try {
        const supportSocketService = require('../services/supportSocketService').default;
        ...
        logger.warn('[SUPPORT_TICKET] Socket emit failed during auto-assign (non-fatal)', socketErr);
      }
    }
  } catch (error) {
    logger.error(`[SUPPORT_TICKET] Auto-assign failed for ticket ${ticketId}:`, error);
  }
};

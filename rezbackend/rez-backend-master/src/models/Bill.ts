...
export interface IBill extends Document {
  ...
  autoApproved?: boolean; // True when approved automatically by OCR confidence threshold
  isActive: boolean;
  createdAt: Date;
  updatedAt: Date;
...
BillSchema.post('save', async function (doc) {
  ...
  if (
    ...
    doc.cashbackStatus === 'pending' &&
    doc.cashbackAmount &&
    doc.cashbackAmount > 0
  ) {
    try {
      // FR-D002 FIX: creditBillCashback never existed as a named export on walletService.
      // The correct API is walletService.credit(). Calling a non-existent export caused a
      // silent TypeError at runtime — cashback was silently swallowed and never reached the wallet.
      const { walletService } = require('../services/walletService');
      await walletService.credit({
        userId: doc.user.toString(),
        amount: doc.cashbackAmount,
        source: 'cashback',
        description: `Bill cashback (${doc.cashbackPercentage ?? 0}% on ₹${doc.amount})`,
        operationType: 'cashback_credit',
        referenceId: `bill:${(doc as any)._id}`,
        referenceModel: 'Bill',
        metadata: {
          billId: String((doc as any)._id),
          merchantId: doc.merchant.toString(),
          billAmount: doc.amount,
          cashbackPercentage: doc.cashbackPercentage,
        },
      });

      // Mark cashback as credited after successful wallet credit
      await (doc.constructor as any).findByIdAndUpdate(doc._id, {
        $set: {
      ...
      // Send to Mind for consumer insights
      try {
        const { sendBillUploadEventToMind } = require('../services/mindIntegration');
        sendBillUploadEventToMind({
          userId: doc.user.toString(),
          billId: String((doc as any)._id),
          merchantId: doc.merchant.toString(),
          merchantName: doc.merchantName || '',
          amount: doc.amount,
          cashbackEarned: doc.cashbackAmount,
        }).catch((err: Error) => logger.error('[Bill] Mind send failed', { err }));
      } catch (mindErr) {
        // Fire-and-forget
        logger.warn('[Bill] Mind integration not available', { error: mindErr });
      }

      // Award karma points for bill upload
      try {
        const { recordKarmaCheckIn } = require('../services/karmaIntegration');
        recordKarmaCheckIn(doc.user.toString(), `bill:${(doc as any)._id}`, 'qr').catch((err: Error) =>
          logger.error('[Bill] Karma award failed', { err }),
        );
      } catch (karmaErr) {
        // Fire-and-forget
        logger.warn('[Bill] Karma integration not available', { error: karmaErr });
      }
    } catch (error) {
      logger.error('Error crediting cashback:', error);
      // Mark as failed so admin/job can retry
      try {
        await (doc.constructor as any).findByIdAndUpdate(doc._id, {
          $set: { cashbackStatus: 'failed' },
        });
      } catch (updateErr) {
        logger.error('Error marking cashback as failed:', updateErr);
      }
    }
  }
});
...
BillSchema.statics.findDuplicates = function (

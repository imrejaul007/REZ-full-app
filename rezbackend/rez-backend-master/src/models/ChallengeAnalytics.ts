...
export type ChallengeAnalyticsEvent = 'impression' | 'join' | 'progress_update' | 'completion' | 'claim' | 'drop_off';
...
challengeAnalyticsSchema.statics.getChallengeAnalytics = async function (startDate?: Date, endDate?: Date) {
  const match: any = {};
  if (startDate || endDate) {
    match.timestamp = {};
    if (startDate) match.timestamp.$gte = startDate;

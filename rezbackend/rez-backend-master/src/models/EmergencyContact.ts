...
EmergencyBookingSchema.pre('save', function (next) {
  if (!this.bookingNumber) {
    ...
    const random = crypto.randomUUID().replace('-', '').substring(0, 4).toUpperCase();
...
EmergencyContactSchema.statics.getByType = function (type: string) {
  return this.find({ type, isActive: true }).sort({ priority: 1, name: 1 });
};

// Static method: Get national emergency contacts
EmergencyContactSchema.statics.getNationalContacts = function () {
  return this.find({ isNational: true, isActive: true }).sort({ priority: 1, name: 1 });
};

// Static method: Get contacts by city
EmergencyContactSchema.statics.getByCity = function (city: string) {
  return this.find({
    $or: [{ city: new RegExp(city, 'i') }, { isNational: true }],
    isActive: true,
  }).sort({ priority: 1, name: 1 });
};

// Static method: Get nearby contacts
EmergencyContactSchema.statics.getNearby = function (latitude: number, longitude: number, maxDistanceKm: number = 50) {
  const _maxDistanceMeters = maxDistanceKm * 1000;

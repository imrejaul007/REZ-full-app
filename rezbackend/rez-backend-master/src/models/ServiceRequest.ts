...
ServiceRequestSchema.statics.getActiveRequests = async function (userId: Types.ObjectId): Promise<IServiceRequest[]> {
  return this.find({
    user: userId,
...
ServiceRequestSchema.statics.getOverdueRequests = async function (userId?: Types.ObjectId): Promise<IServiceRequest[]> {
  const query: any = {
...
ServiceRequestSchema.methods.cancelService = async function (reason: string): Promise<IServiceRequest> {

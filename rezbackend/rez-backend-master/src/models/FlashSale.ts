...
FlashSaleSchema.index({ status: 1, enabled: 1 });
FlashSaleSchema.index({ priority: -1 });
FlashSaleSchema.index({ products: 1 });
FlashSaleSchema.index({ category: 1 });
// Flash sale notification job: { enabled, status:$in, startTime:$lte, endTime:$gte, notifyOnStart:true }
FlashSaleSchema.index({ status: 1, enabled: 1, startTime: 1, endTime: 1 });
...
FlashSaleSchema.statics.getLowStock = function (threshold: number = 20) {
  ...
  return this.find({
    ...
    status: { $nin: ['ended', 'sold_out'] },
    $expr: {
      $gte: [{ $divide: ['$soldQuantity', '$maxQuantity'] }, { $divide: [threshold, 100] }],
    },
  }).sort({ priority: -1 });
};

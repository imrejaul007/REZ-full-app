 ...
 */
ProcessedWebhookEventSchema.statics.isEventProcessed = async function (eventId: string): Promise<boolean> {
...
ProcessedWebhookEventSchema.statics.getSubscriptionEventHistory = async function (
  ...
  return await this.find({ subscriptionId }).sort({ processedAt: -1 }).limit(limit);
};
...
ProcessedWebhookEventSchema.statics.recordEvent = async function (
  ...
  try {
    ...
    if (error.code === 11000) {
      logger.warn(`[WEBHOOK] Event already recorded: ${eventId}`, { error: error.message });
 ...
 */
ProcessedWebhookEventSchema.statics.markEventFailed = async function (eventId: string, errorMessage: string) {
 ...
 */
ProcessedWebhookEventSchema.statics.getFailedEvents = async function (hoursAgo: number = 24) {
 ...
 */
ProcessedWebhookEventSchema.statics.getEventStats = async function (hoursAgo: number = 24) {
  const cutoffDate = new Date(Date.now() - hoursAgo * 60 * 60 * 1000);

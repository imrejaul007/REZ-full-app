...
export class ProductModel {
  static async findByMerchantId(merchantId: string | Types.ObjectId, storeId?: string | Types.ObjectId): Promise<IProduct[]> {
    ...
    return await (MProduct as any).find(query).sort({ createdAt: -1 });
  }
  ...
  static async findLowStock(merchantId: string | Types.ObjectId): Promise<IProduct[]> {
    const merchantObjectId = typeof merchantId === 'string' ? new Types.ObjectId(merchantId) : merchantId;
    return await (MProduct as any).find({
  ...
  static async search(params: {
    ...
    const [products, totalCount] = await Promise.all([
      (MProduct as any).find(query).sort(sortQuery).skip(skip).limit(limit),
      (MProduct as any).countDocuments(query)
    ]);
  ...
  static async createSampleProducts(merchantId: string) {
    ...
    for (const productData of sampleProducts) {
      const existingProduct = await (MProduct as any).findOne({
        merchantId,
        sku: productData.sku
      });

      if (!existingProduct) {
        await (MProduct as any).create(productData);
      }
    }
  }

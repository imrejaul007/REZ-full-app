...
// Static methods
DailyCheckInSchema.statics.getLastCheckIn = async function (userId: Types.ObjectId): Promise<IDailyCheckIn | null> {
  return this.findOne({ userId }).sort({ date: -1 }).limit(1);
};

DailyCheckInSchema.statics.hasCheckedInToday = async function (userId: Types.ObjectId): Promise<boolean> {
  const today = getStartOfDayUTC();
  const checkIn = await this.findOne({ userId, date: today });
  return !!checkIn;
};

DailyCheckInSchema.statics.getCurrentStreak = async function (userId: Types.ObjectId): Promise<number> {
  const today = getStartOfDayUTC();
...
DailyCheckInSchema.statics.getWeeklyEarnings = async function (userId: Types.ObjectId): Promise<number> {
...
const DailyCheckIn = mongoose.model<IDailyCheckIn, IDailyCheckInModel>('DailyCheckIn', DailyCheckInSchema);

export default DailyCheckIn;

// Export streak bonus calculation helper
export const calculateStreakBonus = (streak: number): number => {
  const _baseReward = 10; // ReZ coins

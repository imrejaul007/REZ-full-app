...
// KAVITA: compound index for abandoned cart cleanup and reactivation queries — prevents collection scan on stale cart sweeps
CartSchema.index({ isActive: 1, updatedAt: -1 });

// TTL: auto-delete expired carts
CartSchema.index({ expiresAt: 1 }, { expireAfterSeconds: 0 });
// Reservation cleanup job: { reservedItems.expiresAt: $lt }
CartSchema.index({ 'reservedItems.expiresAt': 1 });
...
CartSchema.virtual('storeCount').get(function () {
  const uniqueStores = new Set(
    this.items.filter((item: ICartItem) => item.store != null).map((item: ICartItem) => item.store!.toString()),
  );
...
CartSchema.methods.addItem = async function (productId: string, quantity: number = 1, variant?: any): Promise<void> {
  ...
  const existingItemIndex = this.items.findIndex((item: ICartItem) => {
    ...
    const productId_ = (item.product as any)._id ? (item.product as any)._id.toString() : item.product.toString();
    const productMatch = productId_ === productId;
...
CartSchema.methods.removeItem = async function (productId: string, variant?: any): Promise<void> {
...
CartSchema.methods.updateItemQuantity = async function (
  ...
  logger.info(
    '🛒 [UPDATE ITEM QTY] Cart has items:',
...
CartSchema.methods.calculateTotals = async function (): Promise<void> {
  ...
  this.items.forEach((item: ICartItem) => {
    ...
    const _regularQty = item.quantity - lockedQty;
...
CartSchema.methods.lockItem = async function (
  ...
  const existingLockIndex = this.lockedItems.findIndex(
    (item: any) =>
  ...
  if (existingLockIndex > -1) {
    ...
    const storeId = typeof product.store === 'object' && product.store?._id ? product.store._id : product.store || null;
...
// Unlock item
CartSchema.methods.unlockItem = async function (productId: string, variant?: any): Promise<void> {
...
CartSchema.methods.moveLockedToCart = async function (productId: string, variant?: any): Promise<void> {
  ...
  if (lockedItem.isPaidLock && lockedItem.lockFee) {
    ...
    if (existingItemIndex > -1) {
      ...
      this.items[existingItemIndex].lockedQuantity =
        (this.items[existingItemIndex].lockedQuantity || 0) + lockedItem.quantity;

...
UserOfferInteractionSchema.statics.getUserInteractions = function (
  ...
  return this.find(query).populate('offer', 'title image cashbackPercentage category').sort({ timestamp: -1 });
};
...
UserOfferInteractionSchema.statics.getOfferInteractions = function (
  ...
  return this.find(query).populate('user', 'name email').sort({ timestamp: -1 });
};

UserOfferInteractionSchema.statics.getInteractionStats = async function (offerId: Types.ObjectId): Promise<any> {
...
UserOfferInteractionSchema.statics.getUserEngagementStats = async function (userId: Types.ObjectId): Promise<any> {
  const stats = await this.aggregate([
    { $match: { user: userId } },
    {
      $group: {
...
const UserOfferInteraction = mongoose.model<IUserOfferInteraction, IUserOfferInteractionModel>(

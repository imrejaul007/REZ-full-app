...
// Unique partial index: prevents a race condition where two concurrent requests
// could both create an 'active' subscription for the same user+tier.
// The partialFilterExpression ensures the uniqueness constraint only fires for
// active subscriptions — cancelled/expired records are excluded, allowing
// a user to re-subscribe to the same tier after their previous one lapses.
SubscriptionSchema.index(
  { user: 1, tier: 1 },
  {
    unique: true,
    partialFilterExpression: { status: 'active' },
    name: 'unique_active_subscription_per_user_tier',
  },
);

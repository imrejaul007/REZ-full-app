...
const CashbackSchema = new Schema(
  {
    ...
    // NOTE: callers (e.g. cashbackService) must pass valid ObjectId strings for these fields.
    // Mongoose auto-casts string values to ObjectId, so string-based query helpers
    // (findByMerchantId, search, getAnalytics) continue to work without changes.
    merchantId: { type: Schema.Types.ObjectId, ref: 'Merchant', required: true, index: true },
    customerId: { type: Schema.Types.ObjectId, ref: 'User', required: true, index: true },
    orderId: { type: Schema.Types.ObjectId, ref: 'Order', required: true },
    // H7: Bridge field linking a CashbackRequest to the corresponding UserCashback document.
    // Populated by the reconcileCashbacks script (src/scripts/reconcileCashbacks.ts).
    merchantCashbackId: { type: Schema.Types.ObjectId, ref: 'UserCashback', default: null, index: true },
    customer: {
      id: String,
      name: String,
      email: String,
...
export class CashbackModel {
  static generateRequestNumber(): string {
    ...
    // Format: CB-YYYYMMDD-NNNNNN  (matches /^CB-\d{8}-\d{6}$/)
    // Use random 6-digit number to guarantee uniqueness even within the same millisecond
  ...
  static async create(
    requestData: Omit<CashbackRequest, 'id' | 'requestNumber' | 'createdAt' | 'updatedAt' | 'expiresAt'>,
  ...
  static async search(params: {
    ...
    const [requests, totalCount] = await Promise.all([
      CashbackMongoModel.find(query).sort(sortOptions).skip(skip).limit(limit).exec(),
  ...
  static async getMetrics(merchantId: string): Promise<CashbackMetrics> {
    ...
    const autoApprovedToday = requests.filter(
      (req) => req.status === 'approved' && req.reviewedAt && req.reviewedAt >= today && !req.flaggedForReview,
    ).length;
    ...
    const avgOrderValue =
      paidRequests.length > 0
        ? paidRequests.reduce((sum, req) => sum + ((req as any).orderAmount || req.requestedAmount || 0), 0) /
          paidRequests.length
        : 0;
    const estimatedRevenue = avgOrderValue * paidRequests.length * 1.5; // Assume 1.5x multiplier from repeat customers
    const cashbackROI =
      totalCashbackPaid > 0 ? Math.round(((estimatedRevenue - totalCashbackPaid) / totalCashbackPaid) * 100) : 0;
    ...
    const repeatCustomerRate = uniqueCustomers.size > 0 ? paidRequests.length / uniqueCustomers.size : 0;
  ...
  static assessRisk(
    ...
    if (requestData.customer?.accountAge !== undefined && requestData.customer.accountAge < 7) {
      riskFactors.push({
        type: 'account',
        severity: 'high',
    ...
    if (requestData.customer?.verificationStatus && requestData.customer.verificationStatus !== 'verified') {
      riskFactors.push({
        type: 'account',
        severity: 'medium',
    ...
    const weights: Record<'low' | 'medium' | 'high', number> = { low: 10, medium: 25, high: 60 };
    const riskScore = Math.min(
      riskFactors.reduce((score, factor) => score + weights[factor.severity], 0),
      100,
    );

    const flaggedForReview = riskScore >= 70 || riskFactors.some((factor) => factor.severity === 'high');
  ...
  static async bulkApprove(
    ...
    notes?: string,
    reviewedBy?: string,
  ): Promise<Array<{ success: boolean; requestId: string; message?: string }>> {
  ...
  static async getAnalytics(merchantId: string, dateRange?: { start: Date; end: Date }): Promise<CashbackAnalytics> {
    ...
    const _approvedAmount = approvedRequests.reduce((sum, req) => sum + (req.approvedAmount || req.requestedAmount), 0);
    const pendingRequests = requests.filter((req) => req.status === 'pending').length;
    const _rejectedRequests = requests.filter((req) => req.status === 'rejected').length;
    const paidRequests = requests.filter((req) => req.status === 'paid').length;
    const paidAmount = requests
      .filter((req) => req.status === 'paid')
      .reduce((sum, req) => sum + (req.approvedAmount || req.requestedAmount), 0);

    const _averageRequestAmount = totalRequests > 0 ? totalAmount / totalRequests : 0;
    ...
    const _topCustomers = Array.from(customerCashback.entries())
      .map(([customerId, data]) => ({
        customerId,
    ...
    const fraudDetectionRate =
      totalRequests > 0 ? (requests.filter((req) => req.flaggedForReview).length / totalRequests) * 100 : 0;
    ...
    const totalOrderRevenue = paidRequestsForRetention.reduce(
      (sum: number, req: any) => sum + ((req as any).orderAmount || req.requestedAmount || 0),
      0,
    );
  ...
  static async createSampleRequests(merchantId: string) {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const randomPriority = priorities[randomInt(0, priorities.length - 1)];

      const requestedAmount = randomInt(5, 55);
      const cashbackRate = 5 + randomInt(0, 5);

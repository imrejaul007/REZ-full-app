...
export type BonusCampaignStatus = 'draft' | 'scheduled' | 'active' | 'paused' | 'exhausted' | 'expired' | 'cancelled';
...
// KAVITA: Compound index for getActiveCampaigns() and getCampaignsByType() — compound sort order critical
BonusCampaignSchema.index({ status: 1, startTime: -1, endTime: -1 }); // Bonus: reverse chrono ordering
...
const BonusCampaign = mongoose.model<IBonusCampaign, IBonusCampaignModel>('BonusCampaign', BonusCampaignSchema);

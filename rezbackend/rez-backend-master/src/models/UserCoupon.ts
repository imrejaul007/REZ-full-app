...
// PERF-B10: 3-field compound used by couponService.countDocuments({ user, coupon, status })
// on every checkout. Unique index above can serve {user,coupon} but the status
// filter then scans matching rows; adding it to the key lets Mongo answer
...
//
// BUG FIX: see Coupon.ts checkExpiry — same fire-and-forget `this.save()`
// pattern in a non-async method. Now async + awaited.
UserCouponSchema.methods.checkExpiry = async function (): Promise<boolean> {
  const now = new Date();
...
/** Static methods on the UserCoupon model */
export interface IUserCouponModel extends Model<IUserCoupon> {
  hasUserClaimedCoupon(userId: Types.ObjectId, couponId: Types.ObjectId): Promise<boolean>;
  getUserCouponUsageCount(userId: Types.ObjectId, couponId: Types.ObjectId): Promise<number>;
  getExpiringSoonCoupons(days?: number): Promise<IUserCoupon[]>;
}

export const UserCoupon = mongoose.model<IUserCoupon, IUserCouponModel>('UserCoupon', UserCouponSchema);

...
import { Order, OrderStatus, PaymentStatus, OrderItem, OrderCustomer } from '../types/shared';
...
export class OrderModel {
  ...
  static generateOrderNumber(): string {
    ...
    const random = crypto.randomUUID().replace('-', '').substring(0, 3).toUpperCase(); // Random 3 chars
  ...
  static async search(params: {
    ...
    if (params.storeId) {
      ...
      const merchantObjectId =
        typeof params.merchantId === 'string' ? new Types.ObjectId(params.merchantId) : params.merchantId;
      const storeObjectId = typeof params.storeId === 'string' ? new Types.ObjectId(params.storeId) : params.storeId;

      const products = await (MProduct as any)
        .find({
    ...
    const [orders, totalCount] = await Promise.all([
      OrderMongoModel.find(query).sort(sortOptions).skip(skip).limit(limit).exec(),
      ...
      orders.map(async (order) => {
        ...
        if (order.items && order.items.length > 0) {
          try {
            ...
            const product = await (MProduct as any).findById(firstProductId);
  ...
  static async createSampleOrders(merchantId: string) {
    ...
    const items: OrderItem[] = [
      ...
      },
      {
        ...
        productImage: 'https://images.unsplash.com/photo-1549931319-a545dcf3bc73',
        sku: 'BREAD-001',
        quantity: 1,
        price: 8.5,
        total: 8.5,
      ...
      },
      {
        ...
        productImage: 'https://images.unsplash.com/photo-1524594081293-190a2fe0baae',
        sku: 'HONEY-001',
        quantity: 1,
        price: 15.99,
        total: 15.99,
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const randomSource = sources[randomInt(0, sources.length - 1)];

      const subtotal = randomInt(20, 120);
      const taxAmount = subtotal * 0.08;
      const shippingAmount = randomInt(0, 10) < 3 ? 0 : 5.99;
      const discountAmount = randomInt(0, 10) < 2 ? subtotal * 0.1 : 0;
      const totalAmount = subtotal + taxAmount + shippingAmount - discountAmount;

      const createdAt = new Date();
      createdAt.setDate(createdAt.getDate() - randomInt(0, 30));
      createdAt.setHours(randomInt(0, 24), randomInt(0, 60));
      ...
      const order = await this.create({
        ...
        items: items.slice(0, randomInt(1, items.length)),
        pricing: { subtotal, taxAmount, shippingAmount, discountAmount, totalAmount, currency: 'USD' },
        delivery: {
          method: randomInt(0, 10) < 7 ? 'delivery' : 'pickup',
          address: { street: '123 Main St', city: 'Anytown', state: 'CA', zipCode: '12345', country: 'USA' },
        },
        payment: { method: 'card', transactionId: `txn_${crypto.randomUUID().replace('-', '').substring(0, 12)}` },

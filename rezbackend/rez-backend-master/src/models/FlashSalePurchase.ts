...
FlashSalePurchaseSchema.index({ razorpayOrderId: 1 }, { sparse: true });
...
FlashSalePurchaseSchema.methods.isValid = function (): boolean {
  return this.paymentStatus === 'paid' && !this.isRedeemed && new Date() < this.voucherExpiresAt;
};
...
FlashSalePurchaseSchema.methods.markAsRedeemed = async function (storeId?: string, orderId?: string): Promise<void> {
...
// Only counts PAID purchases — pending/failed attempts must not consume the
// user's allocation, otherwise a failed payment permanently locks them out.

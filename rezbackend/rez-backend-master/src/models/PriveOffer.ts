...
export interface IPriveOffer extends Document {
  ...
  getExpiresIn(): string;
  canRedeem(_userId: Types.ObjectId): Promise<{ canRedeem: boolean; reason?: string }>;
}
...
PriveOfferSchema.statics.findFeaturedOffers = function (tier: string, limit: number = 5): Promise<IPriveOffer[]> {
  const now = new Date();
  const userTierLevel = TIER_HIERARCHY[tier] ?? 0;
  const accessibleTiers = Object.keys(TIER_HIERARCHY).filter((t) => TIER_HIERARCHY[t] <= userTierLevel);
...
PriveOfferSchema.statics.findByCategory = function (category: string, tier: string): Promise<IPriveOffer[]> {
  const now = new Date();
  const userTierLevel = TIER_HIERARCHY[tier] ?? 0;
  const accessibleTiers = Object.keys(TIER_HIERARCHY).filter((t) => TIER_HIERARCHY[t] <= userTierLevel);

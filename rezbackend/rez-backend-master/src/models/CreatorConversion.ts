...
CreatorConversionSchema.methods.updateStatus = async function (
  ...
  try {
    ...
    logger.warn('[CREATOR_CONVERSION] Failed to invalidate earnings cache on status change', {
      conversionId: this._id?.toString(),
    });
  }
};
...
export const CreatorConversion = mongoose.model<ICreatorConversion>('CreatorConversion', CreatorConversionSchema);

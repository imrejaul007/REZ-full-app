...
// KAVITA: compound index for reward queue processing (pending→approved→credited flow) — prevents full collection scan on bulk reward operations
PendingCoinRewardSchema.index({ status: 1, createdAt: 1 });

// KAVITA: compound index for user-specific reward lookups by status — prevents N+1 queries on reward retrieval
PendingCoinRewardSchema.index({ user: 1, status: 1, submittedAt: -1 });
...
PendingCoinRewardSchema.statics.getPendingBySource = async function (source: string): Promise<IPendingCoinReward[]> {
  return this.find({ source, status: 'pending' }).sort({ submittedAt: -1 });
};

// Instance method: Approve reward
PendingCoinRewardSchema.methods.approve = async function (adminId: Types.ObjectId, notes?: string): Promise<void> {
  ...
  try {
    ...
    logger.warn('[PENDING REWARD] Failed to invalidate earnings cache on approval', {
      rewardId: this._id?.toString(),
  ...
  // FIX: Immediately credit coins after approval — previously approve() only changed status
  // to 'approved' but never triggered creditCoins(), leaving coins in limbo indefinitely.
  // creditCoins() validates status === 'approved' before proceeding, so this is safe.
  try {
    await this.creditCoins();
  } catch (creditErr) {
    // Log but do not re-throw — the approval is already recorded. A cron job or the
    // /admin/fix-missing-credits endpoint can recover any failed credit operations.
    logger.error(`[PENDING REWARD] creditCoins failed after approval for reward ${this._id}`, {
...
PendingCoinRewardSchema.methods.reject = async function (adminId: Types.ObjectId, reason: string): Promise<void> {
  if (this.status !== 'pending') {
  ...
  try {
    ...
    logger.warn('[PENDING REWARD] Failed to invalidate earnings cache on rejection', {
      rewardId: this._id?.toString(),
      userId: this.user?.toString(),
    });
  }
...
PendingCoinRewardSchema.methods.creditCoins = async function (): Promise<void> {
  ...
  try {
    ...
    logger.warn('[PENDING REWARD] Failed to invalidate earnings cache after coin credit', {
      rewardId: this._id?.toString(),
      userId: this.user?.toString(),
    });
  }

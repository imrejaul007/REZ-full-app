 ...
 * ROUTE-SEC-003 FIX: Now accepts optional session so the audit log can be
 * included in the same MongoDB transaction as the wallet mutation it records.
 */
export async function logTransaction(
  data: Partial<ITransactionAuditLog>,
  session?: mongoose.ClientSession,
): Promise<void> {
  try {
    await TransactionAuditLog.create([data], session ? { session } : {});
  } catch (error) {

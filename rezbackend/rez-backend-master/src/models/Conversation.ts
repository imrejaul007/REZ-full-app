...
ConversationSchema.virtual('isStoreOpen').get(function () {
  ...
  return currentTime >= this.businessHours.openTime && currentTime <= this.businessHours.closeTime;
});
...
ConversationSchema.statics.getTotalUnreadCount = async function (customerId: Types.ObjectId) {

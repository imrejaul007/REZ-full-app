...
WishlistSchema.methods.addItem = async function (itemType: string, itemId: string, options: any = {}): Promise<void> {
  // Check if item already exists
  const existingItem = this.items.find(
    (item: IWishlistItem) => item.itemType === itemType && item.itemId.toString() === itemId,
  );
...
WishlistSchema.methods.removeItem = async function (itemType: string, itemId: string): Promise<void> {
...
WishlistSchema.methods.updateItem = async function (itemType: string, itemId: string, updates: any): Promise<void> {
...
WishlistSchema.methods.moveItem = async function (itemId: string, targetWishlistId: string): Promise<void> {
...
WishlistSchema.methods.checkPriceChanges = async function (): Promise<void> {
  const productItems = this.items.filter(
    (item: IWishlistItem) => item.itemType === 'Product' && item.priceWhenAdded && item.notifyOnPriceChange,
  );
  ...
  for (const product of products) {
    const wishlistItem = productItems.find((item: IWishlistItem) => item.itemId.toString() === product._id.toString());
...
WishlistSchema.statics.getUserWishlists = function (userId: string) {
  return this.find({ user: userId }).sort({ isDefault: -1, createdAt: -1 }).populate('items.itemId');
};

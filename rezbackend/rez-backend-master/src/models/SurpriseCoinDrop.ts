...
SurpriseCoinDropSchema.statics.getAvailableDrops = async function (userId: Types.ObjectId | string) {
  return this.find({
    userId,
    status: 'available',
...
SurpriseCoinDropSchema.statics.createRandomDrop = async function (
  ...
  const coins = randomInt(minCoins, maxCoins);
...
export const SurpriseCoinDrop = mongoose.model<ISurpriseCoinDrop>('SurpriseCoinDrop', SurpriseCoinDropSchema);

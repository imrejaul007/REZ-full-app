...
export interface IOffer extends Document {
  ...
  // R26 fix: targeting fields (user segments, location radius, age range)
  targeting?: {
    userSegments?: string[];
  ...
  // Computed virtual (consumer app reads this)
...
const OfferSchema = new Schema<IOffer>(
  {
    ...
    // R26 fix: targeting fields
    targeting: {
      userSegments: { type: [String], default: undefined },
      locationRadiusKm: { type: Number, min: 0 },
      ageRange: {
...
// Virtual: computed status for consumer app
// Derives a status string from adminApproved + validity.isActive
OfferSchema.virtual('status').get(function () {
  if (this.adminApproved === false) return 'rejected';
  if (!this.adminApproved) return 'pending_approval';
  if (this.validity?.isActive) return 'active';
  return 'inactive';
});
...
OfferSchema.index({ 'validity.isActive': 1, 'validity.endDate': 1 }); // Tag-offers job: no category prefix
...
OfferSchema.index({ 'store.id': 1, 'validity.isActive': 1, type: 1, cashbackPercentage: -1 });
...
OfferSchema.index({ 'eligibility.rezPlusTiers': 1, 'validity.isActive': 1 });
...
OfferSchema.methods.isActiveForUser = async function (
  ...
  if (this.eligibility) {
    try {
      ...
      if (this.eligibility.rezPlusTiers?.length > 0 && !this.eligibility.rezPlusTiers.includes(priv.subscriptionTier)) {
        return { canUse: false, reason: 'Offer requires a different subscription tier' };
      }

      // Check Prive tier
      if (
        this.eligibility.priveTiers?.length > 0 &&
  ...
  // Check time window (active days and hours) — converted to IST (UTC+5:30)
  if (this.timeWindow) {
    const IST_OFFSET_MS = 5.5 * 60 * 60 * 1000;
    const nowIST = new Date(now.getTime() + IST_OFFSET_MS);
    const currentDay = nowIST.getUTCDay(); // 0=Sunday, 6=Saturday
    const currentHour = nowIST.getUTCHours();

    if (this.timeWindow.activeDays?.length > 0 && !this.timeWindow.activeDays.includes(currentDay)) {
      return { canUse: false, reason: 'Offer is not available on this day' };
    }

    if (this.timeWindow.startHour != null && this.timeWindow.endHour != null) {
      if (currentHour < this.timeWindow.startHour || currentHour >= this.timeWindow.endHour) {
        return {
          canUse: false,
          reason: `Offer is only available between ${this.timeWindow.startHour}:00 and ${this.timeWindow.endHour}:00`,
        };
      }
    }
  }
...
OfferSchema.methods.incrementEngagement = async function (action: 'view' | 'like' | 'share'): Promise<void> {
  ...
  if (updateField in this.engagement) {
    this.engagement[updateField as keyof typeof this.engagement] =
      (this.engagement[updateField as keyof typeof this.engagement] as number) + 1;
    await this.save();
  }
};
...
OfferSchema.statics.findNearbyOffers = function (
...
OfferSchema.statics.findStudentOffers = function (): Promise<IOffer[]> {
  const now = new Date();
  return this.find({
    $or: [{ category: 'student' }, { 'restrictions.userTypeRestriction': 'student' }],
    'validity.isActive': true,
    'validity.startDate': { $lte: now },
    'validity.endDate': { $gte: now },
  })

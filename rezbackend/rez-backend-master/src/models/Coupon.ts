...
export interface ICoupon extends Document {
  ...
  metadata?: {
    // Generic provenance — source name.
    // Known values: 'spin_wheel', 'scratch_card', 'campaign-template' (Phase C), ...
    source?: string;
    ...
    // Phase C — campaign-template launch provenance. Set when the
    // coupon was created via POST /api/merchant/campaign-templates/:id/launch
    // so we can trace a redemption back to its template row.
    templateId?: string;
    launchedAt?: string;
  };
  createdAt: Date;
...
//
// BUG FIX: previously non-async with fire-and-forget `this.save()`. Callers
// acting on `return true` could race the persistence; a rejected save went
// silently to unhandledRejection. Now async + awaited — grep confirms no
// callers in src, so no caller migration needed today.
CouponSchema.methods.checkExpiry = async function (): Promise<boolean> {
  const now = new Date();
...
/** ICoupon extended with instance methods */
export interface ICouponDocument extends ICoupon {
  incrementClaimCount(): Promise<void>;
  incrementUsageCount(): Promise<void>;
}

/** Static methods on the Coupon model */
export interface ICouponModel extends Model<ICouponDocument> {
  getActiveCoupons(filters?: Partial<ICoupon>): Promise<ICouponDocument[]>;
  markExpiredCoupons(): Promise<number>;
}

...
NotificationSchema.statics.getUserNotifications = function (
  ...
  return this.find(query)
    .select('title message type category priority isRead readAt createdAt data.deepLink data.imageUrl')
...
NotificationSchema.statics.createBulkNotifications = async function (
  ...
  const batch = batchId || `BATCH_${Date.now()}_${crypto.randomUUID().replace('-', '').substring(0, 9)}`;
...
NotificationSchema.statics.getAnalytics = function (filters: any = {}, dateRange: { start: Date; end: Date }) {

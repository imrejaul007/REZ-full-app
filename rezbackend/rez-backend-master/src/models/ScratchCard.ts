...
ScratchCardSchema.statics.isEligibleForScratchCard = async function (userId: string): Promise<boolean> {
  ...
  // Enforce daily cap: max 1 new scratch card per calendar day (UTC) to prevent reward farming
  const startOfToday = new Date();
  startOfToday.setUTCHours(0, 0, 0, 0);
  const cardsCreatedToday = await this.countDocuments({
    userId,
    createdAt: { $gte: startOfToday },
  });
  const DAILY_SCRATCH_CARD_LIMIT = 1;
  if (cardsCreatedToday >= DAILY_SCRATCH_CARD_LIMIT) return false;

  return completionPercentage >= 80;
};

// KAVITA: Compound indexes for ScratchCard queries
// User claim history with expiry status
ScratchCardSchema.index({ userId: 1, isClaimed: 1, expiresAt: 1 }, { name: 'user_claim_expiry_idx' });
// User reward history timeline
ScratchCardSchema.index({ userId: 1, createdAt: -1 }, { name: 'user_timeline_idx' });
// Eligibility check: unclaimed cards within 24h
ScratchCardSchema.index({ userId: 1, isClaimed: 1, createdAt: -1 }, { name: 'user_unclaimed_timeline_idx' });

...
PriveVoucherSchema.statics.getActiveVouchers = async function (userId: Types.ObjectId): Promise<IPriveVoucher[]> {
  const now = new Date();
  return this.find({
    userId,
    status: 'active',
...
PriveVoucherSchema.statics.markAsUsed = async function (voucherId: Types.ObjectId): Promise<IPriveVoucher | null> {
...
// Get voucher by code
PriveVoucherSchema.statics.getByCode = async function (code: string): Promise<IPriveVoucher | null> {
...
export const calculateVoucherValue = (coins: number, type: VoucherType): number => {
  ...
  const conversionRates: Record<VoucherType, number> = {
    gift_card: 0.1, // 1000 coins = 100 INR
    bill_pay: 0.1,

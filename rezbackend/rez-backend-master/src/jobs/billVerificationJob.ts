...
async function verifyBill(bill: IBill): Promise<{ passed: boolean; reason?: string }> {
  ...
  try {
    ...
    // Check for same date
    const startOfDay = startOfDayIST(new Date(bill.billDate));
    const endOfDay = endOfDayIST(new Date(bill.billDate));
  ...
  if (bill.extractedData?.amount && bill.amount) {
    ...
    if (diff > tolerance) {
      return {
        passed: false,
        reason: `Amount mismatch: user entered ₹${bill.amount}, OCR extracted ₹${bill.extractedData.amount}`,
      };
    }
  }

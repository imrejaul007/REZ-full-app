...
export async function runStuckTransactionRecovery(): Promise<void> {
  ...
  try {
    ...
    // QF-004 FIX: Lock was previously released inside the try block, meaning any
    // early-return (stuckTransfers.length === 0) or thrown error would bypass the
    // releaseLock call and leave the key locked for the full 5-minute TTL.
    // Moved releaseLock to the finally block so it always runs regardless of path.
    logger.error('Stuck transaction recovery job failed', error);
  } finally {
    // QF-004: Always release the lock — whether the job succeeded, was empty, or threw.
    if (lockToken) {

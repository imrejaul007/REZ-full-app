...
import LeaderboardPrizeDistribution, {
  ILeaderboardPrizeDistribution,
  IPrizeEntry,
} from '../models/LeaderboardPrizeDistribution';
import { awardCoins } from '../services/coinService';
import { invalidateWalletCache } from '../services/walletCacheService';
import { logger } from '../config/logger';
import { startOfDayIST } from '../utils/istTime';
...
function shouldDistribute(period: LeaderboardPeriod): boolean {
  ...
  switch (period) {
    case 'daily': {
      ...
      const todayMidnight = startOfDayIST(now);
    ...
    case 'monthly': {
      ...
      const firstOfMonthIST = startOfDayIST(firstOfMonth);
      return now.getDate() === 1 && firstOfMonthIST >= oneHourAgo && firstOfMonthIST <= now;
    }
    case 'all-time': {
...
async function distributeForConfig(config: ILeaderboardConfig): Promise<{
  ...
  if (existingDistribution) {
    logger.info(
      `[PRIZE DIST] ${slug}: Distribution already exists for cycle ${cycleStartDate.toISOString()} - ${cycleEndDate.toISOString()} (status: ${existingDistribution.status})`,
    );
    return { slug, distributed: 0, flagged: 0, skipped: true };
  }
  ...
  logger.info(
    `[PRIZE DIST] ${slug}: Processing prize distribution for cycle ${cycleStartDate.toISOString()} - ${cycleEndDate.toISOString()}`,
  );
  ...
  try {
    ...
    for (const prizeSlot of config.prizePool) {
      const slotEntries = entries.filter((e) => e.rank >= prizeSlot.rankStart && e.rank <= prizeSlot.rankEnd);
          ...
          for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
            try {
              ...
              if (attempt < MAX_RETRIES) {
                logger.warn(
                  `[PRIZE DIST] ${slug}: Retry ${attempt + 1}/${MAX_RETRIES} for user ${userId}: ${awardError.message}`,
                );
                await new Promise((r) => setTimeout(r, 500 * (attempt + 1)));
              } else {
                logger.error(
                  `[PRIZE DIST] ${slug}: Failed to award prize to user ${userId} after ${MAX_RETRIES + 1} attempts:`,
                  awardError.message,
                );
    ...
    logger.info(
      `[PRIZE DIST] ${slug}: Distribution complete - ${totalDistributed} distributed, ${totalFlagged} flagged`,
    );

...
async function confirmPendingConversions(): Promise<void> {
  ...
  // QF-007 FIX: lockToken hoisted outside try so finally can always release it.
  // Previously the lock was released only at the end of try — any thrown error
  // skipped releaseLock and left the key held until the 5-minute TTL expired.
  ...
  try {
    ...
    for (const conversion of pendingConversions) {
      try {
        ...
        if (creatorProfile) {
          ...
          try {
            await redisService.delPattern(`earnings:consolidated:${creatorProfile.user.toString()}:*`);
          } catch (_e) {
            logger.warn('[CREATOR_JOBS] Failed to invalidate earnings cache for creator', {
              creatorId: creatorProfile.user?.toString(),
            });
          }
        }
        if (creatorProfile) {
          const result = await awardCoins(
            ...
            `Commission from pick conversion`,
            {
              ...
              referenceId: `creator_commission:${conversion._id}`,
            },
          );
    ...
    // QF-007: Always release — guards every exit path including early returns and throws.
    if (lockToken) {
...
async function refreshTrendingScores(): Promise<void> {
  ...
  // QF-007 (continued): same pattern — hoist lockToken for finally-block release.
  let lockToken: string | null = null;

  try {
    lockToken = await redisService.acquireLock(lockKey, 180);
    if (!lockToken) return;
    ...
    for (const pick of picks) {
      const ageInHours = Math.max((now.getTime() - new Date(pick.createdAt).getTime()) / (1000 * 60 * 60), 1);
      ...
      if (pick.trendingScore !== trendingScore) {
        await CreatorPick.updateOne({ _id: pick._id }, { $set: { trendingScore, isTrending } });
        updated++;
      }
    }
...
async function refreshCreatorStats(): Promise<void> {
  ...
  // QF-007 (continued): hoist for finally-block release.
  let lockToken: string | null = null;
    ...
    for (const creator of creators) {
      try {
        ...
        const engagementRate =
          totalViews > 0 ? ((totalLikes + (ps.totalShares || 0) + (ps.totalClicks || 0)) / totalViews) * 100 : 0;
  ...
  try {
    ...
    for (const creator of creators) {
      ...
      if (creator.tier !== newTier) {
        await CreatorProfile.updateOne({ _id: creator._id }, { $set: { tier: newTier } });
        upgraded++;
...
export { confirmPendingConversions, refreshTrendingScores, refreshCreatorStats, autoTierUpgrade };

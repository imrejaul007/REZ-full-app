...
export async function runGiftDelivery(): Promise<void> {
  ...
  try {
    ...
    if (dueGifts.length === 0) {
      // QF-005 FIX: removed early releaseLock() here — the finally block now
      // handles it unconditionally.  The previous two-path release pattern
      // (early return + end of try) was fragile: if a new code path was added
      // between the two release points, the lock could leak.
      return;
    }
    ...
    // QF-005: Always release — whether we returned early (no gifts), finished
    // normally, or threw an exception.  Eliminates the lock-leak paths.
    if (lockToken) {

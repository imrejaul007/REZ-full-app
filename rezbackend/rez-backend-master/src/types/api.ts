...
export interface UserProfile {
  ...
  preferences: {
    language?: string;
    // FM-12 FIX: notifications is a nested object (matching rez-shared User.preferences.notifications),
    // not a flat boolean. Consumer app sends { push, email, sms } object.
    notifications?: {
      push?: boolean;
      email?: boolean;
      sms?: boolean;
  ...
  // FM-13 FIX: wallet can be returned as a populated object (with balance/totalEarned/etc.)
  // in /api/user/me responses. Consumer app accesses user.wallet.balance, user.wallet.totalEarned.
  // Type relaxed from string to string | WalletData to match actual API behavior.
  wallet?:
    | string
    | {
        balance?: number;
...
export interface UpdateProfileRequest {
  ...
  preferences?: {
    language?: string;
    // FM-12 FIX: notifications is a nested object (matching consumer app UpdateProfileRequest)
    notifications?: {
      push?: boolean;
      email?: boolean;
      sms?: boolean;
...
export interface OrderListQuery {
  ...
  dateFrom?: string;
  dateTo?: string;
  sort?: 'newest' | 'oldest' | 'amount_high' | 'amount_low' | 'total_desc' | 'total_asc';
}

...
export const initiateTransferSchema = Joi.object({
  ...
  coinType: Joi.string().valid('rez').default('rez').messages({
    'any.only': 'coinType must be "rez"',
  }),
...
export const redeemCoinsSchema = Joi.object({
  ...
  coinType: Joi.string().valid('rez', 'prive', 'branded', 'promo').default('prive'),

...
const CATEGORY_IMAGES: Record<string, { logo: string; banner: string; productDefault: string }> = {
  'street-food': {
    logo: 'indian,street,food,vendor',
    banner: 'indian,street,food,market',
    productDefault: 'indian,snack,food',
  },
  'meat-fish': { logo: 'seafood,market,fresh', banner: 'fish,market,seafood', productDefault: 'fresh,fish,meat' },
  'packaged-goods': {
    logo: 'grocery,store,packaged',
    banner: 'supermarket,grocery,shelf',
    productDefault: 'packaged,food,grocery',
  },
  'beauty-services': {
    logo: 'beauty,salon,makeup',
    banner: 'beauty,salon,spa',
    productDefault: 'makeup,beauty,cosmetic',
  },
  cosmetology: {
    logo: 'skincare,clinic,beauty',
    banner: 'dermatology,skin,clinic',
    productDefault: 'skincare,treatment,facial',
  },
  'skincare-cosmetics': {
    logo: 'cosmetics,skincare,beauty',
    banner: 'skincare,cosmetics,products',
    productDefault: 'skincare,cream,cosmetic',
  },
  'nail-studios': { logo: 'nail,salon,manicure', banner: 'nail,art,manicure', productDefault: 'nail,polish,manicure' },
  'grooming-men': {
    logo: 'barber,grooming,men',
    banner: 'barbershop,haircut,men',
    productDefault: 'barber,shave,grooming',
  },
  dental: { logo: 'dental,clinic,dentist', banner: 'dental,care,clinic', productDefault: 'dental,teeth,dentist' },
  'vision-eyewear': {
    logo: 'eyewear,glasses,optical',
    banner: 'eyeglasses,optical,store',
    productDefault: 'glasses,eyewear,lens',
  },
  'bags-accessories': {
    logo: 'bags,fashion,accessories',
    banner: 'handbag,fashion,accessories',
    productDefault: 'bag,leather,fashion',
  },
  watches: {
    logo: 'watches,luxury,timepiece',
    banner: 'watch,collection,luxury',
    productDefault: 'wristwatch,luxury,time',
  },
  ...
  'martial-arts': {
    logo: 'martial,arts,karate',
    banner: 'karate,martial,training',
    productDefault: 'martial,arts,training',
  },
  'sports-academies': {
    logo: 'sports,academy,training',
    banner: 'sports,field,training',
    productDefault: 'sports,cricket,football',
  },
  sportswear: {
    logo: 'sportswear,athletic,wear',
    banner: 'sports,shoes,gear',
    productDefault: 'sportswear,shoes,athletic',
  },
  'music-dance-classes': {
    logo: 'music,guitar,piano',
    banner: 'music,class,instrument',
    productDefault: 'music,instrument,guitar',
  },
  'art-craft': { logo: 'art,painting,craft', banner: 'art,studio,painting', productDefault: 'painting,art,craft' },
  vocational: {
    logo: 'computer,training,education',
    banner: 'classroom,computer,training',
    productDefault: 'computer,course,training',
  },
  'language-training': {
    logo: 'language,learning,education',
    banner: 'classroom,language,teaching',
    productDefault: 'language,book,learning',
  },
  plumbing: { logo: 'plumber,tools,repair', banner: 'plumbing,pipes,repair', productDefault: 'plumbing,tap,pipe' },
  electrical: {
    logo: 'electrician,electrical,tools',
    banner: 'electrical,wiring,work',
    productDefault: 'electrical,switch,wire',
  },
  cleaning: { logo: 'cleaning,service,home', banner: 'cleaning,house,service', productDefault: 'cleaning,mop,house' },
  'house-shifting': {
    logo: 'moving,truck,packers',
    banner: 'moving,boxes,relocation',
    productDefault: 'moving,box,packing',
  },
  'home-tutors': {
    logo: 'tutor,education,teaching',
    banner: 'home,tuition,study',
    productDefault: 'study,books,education',
  },
  'intercity-travel': {
    logo: 'bus,travel,transport',
    banner: 'bus,highway,travel',
    productDefault: 'bus,travel,journey',
  },
  taxis: { logo: 'taxi,cab,car', banner: 'taxi,city,transport', productDefault: 'taxi,car,ride' },
  'weekend-getaways': {
    logo: 'travel,vacation,resort',
    banner: 'resort,vacation,weekend',
    productDefault: 'resort,vacation,travel',
  },
  tours: {
    logo: 'tour,travel,adventure',
    banner: 'tourism,travel,destination',
    productDefault: 'tour,travel,monument',
  },
  activities: {
    logo: 'adventure,outdoor,activity',
    banner: 'adventure,outdoor,extreme',
    productDefault: 'adventure,activity,outdoor',
  },
  movies: { logo: 'cinema,movie,theater', banner: 'cinema,theater,movie', productDefault: 'movie,popcorn,cinema' },
  'live-events': {
    logo: 'concert,event,live',
    banner: 'concert,stage,event',
    productDefault: 'concert,event,performance',
  },
  festivals: {
    logo: 'festival,celebration,diwali',
    banner: 'festival,lights,celebration',
    productDefault: 'festival,celebration,party',
  },
  workshops: {
    logo: 'workshop,learning,creative',
    banner: 'workshop,class,creative',
    productDefault: 'workshop,craft,learning',
  },
  'bill-payments': {
    logo: 'bill,payment,finance',
    banner: 'payment,bills,utility',
    productDefault: 'bill,payment,receipt',
  },
  'mobile-recharge': {
    logo: 'mobile,recharge,phone',
    banner: 'mobile,phone,recharge',
    productDefault: 'mobile,sim,recharge',
  },
  'cable-ott': {
    logo: 'streaming,tv,entertainment',
    banner: 'television,streaming,ott',
    productDefault: 'streaming,tv,netflix',
  },
  insurance: {
    logo: 'insurance,protection,shield',
    banner: 'insurance,family,protection',
    productDefault: 'insurance,health,life',
  },
  donations: {
    logo: 'charity,donation,heart',
    banner: 'charity,helping,donation',
    productDefault: 'charity,donation,help',
  },
};
...
const MISSING_SUBCATEGORIES: Record<
  string,
  {
    mainCategory: string;
...
async function seedMissingStores() {
  logger.info('🚀 Starting to seed missing stores (FAST VERSION)...\n');
  ...
  logger.info('📦 Using merchant:', existingMerchant?.businessName || 'None found');
  ...
  for (const [subcatSlug, data] of Object.entries(MISSING_SUBCATEGORIES)) {
    ...
    if (!mainCategory) {
      ...
      logger.info(`  ✅ Created main category: ${data.mainCategoryName}`);
    }
    ...
    if (!subcategory) {
      ...
      logger.info(`  ✅ Created subcategory: ${data.name}`);
    }
    ...
    if (existingCount > 0) {
      logger.info(`  ⏭️ Skipping - ${existingCount} stores already exist`);
    ...
    for (const storeData of data.stores) {
      const storeSlug = storeData.name
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/-+$/, '');
      ...
      if (existingStore) {
        logger.info(`  ⏭️ Store exists: ${storeData.name}`);
        skippedStores++;
        continue;
      }
      ...
      totalStoresCreated++;
      logger.info(`  ✅ Created store: ${storeData.name}`);
      ...
      const products = storeData.products.map((p, idx) => {
        ...
        const imageKeywords = productNameKey ? PRODUCT_IMAGES[productNameKey] : categoryImages.productDefault;
  ...
  logger.info(`   📦 Stores created: ${totalStoresCreated}`);
  logger.info(`   🏷️ Products created: ${totalProductsCreated}`);
  logger.info(`   ⏭️ Stores skipped: ${skippedStores}`);
...
seedMissingStores().catch((err) => { console.error(err); process.exit(1); }); // D19: fail loud so CI catches broken seeds

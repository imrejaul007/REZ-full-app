...
async function setFeaturedProducts() {
  try {
    logger.info('Starting Featured Products Setup...');
    ...
    logger.info('Reset all products to isFeatured: false\n');
    ...
    for (const store of stores) {
      ...
      if (products.length > 0) {
        ...
        await productsCollection.updateMany({ _id: { $in: productIds } }, { $set: { isFeatured: true } });

        totalFeatured += numToFeature;
        logger.info(`  Featured ${numToFeature} products from ${store.name}`);
      }
    }

    logger.info('\n========================================');
    logger.info('FEATURED PRODUCTS SUMMARY');
    logger.info('========================================');
    logger.info(`Total featured products: ${totalFeatured}`);

    // Verify
    const featuredCount = await productsCollection.countDocuments({ isFeatured: true });
    logger.info(`Verified count: ${featuredCount} products with isFeatured=true`);
    ...
    for (const product of sampleFeatured) {
      ...
      logger.info(`  - ${product.name} (${store?.name || 'Unknown store'}) - Rating: ${product.ratings?.average || 0}`);
    }
  } catch (error) {
...
setFeaturedProducts()
  .then(() => {
    logger.info('Featured products setup completed!');
    process.exit(0);
  })
  .catch((error) => {

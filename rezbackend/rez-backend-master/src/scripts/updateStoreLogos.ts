...
async function updateStoreLogos() {
  try {
    logger.info('🚀 Starting store logo update...');
    ...
    logger.info('========================================');
    logger.info('UPDATING STORE LOGOS');
    logger.info('========================================\n');
    ...
    for (const store of stores) {
      ...
      if (logoData) {
        ...
        logger.info(`✅ Updated: ${storeName}`);
        updatedCount++;
      } else {
        // Generate a placeholder logo using ui-avatars
        const _initials = storeName
          .split(' ')
          .map((w: string) => w[0])
          .join('+');
        ...
        logger.info(`⚠️ Generated placeholder for: ${storeName}`);
        notFoundCount++;
      }
    }
    ...
    logger.info(`Updated with specific logos: ${updatedCount}`);
    logger.info(`Generated placeholders: ${notFoundCount}`);
    ...
    for (const s of sampleStores) {
      ...
      logger.info(`      Logo: ${store.logo?.substring(0, 60)}...`);
      logger.info(`      Cover: ${store.coverImage?.substring(0, 60)}...`);
    }
  } catch (error) {

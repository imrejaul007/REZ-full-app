...
async function migrateStore(
  store: any,
  _categoryId: mongoose.Types.ObjectId,
  ...
  if (!store.tags || !Array.isArray(store.tags) || store.tags.length === 0) {
    ...
    const hasCuisineTag = cuisineKeywords.some((keyword) => existingTags.some((tag: string) => tag.includes(keyword)));
    ...
    if (!hasCuisineTag) {
      ...
      changes.push(
        `Added cuisine tags: ${inferredTags
          .filter((t) => !existingTags.includes(t.toLowerCase()))
          .slice(0, 3)
          .join(', ')}`,
      );
    }
  ...
  if (Object.keys(updates).length > 0) {
    try {
      ...
      await Store.collection.updateOne({ _id: store._id }, { $set: updates });
...
async function main() {
  ...
  try {
    ...
    logger.info('├──────────────────────────────────────┼──────────────┤');
    logger.info(`│ Stores Migrated                      │ ${String(result.migrated).padStart(12)} │`);
    logger.info(`│ Stores Skipped (No Changes)          │ ${String(result.skipped).padStart(12)} │`);
    logger.info('└──────────────────────────────────────┴──────────────┘');

...
async function updateAllStoreLogos() {
  logger.info('🖼️ Updating ALL store logos with proper images...\n');
  ...
  for (const store of stores) {
    ...
    if (logoData) {
      ...
      skipped++;
      logger.info(`  ⏭️ ${store.name} (no logo found)`);
    }
  }

  logger.info('\n' + '='.repeat(50));
  logger.info('🎉 Store logo update complete!');
...
updateAllStoreLogos().catch((err) => { console.error(err); process.exit(1); }); // D19: fail loud so CI catches broken seeds

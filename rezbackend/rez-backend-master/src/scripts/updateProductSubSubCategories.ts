...
const PRODUCT_NAME_HINTS: Record<string, string[]> = {
  ...
  'cricket|football|swimming|badminton': [
    'Cricket Coaching',
    'Football Training',
    'Swimming Lessons',
    'Badminton Coaching',
...
async function updateProductSubSubCategories() {
  try {
    logger.info('🚀 Starting product sub-sub-category update...');
    ...
    const _isSubcategory = (categoryId: string): boolean => {
    ...
    logger.info('========================================');
    logger.info('UPDATING PRODUCTS WITH SUB-SUB-CATEGORIES');
    logger.info('========================================\n');
    ...
    for (const product of products) {
        ...
        for (const [pattern, categories] of Object.entries(PRODUCT_NAME_HINTS)) {
          ...
          for (const pat of patterns) {
            if (productNameLower.includes(pat)) {
              ...
              await db.collection('products').updateOne({ _id: p._id }, { $set: { subSubCategory: matchedSubSubCat } });
      ...
      // Update the product
      await db.collection('products').updateOne({ _id: p._id }, { $set: { subSubCategory: assignedSubSubCategory } });

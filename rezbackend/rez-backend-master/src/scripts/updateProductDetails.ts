...
async function updateProductDetails() {
  try {
    logger.info('🚀 Starting product details update...');
    ...
    for (const product of products) {
      ...
      if (!storeId) {
        logger.info(`⚠️  Skipping product ${p._id} - no store assigned`);
        continue;
      }
    ...
    logger.info('\n========================================');
    logger.info('UPDATING STORE SUBCATEGORIES');
    logger.info('========================================\n');
    ...
    if (sortedCategories.length > 20) {
      logger.info(`   ... and ${sortedCategories.length - 20} more subcategories`);
    }
  } catch (error) {

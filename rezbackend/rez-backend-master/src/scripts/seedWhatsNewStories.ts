...
async function clearWhatsNewStories() {
  logger.info("🗑️  Clearing existing What's New stories...");
  await WhatsNewStory.deleteMany({});
  logger.info('✅ Existing stories cleared');
}

async function seedWhatsNewStories() {
  try {
    logger.info("🚀 Starting What's New Stories seeding...");
    ...
    logger.info("🔄 Creating What's New stories...");
    const stories = await WhatsNewStory.insertMany(storiesData);

    logger.info(`✅ Created ${stories.length} What's New stories:`);
    stories.forEach((story, index) => {
      logger.info(`   ${index + 1}. ${story.title} (Priority: ${story.priority})`);
    });

    logger.info("\n🎉 What's New Stories seeding completed successfully!");
  } catch (error) {
    logger.error("❌ Error seeding What's New stories:", error);
    throw error;
  } finally {
...
seedWhatsNewStories().catch((err) => { console.error(err); process.exit(1); }); // D19: fail loud so CI catches broken seeds

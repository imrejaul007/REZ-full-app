...
async function seedProfileData() {
  try {
    ...
    logger.info(`Using user: ${user.profile.firstName} (${userId})`);
    ...
    if (products.length === 0 || stores.length === 0) {
      logger.info('⚠️  No products/stores found. Run seed products/stores first.');
    }
        ...
        items: [
          {
            ...
            image: product.images?.[0] || 'https://placehold.co/200',
            quantity: 1,
    ...
    logger.info(`✅ Created ${projects.length} projects with submissions`);
    ...
    const brands = [
      ...
      },
      {
        ...
        termsAndConditions: ['Valid for 6 months', 'Applicable on select products', 'One-time use only'],
        purchaseCount: 4000,
        viewCount: 20000,
      },
      {
        ...
        termsAndConditions: ['Valid for 3 months', 'Minimum order ₹299', 'Cannot be combined with other offers'],
        purchaseCount: 3000,
        viewCount: 15000,
      },
    ];
    ...
    logger.info(`✅ Created ${createdBrands.length} voucher brands`);
    ...
    logger.info(`✅ Created ${userVouchers.length} user vouchers`);
    ...
    logger.info('✅ Updated user wallet balance');

    // ===== SUMMARY =====
    logger.info('\n📊 Profile Data Seeding Summary:');
    logger.info(`   Orders: ${orders.length}`);
    logger.info(`   Projects: ${projects.length}`);
    logger.info(`   Voucher Brands: ${createdBrands.length}`);
    logger.info(`   User Vouchers: ${userVouchers.length}`);
    logger.info(`   Transactions: ${transactions.length}`);
    logger.info(`   Total Earned: ₹${totalEarned}`);
    logger.info(`   Total Spent: ₹${totalSpent}`);
    logger.info(`   Wallet Balance: ₹${totalEarned - totalSpent}`);

    logger.info('\n✅ Profile data seeding completed successfully!');
    logger.info('\nYou can now test the profile sections:');
    logger.info('  - My Products: GET /api/orders');
    logger.info('  - My Services: GET /api/projects (filter by user submissions)');
    logger.info('  - My Vouchers: GET /api/vouchers/my-vouchers');
    logger.info('  - My Earnings: GET /api/wallet/transactions');
  } catch (error) {

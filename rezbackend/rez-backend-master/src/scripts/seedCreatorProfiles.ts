...
async function seedCreatorProfiles() {
  try {
    ...
    const CreatorProfile = mongoose.model(
      'CreatorProfile',
      new mongoose.Schema({}, { strict: false, collection: 'creatorprofiles' }),
    );
    const CreatorPick = mongoose.model(
      'CreatorPick',
      new mongoose.Schema({}, { strict: false, collection: 'creatorpicks' }),
    );
    ...
    if (existingCount > 0) {
      logger.info(`Already have ${existingCount} CreatorProfile documents. Skipping seed.`);
      logger.info('To re-seed, run: db.creatorprofiles.deleteMany({}) in MongoDB shell first.');
    ...
    if (users.length < CREATOR_DATA.length) {
      logger.error(`Need at least ${CREATOR_DATA.length} users, found ${users.length}. Seed users first.`);
      await mongoose.disconnect();
      return;
    }
    ...
    for (let i = 0; i < CREATOR_DATA.length; i++) {
      ...
      logger.info(`✅ Created CreatorProfile: ${data.displayName} (${data.category}, ${data.tier})`);
    }
    ...
    if (products.length > 0) {
      for (const profile of createdProfiles) {
        ...
        logger.info(`   📌 Created ${numPicks} picks for ${(profile as any).displayName}`);
      }
    } else {
      logger.info('⚠️  No products found — skipping pick creation');
    }

    logger.info(`\n🎉 Seeding complete!`);
    logger.info(`   CreatorProfiles: ${createdProfiles.length}`);
    logger.info(`   CreatorPicks: ${totalPicks}`);

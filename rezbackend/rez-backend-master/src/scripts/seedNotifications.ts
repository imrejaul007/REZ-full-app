...
async function seedNotifications() {
  try {
    ...
    logger.info(`Found ${users.length} users, ${orders.length} orders, ${products.length} products`);
      ...
      if (products.length > 0) {
        ...
        notifications.push({
          ...
          data: {
            ...
            discountPercentage:
              Math.floor(
                ((randomProduct.pricing?.original - randomProduct.pricing?.selling) / randomProduct.pricing?.original) *
                  100,
              ) || 20,
          },
          actionButton: {
    ...
    logger.info(`  Total: ${stats.total} notifications`);
    logger.info(`  Read: ${stats.read} | Unread: ${stats.unread}`);
    logger.info(`  By Type:`);
    ...
    for (let i = 0; i < Math.min(users.length, 3); i++) {
      ...
      logger.info(`  ${userName}: ${userNotifications.length} total, ${unreadCount} unread`);
    }

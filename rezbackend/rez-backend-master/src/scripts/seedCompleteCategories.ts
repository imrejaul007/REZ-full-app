...
async function seedCompleteCategories() {
  try {
    logger.info('🚀 Starting Complete Category Seeding...');
    ...
    logger.info('📦 Creating 12 Main Categories...\n');
    ...
    for (const [slug, config] of Object.entries(CATEGORY_HIERARCHY)) {
      ...
      if (existingMain) {
        ...
        logger.info(`   ✏️ Updated: ${config.name} (${slug})`);
      } else {
        ...
        if (existingAsSub) {
          ...
          logger.info(`   🔄 Promoted to main: ${config.name} (${slug}) — was a subcategory`);
        } else {
          // Insert new
          ...
          logger.info(`   ✅ Created: ${config.name} (${slug})`);
        }
      }
    ...
    for (const [mainSlug, config] of Object.entries(CATEGORY_HIERARCHY)) {
      ...
      for (let i = 0; i < config.subcategories.length; i++) {
        ...
        if (existingSub) {
          ...
          logger.info(`      ✏️ ${sub.name} (${sub.slug})`);
        } else {
          // Insert new
          const result = await categoriesCollection.insertOne({
            name: sub.name,
    ...
    for (const [mainSlug, childIds] of Object.entries(childCategoryIds)) {
      await categoriesCollection.updateOne({ _id: categoryMapping[mainSlug] }, { $set: { childCategories: childIds } });
    }

    logger.info('\n========================================');
    logger.info('📊 SEEDING SUMMARY');
    logger.info('========================================');
    logger.info(`Main Categories: ${mainCategoryCount}`);
    logger.info(`Subcategories: ${subcategoryCount}`);
    logger.info(`Total Categories: ${mainCategoryCount + subcategoryCount}`);
    logger.info('========================================\n');
    ...
    logger.info(`Main Categories in DB: ${mainInDb}`);
    logger.info(`Subcategories in DB: ${subInDb}`);
    logger.info('========================================\n');

    // Export mapping for use by other scripts
    logger.info('📋 Category Slug → ObjectId Mapping:');
    logger.info('(Save this for reference)\n');
    ...
    logger.info(`   ... and ${Object.keys(categoryMapping).length - 10} more\n`);
  } catch (error) {

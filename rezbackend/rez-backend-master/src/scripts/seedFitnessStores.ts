...
const fitnessStoresData = [
  // ========== GYMS ==========
  {
    ...
    description:
      'Premium gym with world-class equipment and expert trainers. State-of-the-art facilities including cardio zone, strength training area, and group fitness studio.',
  ...
  },
  {
    ...
    description:
      'Your one-stop fitness destination. Offering gym, group workouts, yoga, and dance classes. Award-winning fitness centers.',
  ...
  // ========== YOGA STUDIOS ==========
  {
    ...
    description:
      'Traditional yoga classes in a serene environment. Experienced instructors teaching Hatha, Vinyasa, and Ashtanga yoga.',
  ...
  },
  {
    ...
    description:
      'Modern yoga studio offering group and private sessions. Perfect for beginners and advanced practitioners.',
  ...
  // ========== PERSONAL TRAINERS ==========
  {
    ...
    slug: 'fitcoach-rahul',
    description:
      'Certified personal trainer specializing in strength training and body transformation. 10+ years experience.',
...
async function seedFitnessStores() {
  logger.info('🏋️ Starting Fitness Stores Seeding...\n');
  ...
  try {
    ...
    if (!fitnessCategory) {
      logger.info('📁 Creating Fitness & Sports category...');
      ...
      logger.info('✅ Created Fitness & Sports category');
    } else {
      logger.info('✅ Fitness & Sports category exists');
    }
    ...
    for (const subCat of subCategoryData) {
      ...
      if (!existingSubCat) {
        ...
        logger.info(`   ✅ Created: ${subCat.name}`);
      } else {
        ...
        if (!existingSubCat.parentCategory) {
          ...
          logger.info(`   🔗 Linked: ${subCat.name} to Fitness & Sports`);
        } else {
          logger.info(`   ✓ Exists: ${subCat.name}`);
        }
      }
    ...
    for (const storeData of fitnessStoresData) {
      ...
      try {
        ...
        createdCount++;
        logger.info(`   ✅ Created: ${store.name} (${categorySlug})`);
      } catch (err: any) {
        logger.error(`   ❌ Error creating ${storeData.name}:`, err.message);
      }
    }
    ...
    logger.info(`   Total fitness stores: ${totalFitnessStores}`);
    logger.info(`   Created: ${createdCount}`);
    logger.info('\n   By Category:');
    logger.info(`   - Gyms: ${gymsCount}`);
    logger.info(`   - Studios: ${studiosCount}`);
    logger.info(`   - Trainers: ${trainersCount}`);
    logger.info(`   - Sports Stores: ${storesCount}`);
    logger.info('═'.repeat(50));

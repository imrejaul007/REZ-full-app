  ...
  for (const storeData of stores) {
    ...
    if (!store) {
      ...
      const generalCategory =
        (await Category.findOne({ slug: 'home-services' })) || (await Category.findOne({ slug: 'general' }));
...
async function main(): Promise<void> {
  ...
  try {
    ...
    logger.info('├────────────────────────────┼───────┤');
    logger.info(`│ Service Categories         │ ${String(categories.length).padStart(5)} │`);
    logger.info(`│ Service Provider Stores    │ ${String(stores.length).padStart(5)} │`);
    logger.info(`│ Services                   │ ${String(servicesCount).padStart(5)} │`);
    logger.info('└────────────────────────────┴───────┘');

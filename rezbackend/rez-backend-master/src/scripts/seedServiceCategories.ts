...
async function seedServiceCategories() {
  try {
    ...
    if (existingCount > 0) {
      logger.info(`Found ${existingCount} existing service categories.`);
      ...
      if (answer.toLowerCase() === 's') {
        logger.info('Skipping seed. No changes made.');
        await disconnectDb();
        return;
      }
      ...
      if (answer.toLowerCase() === 'r') {
        ...
        logger.info('Existing categories deleted.');
      }
    }
    ...
    for (const categoryData of serviceCategoriesData) {
      ...
      await category.save();
      logger.info(`  + Created category: ${categoryData.name} (${categoryData.cashbackPercentage}% cashback)`);
    }

    // Verify insertion
    const finalCount = await ServiceCategory.countDocuments();
    logger.info(`\nService categories seeded successfully!`);
    logger.info(`Total categories in database: ${finalCount}`);

    // List all categories
    const allCategories = await ServiceCategory.find().sort({ sortOrder: 1 });
    logger.info('\nCategories:');
    allCategories.forEach((cat, index) => {
      logger.info(`  ${index + 1}. ${cat.name} - Up to ${cat.cashbackPercentage}% cashback`);
    });
  } catch (error) {

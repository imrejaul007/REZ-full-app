...
async function checkData() {
  ...
  const subCatSlugCount = await db!
    .collection('stores')
    .aggregate([{ $group: { _id: '$subcategorySlug', count: { $sum: 1 } } }, { $sort: { count: -1 } }, { $limit: 15 }])
    .toArray();
  ...
  logger.info(`  Found ${matchingCategories.length} matching categories:`);
  matchingCategories.forEach((cat) => {
    logger.info(`    - ${cat.slug}: ${cat.name} (${cat.parentCategory ? 'subcategory' : 'main'})`);
  });
  ...
  const bangaloreStores = await db!
    ...
    .limit(5)
    .project({ name: 1, 'location.city': 1, subcategorySlug: 1 })
    .toArray();
  bangaloreStores.forEach((s) =>
    console.log(`  ${s.name} - ${s.location?.city} - slug: ${s.subcategorySlug || 'none'}`),
  );

...
async function seedSubscriptionTiers() {
  try {
    ...
    logger.info('🗑️  Cleared existing subscription tiers');

    // Insert new tiers
    const inserted = await SubscriptionTier.insertMany(subscriptionTiers);
    logger.info(`✅ Successfully seeded ${inserted.length} subscription tiers:`);
    inserted.forEach((tier) => {
      logger.info(`   - ${tier.name} (${tier.tier}): ₹${tier.pricing.monthly}/month, ₹${tier.pricing.yearly}/year`);
    });

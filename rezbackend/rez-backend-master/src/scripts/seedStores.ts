...
const storesData = [
  ...
  },
  {
    ...
    slug: 'penny-store',
    description:
      'Ultra-budget store with items starting from 1 rupee. Perfect for students and budget-conscious families.',
...
async function seedStores() {
  try {
    logger.info('🌱 Starting store seeding...');
    ...
    storesData.forEach((store, _index) => {
      let categoryId = null;
      ...
      if (!categoryId) {
        logger.error(`❌ No category found for store: ${store.name}`);
        // Use the first available category as fallback
        categoryId = categories[0]?._id;
      }

      (store as any).category = categoryId;
      logger.info(`✅ Assigned category to ${store.name}: ${categoryId}`);
    });
    ...
    logger.info(`   🚀 Fast Delivery (30 min): ${fastDeliveryStores}`);
    logger.info(`   💰 Budget Friendly (1 rupee): ${budgetStores}`);
    logger.info(`   👑 Premium/Luxury: ${premiumStores}`);
    logger.info(`   🌱 Organic: ${organicStores}`);
    logger.info(`   🤝 Alliance: ${allianceStores}`);
    logger.info(`   💸 Lowest Price: ${lowestPriceStores}`);
    logger.info(`   🏬 Mall: ${mallStores}`);
    logger.info(`   💵 Cash Store: ${cashStores}`);

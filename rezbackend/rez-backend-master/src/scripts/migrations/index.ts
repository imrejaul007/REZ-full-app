 ...
 * RUNNING MIGRATIONS:
 *
 * Option 1: Run individual migrations directly
 *   MONGO_URI=mongodb+srv://... npx ts-node src/scripts/migrations/001-statusHistory-to-timeline.ts
 *   MONGO_URI=mongodb+srv://... npx ts-node src/scripts/migrations/001-statusHistory-to-timeline.ts up
 *   MONGO_URI=mongodb+srv://... npx ts-node src/scripts/migrations/001-statusHistory-to-timeline.ts down
 *
 * Option 2: Use the migration runner (recommended)
 *   # Run all pending migrations
 *   MONGO_URI=... npx ts-node src/scripts/migrations/migration-runner.ts
 *
 *   # Run specific migration
 *   MONGO_URI=... npx ts-node src/scripts/migrations/migration-runner.ts --id 002
 *
 *   # Rollback specific migration
 *   MONGO_URI=... npx ts-node src/scripts/migrations/migration-runner.ts --id 002 --down
 *
 *   # Rollback last N migrations
 *   MONGO_URI=... npx ts-node src/scripts/migrations/migration-runner.ts --rollback=2
 *
 *   # Show migration status
 *   MONGO_URI=... npx ts-node src/scripts/migrations/migration-runner.ts --status
 *
 * Order matters: run in sequence (001 → 008). Each script is idempotent
 * and safe to re-run, but running in order avoids any dependency issues.
 *
 * ROLLBACK CAPABILITY:
 *   All reversible migrations now include a down() method for rollback.
 *   Migrations marked as reversible=false cannot be rolled back (data loss).
 *
...
export const migrations: MigrationEntry[] = [
  ...
  },
  {
    ...
    reversible: true,
    notes:
      'DM-L4 (User.wallet sub-doc) is report-only — requires consumer app audit before removal. DM-L5 (averageOrderValue) is not removed — needs recalculation job instead. NOTE: Down() restores fields with null/default values since original data is lost.',
  },
  {
    id: '008',
    name: 'remove-user-wallet-subdoc',
    description:
      'Remove User.wallet embedded sub-document (denormalized cache). The Wallet collection is the sole source of truth for wallet data.',
    bugRef: 'DM-L4',
    collections: ['users'],
    risk: 'MEDIUM',
    type: 'write',
    reversible: false,
    notes:
      'NOT reversible — data permanently deleted. Down() will exit with error. To re-embed wallet data, implement a sync job from Wallet to User.',
  },
];

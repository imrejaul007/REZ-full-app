 ...
 *
 * Usage:
 *   MONGO_URI=... npx ts-node src/scripts/migrations/004-notification-read-to-isread.ts [up|down]
 */
...
async function down(): Promise<void> {
  ...
  try {
    console.log('[004] DOWN: Reverting isRead → read field...');
    await client.connect();
    const db = client.db();

    const exists = (await db.listCollections({ name: COLLECTION }).toArray()).length > 0;
    if (!exists) {
      console.log(`[004] Collection \`${COLLECTION}\` does not exist — nothing to revert`);
      return;
    }

    const col = db.collection(COLLECTION);

    // Add read field back based on isRead value
    const withIsRead = await col.countDocuments({ isRead: { $exists: true } });
    console.log(`[004] DOWN: Docs with isRead field: ${withIsRead}`);

    // Step 1: Restore read=true where isRead=true
    const step1Result = await col.updateMany(
      { isRead: true },
      { $set: { read: true } },
    );
    console.log(`  Docs with read=true restored: ${step1Result.modifiedCount}`);

    // Step 2: Restore read=false where isRead=false
    const step2Result = await col.updateMany(
      { isRead: false },
      { $set: { read: false } },
    );
    console.log(`  Docs with read=false restored: ${step2Result.modifiedCount}`);

    // Verification
    const totalWithRead = await col.countDocuments({ read: { $exists: true } });
    console.log(`\n[004] DOWN: Rollback complete:`);
    console.log(`  Total docs with read field restored: ${totalWithRead}`);
  } finally {
    await client.close();
    console.log('[004] DOWN: Disconnected');
  }
}

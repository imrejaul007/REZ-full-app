 ...
 *
 * Usage:
 *   MONGO_URI=... npx ts-node src/scripts/migrations/007-dead-fields-cleanup.ts [up|down]
 *
 * DOWN: Restores fields that were removed. Note: original values are lost,
 * so fields are restored with null/default values. Data loss is unavoidable
 * for fields that were legitimately removed.
 */
...
async function down(): Promise<void> {
  ...
  try {
    console.log('[007] DOWN: Restoring removed dead fields...');
    console.warn('[007] DOWN: WARNING: Original values were lost during up(). Fields restored with null.');
    ...
    // Step 1 — DM-L1: Restore profile.ringSize and profile.jewelryPreferences
    console.log('\n[007] DOWN Step 1 (DM-L1): Restoring User.profile jewelry fields...');

    if (existingCollections.has('users')) {
      const col = db.collection('users');

      // Restore with null since we don't have original values
      const result1 = await col.updateMany(
        { 'profile.ringSize': { $exists: false } },
        { $set: { 'profile.ringSize': null } },
      );
      console.log(`  Users with ringSize restored: ${result1.modifiedCount}`);

      const result1b = await col.updateMany(
        { 'profile.jewelryPreferences': { $exists: false } },
        { $set: { 'profile.jewelryPreferences': null } },
      );
      console.log(`  Users with jewelryPreferences restored: ${result1b.modifiedCount}`);

      totalRestored += result1.modifiedCount + result1b.modifiedCount;
    } else {
      console.log('  [skip] users — collection does not exist');
    }

    // Step 2 — DM-L2: Restore Order.payment.coinsUsed.wasilCoins
    console.log('\n[007] DOWN Step 2 (DM-L2): Restoring Order.payment.coinsUsed.wasilCoins...');

    if (existingCollections.has('orders')) {
      const col = db.collection('orders');

      // Restore wasilCoins to null (we don't have original values)
      const result2 = await col.updateMany(
        { 'payment.coinsUsed.wasilCoins': { $exists: false } },
        { $set: { 'payment.coinsUsed.wasilCoins': null } },
      );
      console.log(`  Orders with wasilCoins restored: ${result2.modifiedCount}`);
      totalRestored += result2.modifiedCount;
    } else {
      console.log('  [skip] orders — collection does not exist');
    }

    // Step 3 — DM-L3: Restore Wallet.categoryBalances
    console.log('\n[007] DOWN Step 3 (DM-L3): Restoring Wallet.categoryBalances...');

    if (existingCollections.has('wallets')) {
      const col = db.collection('wallets');

      // Restore categoryBalances as empty object (original state before removal)
      const result3 = await col.updateMany(
        { categoryBalances: { $exists: false } },
        { $set: { categoryBalances: {} } },
      );
      console.log(`  Wallets with categoryBalances restored: ${result3.modifiedCount}`);
      totalRestored += result3.modifiedCount;
    } else {
      console.log('  [skip] wallets — collection does not exist');
    }

    console.log(`\n[007] DOWN: Rollback complete:`);
    console.log(`  Total fields restored: ${totalRestored}`);
    console.warn('[007] DOWN: NOTE: Original field values were lost. Fields restored with null/empty defaults.');
  } finally {
    await client.close();
    console.log('[007] DOWN: Disconnected');
  }
}
...
async function up(): Promise<void> {
  ...
  try {
    ...
    if (!existingCollections.has('orders')) {
      ...
      const wasilCoinsPath = 'payment.coinsUsed.wasilCoins';
      // Use $and to combine multiple $ne conditions (avoid duplicate key error in TypeScript)
      const nonZeroWasilFilter = {
        $and: [
          { [wasilCoinsPath]: { $exists: true } },
          { [wasilCoinsPath]: { $ne: null } },
          { [wasilCoinsPath]: { $ne: 0 } },
          { [wasilCoinsPath]: { $gt: 0 } },
        ],
      };
      const nonZeroWasil = await col.countDocuments(nonZeroWasilFilter);
      ...
      const toRemoveFilter = {
        [wasilCoinsPath]: { $exists: true },
        $or: [{ [wasilCoinsPath]: null }, { [wasilCoinsPath]: 0 }],
      };
      const toRemove = await col.countDocuments(toRemoveFilter);

      console.log(`  Orders with wasilCoins=null or 0: ${toRemove}`);

      const result2 = await col.updateMany(toRemoveFilter, { $unset: { [wasilCoinsPath]: '' } });

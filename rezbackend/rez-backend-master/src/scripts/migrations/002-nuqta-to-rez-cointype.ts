 ...
 *
 * Usage:
 *   MONGO_URI=... npx ts-node src/scripts/migrations/002-nuqta-to-rez-cointype.ts [up|down]
 */
...
async function down(): Promise<void> {
  ...
  try {
    console.log('[002] DOWN: Reverting coinType rez → nuqta...');
    ...
    // --- 1. cointransactions ---
    const c1 = await safeUpdateMany('cointransactions', { coinType: 'rez' }, { $set: { coinType: 'nuqta' } });
    console.log(`[1/4] cointransactions: ${c1} documents reverted`);
    totalReverted += c1;

    // --- 2. coinledgers ---
    const c2 = await safeUpdateMany('coinledgers', { coinType: 'rez' }, { $set: { coinType: 'nuqta' } });
    console.log(`[2/4] coinledgers: ${c2} documents reverted`);
    totalReverted += c2;

    // --- 3. wallets (coins array) ---
    const c3 = await safeUpdateMany(
      'wallets',
      { 'coins.type': 'rez' },
      { $set: { 'coins.$[elem].type': 'nuqta' } },
      { arrayFilters: [{ 'elem.type': 'rez' }] } as object,
    );
    console.log(`[3/4] wallets (coins array): ${c3} documents reverted`);
    totalReverted += c3;

    // --- 4. userloyalties ---
    const c4a = await safeUpdateMany('userloyalties', { coinType: 'rez' }, { $set: { coinType: 'nuqta' } });
    const c4b = await safeUpdateMany(
      'userloyalties',
      { 'history.coinType': 'rez' },
      { $set: { 'history.$[elem].coinType': 'nuqta' } },
    );
    const c4c = await safeUpdateMany(
      'userloyalties',
      { 'transactions.coinType': 'rez' },
      { $set: { 'transactions.$[elem].coinType': 'nuqta' } },
    );
    const c4 = c4a + c4b + c4c;
    console.log(`[4/4] userloyalties: ${c4} total reverted`);
    totalReverted += c4;

    console.log(`\n[002] DOWN: Rollback complete:`);
    console.log(`  Total documents reverted: ${totalReverted}`);
  } finally {
    await client.close();
    console.log('[002] DOWN: Disconnected');
  }
}

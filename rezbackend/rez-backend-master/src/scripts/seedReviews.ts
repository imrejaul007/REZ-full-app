...
async function seedReviews() {
  try {
    ...
    if (users.length === 0 || products.length === 0) {
      logger.info('❌ Please run basic seeding first (users, products)');
      process.exit(1);
    }

    logger.info(`Found ${users.length} users, ${products.length} products, ${orders.length} orders`);

    // Clear existing reviews
    await Review.deleteMany({});
    logger.info('🗑️  Cleared existing reviews');
    ...
    const reviews = [
      {
        ...
        content:
          'I have been using this iPhone 15 Pro for two weeks now and I am absolutely blown away by the camera quality. The night mode is incredible and the battery life easily gets me through a full day of heavy usage. The titanium build feels premium and the Action Button is surprisingly useful. Definitely worth the upgrade!',
      ...
      },
      {
        ...
        content:
          'Really happy with this purchase. The cotton feels premium and soft, exactly as described. The fit is true to size and the color matches the website perfectly. Only minor complaint is the price - could be a bit more affordable, but the quality justifies it. Would definitely buy again in different colors.',
      ...
      },
      {
        ...
        content:
          'This is my third t-shirt from this brand and they never disappoint. The 100% cotton is so comfortable and breathable. Perfect for everyday wear or working out. The fit is relaxed but not baggy. Highly recommend!',
      ...
      },
      {
        ...
        content:
          'Overall a solid upgrade from my iPhone 13. The cameras are noticeably better and the display is brighter. Performance is snappy as expected. However, I was hoping for better battery life given the hype. It gets me through the day but just barely with my usage patterns. Still a good phone though.',
      ...
      },
      {
        ...
        content:
          'The phone is good, do not get me wrong. But for the price premium, I expected more significant improvements. The camera is better but not dramatically so. The titanium build is nice but does not feel that much different from stainless steel. Maybe wait for a price drop.',
    ...
    for (const [productName, data] of Object.entries(avgRatings)) {
      ...
      logger.info(`  ${productName}: ${avgRating}★ avg (${data.count} reviews) [${data.reviews.join(', ')}]`);
    }

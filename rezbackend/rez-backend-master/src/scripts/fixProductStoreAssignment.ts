...
const SUBCATEGORY_PRODUCTS: Record<
  string,
  Array<{ name: string; subSubCategory: string; description: string; price: number }>
> = {
  cafes: [
    {
      name: 'Espresso',
  ...
  'qsr-fast-food': [
    ...
    {
      name: 'Chicken Tacos',
      subSubCategory: 'Tacos/Burritos',
      description: 'Spiced chicken in corn tortilla',
      price: 280,
    },
  ...
  'ice-cream-dessert': [
    ...
    },
    {
      name: 'Pista Kulfi',
      subSubCategory: 'Indian Desserts (Kulfi)',
      description: 'Pistachio flavored kulfi',
      price: 150,
    },
  ],
...
async function fixProductAssignments() {
  try {
    logger.info('🚀 Starting product-store assignment fix...');
    ...
    const stores = await db
      .collection('stores')
      .find({ subcategorySlug: { $exists: true } })
      .toArray();
    logger.info(`📦 Found ${stores.length} stores with subcategory assigned\n`);
    ...
    for (const store of stores) {
      ...
      if (!validSubSubCategories) {
        logger.info(`⚠️ ${store.name}: No valid subSubCategories defined for '${subcategorySlug}'`);
        continue;
      }
      ...
      const invalidProducts = currentProducts.filter(
        (p) =>
          !validSubSubCategories.some(
            (valid) =>
      ...
      logger.info(`   Invalid products: ${invalidProducts.length}`);
      ...
      if (productTemplates && remainingProducts < 6) {
        ...
        for (let i = 0; i < productsToCreate; i++) {
          ...
          const newProduct = {
            ...
            images: [
              `https://images.unsplash.com/photo-${1500000000000 + Math.floor(Math.random() * 100000000)}?w=400&q=80`,
            ],
        ...
        logger.info(`   ✅ Created ${productsToCreate} new products`);
      }
    ...
    logger.info('========================================');
    logger.info(`Stores processed: ${totalUpdated}`);

...
async function main() {
  ...
  try {
    ...
    logger.info('├────────────────────────────┼───────┤');
    logger.info(`│ Stores Created             │ ${String(createdStores.length).padStart(5)} │`);
    if (skippedCount > 0) {
      logger.info(`│ Stores Skipped             │ ${String(skippedCount).padStart(5)} │`);
    }
    logger.info(`│ Total Stores in Category   │ ${String(totalStores).padStart(5)} │`);
    logger.info('└────────────────────────────┴───────┘');
    ...
    createdStores.forEach((store) => {
      ...
      if (tags.includes('italian') || tags.includes('pizza'))
        cuisineBreakdown['Italian'] = (cuisineBreakdown['Italian'] || 0) + 1;
      ...
      if (tags.includes('japanese') || tags.includes('sushi'))
        cuisineBreakdown['Japanese'] = (cuisineBreakdown['Japanese'] || 0) + 1;
    });

    if (Object.keys(cuisineBreakdown).length > 0) {
      logger.info('\nCuisine Breakdown:');
      Object.entries(cuisineBreakdown).forEach(([cuisine, count]) => {
        logger.info(`  ${cuisine.padEnd(20)} ${count} stores`);
      });
    }

...
async function fixSeededImages() {
  logger.info('🖼️ Fixing images for seeded stores & products...\n');
  ...
  for (const store of stores) {
    ...
    const _images = CATEGORY_IMAGES[subcatSlug] || { logo: 100, banner: 200, products: [300, 301, 302, 303] };
...
fixSeededImages().catch((err) => { console.error(err); process.exit(1); }); // D19: fail loud so CI catches broken seeds

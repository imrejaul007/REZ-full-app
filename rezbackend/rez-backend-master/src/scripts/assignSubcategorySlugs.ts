...
function determineSubcategorySlug(store: any): string | null {
  ...
  for (const [_section, slugMappings] of Object.entries(SUBCATEGORY_MAPPINGS)) {
...
async function assignSubcategorySlugs() {
  ...
  logger.info(`✅ Will update ${withSlug.length} stores with subcategorySlug`);
  logger.info(`⏭️  ${skipped.length} stores have no matching subcategory (but will get region)\n`);
  ...
  logger.info('📋 Update summary by subcategory:');
  for (const [slug, names] of Object.entries(slugGroups)) {
    logger.info(`  ${slug}: ${names.length} stores`);
  ...
  for (const [region, count] of Object.entries(regionGroups)) {
    logger.info(`  ${region}: ${count} stores`);
  }
  ...
  for (const update of updates) {
    ...
    await db!.collection('stores').updateOne({ _id: update.id }, { $set: setFields });
    updateCount++;
  }

  logger.info(`✅ Updated ${updateCount} stores with region field`);
  ...
  const regionDist = await db!
    .collection('stores')
    .aggregate([{ $group: { _id: '$region', count: { $sum: 1 } } }, { $sort: { count: -1 } }])
    .toArray();
...
assignSubcategorySlugs().catch((err) => { console.error(err); process.exit(1); }); // D19: fail loud so CI catches broken seeds

...
async function verifyGameProduction(): Promise<VerificationReport> {
  ...
  try {
    logger.info('🔍 Starting Game Production Readiness Verification...\n');
    ...
    logger.info(`✅ Database connected successfully (${report.database.connectionTime}ms)`);
    logger.info(`   URI: ${report.database.uri}\n`);

    // Step 2: Verify QuizQuestion Model
    logger.info('📊 Step 2: Verifying QuizQuestion Model...');
    try {
      ...
      const categoryAgg = await QuizQuestion.aggregate([{ $group: { _id: '$category', count: { $sum: 1 } } }]);
      categoryAgg.forEach((item) => {
        report.models.QuizQuestion.categories[item._id] = item.count;
      });

      // Get difficulty breakdown
      const difficultyAgg = await QuizQuestion.aggregate([{ $group: { _id: '$difficulty', count: { $sum: 1 } } }]);
      difficultyAgg.forEach((item) => {
        report.models.QuizQuestion.difficulties[item._id] = item.count;
      });

      logger.info(`✅ QuizQuestion Model verified`);
      logger.info(`   Total: ${totalQuizCount}, Active: ${activeQuizCount}`);
      logger.info(`   Categories:`, report.models.QuizQuestion.categories);
      logger.info(`   Difficulties:`, report.models.QuizQuestion.difficulties);
      ...
      logger.error('❌ QuizQuestion verification failed:', error);
    }
    logger.info('');

    // Step 3: Verify TriviaQuestion Model
    logger.info('📊 Step 3: Verifying TriviaQuestion Model...');
    try {
      ...
      // Get category breakdown
      const categoryAgg = await TriviaQuestion.aggregate([{ $group: { _id: '$category', count: { $sum: 1 } } }]);
      categoryAgg.forEach((item) => {
        report.models.TriviaQuestion.categories[item._id] = item.count;
      });

      // Get difficulty breakdown
      const difficultyAgg = await TriviaQuestion.aggregate([{ $group: { _id: '$difficulty', count: { $sum: 1 } } }]);
      difficultyAgg.forEach((item) => {
        report.models.TriviaQuestion.difficulties[item._id] = item.count;
      });

      logger.info(`✅ TriviaQuestion Model verified`);
      logger.info(`   Total: ${totalTriviaCount}, Active: ${activeTriviaCount}`);
      logger.info(`   Categories:`, report.models.TriviaQuestion.categories);
      logger.info(`   Difficulties:`, report.models.TriviaQuestion.difficulties);
      ...
      logger.error('❌ TriviaQuestion verification failed:', error);
    }
    logger.info('');

    // Step 4: Verify GameSession Model
    logger.info('📊 Step 4: Verifying GameSession Model...');
    try {
      ...
      const statusAgg = await GameSession.aggregate([{ $group: { _id: '$status', count: { $sum: 1 } } }]);
      statusAgg.forEach((item) => {
        report.models.GameSession.statusBreakdown[item._id] = item.count;
      });

      // Get game type breakdown
      const gameTypeAgg = await GameSession.aggregate([{ $group: { _id: '$gameType', count: { $sum: 1 } } }]);
      gameTypeAgg.forEach((item) => {
        report.models.GameSession.gameTypeBreakdown[item._id] = item.count;
      });

      logger.info(`✅ GameSession Model verified`);
      logger.info(`   Total: ${totalSessionCount}`);
      logger.info(`   Status:`, report.models.GameSession.statusBreakdown);
      logger.info(`   Game Types:`, report.models.GameSession.gameTypeBreakdown);
    } catch (error) {
      report.issues.push(`GameSession model verification failed: ${error}`);
      logger.error('❌ GameSession verification failed:', error);
    }
    logger.info('');

    // Step 5: Test QuizQuestion Static Methods
    logger.info('📊 Step 5: Testing QuizQuestion Static Methods...');
    ...
    try {
      ...
      logger.info(`✅ getRandomQuestions(5) returned ${randomQuestions.length} questions`);
    } catch (error) {
      ...
      logger.error(`❌ getRandomQuestions failed:`, error);
    }
    ...
    try {
      ...
      logger.info(`✅ getQuestionsByDifficulty('easy', 5) returned ${easyQuestions.length} questions`);
    } catch (error) {
      ...
      logger.error(`❌ getQuestionsByDifficulty failed:`, error);
    }
    ...
    try {
      ...
      logger.info(`✅ getQuestionsByCategory('shopping', 5) returned ${shoppingQuestions.length} questions`);
    } catch (error) {
      ...
      logger.error(`❌ getQuestionsByCategory failed:`, error);
    }
    logger.info('');

    // Step 6: Test TriviaQuestion Static Methods
    logger.info('📊 Step 6: Testing TriviaQuestion Static Methods...');
    ...
    try {
      ...
      logger.info(`✅ getTriviaByCategory('science', 3) returned ${scienceTrivia.length} questions`);
    } catch (error) {
      ...
      logger.error(`❌ getTriviaByCategory failed:`, error);
    }
    logger.info('');

    // Step 7: Test GameSession Static Methods
    logger.info('📊 Step 7: Testing GameSession Static Methods...');
    ...
    try {
      ...
      logger.info(`✅ expireSessions() executed successfully (${expireResult.modifiedCount} sessions expired)`);
    } catch (error) {
      report.staticMethods.GameSession.expireSessions.error = String(error);
      report.issues.push(`expireSessions failed: ${error}`);
      logger.error(`❌ expireSessions failed:`, error);
    }
    logger.info('');

    // Step 8: Check Cron Job Files
    logger.info('📊 Step 8: Verifying Cron Job Files...');
    const fs = require('fs');
    ...
    if (report.cronJobs.sessionCleanup.exists) {
      logger.info(`✅ cleanupExpiredSessions.ts exists at ${sessionCleanupPath}`);
    } else {
      report.issues.push('cleanupExpiredSessions.ts not found');
      logger.error(`❌ cleanupExpiredSessions.ts not found at ${sessionCleanupPath}`);
    }

    if (report.cronJobs.coinExpiry.exists) {
      logger.info(`✅ expireCoins.ts exists at ${coinExpiryPath}`);
    } else {
      report.issues.push('expireCoins.ts not found');
      logger.error(`❌ expireCoins.ts not found at ${coinExpiryPath}`);
    }
    logger.info('');

    // Step 9: Check server.ts for cron job initialization
    logger.info('📊 Step 9: Checking server.ts for cron job initialization...');
    ...
    if (fs.existsSync(serverPath)) {
      ...
      if (hasSessionCleanupInit) {
        logger.info('✅ initializeSessionCleanupJob() found in server.ts');
      } else {
        report.issues.push('initializeSessionCleanupJob() not called in server.ts');
        logger.error('❌ initializeSessionCleanupJob() not called in server.ts');
      }

      if (hasCoinExpiryInit) {
        logger.info('✅ initializeCoinExpiryJob() found in server.ts');
      } else {
        report.issues.push('initializeCoinExpiryJob() not called in server.ts');
        logger.error('❌ initializeCoinExpiryJob() not called in server.ts');
      }
    } else {
      report.issues.push('server.ts not found');
      logger.error('❌ server.ts not found');
    }
    logger.info('');
    ...
    if (report.productionReady) {
      ...
      logger.info('   All game features are properly configured and working.');
    } else {
      logger.info('⚠️  STATUS: NOT PRODUCTION READY');
      logger.info(`   ${report.issues.length} issue(s) found:`);
    ...
    logger.info('\n📊 SUMMARY:');
    logger.info(
      `   Quiz Questions: ${report.models.QuizQuestion.totalCount} total, ${report.models.QuizQuestion.activeCount} active`,
    );
    logger.info(
      `   Trivia Questions: ${report.models.TriviaQuestion.totalCount} total, ${report.models.TriviaQuestion.activeCount} active`,
    );
    logger.info(`   Game Sessions: ${report.models.GameSession.totalCount} total`);
    logger.info(`   Database Connection: ${report.database.connectionTime}ms`);
    logger.info('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
  } catch (error) {

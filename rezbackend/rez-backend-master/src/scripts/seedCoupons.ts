...
async function seedCoupons() {
  try {
    logger.info('🎫 Starting Coupon Seeding...');
    logger.info('=====================================\n');
    ...
    logger.info('🗑️  Clearing existing coupons...');
    ...
    logger.info('🔄 Creating user coupons...');
    ...
    logger.info(`✅ Created ${userCoupons.length} user coupons\n`);
    ...
    logger.info('\n=====================================');
    logger.info('🎉 Coupon seeding completed successfully!');
    logger.info('=====================================');
    logger.info('\n📊 Summary:');
    logger.info(`🎫 Total Coupons: ${coupons.length}`);
    logger.info(`   - Active: ${coupons.filter((c) => c.status === 'active').length}`);
    logger.info(`   - Expired: ${coupons.filter((c) => c.status === 'expired').length}`);
    logger.info(`   - Featured: ${coupons.filter((c) => c.isFeatured).length}`);
    logger.info(`👥 User Coupons: ${userCoupons.length}`);
    logger.info(`   - Available: ${userCoupons.filter((uc: any) => uc.status === 'available').length}`);
    logger.info(`   - Used: ${userCoupons.filter((uc: any) => uc.status === 'used').length}`);
    logger.info(`   - Expired: ${userCoupons.filter((uc: any) => uc.status === 'expired').length}`);

    logger.info('\n🔗 Coupon Codes Created:');
    coupons.forEach((coupon) => {
      logger.info(`   - ${coupon.couponCode}: ${coupon.title} (${coupon.status})`);
    });

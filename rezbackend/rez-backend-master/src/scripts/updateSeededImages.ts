...
async function updateSeededImages() {
  logger.info('🖼️ Updating images for seeded stores & products...\n');
  ...
  logger.info(`📦 Found ${stores.length} stores to update\n`);
  ...
  for (const store of stores) {
    ...
    storesUpdated++;
    logger.info(`  ✅ Updated store: ${store.name}`);
    ...
    logger.info(`    📸 Updated ${products.length} product images`);
  }

  logger.info('\n' + '='.repeat(50));
  logger.info('🎉 Image update complete!');
  logger.info(`   📦 Stores updated: ${storesUpdated}`);
  logger.info(`   🏷️ Products updated: ${productsUpdated}`);
...
updateSeededImages().catch((err) => { console.error(err); process.exit(1); }); // D19: fail loud so CI catches broken seeds

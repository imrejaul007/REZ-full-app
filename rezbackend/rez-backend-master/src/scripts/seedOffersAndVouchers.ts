...
const seedOffers = async (): Promise<void> => {
  try {
    ...
    logger.error('❌ Error seeding offers:', error);
    throw error;
  }
};
...
const seedVoucherBrands = async () => {
  try {
    ...
    logger.info('🗑️  Cleared existing voucher brands');
    ...
    const voucherBrandsData = [
      ...
      },
      {
        ...
        termsAndConditions: ['Valid for 3 months', 'Minimum order value ₹500', 'Delivery charges may apply'],
        purchaseCount: 1876,
        viewCount: 10234,
      },
      {
        ...
        termsAndConditions: ['Valid for 1 year', 'Applicable in-store and online', 'Check for product exclusions'],
        purchaseCount: 987,
        viewCount: 6543,
      },
      {
        ...
        isActive: true,
        denominations: [500, 1000, 2000, 5000],
        termsAndConditions: ['Valid for 1 year', 'Can be used at all Decathlon stores', 'Online redemption available'],
        purchaseCount: 1234,
        viewCount: 7890,
      },
      {
        ...
        termsAndConditions: ['Valid for 2 months', 'Applicable at all outlets', 'Cannot be used with other vouchers'],
        purchaseCount: 2456,
        viewCount: 13456,
      },
    ];
    ...
    logger.info(`✅ Created ${brands.length} voucher brands`);

    return brands;
  } catch (error) {
    logger.error('❌ Error seeding voucher brands:', error);
    throw error;
  }
};
...
const seedAll = async () => {
  try {
    logger.info('🌱 Starting Offers & Vouchers Seeding Process...\n');
    ...
    logger.info(`📊 Summary:`);
    logger.info(`   - Offers: Complete`);
    logger.info(`   - Voucher Brands: Complete`);

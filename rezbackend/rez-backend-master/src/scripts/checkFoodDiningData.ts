  ...
  try {
    ...
    if (exists) {
      ...
      if (fieldPath === 'banner') {
        ...
        const _expectedType = fieldConfig.type.replace('|string', '').replace('|', ' or ');
...
async function checkStores(categoryId: mongoose.Types.ObjectId): Promise<AuditSummary> {
  ...
  try {
    ...
    // Display results
    logger.info('\n📊 Store Audit Results:');
    logger.info(`   Stores with all fields (100%): ${storesWithAllFields}/${activeStores.length}`);
    logger.info(`   Stores needing migration: ${storesNeedingMigration}`);
    logger.info(`   Stores needing seeding: ${storesNeedingSeeding}`);

    // Field coverage table
    log.section('Field Coverage');
    logger.info('\n┌──────────────────────────────────────┬─────────┬───────────┬────────────┐');
    logger.info('│ Field                                 │ Populated│ Total     │ Coverage   │');
    logger.info('├──────────────────────────────────────┼─────────┼───────────┼────────────┤');
    ...
    for (const [fieldPath, coverage] of Object.entries(fieldCoverage)) {
      ...
      logger.info(`│ ${fieldName} │ ${populated} │ ${total} │ ${percentage} ${status} │`);
    }
    ...
    if (activeStores.length > 0) {
      ...
      logger.info(`Score: ${sampleAudit.score}%`);
      ...
      sampleAudit.fields.forEach((field) => {
        ...
        logger.info(`   ${status} ${field.field.padEnd(30)} ${field.populated ? 'OK' : 'MISSING'}${issues}`);
      });
      ...
      if (sampleStore.tags && Array.isArray(sampleStore.tags)) {
        ...
        logger.info(`   Tag samples: ${tags.slice(0, 5).join(', ')}`);
    ...
    if (storesNeedingSeeding > 0) {
      recommendations.push(
        `${storesNeedingSeeding} stores need complete data - Run seed script: npx ts-node src/scripts/seedFoodDiningStores.ts`,
      );
    }

    if (storesNeedingMigration > 0) {
      recommendations.push(
        `${storesNeedingMigration} stores need data migration - Update banner format, add tags, etc.`,
      );
    }
...
async function main() {
  ...
  try {
    ...
    logger.info('├──────────────────────────────────────┼──────────────┤');
    logger.info(`│ Category Exists                      │ ${summary.categoryExists ? '✓ Yes' : '✗ No'.padEnd(12)} │`);
    logger.info(`│ Total Stores                         │ ${String(summary.totalStores).padStart(12)} │`);
    logger.info(`│ Active Stores                        │ ${String(summary.activeStores).padStart(12)} │`);
    logger.info(`│ Stores with All Fields (100%)        │ ${String(summary.storesWithAllFields).padStart(12)} │`);
    logger.info(`│ Stores Needing Migration             │ ${String(summary.storesNeedingMigration).padStart(12)} │`);
    logger.info(`│ Stores Needing Seeding               │ ${String(summary.storesNeedingSeeding).padStart(12)} │`);
    logger.info('└──────────────────────────────────────┴──────────────┘');

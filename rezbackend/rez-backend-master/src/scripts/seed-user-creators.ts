...
const INDIAN_CITIES = [
  { city: 'Mumbai', state: 'Maharashtra', coordinates: [72.8777, 19.076] as [number, number] },
  { city: 'Delhi', state: 'Delhi', coordinates: [77.209, 28.6139] as [number, number] },
  { city: 'Bangalore', state: 'Karnataka', coordinates: [77.5946, 12.9716] as [number, number] },
  { city: 'Hyderabad', state: 'Telangana', coordinates: [78.4867, 17.385] as [number, number] },
...
async function seedUserCreators() {
  try {
    logger.info('🚀 Starting UGC Content Creator Users seeding...');
    ...
    logger.info('🧹 Checking for existing creator users...');
    const existingCreators = await User.countDocuments({
      'profile.bio': { $regex: /fashion|beauty|lifestyle|tech/i },
    });

    if (existingCreators > 0) {
      logger.info(`⚠️  Found ${existingCreators} existing creator users`);
      logger.info('💡 Skipping deletion to preserve existing data');
      logger.info('💡 To clear existing creators, uncomment the deleteMany line\n');
      // await User.deleteMany({ 'profile.bio': { $regex: /fashion|beauty|lifestyle|tech/i } });
    }

    // Create creator users
    logger.info('👥 Creating UGC content creator users...\n');
    ...
    for (let i = 0; i < CREATOR_PROFILES.length; i++) {
      ...
      try {
        ...
        if (existingUser) {
          logger.info(
            `⏭️  [${i + 1}/${CREATOR_PROFILES.length}] User already exists: ${profile.firstName} ${profile.lastName}`,
          );
        ...
        errorCount++;
        logger.error(
          `❌ [${i + 1}/${CREATOR_PROFILES.length}] Error creating ${profile.firstName} ${profile.lastName}:`,
          error.message,
        );
      }
    }
    ...
    logger.info('\n' + '='.repeat(80));
    logger.info('📊 UGC Content Creator Users Seeding Summary');
    logger.info('='.repeat(80));

    // Category breakdown
    const categoryStats = {
      fashion: createdUsers.filter((u) =>
        CREATOR_PROFILES.find((p) => p.firstName === u.profile?.firstName && p.category === 'fashion'),
      ).length,
      beauty: createdUsers.filter((u) =>
        CREATOR_PROFILES.find((p) => p.firstName === u.profile?.firstName && p.category === 'beauty'),
      ).length,
      lifestyle: createdUsers.filter((u) =>
        CREATOR_PROFILES.find((p) => p.firstName === u.profile?.firstName && p.category === 'lifestyle'),
      ).length,
      tech: createdUsers.filter((u) =>
        CREATOR_PROFILES.find((p) => p.firstName === u.profile?.firstName && p.category === 'tech'),
      ).length,
    };
    ...
    logger.info('\n📁 Category Breakdown:');
    logger.info(`   👗 Fashion Influencers: ${categoryStats.fashion}`);
    logger.info(`   💄 Beauty Creators: ${categoryStats.beauty}`);
    logger.info(`   🌟 Lifestyle Bloggers: ${categoryStats.lifestyle}`);
    logger.info(`   📱 Tech Reviewers: ${categoryStats.tech}`);
    ...
    logger.info('\n👥 Demographics:');
    logger.info(`   Female: ${genderStats.female}`);
    logger.info(`   Male: ${genderStats.male}`);
    ...
    logger.info('\n💡 Next Steps:');
    logger.info('   1. Run seed-ugc-videos.ts to create UGC videos for these users');
    logger.info('   2. Test user API endpoints: GET /api/users');
    logger.info('   3. Verify user profiles in database');

    logger.info('\n📋 Sample User IDs for reference:');
    createdUsers.slice(0, 3).forEach((user) => {
      logger.info(`   ${user._id} - ${user.profile?.firstName} ${user.profile?.lastName} (${user.phoneNumber})`);
    });

...
async function seedCashback() {
  try {
    logger.info('🌱 Starting Cashback Seed...\n');
    ...
    logger.info('👥 Fetching users and orders...');
    const users = await User.find({}).limit(5);
    const orders = await Order.find({}).limit(10);

    logger.info(`✅ Found ${users.length} users and ${orders.length} orders\n`);
    ...
    logger.info('🗑️  Clearing existing cashback...');
    await UserCashback.deleteMany({});
    logger.info('✅ Cleared existing cashback\n');

    // Create cashback transactions
    logger.info('💰 Creating cashback transactions...');
    const cashbackData = [];
    ...
    logger.info(`✅ Created ${cashbackData.length} cashback transactions\n`);
    ...
    logger.info('📊 Cashback Statistics:');
    stats.forEach((stat) => {
      logger.info(`   ${stat._id}: ${stat.count} transactions, ₹${stat.totalAmount}`);
    });
    ...
    if (sample) {
      ...
      logger.info(`   Amount: ₹${sample.amount}`);
      logger.info(`   Source: ${sample.source}`);
      logger.info(`   Status: ${sample.status}`);
      logger.info(`   Description: ${sample.description}`);
      logger.info(`   Expires: ${sample.expiryDate.toLocaleDateString()}`);
    }

    logger.info('\n✅ Cashback Seed Complete!\n');
    process.exit(0);
  } catch (error) {
    logger.error('❌ Error seeding cashback:', error);
    process.exit(1);
  }
}

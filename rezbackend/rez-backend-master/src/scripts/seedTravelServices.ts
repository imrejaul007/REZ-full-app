  ...
  for (const storeData of stores) {
    ...
    if (!store) {
      ...
      const generalCategory =
        (await Category.findOne({ slug: 'travel-experiences' })) || (await Category.findOne({ slug: 'general' }));
...
async function main(): Promise<void> {
  ...
  try {
    ...
    logger.info('├────────────────────────────┼───────┤');
    logger.info(`│ Travel Categories          │ ${String(categories.length).padStart(5)} │`);
    logger.info(`│ Travel Provider Stores     │ ${String(stores.length).padStart(5)} │`);
    logger.info(`│ Travel Services            │ ${String(servicesCount).padStart(5)} │`);
    logger.info('└────────────────────────────┴───────┘');

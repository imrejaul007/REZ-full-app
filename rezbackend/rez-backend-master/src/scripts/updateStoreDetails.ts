...
async function updateStoreDetails() {
  try {
    logger.info('🚀 Starting store details update...');
    ...
    for (let i = 0; i < stores.length; i++) {
      ...
      if (!categorySlug || !STORE_TEMPLATES[categorySlug]) {
        logger.info(`⚠️  Skipping store ${store._id} - no valid category`);
        continue;
      }
      ...
      logger.info(`✅ Updated: ${newName} (${categorySlug})`);
    }
    ...
    for (const [category, count] of Object.entries(categoryStats).sort((a, b) => b[1] - a[1])) {
      logger.info(`   ${category}: ${count} stores`);
    }
  } catch (error) {

...
if (process.env.NODE_ENV === 'production') {
  console.error('ERROR: seedDemoData must not be run in production. Exiting.');
  process.exit(1);
}
  ...
  try {
    ...
    if (adminUser) {
      ...
      if (adminUser.role !== 'admin') {
        ...
        logger.info('   ✅ Updated to admin role');
      }
    } else {
    ...
    if (customerUser) {
      logger.info('   ⚠️  Customer user already exists');
      ...
      if (existingWallet) {
        ...
        if (!existingWallet.coins || existingWallet.coins.length === 0) {
          ...
          logger.info('   ✅ Customer wallet updated with correct coin structure');
        }
      }
    } else {
      ...
      logger.info('   ✅ Customer wallet created with 500 Rez Coins + 100 Promo Coins');
    }
    ...
    if (merchantUser) {
      logger.info('   ⚠️  Merchant user already exists');
    } else {
    ...
    logger.info('🏪 Creating Merchant & Store...');
    ...
    if (!merchant) {
      ...
      logger.warn(`[SEED] Demo merchant password for this run: ${seedPassword} — record this now.`);
      ...
      logger.info('   ⚠️  Merchant already exists');
    }
    ...
    if (!store) {
      store = await Store.create({
        ...
        logo: 'https://placehold.co/200x200?text=Demo+Store',
        coverImage: 'https://placehold.co/800x400?text=Demo+Fashion+Store',
        status: 'active',
        isVerified: true,
        location: {
          ...
          state: 'Maharashtra',
          pincode: '400001',
          coordinates: [72.8777, 19.076], // [longitude, latitude] Mumbai
        },
        contact: {
          phone: '+919123456789',
      ...
      logger.info('   ⚠️  Store already exists');
      // Update merchantId reference if not set
      if (!store.merchantId) {
        store.merchantId = merchant._id as any;
        await store.save();
        logger.info('   ✅ Updated store merchantId reference');
      }
    }
    ...
    const products = [
      ...
      },
      {
        ...
        sku: 'DRESS-001',
        category: fashionCategory?._id,
        images: ['https://placehold.co/400x400?text=Dress'],
        pricing: {
          original: 2499,
          selling: 1499,
          currency: 'INR',
    ...
    for (const prod of products) {
      ...
      if (!exists) {
        ...
        logger.info(`   ✅ Product "${prod.name}" created`);
      } else {
        logger.info(`   ⚠️  Product "${prod.name}" already exists`);
      }
    }

    // ============ 7. CREATE CAMPAIGNS/DEALS ============
    logger.info('🎯 Creating Campaigns/Deals...');
        ...
        deals: [
          {
            ...
            storeId: store._id,
            image: 'https://placehold.co/400x200?text=Summer+Sale',
            cashback: '10%',
            discount: '50%',
          },
        ],
        ...
        deals: [
          {
            store: 'Demo Fashion Store',
            storeId: store._id,
            image: 'https://placehold.co/400x200?text=Triple+Coins',
            coins: '3X',
          },
        ],
        startTime: new Date(),
        ...
        deals: [
          {
            store: 'Demo Fashion Store',
            storeId: store._id,
            image: 'https://placehold.co/400x200?text=Flash+Deal',
            discount: '60%',
            endsIn: '2 days',
          },
        ],
    ...
    for (const campaign of campaigns) {
      ...
      if (!exists) {
        try {
          ...
          logger.info(`   ✅ Campaign "${campaign.title}" created`);
        } catch (err: any) {
          logger.info(`   ⚠️  Campaign "${campaign.title}" skipped: ${err.message}`);
        }
      } else {
        logger.info(`   ⚠️  Campaign "${campaign.title}" already exists`);
      }
    }

    // ============ 8. CREATE MERCHANT WALLET ============
    logger.info('💰 Creating Merchant Wallet...');
    ...
    if (!merchantWallet) {
      ...
      logger.info('   ✅ Merchant wallet created');
    } else {
      logger.info('   ⚠️  Merchant wallet already exists');
    }
    ...
    logger.info('─'.repeat(60));
    logger.info('');
    logger.info('🔴 ADMIN PORTAL (rez-admin - port 8083):');
    logger.info(`   Email:    ${ADMIN_USER.email}`);
    logger.info(`   Password: [REDACTED — check ADMIN_USER constant or .env]`);
    logger.info('');
    logger.info('🟣 MERCHANT PORTAL (rez-merchant - port 8082):');
    logger.info(`   Email:    merchant@rez.app`);
    logger.info(`   Password: [REDACTED — check seed constants or .env]`);
    logger.info('');
    logger.info('🟢 CUSTOMER APP (rez-frontend - port 8081):');
    logger.info(`   Phone:    ${CUSTOMER_USER.phoneNumber}`);
    logger.info(`   Password: [REDACTED — check CUSTOMER_USER constant or .env]`);
    logger.info(`   Email:    ${CUSTOMER_USER.email}`);
    logger.info('');
    logger.info('─'.repeat(60));
    logger.info('');
    logger.info('🎯 DEMO FLOW TEST:');
    logger.info('1. Open Customer App → Search "Summer Sale" or "Fashion"');
    logger.info('2. Add products to cart and checkout');
    logger.info('3. Merchant receives order in Merchant Portal');
    logger.info('4. Admin sees order in Admin Portal');
    logger.info('5. Customer gets 5% coins automatically');
    logger.info('6. Customer shares purchase → Gets 5% pending coins');
    logger.info('7. Admin approves share coins in Coin Rewards tab');
    logger.info('8. Merchant wallet shows sales with 15% fee deduction');
    logger.info('');
    logger.info('═'.repeat(60));
  } catch (error) {

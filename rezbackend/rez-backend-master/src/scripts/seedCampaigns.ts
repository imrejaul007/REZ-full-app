...
async function seedCampaigns(): Promise<number> {
  ...
  const bangaloreCampaigns = [
    {
      ...
      terms: ['Minimum order value: ₹500', 'Cashback credited within 24 hours', 'Valid only on weekends'],
    },
    {
  ...
  const dubaiCampaigns = [
    {
      ...
      priority: 100,
      eligibleCategories: ['electronics', 'fashion', 'home', 'beauty'],
      terms: ['Minimum order value: AED 100', 'Cashback credited within 24 hours', 'Valid only on weekends'],
    },
    {
  ...
  for (const campaignData of campaigns) {
    try {
      const _campaign = await Campaign.findOneAndUpdate({ campaignId: campaignData.campaignId }, campaignData, {
        upsert: true,
        new: true,
      });
      createdCount++;
...
async function main(): Promise<void> {
  ...
  try {
    ...
    logger.info('├────────────────────────────┼───────┤');
    logger.info(`│ Campaigns                  │ ${String(campaignsCount).padStart(5)} │`);
    logger.info('└────────────────────────────┴───────┘');

...
function generateCityMapping(config: NewRegionConfig): string {
  return config.cities.map((city) => `  '${city.toLowerCase()}': '${config.id}',`).join('\n');
}
...
${config.cities
  ...
  .map(
    (city, i) =>
      `    { name: '${city}', coords: [${config.defaultCoordinates.longitude + i * 0.01}, ${config.defaultCoordinates.latitude + i * 0.01}], pincode: '${100000 + i}' },`,
  )
  .join('\n')}
  ],
};
...
async function seed${config.name}Stores() {
  ...
  if (storesToInsert.length > 0) {
    ...
    logger.info(\`✅ Inserted \${storesToInsert.length} stores for ${config.name}\`);
  }
...
seed${config.name}Stores().catch((err) => { console.error(err); process.exit(1); }); // D19: fail loud so CI catches broken seeds
`;
}
...
async function seed${config.name}Events() {
  ...
  if (eventsToInsert.length > 0) {
    ...
    logger.info(\`✅ Inserted \${eventsToInsert.length} events for ${config.name}\`);
  }
...
seed${config.name}Events().catch((err) => { console.error(err); process.exit(1); }); // D19: fail loud so CI catches broken seeds
`;
}
...
async function generateRegionFiles(config: NewRegionConfig) {
  ...
  logger.info(`\n${'='.repeat(60)}`);
  logger.info(`🌍 Generating files for region: ${config.name} (${config.id})`);
  logger.info(`${'='.repeat(60)}\n`);

  // 1. Backend region config
  logger.info('📁 1. BACKEND CONFIG (rez-backend/src/config/regions.ts)');
  logger.info('   Add to RegionId type:');
  logger.info(`   export type RegionId = 'bangalore' | 'dubai' | 'china' | '${config.id}';`);
  logger.info('\n   Add to REGIONS object:');
  logger.info(generateBackendRegionConfig(config));
  logger.info('\n   Add to getRegionFromCoordinates():');
  logger.info(generateCoordinateDetection(config));
  ...
  logger.info(generateFrontendRegionConfig(config));
  logger.info('\n   Update isValidRegion():');
  logger.info(`   return ['bangalore', 'dubai', 'china', '${config.id}'].includes(region);`);

  // 3. City mapping
  logger.info('\n📁 3. CITY MAPPING (rez-backend/src/scripts/assignSubcategorySlugs.ts)');
  logger.info('   Add to CITY_TO_REGION:');
  logger.info(generateCityMapping(config));
  ...
  logger.info(`   ✅ ${storesSeedFile}`);
  logger.info(`   ✅ ${eventsSeedFile}`);
...
async function interactiveMode() {
  ...
  logger.info('Available sample regions:');
  Object.keys(SAMPLE_REGIONS).forEach((key, i) => {
    const region = SAMPLE_REGIONS[key];
    logger.info(`  ${i + 1}. ${key} - ${region.displayName} (${region.currency})`);
  });
  logger.info(`  ${Object.keys(SAMPLE_REGIONS).length + 1}. Custom - Enter your own region details\n`);
  ...
  if (choiceNum > 0 && choiceNum <= regionKeys.length) {
    ...
    const customRegion: NewRegionConfig = {
      ...
      countries: (await question('Countries (comma-separated, e.g., "Singapore,SG,SGP"): '))
        .split(',')
        .map((s) => s.trim()),
      defaultCoordinates: {
      ...
      deliveryRadius: parseInt(await question('Delivery Radius (km, 10): ')) || 15,
    };
...
interactiveMode().catch((err) => { console.error(err); process.exit(1); }); // D19: fail loud so CI catches broken seeds

...
async function seedMallData() {
  try {
    ...
    // Clear existing data
    logger.info('🗑️ Clearing existing mall data...');
    await Promise.all([
    ...
    const brandsWithRefs = brandsData.map((brand) => ({
      ...
      collections:
        brand.tier === 'premium' || brand.tier === 'exclusive'
    ...
    const cleanBrands = brandsWithRefs.map(({ categorySlug: _categorySlug, ...rest }) => rest);
    ...
    logger.info('📊 Updating category brand counts...');
    ...
    logger.info('✅ Category brand counts updated');
    ...
    logger.info(`🖼️ Banners: ${banners.length}`);
    logger.info('========================================\n');
  } catch (error) {
    logger.error('❌ Error seeding mall data:', error);
    throw error;
  } finally {
    await disconnectDb();

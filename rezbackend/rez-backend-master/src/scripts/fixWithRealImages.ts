...
async function fixWithRealImages() {
  logger.info('🖼️ Fixing images with REAL category-specific images...\n');
  ...
  for (const store of stores) {
    ...
    for (const product of products) {
      ...
      let imageUrl =
        DEFAULT_CATEGORY_IMAGES[subcatSlug] ||
  ...
  logger.info('\n' + '='.repeat(50));
  logger.info('🎉 Real image update complete!');
...
fixWithRealImages().catch((err) => { console.error(err); process.exit(1); }); // D19: fail loud so CI catches broken seeds

...
const _BANGALORE_CENTER = [77.5946, 12.9716]; // [lng, lat]

// Dubai coordinates
const _DUBAI_CENTER = [55.2708, 25.2048]; // [lng, lat]
    ...
    const baseOffers = [
      // Mega Offers
      {
        ...
        store: {
          id: store._id,
          name: store.name,
          logo: store.logo || 'https://placehold.co/100',
...
async function seedOffers() {
  ...
  try {
    ...
    const bangaloreStores = (await Store.find({
      ...
      .select('_id name logo rating location')
      .lean()) as unknown as StoreInfo[];
    ...
    const dubaiStores = (await Store.find({
      isActive: true,
      $or: [{ 'location.city': { $regex: /dubai/i } }, { 'address.city': { $regex: /dubai/i } }],
    })
      .select('_id name logo rating location')
      .lean()) as unknown as StoreInfo[];
    ...
    logger.info('┌────────────────────────┬───────────┬────────────┐');
    logger.info('│ Region                 │ Stores    │ Offers     │');
    logger.info('├────────────────────────┼───────────┼────────────┤');
    logger.info(
      `│ Bangalore (INR)        │ ${String(bangaloreStores.length).padStart(9)} │ ${String(bangaloreOffersCount).padStart(10)} │`,
    );
    logger.info(
      `│ Dubai (AED)            │ ${String(dubaiStores.length).padStart(9)} │ ${String(dubaiOffersCount).padStart(10)} │`,
    );
    logger.info('├────────────────────────┼───────────┼────────────┤');
    logger.info(
      `│ Total                  │ ${String(bangaloreStores.length + dubaiStores.length).padStart(9)} │ ${String(bangaloreOffersCount + dubaiOffersCount).padStart(10)} │`,
    );
    logger.info('└────────────────────────┴───────────┴────────────┘');
    ...
    categoryDistribution.forEach((cat) => {
      logger.info(`  ${cat._id}: ${cat.count} offers`);
    });
  } catch (error: any) {

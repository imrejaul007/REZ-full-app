...
const _TIER_GRADIENTS: Record<string, [string, string]> = {
...
async function seedMallBrands() {
  try {
    logger.info('========================================');
    logger.info('   MallBrand & MallCategory Seeder');
    logger.info('========================================\n');
    ...
    logger.info('--- Step 1: Checking MallCategory Collection ---');
    const existingCategories = await MallCategory.countDocuments();
    logger.info(`Found ${existingCategories} existing categories`);

    const categoryMap: Record<string, mongoose.Types.ObjectId> = {};

    if (existingCategories === 0) {
      logger.info('No categories found. Seeding categories...\n');
      ...
      for (const categoryData of CATEGORIES_DATA) {
        ...
        logger.info(`  Created category: ${categoryData.name} (${category._id})`);
      }

      logger.info(`\nSeeded ${CATEGORIES_DATA.length} categories`);
    } else {
      logger.info('Categories already exist. Fetching existing categories...\n');

      const categories = await MallCategory.find({});
      for (const cat of categories) {
        categoryMap[cat.slug] = cat._id;
        logger.info(`  Found category: ${cat.name} (${cat._id})`);
      }
      ...
      for (const categoryData of CATEGORIES_DATA) {
        if (!categoryMap[categoryData.slug]) {
          ...
          categoryMap[categoryData.slug] = category._id;
          logger.info(`  Created missing category: ${categoryData.name} (${category._id})`);
        }
      }
    }
    ...
    logger.info(`Found ${existingBrands} existing brands`);

    if (existingBrands === 0) {
      logger.info('No brands found. Seeding brands...\n');

      for (const brandData of BRANDS_DATA) {
        const categoryId = categoryMap[brandData.categorySlug];

        if (!categoryId) {
          logger.info(`  Skipping ${brandData.name}: Category '${brandData.categorySlug}' not found`);
          continue;
        }
        ...
        logger.info(`  Created brand: ${brandData.name} (${brand._id}) - ${brandData.tier}`);
      }

      logger.info(`\nSeeded ${BRANDS_DATA.length} brands`);
    } else {
      logger.info('Brands already exist. Checking for missing brands...\n');
      ...
      for (const brandData of BRANDS_DATA) {
        if (!existingBrandSlugs.includes(brandData.slug)) {
          ...
          logger.info(`  Created missing brand: ${brandData.name} (${brand._id})`);
        } else {
          logger.info(`  Brand exists: ${brandData.name}`);
        }
      }
    }
    ...
    for (const [slug, categoryId] of Object.entries(categoryMap)) {
      ...
      logger.info(`  ${slug}: ${brandCount} brands`);
    }
    ...
    logger.info(`\nFinal counts:`);
    logger.info(`  - Categories: ${finalCategoryCount}`);
    logger.info(`  - Total Brands: ${finalBrandCount}`);
    logger.info(`  - Featured Brands: ${featuredBrandCount}`);
    ...
    featuredBrands.forEach((brand: any) => {
      logger.info(`  - ${brand.name} (${brand.tier}) - ${brand.cashback.percentage}% cashback`);
    });

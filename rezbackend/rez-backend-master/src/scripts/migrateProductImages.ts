...
async function migrateProductImages() {
  logger.info('🖼️  Product Image Migration');
  ...
  try {
    ...
    logger.info(`Found ${brokenProducts.length} products with expired Google thumbnail images.\n`);

    if (brokenProducts.length === 0) {
      logger.info('✅ No broken images found. All good!');
      return;
    }
    ...
    for (const product of brokenProducts) {
      ...
      await Product.updateOne({ _id: product._id }, { $set: { images: [newImageUrl] } });

      logger.info(`  ✅ ${product.name} → ${newImageUrl.substring(0, 70)}...`);
      updated++;
    }

    logger.info('');
    logger.info('─'.repeat(60));
    logger.info(`📊 Updated ${updated}/${brokenProducts.length} products with working images.`);
    logger.info('─'.repeat(60));
  } catch (error) {

...
async function seedWishlists() {
  try {
    ...
    logger.info('🗑️  Cleared existing wishlists');
    ...
    for (let i = 0; i < Math.min(users.length, 4); i++) {
      ...
      if (userProducts.length > 0) {
        ...
        wishlists.push({
          user: user._id,
          items: wishlistItems,
          name: ['My Favorites', 'Want to Buy', 'Gift Ideas', 'Future Purchases', 'Birthday Wishlist'][i % 5],
          description: [
    ...
    for (let i = 0; i < createdWishlists.length; i++) {
      ...
      logger.info(`  ${i + 1}. "${wishlist.name}" by ${userName} - ${wishlist.items.length} items ${isPublic}`);
      ...
      for (let j = 0; j < Math.min(wishlist.items.length, 2); j++) {
        ...
        logger.info(`     - ${productName} (₹${item.priceWhenAdded}) - ${item.priority} priority`);
      }
    }

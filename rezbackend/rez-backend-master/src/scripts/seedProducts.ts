...
const createDefaultData = async () => {
  ...
  for (const store of stores) {
    await Store.findOneAndUpdate({ slug: store.slug }, store, { upsert: true, new: true });
  }

  logger.info('✅ Created default categories and stores');
};
...
const seedProducts = async () => {
  try {
    ...
    for (const productData of productsToSeed) {
      const { categoryName, subcategoryName: _subcategoryName, storeName, ...productFields } = productData;
    ...
    logger.info('🎉 Successfully seeded all products!');
    ...
    logger.info(`Featured Products: ${featuredProducts}`);

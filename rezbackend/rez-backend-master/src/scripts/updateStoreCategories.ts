...
async function updateStoreCategories() {
  try {
    logger.info('🚀 Starting Store Category Update...');
    ...
    logger.info(`📦 Processing ${stores.length} stores...\n`);
    ...
    for (const store of stores) {
      ...
      if (!subcategorySlug) {
        logger.info(`   ⚠️ ${store.name}: No subcategorySlug assigned`);
      ...
      if (!parentSlug) {
        logger.info(`   ⚠️ ${store.name}: Unknown subcategory '${subcategorySlug}'`);
      ...
      if (!mainCategoryId) {
        logger.info(`   ⚠️ ${store.name}: Main category '${parentSlug}' not found in DB`);
      ...
      if (!subcategoryId) {
        logger.info(`   ⚠️ ${store.name}: Subcategory '${subcategorySlug}' not found in DB`);
      ...
      logger.info(`   ✅ ${store.name}: ${parentSlug} → ${subcategorySlug}`);
      updatedCount++;
    }
    ...
    logger.info('📊 VERIFICATION (Sample Stores):');
    ...
    for (const store of sampleStores) {
      ...
      logger.info(`      subcategorySlug: ${store.subcategorySlug}`);
    }
  } catch (error) {
...
updateStoreCategories()
  .then(() => {
    logger.info('✅ Store category update completed successfully!');
    process.exit(0);
  })
  .catch((error) => {

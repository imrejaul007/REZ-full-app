...
async function fixCategoryImages() {
  ...
  for (const [slug, images] of Object.entries(categoryImages)) {
    ...
    if (result.matchedCount > 0) {
      logger.info(`  Updated: ${slug}`);
      updated++;
    } else {
      logger.info(`  Not found: ${slug}`);
      notFound++;
    }
  }
  ...
  if (sample) {
    logger.info(`\nSample: ${sample.name}`);
    logger.info(`  image: ${sample.image?.substring(0, 50)}...`);
    logger.info(`  bannerImage: ${sample.bannerImage?.substring(0, 50)}...`);
  }
...
fixCategoryImages().catch((err) => { console.error(err); process.exit(1); }); // D19: fail loud so CI catches broken seeds

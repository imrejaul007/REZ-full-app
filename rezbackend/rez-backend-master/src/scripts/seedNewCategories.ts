...
async function seedCategories() {
  try {
    ...
    logger.info('🗑️  Clearing existing categories...');
    const deleteResult = await Category.deleteMany({});
    logger.info(`   Deleted ${deleteResult.deletedCount} categories`);
    ...
    for (let i = 0; i < CATEGORIES.length; i++) {
      ...
      logger.info(`   ✅ Created: ${cat.name} (${cat.slug})`);
    }
    ...
    logger.info('========================================');
    logger.info(`   Main Categories: ${CATEGORIES.length}`);
    logger.info(`   Subcategories: ${totalSubcategories}`);
    logger.info(`   Total: ${CATEGORIES.length + totalSubcategories}`);
    logger.info('========================================\n');

    // Verify by fetching all categories
    const allCategories = await Category.find({}).lean();
    logger.info('📋 Verification:');
    logger.info(`   Total documents in collection: ${allCategories.length}`);

    // List main categories
    const mainCats = allCategories.filter((c) => !c.parentCategory);
    logger.info(`   Main categories: ${mainCats.map((c) => c.slug).join(', ')}`);
  } catch (error) {

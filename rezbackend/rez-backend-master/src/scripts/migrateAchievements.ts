...
async function migrateAchievementDefinitions(): Promise<number> {
  ...
  for (const def of ACHIEVEMENT_DEFINITIONS) {
    ...
    const result = await Achievement.updateOne({ type: def.type }, { $set: updateData }, { upsert: true });
...
async function linkUserAchievements(): Promise<number> {
  ...
  const unlinked = await UserAchievement.find({
    $or: [{ achievement: { $exists: false } }, { achievement: null }],
  });

  logger.info(`  Found ${unlinked.length} unlinked UserAchievement records`);
  ...
  for (const ua of unlinked) {
    ...
    if (!achievement) {
      logger.info(`  [SKIP] No Achievement found for type="${ua.type}" (user=${ua.user})`);
      continue;
    }
...
async function seedQuickActions(): Promise<number> {
  ...
  for (const qa of quickActions) {
    const result = await QuickAction.updateOne({ slug: qa.slug }, { $set: qa }, { upsert: true });
...
async function seedValueCards(): Promise<number> {
  ...
  for (const vc of valueCards) {
    const result = await ValueCard.updateOne({ title: vc.title }, { $set: vc }, { upsert: true });
...
async function seedLeaderboardConfigs(): Promise<number> {
  ...
  for (const config of configs) {
    const result = await LeaderboardConfig.updateOne({ slug: config.slug }, { $set: config }, { upsert: true });
...
async function main() {
  logger.info('==========================================================');
  logger.info('  Achievement System Migration');
  logger.info('==========================================================');
  ...
  try {
    ...
    // Step 1: Migrate Achievement definitions
    logger.info('--- Step 1: Migrate Achievement Definitions ---');
    const achievementsMigrated = await migrateAchievementDefinitions();
    logger.info(`Achievements migrated/updated: ${achievementsMigrated}\n`);

    // Step 2: Link UserAchievement records
    logger.info('--- Step 2: Link UserAchievement Records ---');
    const userAchievementsLinked = await linkUserAchievements();
    logger.info(`UserAchievements linked: ${userAchievementsLinked}\n`);

    // Step 3: Seed QuickAction data
    logger.info('--- Step 3: Seed QuickAction Data ---');
    const quickActionsSeeded = await seedQuickActions();
    logger.info(`QuickActions seeded: ${quickActionsSeeded}\n`);

    // Step 4: Seed ValueCard data
    logger.info('--- Step 4: Seed ValueCard Data ---');
    const valueCardsSeeded = await seedValueCards();
    logger.info(`ValueCards seeded: ${valueCardsSeeded}\n`);

    // Step 5: Seed LeaderboardConfig data
    logger.info('--- Step 5: Seed LeaderboardConfig Data ---');
    const leaderboardConfigsSeeded = await seedLeaderboardConfigs();
    logger.info(`LeaderboardConfigs seeded: ${leaderboardConfigsSeeded}\n`);

    // Summary
    logger.info('==========================================================');
    logger.info('  Migration Summary');
    logger.info('==========================================================');
    logger.info(`  Achievements migrated/updated : ${achievementsMigrated} / ${ACHIEVEMENT_DEFINITIONS.length}`);
    logger.info(`  UserAchievements linked       : ${userAchievementsLinked}`);
    logger.info(`  QuickActions seeded           : ${quickActionsSeeded} / 6`);
    logger.info(`  ValueCards seeded             : ${valueCardsSeeded} / 4`);
    logger.info(`  LeaderboardConfigs seeded     : ${leaderboardConfigsSeeded} / 6`);
    logger.info('==========================================================');

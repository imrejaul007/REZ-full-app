...
async function seedVouchersWithStores() {
  try {
    logger.info('🌱 Starting Voucher Seeding with Store Links...\n');
    ...
    const storeByCategory: { [key: string]: typeof stores } = {
      ...
      beauty: stores.filter((s) => {
        const name = s.name.toLowerCase();
        return name.includes('beauty') || name.includes('cosmetic') || name.includes('salon') || name.includes('spa');
      }),
    ...
    const voucherBrandsData = [
      ...
      },
      {
        ...
        termsAndConditions: ['Valid for 6 months', 'Applicable on all fashion items', 'Non-refundable'],
        purchaseCount: 1234,
        viewCount: 5432,
      ...
      },
      {
        ...
        termsAndConditions: ['Valid for 12 months', 'Applicable at all Zara stores and online', 'Non-transferable'],
        purchaseCount: 987,
        viewCount: 4321,
        store: storeByCategory.fashion[0]?._id || stores[0]?._id,
      },
      {
        ...
        isActive: true,
        denominations: [250, 500, 1000, 2000],
        termsAndConditions: ['Valid for 6 months', 'Can be used multiple times', 'Non-refundable'],
        purchaseCount: 876,
        viewCount: 3456,
        store: storeByCategory.fashion[1]?._id || stores[0]?._id,
      },
      ...
      },
      {
        ...
        termsAndConditions: ['Valid for 2 months', 'Applicable at all KFC outlets', 'Cannot be used with other offers'],
        purchaseCount: 543,
        viewCount: 2876,
      ...
      },
      {
        ...
        termsAndConditions: ['Valid for 3 months', 'Fastest grocery delivery', 'Non-refundable'],
        purchaseCount: 1876,
        viewCount: 9234,
      ...
      },
      {
        ...
        isActive: true,
        denominations: [200, 500, 1000],
        termsAndConditions: ['Valid for 2 months', '10-minute delivery guarantee', 'Non-refundable'],
        purchaseCount: 1543,
        viewCount: 6543,
        store: storeByCategory.grocery[0]?._id || stores[0]?._id,
      },
      ...
      // Beauty vouchers
      {
        ...
        isActive: true,
        denominations: [250, 500, 1000, 2000],
        termsAndConditions: ['Valid for 6 months', 'Applicable on all beauty products', 'Check for brand exclusions'],
        purchaseCount: 2134,
        viewCount: 11234,
      ...
      },
      {
        ...
        isActive: true,
        denominations: [500, 1000, 2000, 5000],
        termsAndConditions: ['Valid for 12 months', 'Applicable at all Sephora stores and online', 'Non-transferable'],
        purchaseCount: 987,
        viewCount: 4321,
        store: storeByCategory.beauty[0]?._id || stores[0]?._id,
      },
      ...
      },
      {
        ...
        termsAndConditions: ['Valid for 12 months', 'Applicable in-store and online', 'Check for product exclusions'],
        purchaseCount: 345,
        viewCount: 1890,
      ...
      },
      {
        ...
        termsAndConditions: ['Valid for 12 months', 'Applicable on all IndiGo flights', 'Non-refundable'],
        purchaseCount: 321,
        viewCount: 1876,
        // No specific store - airline
      },
      ...
      },
      {
        ...
        isActive: true,
        denominations: [200, 500, 1000, 2000],
        termsAndConditions: ['Valid for 6 months', 'Applicable at all PVR locations', 'Non-refundable'],
        purchaseCount: 876,
        viewCount: 4321,
      ...
      // More fashion brands
      {
        ...
        isActive: true,
        denominations: [500, 1000, 2000, 5000],
        termsAndConditions: ['Valid for 12 months', 'Applicable at all Hollister stores', 'Non-transferable'],
        purchaseCount: 432,
        viewCount: 2345,
        store: storeByCategory.fashion[0]?._id || stores[0]?._id,
      },
      {
        ...
        termsAndConditions: ['Valid for 12 months', 'Can be used multiple times', 'Non-refundable'],
        purchaseCount: 654,
        viewCount: 3210,
        store: storeByCategory.fashion[1]?._id || stores[0]?._id,
      },
      {
        ...
        isActive: true,
        denominations: [500, 1000, 2000, 5000],
        termsAndConditions: ['Valid for 12 months', 'Applicable at all CK stores', 'Non-transferable'],
        purchaseCount: 543,
        viewCount: 2876,
        store: storeByCategory.fashion[0]?._id || stores[0]?._id,
      },
      ...
      // More Entertainment vouchers
      {
        ...
        isActive: true,
        denominations: [100, 200, 500, 1000],
        termsAndConditions: ['Valid for 6 months', 'Applicable for movies and shows', 'Non-refundable'],
        purchaseCount: 890,
        viewCount: 5432,
      },
      {
        ...
        isActive: true,
        denominations: [200, 500, 1000],
        termsAndConditions: ['Valid for 6 months', 'Applicable at all INOX locations', 'Non-refundable'],
        purchaseCount: 543,
        viewCount: 2876,
      },
    ];
    ...
    logger.info('📊 Checking existing voucher brands...');
    const existingCount = await VoucherBrand.countDocuments();
    logger.info(`✅ Found ${existingCount} existing voucher brands\n`);

    // Seed voucher brands
    logger.info('🌱 Seeding voucher brands...');
    let created = 0;
    let skipped = 0;
    ...
    for (const voucherData of voucherBrandsData) {
      ...
      if (existingVoucher) {
        ...
        if (!existingVoucher.store && voucherData.store) {
          ...
          logger.info(`🔄 Updated ${voucherData.name} with store link`);
          created++;
        } else {
          skipped++;
          logger.info(`⚠️  ${voucherData.name} already exists, skipping...`);
        }
      } else {
        const voucher = new VoucherBrand(voucherData);
        await voucher.save();
        logger.info(`✅ Created voucher brand: ${voucherData.name}${voucherData.store ? ' (linked to store)' : ''}`);
        created++;
      }
    }
    ...
    logger.info(`✅ Created/Updated: ${created} vouchers`);
    logger.info(`⏭️  Skipped: ${skipped} vouchers`);
    ...
    vouchersWithStores.forEach((v) => {
      logger.info(`   - ${v.name} → ${(v.store as any)?.name || 'Store'}`);
    });

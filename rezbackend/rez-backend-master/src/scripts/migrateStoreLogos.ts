...
async function migrateStoreLogos() {
  logger.info('🏪 Store Logo Migration');
  ...
  try {
    ...
    logger.info(`Found ${stores.length} stores without logos.\n`);

    if (stores.length === 0) {
      logger.info('All stores already have logos!');
      return;
    }
    ...
    for (const store of stores) {
      ...
      await Store.updateOne({ _id: store._id }, { $set: { logo: logoUrl } });

      const marker = STORE_LOGOS[store.name] ? '✅' : '⚠️';
      logger.info(`  ${marker} ${store.name} → ${logoUrl.substring(0, 65)}...`);
      updated++;
    }

    logger.info('');
    logger.info('─'.repeat(60));
    logger.info(`📊 Updated ${updated} stores.`);
    if (usedFallback > 0) {
      logger.info(`   ${usedFallback} used generic fallback image.`);
    }
    logger.info('─'.repeat(60));
  } catch (error) {

...
async function seedVideos() {
  ...
  logger.info(`✅ Created ${createdVideos.length} videos`);
  ...
  const reviews = [
    {
      ...
      comment:
        'Absolutely love this phone! The camera quality is outstanding, especially in low light conditions. The build quality feels premium and the battery life is impressive. Highly recommended for anyone looking for a flagship device.',
      images: [
    ...
    },
    {
      ...
      comment:
        "Really comfortable t-shirt with good fabric quality. The fit is perfect and it doesn't shrink after washing. Only minor issue is that the color is slightly different from the website photos, but still looks good.",
    ...
    },
    {
      ...
      comment:
        'These organic coffee beans are absolutely fantastic! The aroma is incredible and the taste is smooth with perfect balance. You can really taste the quality difference. Will definitely order again.',
      images: [],
      isVerified: true,
      isRecommended: true,
      likes: 18,
    ...
    },
    {
      ...
      comment:
        'The lamp looks exactly like the photos and the quality is good for the price. Installation was easy and the LED bulb is bright enough for reading. The only downside is the cord could be a bit longer.',
    ...
    },
    {
      ...
      comment:
        'Excellent quality dumbbells! The adjustment mechanism is smooth and the weight plates are well-balanced. Perfect for my home gym setup. The rubber coating protects my floors. Worth every penny!',
      images: ['https://images.unsplash.com/photo-1571019613540-996a8a8a2b6d?w=300'],
      isVerified: false,
      isRecommended: true,
      likes: 15,
      dislikes: 0,
    ...
    },
    {
      ...
      comment:
        "The S24 Ultra is a powerful phone with excellent display and camera. The S Pen is really useful for note-taking. However, it's quite expensive and the battery could be better for heavy usage.",
      images: [],
      isVerified: false,
      isRecommended: true,
      likes: 9,
...
async function seedWishlists() {
  ...
  logger.info(`✅ Created ${createdWishlists.length} wishlists`);
...
async function main() {
  try {
    ...
    logger.info(`🏪 Stores: ${storeIds.length}`);
    logger.info(`📦 Products: ${productIds.length}`);
    logger.info(`🛒 Carts: 3`);
    logger.info(`📋 Orders: ${orderIds.length}`);
    logger.info(`🎥 Videos: ${videoIds.length}`);
    logger.info(`📋 Projects: ${projectIds.length}`);
    logger.info(`💳 Transactions: 5`);
    logger.info(`🔔 Notifications: 8`);
    logger.info(`⭐ Reviews: 6`);
    logger.info(`💝 Wishlists: 4`);

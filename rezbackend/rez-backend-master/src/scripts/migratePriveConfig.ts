...
async function migrate() {
  ...
  if (existing?.priveProgramConfig) {
    logger.info('ℹ️  priveProgramConfig already exists — skipping migration (idempotent)');
  } else if (existing) {
    ...
    if (result.modifiedCount > 0) {
      logger.info('✅ Added priveProgramConfig to existing WalletConfig');
    } else {
      logger.info('ℹ️  No update needed (config may have been added concurrently)');
    }
  } else {
    ...
    logger.info('✅ Created WalletConfig with priveProgramConfig');
  }

  await disconnectDb();
  logger.info('🔌 Disconnected. Migration complete.');
}

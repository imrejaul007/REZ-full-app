...
const NEW_STORES = [
  ...
  // Beauty
  {
    ...
    slug: 'sephora',
    categorySlug: 'beauty',
    image: 'https://images.unsplash.com/photo-1522335789203-abd65232a2fc?w=800',
  },
...
async function addMoreStores() {
  logger.info('🚀 Starting addMoreStores script...');
  ...
  for (const cat of EXTRA_CATEGORIES) {
    ...
    logger.info(`   ✅ Checked/Created category: ${cat.name}`);
  }

  // 2. Add Stores without deleting
  logger.info('🏬 Adding stores...');
  let addedCount = 0;
  ...
  for (const s of NEW_STORES) {
    ...
    if (!category) {
      logger.warn(`   ⚠️ Category ${s.categorySlug} not found for ${s.name} (Skipping)`);
      continue;
    }
    ...
    if (store) {
      logger.info(`   ℹ️ Store ${s.name} already exists (Skipping creation)`);
    } else {
      logger.info(`   🆕 Creating store: ${s.name}`);
    ...
    if (productCount < 2) {
      logger.info(`      Creating products for ${s.name}...`);
      for (let i = 1; i <= 3; i++) {
  ...
  logger.info(`✅ Added ${addedCount} new stores.`);
  logger.info('🎉 Done!');
}

...
const vibesData: Record<
  string,
  Array<{ id: string; name: string; icon: string; color: string; description: string }>
> = {
  'food-dining': [
...
const occasionsData: Record<
  string,
  Array<{ id: string; name: string; icon: string; color: string; tag: string | null; discount: number }>
> = {
  'food-dining': [
...
// Hashtags data
const hashtagsData: Record<
  string,
  Array<{ id: string; tag: string; count: number; color: string; trending: boolean }>
> = {
  'food-dining': [
...
async function main(): Promise<void> {
  ...
  try {
    ...
    logger.info('├────────────────────────────┼───────┤');
    logger.info(`│ Vibes                      │ ${String(metadataCounts.vibes).padStart(5)} │`);
    logger.info(`│ Occasions                  │ ${String(metadataCounts.occasions).padStart(5)} │`);
    logger.info(`│ Hashtags                    │ ${String(metadataCounts.hashtags).padStart(5)} │`);
    logger.info(`│ Bank Offers                │ ${String(bankOffersCount).padStart(5)} │`);
    logger.info(`│ Exclusive Offers           │ ${String(exclusiveOffersCount).padStart(5)} │`);
    logger.info(`│ Stores Updated             │ ${String(storesUpdated).padStart(5)} │`);
    logger.info(`│ Social Proof Stats         │ ${String(socialProofCount).padStart(5)} │`);
    logger.info('└────────────────────────────┴───────┘');

    const total =
      metadataCounts.vibes +
      metadataCounts.occasions +
      metadataCounts.hashtags +
      bankOffersCount +

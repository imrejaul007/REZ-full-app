...
async function addIndexes() {
  try {
    ...
    // USER MODEL INDEXES
    logger.info('Adding indexes to User model...');
    try {
      ...
      logger.error('✗ Error adding User indexes:', error.message);
      results.push({
        model: 'User',
        indexes: [],
        status: 'error',
    ...
    // STORE MODEL INDEXES
    logger.info('Adding indexes to Store model...');
    try {
      ...
      logger.info('✓ Store indexes added');
    } catch (error: any) {
      logger.error('✗ Error adding Store indexes:', error.message);
      results.push({
        model: 'Store',
        indexes: [],
        status: 'error',
    ...
    logger.info('Adding indexes to Product model...');
    try {
      ...
      results.push({
        ...
        indexes: [
          ...
          {
            name: 'featured_rating_active',
            fields: 'isFeatured: 1, ratings.average: -1, isActive: 1',
            type: 'compound',
          },
      ...
      logger.info('✓ Product indexes added');
    } catch (error: any) {
      logger.error('✗ Error adding Product indexes:', error.message);
      results.push({
        model: 'Product',
        indexes: [],
        status: 'error',
    ...
    // ORDER MODEL INDEXES
    logger.info('Adding indexes to Order model...');
    try {
      ...
      logger.info('✓ Order indexes added');
    } catch (error: any) {
      logger.error('✗ Error adding Order indexes:', error.message);
      results.push({
        model: 'Order',
        indexes: [],
        status: 'error',
    ...
    logger.info('Adding indexes to Review model...');
    try {
      ...
      logger.info('✓ Review indexes added');
    } catch (error: any) {
      logger.error('✗ Error adding Review indexes:', error.message);
      results.push({
        model: 'Review',
        indexes: [],
        status: 'error',
    ...
    // CART MODEL INDEXES
    logger.info('Adding indexes to Cart model...');
    try {
      ...
      logger.info('✓ Cart indexes added');
    } catch (error: any) {
      logger.error('✗ Error adding Cart indexes:', error.message);
      results.push({
        model: 'Cart',
        indexes: [],
        status: 'error',
    ...
    logger.info('Adding indexes to Wishlist model...');
    try {
      ...
      logger.info('✓ Wishlist indexes added');
    } catch (error: any) {
      logger.error('✗ Error adding Wishlist indexes:', error.message);
      results.push({
        model: 'Wishlist',
        indexes: [],
        status: 'error',
    ...
    logger.info('Adding indexes to Notification model...');
    try {
      ...
      logger.error('✗ Error adding Notification indexes:', error.message);
      results.push({
        model: 'Notification',
        indexes: [],
        status: 'error',
    ...
    // VIDEO MODEL INDEXES
    logger.info('Adding indexes to Video model...');
    try {
      ...
      logger.info('✓ Video indexes added');
    } catch (error: any) {
      logger.error('✗ Error adding Video indexes:', error.message);
      results.push({
        model: 'Video',
        indexes: [],
        status: 'error',
    ...
    logger.info('Adding indexes to Wallet model...');
    try {
      ...
      logger.info('✓ Wallet indexes added');
    } catch (error: any) {
      logger.error('✗ Error adding Wallet indexes:', error.message);
      results.push({
        model: 'Wallet',
        indexes: [],
        status: 'error',
    ...
    logger.info('Adding indexes to Transaction model...');
    try {
      ...
      logger.info('✓ Transaction indexes added');
    } catch (error: any) {
      logger.error('✗ Error adding Transaction indexes:', error.message);
      results.push({
        model: 'Transaction',
        indexes: [],
        status: 'error',
    ...
    logger.info('Adding indexes to Coupon model...');
    try {
      ...
      logger.info('✓ Coupon indexes added');
    } catch (error: any) {
      logger.error('✗ Error adding Coupon indexes:', error.message);
      results.push({
        model: 'Coupon',
        indexes: [],
        status: 'error',
    ...
    // OFFER MODEL INDEXES
    logger.info('Adding indexes to Offer model...');
    try {
      ...
      logger.info('✓ Offer indexes added');
    } catch (error: any) {
      logger.error('✗ Error adding Offer indexes:', error.message);
      results.push({
        model: 'Offer',
        indexes: [],
        status: 'error',
    ...
    logger.info('Adding indexes to Activity model...');
    try {
      ...
      logger.info('✓ Activity indexes added');
    } catch (error: any) {
      logger.error('✗ Error adding Activity indexes:', error.message);
      results.push({
        model: 'Activity',
        indexes: [],
        status: 'error',
    ...
    logger.info('Adding indexes to CoinTransaction model...');
    try {
      ...
      logger.info('✓ CoinTransaction indexes added');
    } catch (error: any) {
      logger.error('✗ Error adding CoinTransaction indexes:', error.message);
      results.push({
        model: 'CoinTransaction',
        indexes: [],
        status: 'error',
    ...
    logger.info(`Summary: ${successCount} models indexed successfully, ${errorCount} with errors\n`);

    // Print detailed results
    logger.info('Detailed Results:\n');
    results.forEach((result, index) => {
      logger.info(`${index + 1}. ${result.model}`);
      if (result.status === 'success') {
        logger.info(`   Status: ✓ SUCCESS`);
        logger.info(`   Indexes added: ${result.indexes.length}`);
        result.indexes.forEach((idx) => {
          const type = idx.type ? ` (${idx.type})` : '';
          logger.info(`     - ${idx.name}: ${idx.fields}${type}`);
        });
      } else {

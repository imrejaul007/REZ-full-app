...
export async function triggerGamificationEvent(
  ...
  try {
    ...
    if (reward > 0) {
      await coinService.awardCoins(userId, reward, event, `Earned ${reward} coins from ${event.replace(/_/g, ' ')}`, {
        ...metadata,
        referenceId: metadata?.referenceId || `gamification_trigger:${userId}:${event}:${Date.now()}`,
      });
 ...
 */
async function checkAchievements(userId: string, event: string, _metadata: any): Promise<void> {
  try {
          ...
          try {
            await coinService.awardCoins(
              ...
              `Unlocked achievement: ${achievement.title}`,
              {
                ...
                referenceId: `achievement:${achievement._id}:${userId}`,
              },
            );
          } catch (err: any) {
            if (err.code === 11000) {
 ...
 */
async function updateChallengeProgress(userId: string, event: string, _metadata: any): Promise<void> {
  try {
...
async function getUserStats(userId: string, event: string): Promise<Record<string, number>> {
  ...
  try {
    ...
    if (event === 'order_placed' || event === 'review_submitted') {
      const [orderStats, reviewCount, _offerCount] = await Promise.all([
        Order.aggregate([
    ...
    stats.totalActivity =
      (stats.totalOrders || 0) + (stats.totalVideos || 0) + (stats.totalProjects || 0) + (stats.totalReviews || 0);
  } catch (error) {
...
export async function recalculateUserGamification(userId: string): Promise<void> {
  ...
  try {
    ...
    const events = ['order_placed', 'review_submitted', 'video_created', 'project_completed'];

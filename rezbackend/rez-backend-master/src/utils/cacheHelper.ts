...
export const CacheKeys = {
  ...
  walletSnapshot: (userId: string) => `wallet:snapshot:${userId}`, // ISSUE #6a: Balance snapshot with 60s TTL
  ...
  // ISSUE #6b: Merchant store listing cache (rarely changes, 5min TTL)
  merchantStores: (merchantId: string) => `merchant:stores:${merchantId}`,

  // ISSUE #6c: Campaign feed cache (3min TTL)
  campaignsFeed: (city: string = 'all') => `campaigns:active:${city}`,

  // ISSUE #6d: Booking history cache (30s TTL, user might just-booked)
  bookingHistory: (userId: string, page: number = 1) => `bookings:consumer:${userId}:page:${page}`,
};
...
export class CacheInvalidator {
  ...
  static async invalidateUser(userId: string): Promise<void> {
    ...
    await CacheInvalidator.safeInvalidate(`user:${userId}`, [
      ...
      // ISSUE #6: Invalidate wallet snapshot when user cache is invalidated
      redisService.del(CacheKeys.walletSnapshot(userId)),
      redisService.del(CacheKeys.bookingHistory(userId)),
    ]);
  }

  /**
   * ISSUE #6a: Invalidate wallet snapshot cache (called on debit/credit)
   */
  static async invalidateWalletSnapshot(userId: string): Promise<void> {
    logger.info(`Invalidating wallet snapshot cache for user: ${userId}`);
    await CacheInvalidator.safeInvalidate(`wallet:snapshot:${userId}`, [
      redisService.del(CacheKeys.walletSnapshot(userId)),
    ]);
  }

  /**
   * ISSUE #6b: Invalidate merchant stores cache (called on store update)
   */
  static async invalidateMerchantStores(merchantId: string): Promise<void> {
    logger.info(`Invalidating merchant stores cache for merchant: ${merchantId}`);
    await CacheInvalidator.safeInvalidate(`merchant:stores:${merchantId}`, [
      redisService.del(CacheKeys.merchantStores(merchantId)),
    ]);
  }

  /**
   * ISSUE #6c: Invalidate campaigns feed cache (called on campaign create/update)
   */
  static async invalidateCampaignsFeed(city?: string): Promise<void> {
    logger.info(`Invalidating campaigns feed cache${city ? ` for city: ${city}` : ' (all)'}`);
    await CacheInvalidator.safeInvalidate(`campaigns:feed:${city || 'all'}`, [
      redisService.delPattern('campaigns:active:*'),
    ]);
  }

  /**
   * ISSUE #6d: Invalidate booking history cache (called on booking create/update)
   */
  static async invalidateBookingHistory(userId: string): Promise<void> {
    logger.info(`Invalidating booking history cache for user: ${userId}`);
    await CacheInvalidator.safeInvalidate(`bookings:${userId}`, [
      redisService.delPattern(`bookings:consumer:${userId}:page:*`),
    ]);
  }

...
// Mock adminActionRateLimit as passthrough — rate limiting is not under test here.
// The real limiter uses Redis with failOpen:false; without a working Redis client
// it throws "Rate limiter backend unavailable", causing 503 on all wallet operations.
jest.mock('../../middleware/rateLimiter', () => ({
  adminActionRateLimit: (_req: any, _res: any, next: any) => next(),
...
jest.mock('../../config/walletMetrics', () => {
  ...
  const histogram = () => ({ startTimer: jest.fn(() => jest.fn()), observe: jest.fn() });
  const gauge = () => ({ set: jest.fn(), inc: jest.fn(), dec: jest.fn() });
  return {
    walletTransactionTotal: counter(),
    walletTransactionDuration: histogram(),
    walletBalanceDriftTotal: counter(),
    walletActiveLocks: gauge(),
    walletVelocityBlockedTotal: counter(),
    walletTransferAmount: histogram(),
    walletGiftAmount: histogram(),
    walletLedgerEntriesTotal: counter(),
    walletCacheOps: counter(),
    walletWriteTotal: counter(),
    walletCommitRetryTotal: counter(),
    walletCacheStaleTotal: counter(),
  };
});
...
// JWT secrets (≥32 chars) — keep distinct so user tokens pass auth but fail authorization
const TEST_SECRET = 'test-jwt-secret-for-integration-tests-32chars-plus';
const TEST_ADMIN_SECRET = 'test-admin-jwt-secret-integration-tests-32chars!!';

// Build minimal Express app with real auth middleware + admin routes
function buildTestApp() {
  process.env.JWT_SECRET = TEST_SECRET;
  // Admin tokens must be signed with JWT_ADMIN_SECRET (enforced by auth middleware)
  // Use a distinct secret so user tokens (signed with JWT_SECRET) pass auth and hit requireAdmin
...
beforeEach(() => {
  ...
  // Bypass withTransaction — standalone MongoMemoryServer doesn't support multi-document
  // transactions; the route wraps wallet ops in withTransaction for atomicity in production.
  // We replace withTransaction with a direct fn call so the underlying queries still run
  // against the real DB (using a real ClientSession for session-aware queries).
  jest.spyOn(mongoose, 'startSession').mockImplementation(async () => {
    const session = await mongoose.connection.startSession();
    session.withTransaction = async (fn: any) => fn(session);
...
function adminToken(adminId: string): string {
  return jwt.sign({ userId: adminId, role: 'super_admin' }, TEST_ADMIN_SECRET, { expiresIn: '1h' });
}
...
describe('HTTP Integration: Frontend → Backend → DB', () => {
  ...
  describe('GET /api/admin/user-wallets (Search)', () => {
    it('should find user by phone and return wallet data', async () => {
      ...
      const found = res.body.data.users.find((u: any) => u.user.phoneNumber === user.phoneNumber);
  ...
  describe('POST /api/admin/user-wallets/:id/reverse-cashback (Exact with TX ID)', () => {
    it('should reverse specific transaction via rewardEngine', async () => {
      ...
      const originalTxId = new Types.ObjectId().toString();
      ...
      expect(mockReverseReward).toHaveBeenCalledWith(originalTxId, 'Order cancelled', { partialAmount: 200 });
    });

    it('should be idempotent — second call returns same result', async () => {
      const sameTxId = new Types.ObjectId();
      const txAbcId = new Types.ObjectId().toString();
      mockReverseReward
        .mockResolvedValueOnce({
          success: true,
          reversalTransactionId: sameTxId,
          amount: 100,
          newBalance: 900,
          originalTransactionId: txAbcId,
          reason: 'First',
        })
        .mockResolvedValueOnce({
          success: true,
      ...
      const first = await request(testApp)
        .post(`/api/admin/user-wallets/${user._id}/reverse-cashback`)
        .set('Authorization', `Bearer ${token}`)
        .send({ amount: 100, originalTransactionId: txAbcId, reason: 'First' });

      const second = await request(testApp)
        .post(`/api/admin/user-wallets/${user._id}/reverse-cashback`)
        .set('Authorization', `Bearer ${token}`)
        .send({ amount: 100, originalTransactionId: txAbcId, reason: 'Dup' });
    ...
    it('should return 404 for non-existent TX', async () => {
      const fakeTxId = new Types.ObjectId().toString();
      mockReverseReward.mockRejectedValueOnce(new Error(`Transaction ${fakeTxId} not found`));
      ...
      const res = await request(testApp)
        ...
        .send({
          amount: 50,
          originalTransactionId: fakeTxId,
  ...
  describe('Authentication & authorization', () => {
    ...
    it('should reject non-admin tokens → 403', async () => {
      const userToken = jwt.sign({ userId: user._id.toString(), role: 'user' }, TEST_SECRET, { expiresIn: '1h' });

      const res = await request(testApp).get('/api/admin/user-wallets').set('Authorization', `Bearer ${userToken}`);

...
describe('Gift Coins E2E Flow', () => {
  ...
  beforeEach(async () => {
    ...
    await CoinTransaction.createTransaction(String(sender._id), 'earned', 5000, 'admin', 'Initial balance seed', {
      idempotencyKey: `seed-sender-${sender._id}`,
    });
    await CoinTransaction.createTransaction(String(recipient._id), 'earned', 1000, 'admin', 'Initial balance seed', {
      idempotencyKey: `seed-recipient-${recipient._id}`,
    });
  ...
  describe('Recipient Validation', () => {
    ...
    it('should mask name correctly', () => {
      const maskName = (name: string) =>
        name
          .split(' ')
          .map((w) => (w.length <= 2 ? w : w[0] + '***' + w[w.length - 1]))
          .join(' ');
  ...
  describe('Get Received Gifts', () => {
    it('should show delivered gifts with valid expiry', async () => {
      ...
      const gifts = await CoinGift.find({
        recipient: recipient._id,
        $or: [{ status: 'claimed' }, { status: 'delivered', expiresAt: { $gte: new Date() } }],
      });
  ...
  describe('Claim Gift', () => {
    ...
    it('should prevent double-claim (atomic guard)', async () => {
      await CoinGift.findOneAndUpdate({ _id: giftToClaim._id, status: 'delivered' }, { $set: { status: 'claimed' } });
  ...
  describe('Admin Operations', () => {
    ...
    it('should list gifts with pagination', async () => {
      const gifts = await CoinGift.find({}).sort({ createdAt: -1 }).skip(0).limit(20).lean();

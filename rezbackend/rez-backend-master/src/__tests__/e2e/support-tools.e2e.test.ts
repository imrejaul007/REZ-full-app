...
// Mock wallet metrics — include ALL exported counters/histograms/gauges
...
const hasReplicaSet = process.env.TEST_USE_REPLICA_SET === 'true';

// Only run transaction-dependent tests when replica set is available
const runTransactionTests = hasReplicaSet;
...
describe('Support Tools — Wallet Operations', () => {
  ...
  // Skip entire section if no replica set (MongoDB transactions required)
  const describeSection = runTransactionTests ? describe : describe.skip;

  describeSection('1. Credit Wallet', () => {
  ...
  describe('4. Reverse Cashback (with TX ID — exact reversal)', () => {
    it('should reverse a specific cashback transaction via rewardEngine', async () => {
      ...
      const result = await rewardEngine.reverseReward(cashbackTx._id.toString(), 'Incorrectly awarded cashback', {
        partialAmount: 150,
      });
    ...
    it('should support partial reversal', async () => {
      ...
      const { rewardEngine } = await import('../../core/rewardEngine');
      const result = await rewardEngine.reverseReward(cashbackTx._id.toString(), 'Partial reversal', {
        partialAmount: 80,
      });
    ...
    it('should reject reversal of non-existent transaction', async () => {
      ...
      const { rewardEngine } = await import('../../core/rewardEngine');
      await expect(rewardEngine.reverseReward(fakeId, 'Does not exist')).rejects.toThrow('not found');
    });
  });
...
describe('Support Tools — Campaign Management', () => {
  describe('8. Pause/Resume Campaigns', () => {
    it('should pause an active campaign by setting status to paused', async () => {
      ...
      // Pause it
      const paused = await BonusCampaign.findByIdAndUpdate(campaign._id, { status: 'paused' }, { new: true });
    ...
    it('should resume a paused campaign by setting status to active', async () => {
      ...
      // Resume it
      const resumed = await BonusCampaign.findByIdAndUpdate(campaign._id, { status: 'active' }, { new: true });

...
import { getMyChallengeProgress, getGamificationStats } from '../controllers/gamificationController';
...
describe('New Gamification Endpoints', () => {
  ...
  describe('GET /api/gamification/challenges/my-progress', () => {
    ...
    it('should require authentication', async () => {
      ...
      await getMyChallengeProgress(mockReq as Request, mockRes as Response, mockNext);
  ...
  describe('GET /api/gamification/streaks', () => {
    it('should return current user login streak', async () => {
      ...
      await streakController.getCurrentUserStreak(mockReq as Request, mockRes as Response, jest.fn());
  ...
  describe('GET /api/gamification/stats', () => {
    it('should return comprehensive gamification stats', async () => {
      ...
      GameSession.aggregate = jest.fn().mockResolvedValue([{ totalGames: 50, gamesWon: 35 }]);
    ...
    it('should require authentication', async () => {
      ...
      await getGamificationStats(mockReq as Request, mockRes as Response, mockNext);

      // asyncHandler catches the AppError and forwards it to next() — check next was called with the error
      expect(mockNext).toHaveBeenCalledWith(expect.objectContaining({ message: 'Authentication required' }));
    });

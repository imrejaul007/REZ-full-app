 ...
 */
export const razorpayIPWhitelist = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (!isAuthorized) {
    logger.error(`[WEBHOOK-SECURITY] Unauthorized webhook attempt from IP: ${clientIP}`, {
  ...
  logger.info(`[WEBHOOK-SECURITY] Authorized IP: ${clientIP}`, {
    timestamp: new Date().toISOString(),
  });
...
export const webhookRateLimiter = rateLimit({
  ...
  handler: (req: Request, res: Response) => {
    ...
    res.status(429).json({
      ...
      retryAfter: req.rateLimitData?.resetTime,
    });
  },
});
...
export const validateWebhookPayload = (req: Request, res: Response, next: NextFunction): void => {
  try {
    ...
    if (missingFields.length > 0) {
      logger.error(`[WEBHOOK-SECURITY] Missing required fields: ${missingFields.join(', ')}`, {
    ...
    if (!validEventTypes.includes(webhookBody.event)) {
      logger.error(`[WEBHOOK-SECURITY] Invalid event type: ${webhookBody.event}`, {
        timestamp: new Date().toISOString(),
      });
    ...
    if (webhookAge > WEBHOOK_MAX_AGE_SECONDS) {
      logger.error(`[WEBHOOK-SECURITY] Webhook too old: ${webhookBody.id}`, {
 ...
 */
export const logWebhookSecurityEvent = (req: Request, res: Response, next: NextFunction): void => {
 ...
 */
export const sendSecureWebhookSuccess = (res: Response, message: string, eventId: string): void => {

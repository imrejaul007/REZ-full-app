...
// Paths that must never be blocked — Render/K8s health probes come from
// platform IPs that could accumulate violations in a shared Redis cluster.
// Blocking /health would cause Render to mark the service unhealthy and
// restart it in a crash loop.
const HEALTH_PATHS = new Set(['/health', '/health/ready', '/health/live', '/health/deep']);
...
export const ipBlocker = async (req: Request, res: Response, next: NextFunction) => {
  // Fast-path: health probes bypass the block list entirely
  ...
  try {
    ...
    if (cachedBlockedSet.has(clientIP)) {
      return res.status(403).json({
        success: false,
        error:
          'Your IP has been blocked due to suspicious activity. Please contact support if you believe this is a mistake.',
      });
    }
...
export const recordRateLimitViolation = (req: Request, res: Response, next: NextFunction) => {
  const clientIP = req.ip || req.socket.remoteAddress || 'unknown';
  recordViolation(clientIP, 'Rate Limit Exceeded').catch((err) =>
    logger.error('[IPBlocker] Rate limit violation recording failed', { error: err.message, clientIP }),
  );
  next();
};

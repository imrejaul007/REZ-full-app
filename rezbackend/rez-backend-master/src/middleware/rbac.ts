...
export function checkPermission(permission: Permission) {
  return (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const role: MerchantUserRole | null = req.merchantUser?.role ?? (req.merchantId ? 'owner' : null);
      if (!role) {
...
/**
 * Require merchant team admin role (owner or admin within a store team).
 *
 * IMPORTANT: This is a *merchant team* role check, NOT a platform admin check.
 * For platform admin access use requireAdmin from ../../middleware/auth instead.
 * The previous name `requireAdmin` was a collision risk — any route accidentally
 * importing from rbac.ts instead of auth.ts would grant store team members
 * platform admin access.
 *
 * Usage:
 * router.patch('/config', requireMerchantAdmin, updateConfig);
 */
export const requireMerchantAdmin = requireRole('owner', 'admin');

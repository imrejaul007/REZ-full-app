...
export const validateClaimMilestone = [
  param('milestoneId')
    ...
    .matches(/^milestone-\d+$/)
    .withMessage('Invalid milestone ID format (expected: milestone-{number})')
    .isLength({ max: 50 })
    .withMessage('Milestone ID too long'),
...
export const validateClaimTask = [
  param('taskId')
    ...
    .withMessage('Task ID must be between 3 and 100 characters')
    .matches(/^[a-zA-Z0-9\s]+$/)
    .withMessage('Task ID contains invalid characters'),
  handleValidationErrors,
];
...
export const validateClaimJackpot = [
  param('spendAmount')
    .notEmpty()
    .withMessage('Spend amount is required')
    .isInt({ min: 25000, max: 100000 })
    .withMessage('Spend amount must be between 25000 and 100000')
    .custom((value) => {
...
export const validateUpdateTaskProgress = [
  param('taskType')
    ...
    .isLength({ max: 50 })
    .withMessage('Task type too long'),
  body('progress').optional().isInt({ min: 0, max: 1000 }).withMessage('Progress must be a number between 0 and 1000'),
  handleValidationErrors,
];
...
export const validateClaimOffer = [
  body('offerId')
    ...
    .withMessage('Offer ID must be between 1 and 200 characters'),
  handleValidationErrors,
];
...
export const validateRequestPayout = [
  ...
  body('method')
    ...
    .isString()
    .withMessage('Payout method must be a string')
    .isIn(['bank', 'upi', 'wallet'])
    .withMessage('Invalid payout method. Must be: bank, upi, or wallet'),
  body('details')
    .optional()
    .isObject()

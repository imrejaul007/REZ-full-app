...
/**
 * Verify the requesting merchant owns the store associated with the order.
 * Expects `req.params.orderId` and `req.merchantId` to be set by merchantauth middleware.
 */
export async function verifyOrderOwnership(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const orderId = req.params.orderId || req.params.id;
    // SECURITY FIX: Only accept req.merchantId (set by merchantauth middleware).
    // The previous fallback to req.userId would allow a consumer token to pass
    // the ownership check — a privilege escalation vector.
    const merchantUserId = (req as any).merchantId;
    ...
    const order = await Order.findById(orderId).select('items.store store').lean();
    ...
    const store = await Store.findById(storeIdStr).select('merchantId').lean();

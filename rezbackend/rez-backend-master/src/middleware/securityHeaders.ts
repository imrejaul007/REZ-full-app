...
export const apiSecurityHeaders = (req: Request, res: Response, next: NextFunction) => {
  ...
  // Record start time and write elapsed ms when response finishes
...
export const rateLimitHeaders = (req: Request, res: Response, next: NextFunction) => {
  if (req.rateLimitData) {
    res.setHeader('X-RateLimit-Limit', req.rateLimitData.limit.toString());
    res.setHeader('X-RateLimit-Remaining', req.rateLimitData.remaining.toString());
    res.setHeader('X-RateLimit-Reset', req.rateLimitData.resetTime.toISOString());
  }

...
// NOTE: Keep this as the single source of truth — do NOT redeclare CSRF_EXEMPT_PATHS
// in nested functions (that would shadow this variable and silently use the wrong list).
const CSRF_EXEMPT_PATHS = [
  ...
  '/api/webhooks/payment', // Additional webhook paths (previously only in inner shadow variable)
  '/api/webhooks/razorpay',
  '/api/webhooks/stripe',
];

// Auth endpoints are exempt — they are unauthenticated (no JWT) and called
// cross-origin from web clients (Vercel). CSRF cookies with sameSite=strict
// are not sent cross-origin, so these would always fail. Auth endpoints are
// protected by OTP/rate-limiting instead.
const CSRF_EXEMPT_PREFIXES = [
  '/api/auth/',
  '/api/user/auth/', // Consumer-app auth endpoints called by rez-web-menu
  '/api/merchant/auth/',
  '/api/admin/auth/',
  '/auth/', // When middleware is mounted on /api/merchant, req.path is relative
];
...
function shouldExemptFromCsrf(req: Request): boolean {
  ...
  // Use originalUrl (full path) — req.path is relative to the mount point when
  // this middleware is mounted via app.use('/api/merchant', validateCsrfToken),
  // which would strip the prefix and break prefix-based exemptions.
  ...
  // Exempt specific paths using exact match (uses module-level CSRF_EXEMPT_PATHS)
  if (CSRF_EXEMPT_PATHS.includes(path)) {
    return true;
  }

  // Exempt auth endpoints by prefix (unauthenticated, cross-origin safe)
...
export function setCsrfToken(req: Request, res: Response, next: NextFunction): void {
  ...
  try {
    ...
    if (!csrfToken || csrfToken.length !== CSRF_TOKEN_LENGTH * 2) {
      ...
      // NOTE: httpOnly MUST be false for the Double Submit Cookie pattern — the JS
      // client needs to read this cookie and echo it in the X-CSRF-Token request header.
      // Protection against XSS stealing the token comes from sameSite + short maxAge.
      // SANA: security hardening — All cookie flags properly set:
      // - secure: enforced in production (HTTPS only)
      // - sameSite: 'strict' prevents CSRF via cross-site requests
      // - maxAge: short-lived (1 hour) reduces exposure window

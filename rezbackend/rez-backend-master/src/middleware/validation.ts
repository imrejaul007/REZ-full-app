...
// S-11: Helper to strip HTML tags and event handlers from strings to prevent stored XSS
...
const stripHtmlTags = sanitizeHtml;
...
const sanitizeLogBody = (body: any): any => {
  if (!body || typeof body !== 'object') return body;
  const SENSITIVE = ['password', 'token', 'secret', 'otp', 'pin', 'cvv', 'cardNumber', 'accessToken', 'refreshToken'];
...
export const validate = (schema: Joi.ObjectSchema) => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (error) {
      ...
      logger.info('[VALIDATION ERROR]', JSON.stringify({ body: sanitizeLogBody(req.body), errors }, null, 2));
...
export const commonSchemas = {
  ...
  // S-10: Require min 8 chars with at least one uppercase, one lowercase, one digit, and one special char
  password: Joi.string()
    .min(8)
    .max(128)
    .pattern(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?])/)
...
export const authSchemas = {
  ...
  sendOTP: Joi.object({
    ...
    email: Joi.alternatives().try(Joi.string().valid('', null), Joi.string().email().lowercase()).optional(),
    referralCode: Joi.alternatives()
      .try(Joi.string().valid('', null), Joi.string().trim().uppercase().min(6).max(10))
      .optional(),
    // GF-03: Include flow so the sendOTP controller can distinguish signup vs login intent.
    // Without this, stripUnknown:true silently drops the field before the controller reads it.
    flow: Joi.string().valid('signup', 'login').optional(),
  }),
  ...
  updateProfile: Joi.object({
    // BUG FIX: email was missing from the schema and got silently stripped by stripUnknown:true
    // before the controller could read it, so email updates were silently dropped.
    email: Joi.string()
    ...
    profile: Joi.object({
      ...
      // R01 FIX: Added 'prefer_not_to_say' to match Mongoose schema enum (User.ts line 327)
    ...
    preferences: Joi.object({
      ...
      dietaryPreferences: Joi.array().items(Joi.string().max(50)).max(20),
    }),
...
export const orderSchemas = {
  createOrder: Joi.object({
    ...
    paymentMethod: Joi.string().valid('wallet', 'card', 'upi', 'cod', 'netbanking', 'razorpay', 'stripe').required(),

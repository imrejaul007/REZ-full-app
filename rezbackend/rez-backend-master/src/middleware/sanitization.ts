...
// ISSUE-09 FIX: Fields whose values must NOT be passed through validator.escape().
// validator.escape() converts @ → &#64;, . → &#x2E;, etc., corrupting email addresses,
// URLs, phone numbers, and other structured values before they reach controllers.
// mongoSanitize (applied in config/middleware.ts) already handles NoSQL injection, so
// HTML-escaping these fields provides no additional security benefit.
 ...
 * Skips HTML-escaping for fields listed in SKIP_ESCAPE_KEYS (e.g. email) to avoid
 * corrupting structured values that contain characters such as @ and .
 */
function deepSanitize(input: any, parentKey?: string): any {
  if (typeof input === 'string') {
    // Skip validator.escape() for fields that contain structured data (emails, URLs, etc.)
    // that would be corrupted by HTML entity encoding.
    if (parentKey && SKIP_ESCAPE_KEYS.has(parentKey)) {
      // Only trim and remove null bytes — no HTML-escaping
  ...
  if (typeof input === 'object' && input !== null) {
    ...
    for (const key in input) {
      if (Object.prototype.hasOwnProperty.call(input, key)) {
        // Keys should not be HTML-escaped — only escape values.
        // Pass the current key as parentKey so the child string value can check
        // whether it should be HTML-escaped or left as-is.
        sanitized[key] = deepSanitize(input[key], key);
      }
    }
    return sanitized;
  }
...
/**
 * Strip MongoDB operator keys (keys starting with $) from query objects.
 * Prevents NoSQL injection via GET query parameters.
 */
...
export const sanitizeQuery = (req: Request, res: Response, next: NextFunction) => {
  if (req.query && typeof req.query === 'object') {
    // First strip MongoDB operators to prevent NoSQL injection

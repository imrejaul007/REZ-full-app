...
export class BusinessMetricsService {
   ...
   *
   * BUG FIX: The Store collection has a cross-service field fork — rez-backend
   * writes `merchantId`, rez-merchant-service writes `merchant`. Filtering on
   * only one field silently drops stores created by the other service, so the
   * dashboard would show ₹0 for merchants whose stores were onboarded via
   * rez-merchant-service. Use `$or` to match either shape.
   */
  private static async getMerchantStoreIds(merchantId: string): Promise<mongoose.Types.ObjectId[]> {
    const mongoId = new mongoose.Types.ObjectId(merchantId);
    const stores = await Store.find({
      $or: [{ merchantId: mongoId }, { merchant: mongoId }],
    })
      .select('_id')
      .lean();
  ...
  /**
   * Aggregate paid POS bills into dashboard-ready totals.
   *
   * BUG FIX (C1 — POS data silo): The dashboard previously aggregated only
   * the `Order` collection, completely blind to the `PosBill` collection
   * where every in-store POS sale is recorded. Result: a merchant doing
   * ₹50k/day via POS would see "Today's Revenue: ₹0" on the dashboard.
   *
   * This helper computes POS totals over three windows (all-time, this
   * month, last month) so the caller can fold them into the Order-derived
   * totals without touching the rest of the aggregation code.
   */
  private static async getPosBillTotals(
    ...
    const [allAgg, monthAgg, lastMonthAgg] = await Promise.all([
      ...
      PosBill.aggregate([
        { $match: { ...baseMatch, paidAt: { $gte: startOfMonth } } },
        { $group: { _id: null, count: { $sum: 1 }, revenue: { $sum: '$totalAmount' } } },
      ]),
      PosBill.aggregate([
        { $match: { ...baseMatch, paidAt: { $gte: startOfLastMonth, $lte: endOfLastMonth } } },
        { $group: { _id: null, count: { $sum: 1 }, revenue: { $sum: '$totalAmount' } } },
      ]),
    ]);

    const all = allAgg[0] || { count: 0, revenue: 0 };
    const month = monthAgg[0] || { count: 0, revenue: 0 };
    const lastMonth = lastMonthAgg[0] || { count: 0, revenue: 0 };

    return {
      totalRevenue: all.revenue || 0,
      totalOrders: all.count || 0,
      monthlyRevenue: month.revenue || 0,
      monthlyOrders: month.count || 0,
      lastMonthRevenue: lastMonth.revenue || 0,
      lastMonthOrders: lastMonth.count || 0,
    };
  }
  ...
  static async getDashboardMetrics(merchantId: string, storeId?: string): Promise<DashboardMetrics> {
    ...
    // Get all data in parallel. POS bills are queried from PosBill collection
    // via getPosBillTotals; they are folded into revenue/order totals below.
    const [
      orders,
      monthlyOrders,
      lastMonthOrders,
      ...
      this.getPosBillTotals(filterStoreIds, startOfMonth, startOfLastMonth, endOfLastMonth),
    ]);

    // Revenue metrics — includes both Order (online) and PosBill (in-store) totals.
    // See getPosBillTotals for why this fold-in is needed.
    const orderTotalRevenue = orders.reduce((sum: number, order: any) => sum + (order.totals?.total || 0), 0);
    const orderMonthlyRevenue = monthlyOrders.reduce((sum: number, order: any) => sum + (order.totals?.total || 0), 0);
    const orderLastMonthRevenue = lastMonthOrders.reduce((sum: number, order: any) => sum + (order.totals?.total || 0), 0);

    const totalRevenue = orderTotalRevenue + posTotals.totalRevenue;
    const monthlyRevenue = orderMonthlyRevenue + posTotals.monthlyRevenue;
    const lastMonthRevenue = orderLastMonthRevenue + posTotals.lastMonthRevenue;
    const revenueGrowth = lastMonthRevenue > 0 ? ((monthlyRevenue - lastMonthRevenue) / lastMonthRevenue) * 100 : 0;

    // Order metrics — online orders + POS bills. `pendingOrders` / `completedOrders`
    // remain Order-only because PosBill has its own status vocabulary and the
    // dashboard UI maps these to the delivery pipeline.
    const totalOrdersCount = orders.length + posTotals.totalOrders;
    const averageOrderValue = totalOrdersCount > 0 ? totalRevenue / totalOrdersCount : 0;
    const totalOrders = totalOrdersCount;
    const monthlyOrdersCount = monthlyOrders.length + posTotals.monthlyOrders;
    const lastMonthOrdersCount = lastMonthOrders.length + posTotals.lastMonthOrders;
    const ordersGrowth = lastMonthOrdersCount > 0 ? ((monthlyOrdersCount - lastMonthOrdersCount) / lastMonthOrdersCount) * 100 : 0;

...
export class ReportService {
  ...
  // Cap the number of history entries kept in memory. Once the limit is
  // reached the oldest entry (by insertion order, i.e. Map iteration order)
  // is evicted before the new one is added, keeping memory bounded.
  ...
  static shutdown() {
    ...
    logger.info('📊 Automated reporting service shut down');
  }

  // Create a new report schedule
  static createSchedule(
    scheduleData: Omit<ReportSchedule, 'id' | 'createdAt' | 'updatedAt' | 'nextScheduled'>,
  ): ReportSchedule {
  ...
  private static async processScheduledReports() {
    ...
    const dueSchedules = Array.from(this.schedules.values()).filter(
      (schedule) => schedule.isActive && schedule.nextScheduled <= now,
    );
  ...
  private static async simulateEmailSending(historyEntry: ReportHistory, _exportResult: any) {
    try {
  ...
  // Add entry to report history (bounded).
  // When MAX_HISTORY_ENTRIES is reached the oldest entry is evicted so the
  // Map never grows without bound across long-running deployments.

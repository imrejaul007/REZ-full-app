...
// Encrypted bank fields are stored as AES-256-GCM JSON by rez-merchant-service and
// the monolith's own Merchant pre('save') hook. When stepData is returned directly
// (not via the full toJSON transform), we must mask them manually so callers never
// receive raw ciphertext.
const ENCRYPTED_JSON_PREFIX = '{"encrypted":';

function safeMaskBankField(value: string | undefined, visibleChars: number): string | undefined {
  if (!value) return value;
  try {
    const plain = value.startsWith(ENCRYPTED_JSON_PREFIX) ? decrypt(value) : value;
    return maskSensitiveData(plain, visibleChars);
  } catch {
    return '****';
  }
}

function maskBankDetailsInStepData(stepData: any): any {
  if (!stepData?.bankDetails) return stepData;
  return {
    ...stepData,
    bankDetails: {
      ...stepData.bankDetails,
      accountNumber: safeMaskBankField(stepData.bankDetails.accountNumber, 2),
      ifscCode: safeMaskBankField(stepData.bankDetails.ifscCode, 4),
    },
  };
}
...
export class OnboardingService {
  ...
  static async getOnboardingStatus(merchantId: string): Promise<any> {
    ...
    return {
      ...
      // Bank fields inside stepData may be stored encrypted by rez-merchant-service.
      // maskBankDetailsInStepData decrypts + masks before sending to the caller.
      stepData: maskBankDetailsInStepData(merchant.onboarding.stepData || {}),
   ...
   */
  static async saveStepData(merchantId: string, stepNumber: number, data: any): Promise<any> {
    ...
    return {
      ...
      // Bank fields may be stored encrypted — mask before returning to caller.
      stepData: maskBankDetailsInStepData(merchant.onboarding.stepData),
    };
  }
   ...
   */
  static async completeStep(merchantId: string, stepNumber: number): Promise<any> {
    if (stepNumber < 1 || stepNumber > 5) {
      throw new Error('Invalid step number. Must be between 1 and 5.');
    }
   ...
   */
  static async previousStep(merchantId: string, stepNumber: number): Promise<any> {
    const merchant = await Merchant.findById(merchantId);
   ...
   */
  static async approveOnboarding(merchantId: string, adminId: string): Promise<any> {
    ...
    try {
      ...
      // Send approval email
      await EmailService.sendOnboardingApproved(merchant.email, merchant.ownerName, store._id.toString());
   ...
   */
  static async rejectOnboarding(merchantId: string, reason: string, adminId: string): Promise<any> {
    const merchant = await Merchant.findById(merchantId);
    ...
    await EmailService.sendOnboardingRejected(merchant.email, merchant.ownerName, reason);
   ...
   */
  private static async createStoreFromOnboarding(merchant: IMerchant, session: any): Promise<any> {
    const storeData = merchant.onboarding.stepData.storeDetails;
    const _businessInfo = merchant.onboarding.stepData.businessInfo;
  ...
  private static async validateBankDetails(data: any): Promise<boolean> {
    ...
    // Validate IFSC format - always validate regardless of environment
   ...
   */
  private static async validateStepCompletion(merchant: IMerchant, stepNumber: number): Promise<boolean> {
    const stepData = merchant.onboarding.stepData;

    switch (stepNumber) {
      case 1:
        return !!(stepData.businessInfo && stepData.businessInfo.companyName && stepData.businessInfo.businessType);
      case 2:
        return !!(stepData.storeDetails && stepData.storeDetails.storeName && stepData.storeDetails.category);
      case 3:
        return !!(stepData.bankDetails && stepData.bankDetails.accountNumber && stepData.bankDetails.ifscCode);
      case 4:
   ...
   * Send step completion email
   */
  private static async sendStepCompletionEmail(merchant: IMerchant, stepNumber: number): Promise<void> {
    const stepNames = {
  ...
  private static async notifyAdminNewSubmission(merchant: IMerchant): Promise<void> {
    ...
    await EmailService.sendAdminOnboardingNotification(adminEmail, merchant.businessName, String(merchant._id));
  }
  ...
  private static async calculateAverageCompletionTime(): Promise<number> {
    ...
    completedMerchants.forEach((merchant) => {
      if (merchant.onboarding.startedAt && merchant.onboarding.completedAt) {
        const timeDiff = merchant.onboarding.completedAt.getTime() - merchant.onboarding.startedAt.getTime();
        totalTime += timeDiff;
      }
    });
   ...
   */
  static async getCustomerLTV(storeId: string, limit: number = 50): Promise<any[]> {
    try {
      const { Order } = await import('../models/Order');
      const orders = await Order.find({ store: storeId }).lean().sort({ createdAt: -1 });

      const customerMap = new Map<string, any>();

      for (const order of orders) {
        const custId = (order as any).userId?.toString() || (order as any).user?.toString() || 'unknown';
        ...
        customer.totalSpent += (order as any).total || (order as any).totals?.grandTotal || 0;
      ...
      const ltv: any[] = [];
      for (const [custId, data] of customerMap) {
        const daysBetweenVisits =
          data.visitCount > 1 ? (data.lastVisit - data.firstVisit) / (1000 * 60 * 60 * 24) / (data.visitCount - 1) : 0;

        // Segment customers
        let segment: 'high_value' | 'medium_value' | 'low_value' = 'low_value';
        if (data.totalSpent >= 5000) {
          segment = 'high_value';
        } else if (data.totalSpent >= 2000) {
        ...
        ltv.push({
          ...
          averageOrderValue: data.totalSpent / data.visitCount,
          firstVisit: data.firstVisit,
          lastVisit: data.lastVisit,
          visitFrequencyDays: Math.round(daysBetweenVisits),
          segment,
        });
      }

      // Sort by total spent
      ltv.sort((a, b) => b.totalSpent - a.totalSpent);

      return ltv.slice(0, limit);
    } catch (error) {
   ...
   */
  static async getVisitFrequency(storeId: string): Promise<any> {
    try {
      const { Order } = await import('../models/Order');
      ...
      for (const order of orders) {
        const custId = (order as any).userId?.toString() || (order as any).user?.toString() || 'unknown';
        customerVisits.set(custId, (customerVisits.get(custId) || 0) + 1);
      }

      const totalUniqueCustomers = customerVisits.size;
      const oneTimeVisitors = Array.from(customerVisits.values()).filter((v) => v === 1).length;
      const repeatVisitors = totalUniqueCustomers - oneTimeVisitors;
      const frequentVisitors = Array.from(customerVisits.values()).filter((v) => v >= 5).length;
      const averageVisitsPerCustomer =
        totalUniqueCustomers > 0
          ? Array.from(customerVisits.values()).reduce((a, b) => a + b, 0) / totalUniqueCustomers
          : 0;
      const repeatVisitRate = totalUniqueCustomers > 0 ? Math.round((repeatVisitors / totalUniqueCustomers) * 100) : 0;
      ...
      return {
        ...
        repeatVisitors,
        frequentVisitors,
        averageVisitsPerCustomer: Math.round(averageVisitsPerCustomer * 100) / 100,
        repeatVisitRate,
      };
    } catch (error) {
      logger.error('Error calculating visit frequency:', error);
      throw error;
    }
  }
}

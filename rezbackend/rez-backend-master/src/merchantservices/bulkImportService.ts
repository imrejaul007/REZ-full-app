...
export class BulkImportService {
   ...
   */
  async validateProductRow(row: any, rowNumber: number, storeId: string, merchantId: string): Promise<ImportRow> {
    const errors: string[] = [];
    ...
    if (row.category) {
      ...
      if (Types.ObjectId.isValid(row.category)) {
        ...
        const category = await Category.findOne({
          name: new RegExp(`^${row.category.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'i'),
        });
        if (!category) {
    ...
    if (row.subcategory && categoryId) {
      if (Types.ObjectId.isValid(row.subcategory)) {
        ...
        const subCategory = await Category.findOne({
          name: new RegExp(`^${row.subcategory.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'i'),
   ...
   */
  async processProductRow(validatedRow: ImportRow, storeId: string, merchantId: string): Promise<ImportRow> {
    try {
      ...
      // Parse tags
      const tags = data.tags ? data.tags.split(',').map((tag: string) => tag.trim().toLowerCase()) : [];
      ...
      const productData: Partial<IProduct> = {
        ...
        images: images.length > 0 ? images : ['https://placehold.co/400'],
        pricing: {
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i += this.batchSize) {
        ...
        const validatedBatch = await Promise.all(
          batch.map((row, index) => this.validateProductRow(row, i + index + 1, storeId, merchantId)),
        );

...
export class MerchantCacheService {
  ...
  static invalidateDataType(
    merchantId: string,
    dataType: 'products' | 'orders' | 'cashback' | 'metrics' | 'merchant',
  ): void {
...
export class StreamingCacheService {
  ...
  // Cap on how many unique dataset keys can be held in chunkedCache at once.
  // Each key holds multiple chunk Maps; 200 keys × typical payload = ~20–40 MB max.
  private static readonly MAX_CHUNK_KEYS = 200;
  ...
  static setChunkedData<T>(key: string, data: T[], ttlSeconds?: number): void {
    // Evict the oldest key if we're at the cap and this is a new key
    if (!this.chunkedCache.has(key) && this.chunkedCache.size >= this.MAX_CHUNK_KEYS) {
      const oldestKey = this.chunkedCache.keys().next().value;
      if (oldestKey) this.clearChunkedData(oldestKey);
    }

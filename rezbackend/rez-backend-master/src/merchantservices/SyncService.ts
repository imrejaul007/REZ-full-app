...
import { SyncHistoryModel } from '../models/SyncHistory';
...
export class SyncService {
  ...
  // MP-005 FIX: use a class-level Map instead of global.syncIntervals so that
  // (a) TypeScript can type-check it properly, (b) it is isolated from other
  // global state, and (c) clearAllAutoSyncs() can be called during shutdown to
  // prevent orphaned intervals keeping the process alive after SIGTERM.
  private static syncIntervals = new Map<string, NodeJS.Timeout>();

  /** Cancel all per-merchant auto-sync intervals — call during graceful shutdown. */
  static clearAllAutoSyncs(): void {
    for (const [merchantId, interval] of SyncService.syncIntervals) {
      clearInterval(interval);
      logger.info(`[SyncService] Cleared auto-sync interval for merchant ${merchantId}`);
    }
    SyncService.syncIntervals.clear();
  }
  ...
  static async syncToCustomerApp(config: SyncConfig): Promise<SyncResult> {
    ...
    try {
      ...
      // Keep only last 50 sync results in memory
      ...
      }).catch((err) => logger.warn('SyncHistory persist failed:', err));
    }
  ...
  private static async syncProducts(config: SyncConfig) {
    ...
    try {
      ...
      result.updated = products.filter(
        (p) => config.lastSync && p.updatedAt > config.lastSync && p.createdAt <= config.lastSync,
      ).length;
  ...
  private static async syncOrders(config: SyncConfig) {
    ...
    try {
      ...
      const ordersToSync = config.lastSync ? orders.filter((order) => order.updatedAt > config.lastSync!) : orders;
      ...
      result.updated = ordersToSync.filter(
        (o) => config.lastSync && o.updatedAt > config.lastSync && o.createdAt <= config.lastSync,
      ).length;
  ...
  private static async syncCashback(config: SyncConfig) {
    ...
    try {
      ...
      result.updated = cashbackToSync.filter(
        (c) => config.lastSync && c.updatedAt > config.lastSync && c.createdAt <= config.lastSync,
      ).length;
  ...
  private static async syncProductsToDatabase(
    ...
    for (const merchantProduct of customerAppProducts) {
      try {
        ...
        const productData = {
          ...
          pricing: {
            ...
            discount: merchantProduct.originalPrice
              ? Math.round(
                  ((merchantProduct.originalPrice - merchantProduct.price) / merchantProduct.originalPrice) * 100,
                )
              : 0,
          },
          inventory: {
          ...
          specifications: [],
          tags: Object.keys(merchantProduct.attributes)
            .filter((key) => Array.isArray(merchantProduct.attributes[key]))
        ...
        await Product.findOneAndUpdate({ sku: merchantProduct.productId }, productData, {
          upsert: true,
          new: true,
          session,
        });
  ...
  private static async syncMerchantToDatabase(merchantData: any, merchantId: string, session: any): Promise<number> {
    try {
      ...
      const storeData = {
        ...
        slug: (merchantData.businessName || merchantData.displayName)
          .toLowerCase()
          .replace(/[^a-z0-9\s]/g, '')
          .replace(/\s+/g, '-'),
      ...
      // Upsert store
      await Store.findOneAndUpdate({ merchantId: new mongoose.Types.ObjectId(merchantId) }, storeData, {
        upsert: true,
        new: true,
        session,
      });
  ...
  // Get sync history for a merchant (DB-backed, falls back to in-memory)
  static async getSyncHistory(merchantId: string, limit: number = 10): Promise<SyncResult[]> {
    try {
      const docs = await SyncHistoryModel.find({ merchantId }).sort({ syncedAt: -1 }).limit(limit).lean();
      if (docs.length > 0) {
        return docs.map((d) => ({
          success: d.success,
          syncId: d.syncId,
          ...
          syncedAt: d.syncedAt,
          results: d.results as SyncResult['results'],
          errors: d.syncErrors,
          duration: d.duration,
        }));
      }
  ...
  static scheduleAutoSync(merchantId: string, intervalMinutes: number = 15) {
    // MP-005 FIX: clear existing interval before creating a new one so that
    // repeated calls for the same merchant don't accumulate duplicate timers.
    this.clearAutoSync(merchantId);

    const interval = setInterval(
      async () => {
        try {
          const lastSync = await this.getLastSyncDate(merchantId);
    ...
    // MP-005 FIX: store in class-level Map (not global) for proper lifecycle management
    SyncService.syncIntervals.set(merchantId, interval);
  ...
  static clearAutoSync(merchantId: string) {
    // MP-005 FIX: use class-level Map instead of global.syncIntervals
    if (SyncService.syncIntervals.has(merchantId)) {
      clearInterval(SyncService.syncIntervals.get(merchantId)!);
      SyncService.syncIntervals.delete(merchantId);
      logger.info(`⏹️ Cleared auto-sync for merchant ${merchantId}`);
    }
  }

  // Get last sync date for a merchant — queries MongoDB first so the value
  // survives process restarts, then falls back to in-memory history.
  private static async getLastSyncDate(merchantId: string): Promise<Date | undefined> {
    try {
      const doc = (await SyncHistoryModel.findOne({ merchantId, success: true })
        .sort({ syncedAt: -1 })
        .select('syncedAt')
        .lean()) as { syncedAt?: Date } | null;
      if (doc?.syncedAt) {
        return doc.syncedAt;
      }
    } catch (err) {
      logger.warn(`[SyncService] getLastSyncDate DB query failed, falling back to in-memory: ${err}`);
    }
  ...
  // Get next scheduled sync time (uses in-memory history only — synchronous helper)
  private static getNextScheduledSync(merchantId: string): Date | null {
    ...
    if (lastSync?.syncedAt) {
      return new Date(lastSync.syncedAt.getTime() + 15 * 60 * 1000); // 15 minutes
    }
    return null;
  }
  ...
  // Get sync statistics scoped to a specific merchant
  static getSyncStatistics(merchantId: string) {
    const merchantHistory = this.syncHistory.filter((s) => s.merchantId === merchantId);
    const stats = {
      merchantId,
      totalSyncs: merchantHistory.length,
      successfulSyncs: merchantHistory.filter((s) => s.success).length,
      failedSyncs: merchantHistory.filter((s) => !s.success).length,
      averageDuration: 0,
      isActive: this.activeSyncs.get(merchantId) || false,
      hasAutoSync: SyncService.syncIntervals.has(merchantId),
    };

    if (stats.totalSyncs > 0) {
      stats.averageDuration = merchantHistory.reduce((sum, sync) => sum + sync.duration, 0) / stats.totalSyncs;
    }

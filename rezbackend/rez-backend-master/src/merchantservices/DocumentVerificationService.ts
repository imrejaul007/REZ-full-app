...
export class DocumentVerificationService {
  ...
  /**
   * OCR document extraction (FEAT-3: Graceful fallback when OCR not configured)
   */
  static async extractDocumentData(documentUrl: string, documentType: string): Promise<any> {
    const OCR_CONFIGURED = !!(process.env.GOOGLE_CLOUD_VISION_API_KEY || process.env.AZURE_VISION_KEY);

    if (!OCR_CONFIGURED) {
      logger.info('[OCR] Vision API not configured — manual verification required for:', documentType);
      return {
        extracted: false,
        requiresManualReview: true,
        message: 'Automatic OCR not configured. Document will be reviewed manually within 24 hours.',
        documentType,
        fields: {},
        suggestedData: {}
      };
    ...
    // Real OCR implementation goes here when API key is added
    logger.info('[OCR] OCR is configured but not yet implemented');
    return {
      extracted: false,

...
export class AnalyticsService {
   ...
   */
  static async getSalesOverview(storeId: string, startDate: Date, endDate: Date): Promise<SalesOverview> {
    ...
    // Current period aggregation — secondary replica to avoid primary contention during payments
  ...
  static async getCategoryPerformance(
    ...
    for (const [categoryId, data] of categoryMap.entries()) {
      ...
      // Fetch actual category name, fall back to categoryId if not found
      ...
      if (categoryId !== 'uncategorized') {
        try {
          ...
          logger.warn('[AnalyticsService] Failed to fetch category name', {
            categoryId,
   ...
   */
  static async getCustomerInsights(storeId: string, startDate?: Date, endDate?: Date): Promise<CustomerInsight> {
    // Build date filter
    const dateFilter: any = {
      'items.store': new Types.ObjectId(storeId),
      status: { $nin: ['cancelled', 'refunded'] },
    ...
    // Get top 10 customers with actual user names
    const topCustomersData = customerStats.sort((a, b) => b.totalSpent - a.totalSpent).slice(0, 10);

    const topCustomers = await Promise.all(
      topCustomersData.map(async (c) => {
        let userName = 'Customer ' + c.userId.substring(0, 8);
        try {
          const user = await User.findById(c.userId).select('profile.firstName profile.lastName').lean();
          if (user?.profile?.firstName || user?.profile?.lastName) {
            userName = `${user.profile?.firstName || ''} ${user.profile?.lastName || ''}`.trim();
          }
        } catch (error) {
          logger.warn('[AnalyticsService] Failed to fetch user name', {
            userId: c.userId,
            error: (error as Error).message,
          });
        }
  ...
  static async getInventoryStatus(storeId: string): Promise<InventoryStatus> {
    ...
    const inStockProducts = products.filter(
      (p) => p.inventory.unlimited || (p.inventory.isAvailable && p.inventory.stock > 0),
    ).length;

    const lowStockProducts = products.filter(
      (p) =>
        !p.inventory.unlimited && p.inventory.stock > 0 && p.inventory.stock <= (p.inventory.lowStockThreshold || 5),
    ).length;
   ...
   */
  static async getSalesByTimeOfDay(storeId: string): Promise<
    Array<{
      hour: number;
      revenue: number;
      orders: number;
   ...
   */
  static async getSalesByDayOfWeek(storeId: string): Promise<
    Array<{

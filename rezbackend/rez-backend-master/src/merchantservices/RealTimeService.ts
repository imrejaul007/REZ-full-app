...
export class RealTimeService {
  ...
  static getInstance(io: SocketIOServer): RealTimeService {
    ...
    } else if (RealTimeService.instance.io !== io) {
      // MP-D008 FIX: If a new io instance is provided (e.g. after a test teardown
      // or a server restart within the same process) we must clear the old metrics
      // interval before constructing a new instance.  Without this, the stale
      // interval keeps firing DB queries against a closed io server indefinitely,
      // leaking one setInterval handle every restart.
  ...
  // MP-D007 FIX: Validate merchant IDs supplied in socket event payloads.
  // Without validation, a malicious client can supply arbitrary strings as
  // merchantId and cause the server to join rooms with unbounded names
  // (e.g. a 10 MB string) consuming heap for each room entry in the adapter's
  // internal Map.  We accept only MongoDB ObjectId-shaped strings (24 hex chars).
  private static isValidMerchantId(id: unknown): id is string {
  ...
  private setupEventHandlers() {
    this.io.on('connection', (socket) => {
      ...
      socket.on('join-merchant-dashboard', (merchantId: string) => {
        // MP-D007 FIX: Validate before joining room
        if (!RealTimeService.isValidMerchantId(merchantId)) {
          logger.warn(`[RealTimeService] join-merchant-dashboard rejected — invalid merchantId from ${socket.id}`);
          return;
        }
      ...
      socket.on('subscribe-metrics', (merchantId: string) => {
        if (!RealTimeService.isValidMerchantId(merchantId)) return;
      ...
      socket.on('subscribe-products', (merchantId: string) => {
        if (!RealTimeService.isValidMerchantId(merchantId)) return;
        socket.join(`products-${merchantId}`);
        logger.info(`Subscribed to products for merchant ${merchantId}`);
      });

      socket.on('subscribe-notifications', (merchantId: string) => {
        if (!RealTimeService.isValidMerchantId(merchantId)) return;
        // Notifications are broadcast to merchant-${merchantId} on join-merchant-dashboard.
        // This event ensures clients that join late also join the notification room.
        socket.join(`merchant-${merchantId}`);
        logger.info(`Subscribed to notifications for merchant ${merchantId}`);
      });

      socket.on('leave-merchant-dashboard', (merchantId: string) => {
        const prefixes = ['merchant-', 'dashboard-', 'metrics-', 'orders-', 'cashback-', 'products-'];
        prefixes.forEach((prefix) => socket.leave(`${prefix}${merchantId}`));
        logger.info(`Merchant ${merchantId} left dashboard rooms`);
      });
      ...
      // Echo pong so merchant app can measure round-trip latency
  ...
  public emitProductEvent(merchantId: string, event: RealTimeEvent) {
    this.io.to(`products-${merchantId}`).emit('product-event', event);
    this.io.to(`dashboard-${merchantId}`).emit('dashboard-event', event);

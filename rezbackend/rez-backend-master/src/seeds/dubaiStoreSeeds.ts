...
const DUBAI_MARINA_COORDS: [number, number] = [55.141, 25.0805];
const DEIRA_CITY_COORDS: [number, number] = [55.3333, 25.27];
...
export async function seedDubaiStores(): Promise<void> {
  logger.info('Seeding Dubai stores...');
  ...
  try {
    ...
    for (const storeSeed of dubaiStoreSeeds) {
      ...
      if (!existingStore) {
        ...
        logger.info(`  Created store: ${storeSeed.name}`);
      } else {
        logger.info(`  Store already exists: ${storeSeed.name}`);
      }
    }

    // Seed products
    logger.info('Seeding Dubai products...');
      ...
      if (!existingProduct) {
        await Product.create({
          ...
          pricing: {
            ...
            discount: Math.round(
              ((productSeed.pricing.original - productSeed.pricing.selling) / productSeed.pricing.original) * 100,
            ),
        ...
        logger.info(`  Created product: ${productSeed.name}`);
      } else {
        logger.info(`  Product already exists: ${productSeed.name}`);
      }
    }

    logger.info('Dubai stores and products seeded successfully!');
  } catch (error) {
    logger.error('Error seeding Dubai stores:', error);
    throw error;
  }
}

...
export async function seedExclusiveZoneOffers() {
  logger.info('Seeding Exclusive Zone Offers...');
  ...
  try {
    ...
    if (stores.length === 0) {
      logger.info('  No active stores found. Please seed stores first.');
      logger.info('  Run: npx ts-node -r dotenv/config src/seeds/storeSeeds.ts');
      return;
    }
    ...
    if (existingOffers > 0) {
      logger.info(`  ${existingOffers} exclusive zone offers already exist, skipping seeding`);
      return;
    }
    ...
    logger.info(`  Created ${exclusiveZoneOffers.length} exclusive zone offers`);
    ...
    Object.entries(zoneCounts).forEach(([zone, count]) => {
      logger.info(`     - ${zone}: ${count} offers`);
    });

    logger.info('Exclusive Zone Offers seeded successfully!');
  } catch (error) {
    logger.error('Error seeding Exclusive Zone Offers:', error);
    throw error;
  }
}
...
export async function clearExclusiveZoneOffers() {
  logger.info('Clearing Exclusive Zone Offers...');

  const result = await Offer.deleteMany({
    exclusiveZone: { $exists: true, $ne: null },
  });

  logger.info(`Deleted ${result.deletedCount} exclusive zone offers`);
}

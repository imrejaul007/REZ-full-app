...
const categorySeeds = [
  ...
  },
  {
    ...
    occasions: [{ id: 'coffee-date', name: 'Coffee Date', icon: '☕', color: '#8D6E63', discount: 15 }],
    trendingHashtags: [{ id: '1', tag: '#CoffeeLover', count: 2100, color: '#8D6E63', trending: true }],
  },
  {
    name: 'Bars & Pubs',
    slug: 'bars-pubs',
...
async function runMasterSeeds() {
  logger.info('🚀 Starting Master Seeds...\n');
  ...
  try {
    ...
    logger.info(`   📊 Total categories: ${categorySeeds.length}\n`);
    ...
    logger.info(`   📊 Total stores: ${storeSeeds.length}\n`);

    // Seed Hero Banners
    logger.info('🖼️ Seeding Hero Banners...');
    for (const banner of heroBannerSeeds) {
      await HeroBanner.findOneAndUpdate({ title: banner.title }, banner, { upsert: true, new: true });
      logger.info(`   ✅ Banner: ${banner.title}`);
    }
    logger.info(`   📊 Total banners: ${heroBannerSeeds.length}\n`);
    ...
    // BUG-012 FIX: Use configurable admin email from environment variable
    const ADMIN_EMAIL = process.env.ADMIN_EMAIL || 'admin@rez.app';
    let adminUser = await User.findOne({ $or: [{ email: ADMIN_EMAIL }, { isAdmin: true }] });
    if (!adminUser) {
    ...
    for (const offer of offerSeeds) {
      ...
      logger.info(`   ✅ Offer: ${offer.title}`);
    }
    logger.info(`   📊 Total offers: ${offerSeeds.length}\n`);

    // Seed Campaigns
    logger.info('📢 Seeding Campaigns...');
    for (const campaign of campaignSeeds) {
      await Campaign.findOneAndUpdate({ campaignId: campaign.campaignId }, campaign, { upsert: true, new: true });
      logger.info(`   ✅ Campaign: ${campaign.title}`);
    }
    logger.info(`   📊 Total campaigns: ${campaignSeeds.length}\n`);

    // Seed Store Experiences
    logger.info('🛍️ Seeding Store Experiences...');
    for (const experience of experienceSeeds) {
      await StoreExperience.findOneAndUpdate({ slug: experience.slug }, experience, { upsert: true, new: true });
      logger.info(`   ✅ Experience: ${experience.title}`);
    }
    logger.info(`   📊 Total experiences: ${experienceSeeds.length}\n`);

    // Seed Events
    logger.info('🎭 Seeding Events...');
    for (const event of eventSeeds) {
      await Event.findOneAndUpdate({ title: event.title }, event, { upsert: true, new: true });
      logger.info(`   ✅ Event: ${event.title}`);
    }
    logger.info(`   📊 Total events: ${eventSeeds.length}\n`);

    // Seed Service Categories
    logger.info('🔧 Seeding Service Categories...');
    for (const category of serviceCategorySeeds) {
      await ServiceCategory.findOneAndUpdate({ slug: category.slug }, category, { upsert: true, new: true });
      logger.info(`   ✅ Service Category: ${category.name}`);
    }
    logger.info(`   📊 Total service categories: ${serviceCategorySeeds.length}\n`);
    ...
    logger.info('🎉 Master seeds completed successfully!');
    logger.info('━'.repeat(50));
    logger.info('📊 SUMMARY');
    logger.info('━'.repeat(50));
    logger.info(`   Categories:         ${categorySeeds.length}`);
    logger.info(`   Stores:             ${storeSeeds.length}`);
    logger.info(`   Hero Banners:       ${heroBannerSeeds.length}`);
    logger.info(`   Offers:             ${offerSeeds.length}`);
    logger.info(`   Campaigns:          ${campaignSeeds.length}`);
    logger.info(`   Store Experiences:  ${experienceSeeds.length}`);
    logger.info(`   Events:             ${eventSeeds.length}`);
    logger.info(`   Service Categories: ${serviceCategorySeeds.length}`);
    logger.info('━'.repeat(50));
    logger.info(`   TOTAL ITEMS:        ${totalItems}`);
    logger.info('━'.repeat(50));
  } catch (error) {
    logger.error('❌ Error running master seeds:', error);
    throw error;
  } finally {

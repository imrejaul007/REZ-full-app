...
export async function seedHomepageDeals() {
  logger.info('🌱 Seeding Homepage Deals Section...');
  ...
  try {
    ...
    if (!existingSection) {
      ...
      logger.info('  ✅ Created section config');
    } else {
      logger.info('  ⏭️ Section config already exists');
    }

    // Check if items already exist
    const existingItems = await HomepageDealsItem.countDocuments();
    if (existingItems > 0) {
      logger.info(`  ⏭️ ${existingItems} items already exist, skipping item seeding`);
      return;
    }
    ...
    logger.info(`  ✅ Created ${offersItems.length + cashbackItems.length + exclusiveItems.length} items`);

    logger.info('✅ Homepage Deals Section seeded successfully!');
  } catch (error) {
    logger.error('❌ Error seeding Homepage Deals Section:', error);
    throw error;
  }
}
...
export async function clearHomepageDeals() {
  logger.info('🗑️ Clearing Homepage Deals data...');

  await HomepageDealsSection.deleteMany({});
  await HomepageDealsItem.deleteMany({});

  logger.info('✅ Homepage Deals data cleared');
}

...
const getTournamentData = () => {
  ...
  return [
    ...
    },
    {
      ...
      rules: ['Complete memory match games', 'Fewer moves = Higher score', 'Faster completion = Bonus points'],
      totalPrizePool: 1750,
      featured: false,
    },
    {
      ...
      rules: ['Play Coin Hunt daily', 'Collect as many coins as possible', 'Complete bonus rounds for extra points'],
      totalPrizePool: 6500,
      featured: true,
    },
    {
...
async function seedCreators() {
  ...
  for (const creator of CREATORS) {
    ...
    logger.info(`  ✓ Created creator: ${creator.firstName} ${creator.lastName}`);
  }
...
async function seedVideosForCreators(creators: any[]) {
  ...
  for (const creator of creators) {
    ...
    if (existingVideos >= 3) {
      logger.info(`  → Creator ${creator.profile?.firstName} already has ${existingVideos} videos`);
      continue;
    }
    ...
    logger.info(`  ✓ Created ${numVideos} videos for ${creator.profile?.firstName}`);
  }

  logger.info(`  Total videos created: ${videoCount}`);
}
...
async function seedTournaments() {
  ...
  for (const tournament of tournamentData) {
    ...
    if (existing) {
      logger.info(`  → Tournament "${tournament.name}" already exists`);
      continue;
    }

    const newTournament = new Tournament(tournament);
    await newTournament.save();
    logger.info(`  ✓ Created tournament: ${tournament.name}`);
  }
}

async function seedCoinDrops() {
  logger.info('💰 Seeding coin drops...');

  // Get some stores for coin drops
  const stores = await Store.find({ isActive: true }).limit(10).lean();
  if (stores.length === 0) {
    logger.info('  ⚠ No stores found, skipping coin drops');
    return;
  }
  ...
  for (let i = 0; i < Math.min(stores.length, 10); i++) {
    ...
    if (existing) {
      logger.info(`  → Coin drop for ${store.name} already exists`);
      continue;
    }
    ...
    await coinDrop.save();
    logger.info(`  ✓ Created ${multiplier}x coin drop for ${store.name}`);
  }
}
...
async function runSeeds() {
  logger.info('🚀 Starting Play & Earn Seeds...\n');
  ...
  try {
    ...
    logger.info('🎉 Play & Earn seeds completed successfully!\n');
    ...
    logger.info(`   - Verified Users: ${creatorCount}`);
    logger.info(`   - Published Videos: ${videoCount}`);
    logger.info(`   - Tournaments: ${tournamentCount}`);
    logger.info(`   - Active Coin Drops: ${coinDropCount}`);
  } catch (error) {

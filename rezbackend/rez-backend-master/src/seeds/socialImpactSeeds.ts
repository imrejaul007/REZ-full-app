...
const sponsorSeeds = [
  {
    ...
    description:
      "Tata Group is one of India's largest conglomerates, committed to improving the quality of life of communities through CSR initiatives.",
  ...
  },
  {
    ...
    description:
      'Reliance Industries Limited is committed to sustainable development and community welfare through various CSR programs.',
    brandCoinName: 'Reliance Coins',
    brandCoinLogo:
      'https://upload.wikimedia.org/wikipedia/en/thumb/9/99/Reliance_Industries_Logo.svg/200px-Reliance_Industries_Logo.svg.png',
    contactPerson: {
      name: 'CSR Division',
      email: 'csr@ril.com',
  ...
  },
  {
    ...
    brandCoinLogo:
      'https://upload.wikimedia.org/wikipedia/en/thumb/f/f4/ITC_Limited_Logo.svg/200px-ITC_Limited_Logo.svg.png',
    contactPerson: {
      name: 'CSR Team',
      email: 'csr@itc.in',
  ...
  },
  {
    ...
    brandCoinLogo:
      'https://upload.wikimedia.org/wikipedia/en/thumb/8/88/Sun_Pharmaceutical_Industries.svg/200px-Sun_Pharmaceutical_Industries.svg.png',
    contactPerson: {
      name: 'CSR Department',
  ...
  return [
    {
      ...
      type: 'social_impact',
      description:
        'Join us for a life-saving blood donation drive. Your single donation can save up to 3 lives. All blood types are needed.',
      status: 'active',
    ...
    },
    {
      ...
      type: 'social_impact',
      description:
        'Be part of the Green India Mission. Plant trees and contribute to a greener, cleaner environment for future generations.',
      status: 'active',
    ...
    },
    {
      ...
      type: 'social_impact',
      description:
        'Help us clean Marina Beach and restore its natural beauty. Every piece of trash removed makes a difference.',
      status: 'active',
      startDate: oneWeekFromNow,
    ...
    },
    {
      ...
      benefits: ['Free health screening', 'Refreshments provided', 'Donation certificate', `${BRAND.COIN_NAME}`],
      schedule: [
...
async function seedSocialImpact() {
  try {
    logger.info('🌱 Starting Social Impact seeding...');

    // Connect to database
    await connectDatabase();

    // Clear existing data
    logger.info('🗑️ Clearing existing sponsors, events and allocations...');
    await Sponsor.deleteMany({});
    await Program.deleteMany({ type: 'social_impact' });
    await SponsorAllocation.deleteMany({});

    // Seed sponsors
    logger.info('📝 Seeding sponsors...');
    const createdSponsors = await Sponsor.insertMany(sponsorSeeds);
    logger.info(`✅ Created ${createdSponsors.length} sponsors`);
    ...
    if (testMerchant) {
      ...
      logger.info(`✅ Assigned merchant "${testMerchant.businessName}" to all seeded events`);
    } else {
      logger.info('⚠️ No active merchant found — events created without merchant assignment');
    }

    // Create SponsorAllocation fund entries for each sponsor
    logger.info('💰 Creating sponsor budget allocations...');
    ...
    logger.info(`✅ Created budget allocations for ${allocationsCreated} sponsors`);

    // Update sponsor stats
    logger.info('📊 Updating sponsor statistics...');
    for (const sponsor of createdSponsors) {
    ...
    logger.info('✅ Sponsor statistics updated');

    logger.info('🎉 Social Impact seeding completed successfully!');

    // Summary
    logger.info('\n📊 Seeding Summary:');
    logger.info(`   Sponsors: ${createdSponsors.length}`);
    logger.info(`   Events: ${createdEvents.length}`);
    logger.info(`   Budget allocations: ${allocationsCreated}`);

    process.exit(0);
  } catch (error) {
    logger.error('❌ Error seeding social impact data:', error);
    process.exit(1);
  }
}

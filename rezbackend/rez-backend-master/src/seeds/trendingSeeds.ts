...
const trendingStoreSeeds = [
  // FOOD DELIVERY - 3 stores
  {
    ...
    rewardRules: {
      baseCashbackPercent: 18,
      reviewBonusCoins: 50,
      socialShareBonusCoins: 25,
      minimumAmountForReward: 200,
    },
  ...
  },
  {
    ...
    analytics: {
      ...
      avgOrderValue: 500,
      repeatCustomers: 3100,
      followersCount: 4200,
      views: 12000,
    },
    rewardRules: {
      baseCashbackPercent: 15,
      reviewBonusCoins: 40,
      socialShareBonusCoins: 20,
      minimumAmountForReward: 200,
    },
    orderCount: 20,
  },
  {
    ...
    analytics: {
      ...
      avgOrderValue: 500,
      repeatCustomers: 2500,
      followersCount: 3100,
      views: 9000,
    },
    rewardRules: {
      baseCashbackPercent: 12,
      reviewBonusCoins: 35,
      socialShareBonusCoins: 18,
      minimumAmountForReward: 150,
    },
    orderCount: 15,
  },
  ...
  // SALON & SPA - 2 stores
  {
    ...
    rewardRules: {
      baseCashbackPercent: 25,
      reviewBonusCoins: 75,
      socialShareBonusCoins: 40,
      minimumAmountForReward: 400,
    },
    orderCount: 18,
  },
  {
    ...
    rewardRules: {
      ...
      reviewBonusCoins: 60,
      socialShareBonusCoins: 30,
      minimumAmountForReward: 350,
    },
    orderCount: 12,
  },
  ...
  // GROCERY - 2 stores
  {
    ...
    rewardRules: {
      ...
      reviewBonusCoins: 30,
      socialShareBonusCoins: 15,
      minimumAmountForReward: 300,
    },
    orderCount: 22,
  },
  {
    ...
    analytics: {
      ...
      avgOrderValue: 400,
      repeatCustomers: 2800,
      followersCount: 3200,
      views: 8000,
    },
    rewardRules: {
      baseCashbackPercent: 8,
      reviewBonusCoins: 25,
      socialShareBonusCoins: 12,
      minimumAmountForReward: 250,
    },
    orderCount: 10,
  },
  ...
  // ELECTRONICS - 2 stores
  {
    ...
    rewardRules: {
      baseCashbackPercent: 12,
      reviewBonusCoins: 100,
      socialShareBonusCoins: 50,
      minimumAmountForReward: 800,
    },
    orderCount: 14,
  },
  {
...
async function runTrendingSeeds() {
  logger.info('🔥 Starting Trending Seeds...\n');
  ...
  try {
    ...
    logger.info('📊 Found', Object.keys(categoryMap).length, 'categories\n');
    ...
    logger.info('🏪 Seeding Trending Stores...');
    for (const storeData of trendingStoreSeeds) {
      ...
      if (store) {
        ...
        logger.info(`   📝 Updated: ${storeData.name}`);
      } else {
        // Create new store
        ...
        logger.info(`   ✅ Created: ${storeData.name}`);
      }

      if (!store) {
        logger.info(`   ⚠️ Failed to create/update store: ${storeData.name}`);
        continue;
      }
      ...
      if (!product) {
        ...
        logger.info(`   📦 Created product for: ${storeData.name}`);
      }
      createdProducts.push({ product, store, orderCount });
    }
    logger.info(`   📊 Total stores: ${trendingStoreSeeds.length}\n`);

    // Seed Orders for trending calculation
    logger.info('📝 Seeding Orders for trending calculation...');
    let totalOrders = 0;
    ...
    for (const { product, store, orderCount } of createdProducts) {
      ...
      logger.info(`   ✅ Created ${orderCount} orders for: ${store.name}`);
    }

    logger.info(`\n   📊 Total orders created: ${totalOrders}\n`);

    // Print summary
    logger.info('🎉 Trending seeds completed successfully!');
    logger.info('━'.repeat(50));
    logger.info('📊 SUMMARY');
    logger.info('━'.repeat(50));
    logger.info(`   Trending Stores:    ${trendingStoreSeeds.length}`);
    logger.info(`   Products Created:   ${createdProducts.length}`);
    logger.info(`   Orders Created:     ${totalOrders}`);
    logger.info('━'.repeat(50));
    logger.info('\n📌 Next steps:');
    logger.info('   1. Restart the backend server');
    logger.info('   2. Clear Redis cache (if needed)');
    logger.info('   3. Test /stores/trending endpoint');
    logger.info('   4. Verify videos show in frontend');
  } catch (error) {
    logger.error('❌ Error running trending seeds:', error);
    throw error;
  } finally {
...
if (require.main === module) {
  runTrendingSeeds()
    ...
    .catch((error) => {
      logger.error('❌ Trending seeding failed:', error);
      process.exit(1);
    });
}

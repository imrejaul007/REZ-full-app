...
async function getOrCreatePlatformStore() {
  ...
  if (!store) {
    ...
    logger.info('   ✅ Created platform store for financial services');
  }
...
async function runFinancialServicesSeeds() {
  try {
    logger.info('🌱 Starting financial services seeds...\n');

    // Get or create platform store
    logger.info('🏪 Setting up platform store...');
    const store = await getOrCreatePlatformStore();
    logger.info(`   ✅ Store ready: ${store.name}\n`);

    // Get or create main category
    logger.info('📁 Setting up main category...');
    ...
    if (!mainCategory) {
      ...
      logger.info('   ✅ Created main category');
    } else {
      logger.info('   ✅ Found existing category');
    }
    logger.info('');

    // Seed Service Categories
    logger.info('📂 Seeding Financial Service Categories...');
    const createdCategories = [];
    for (const categoryData of financialCategorySeeds) {
      const category = await ServiceCategory.findOneAndUpdate({ slug: categoryData.slug }, categoryData, {
        upsert: true,
        new: true,
      });
    ...
    // Seed Products
    logger.info('💳 Seeding Financial Service Products...');
    ...
    logger.info('📊 Updating service counts...');
    for (const category of createdCategories) {
      ...
      logger.info(`   ✅ ${category.name}: ${count} services`);
    }
    logger.info('');

    logger.info('🎉 Financial services seeds completed successfully!');
    logger.info(`   Total categories: ${createdCategories.length}`);
    logger.info(`   Total products: ${products.length}`);
  } catch (error) {
    logger.error('❌ Error running financial services seeds:', error);
    throw error;
  }
}

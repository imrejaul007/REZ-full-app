  ...
  },
  {
    ...
    deals: [
      ...
      },
      {
        store: 'First Visit',
        bonus: '1000 coins',
        image: 'https://images.unsplash.com/photo-1555529902-5261145633bf?w=300',
      },
      {
        store: 'Sign Up Bonus',
        bonus: '₹300 cashback',
...
async function runHomepageSeeds() {
  try {
    logger.info('🌱 Starting homepage seeds...\n');
    ...
    logger.info('🏪 Seeding Store Experiences...');
    ...
    logger.info('🎉 Homepage seeds completed successfully!');
    logger.info(
      `   Total items seeded: ${campaignSeeds.length + experienceSeeds.length + serviceCategorySeeds.length + eventSeeds.length}`,
    );
  } catch (error) {
    logger.error('❌ Error running homepage seeds:', error);
    throw error;
  }
}

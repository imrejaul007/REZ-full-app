...
async function seedPromotionalPosters() {
  logger.info('Seeding promotional posters...\n');
  ...
  try {
    ...
    if (!adminUser) {
      logger.error('No user found to set as createdBy. Please seed users first.');
      process.exit(1);
    }
    ...
    for (let i = 0; i < PROMOTIONAL_POSTERS.length; i++) {
      ...
      if (existing) {
        logger.info(`  -> Poster "${poster.title}" already exists, skipping`);
        skipped++;
        continue;
      }
      ...
      logger.info(`  + Created poster: "${poster.title}" (Share bonus: Rs.${poster.shareBonus})`);
      created++;
    }
    ...
    logger.info(`   - Total active promotional posters in DB: ${totalPosters}`);
  } catch (error) {

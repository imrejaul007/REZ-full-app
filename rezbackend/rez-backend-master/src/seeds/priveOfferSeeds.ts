...
const priveOffers = [
  ...
  // Signature tier offers
  {
    ...
    terms: ['Valid with main course order only', 'One dessert per table', 'Dine-in only', 'Subject to availability'],
    howToRedeem: [
  ...
  },
  {
    ...
    terms: ['Valid for Fri-Sun shows only', 'One bonus per transaction', 'Book through CineMax app or website'],
    howToRedeem: ['Book your weekend movie ticket', 'Link your ReZ account', 'Bonus coins credited after movie'],
  ...
  },
  {
    ...
    terms: ['New FitZone members only', 'Valid ID required', 'One trial per person', 'Must be 18+ years old'],
    howToRedeem: [
  ...
  // Building tier offers
  {
    ...
    terms: ['First purchase only', 'Minimum spend of $10', 'One welcome bonus per account'],
    howToRedeem: ['Make your first purchase at any partner', 'Pay using ReZ Pay', 'Bonus coins credited automatically'],
    category: 'Rewards',
  ...
  },
  {
    ...
    terms: ['Valid at participating locations', 'Maximum discount of $5', 'Dine-in and takeaway'],
    howToRedeem: ['Order at a participating restaurant', 'Show your Privé QR code', 'Discount applied at checkout'],
    category: 'Dining',
  ...
  // Special Event Offer
  {
    ...
    terms: ['Valid at all partner stores', 'Triple coins on base earning', 'Limited time offer'],
    howToRedeem: ['Shop at any partner store', 'Pay with ReZ Pay', 'Triple coins credited automatically'],
    category: 'Events',
...
async function seedPriveOffers() {
  try {
    ...
    logger.info('Cleared existing Privé offers');

    // Insert new offers
    const result = await PriveOffer.insertMany(priveOffers);
    logger.info(`Successfully seeded ${result.length} Privé offers`);
    ...
    logger.info('Offers by tier:', tierCounts);

    // Log featured count
    const featuredCount = priveOffers.filter((o) => o.isFeatured).length;
    logger.info(`Featured offers: ${featuredCount}`);

    await mongoose.disconnect();
    logger.info('Disconnected from MongoDB');
    process.exit(0);
  } catch (error) {
    logger.error('Error seeding Privé offers:', error);
    process.exit(1);
  }
}

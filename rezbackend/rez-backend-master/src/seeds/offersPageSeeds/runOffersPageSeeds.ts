...
async function getStoreIds(): Promise<StoreMap> {
  ...
  logger.info('Found stores:', Object.keys(storeMap));
  return storeMap;
}
...
async function getOrCreateAdminUser(): Promise<mongoose.Types.ObjectId> {
  ...
  const ADMIN_EMAIL = process.env.ADMIN_EMAIL || 'admin@rez.com';
...
async function seedFlashSales(storeMap: StoreMap): Promise<mongoose.Types.ObjectId[]> {
  ...
  const flashSalesData = [
    {
      ...
      description:
        'Large Pizza + 2 Sides - Limited time offer! Get our best-selling pizza combo at an unbeatable price.',
    ...
    },
    {
      ...
      enabled: true,
      status: 'active' as const,
      termsAndConditions: ['Valid for delivery only', 'Max 2 per customer', 'While stocks last'],
      minimumPurchase: 0,
      maximumDiscount: 4,
      promoCode: 'BKFLASH',
      viewCount: 2100,
    ...
    },
    {
      ...
      termsAndConditions: ['Valid on all Grande drinks', 'In-store only', 'One per customer per visit'],
      minimumPurchase: 0,
      maximumDiscount: 2,
      promoCode: 'COFFEE33',
      viewCount: 3400,
  ...
  logger.info(`✅ Created ${flashSales.length} flash sales`);
...
async function seedCoupons(adminId: mongoose.Types.ObjectId, storeMap: StoreMap): Promise<void> {
  ...
  const couponsData = [
    ...
    },
    {
      ...
      autoApplyPriority: 0,
      status: 'active' as const,
      termsAndConditions: ['Valid on Burger Bonanza only', 'Max 2 uses per customer', 'Delivery orders only'],
      createdBy: adminId,
    ...
    },
    {
      ...
      autoApplyPriority: 0,
      status: 'active' as const,
      termsAndConditions: ['Valid on Grande drinks only', 'In-store redemption only', 'Max 3 uses per customer'],
      createdBy: adminId,
  ...
  logger.info(`✅ Created ${coupons.length} coupon codes`);
}
...
async function seedFriendRedemptions(
  ...
  logger.info(`✅ Created ${redemptions.length} friend redemptions`);
}

async function main() {
  logger.info('🚀 Starting Offers Page Seeds...\n');
  ...
  try {
    ...
    logger.info(`- Flash Sales: ${flashSaleCount}`);
    logger.info(`- Promo Coupons: ${couponCount}`);
    logger.info(`- Friend Redemptions: ${friendRedemptionCount}`);

...
class OrderModel {
    ...
    calculateItemPrice(menuItem, modifiers) {
    ...
    addItemToOrder(orderId, request) {
    ...
    processRefund(orderId, paymentId, amount) {
        const order = this.orders.get(orderId);
        if (!order)
            return null;
    ...
    calculateApplicableSubtotal(order, applicableItems) {
...
OrderModel.TAX_RATE = 0.08; // 8% tax rate
exports.orderModel = new OrderModel();

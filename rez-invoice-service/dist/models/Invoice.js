...
class InvoiceModel {
    ...
    numberToWords(num) {
        ...
        if (ten)
            result += twoDigits(ten);
        return result.trim();
    }
    calculateTaxes(items, placeOfSupply, businessGstin, customerGstin) {
    ...
    processLineItems(inputItems, placeOfSupply, businessGstin, customerGstin) {
        const isInterState = businessGstin?.substring(0, 2) !== customerGstin?.substring(0, 2);
    ...
    async create(input) {
        ...
        const items = this.processLineItems(input.items, placeOfSupply, businessGstin, customerGstin);
    ...
    async findByInvoiceNumber(invoiceNumber) {
        const row = this.db.prepare('SELECT data FROM invoices WHERE invoice_number = ?').get(invoiceNumber);
        if (!row)
            return null;
        return JSON.parse(row.data);
    }
    ...
    async getGSTSummary(fromDate, toDate) {
        const invoices = await this.list({ fromDate, toDate, status: 'paid' }, { limit: 100000 });
        let cgstCollected = 0;
        let sgstCollected = 0;
        let igstCollected = 0;
        let cessCollected = 0;
    ...
    async getOverdueInvoices() {
        const today = new Date().toISOString().split('T')[0];
        const invoices = await this.list({ toDate: today }, { limit: 1000, sortBy: 'dueDate', sortOrder: 'asc' });
        return invoices.data.filter(inv => inv.dueDate < today &&
            inv.amountDue > 0 &&
            inv.status !== 'paid' &&

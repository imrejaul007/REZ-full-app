...
exports.LoopOrchestrator = exports.ActionDecision = exports.LoopState = void 0;
...
(function (LoopState) {
    ...
    LoopState["INTENT_PROCESSING"] = "intent_processing";
    LoopState["ACTION_DECIDING"] = "action_deciding";
    LoopState["EXECUTING"] = "executing";
    LoopState["PENDING_APPROVAL"] = "pending_approval";
    LoopState["APPROVED"] = "approved";
    LoopState["REJECTED"] = "rejected";
    LoopState["COMPLETED"] = "completed";
    LoopState["FAILED"] = "failed";
})(LoopState || (exports.LoopState = LoopState = {}));
var ActionDecision;
(function (ActionDecision) {
    ActionDecision["AUTO_APPROVE"] = "auto_approve";
    ActionDecision["DRAFT_FOR_APPROVAL"] = "draft_for_approval";
    ActionDecision["SKIP"] = "skip";
    ActionDecision["MANUAL_REVIEW"] = "manual_review";
})(ActionDecision || (exports.ActionDecision = ActionDecision = {}));
// ============================================================================
// Service Clients
// ============================================================================
class IntentGraphClient {
    ...
    async processIntent(event, correlationId) {
...
class ActionEngineClient {
    ...
    async decideAction(intent, tenantId, correlationId) {
        // In production, this would be a gRPC call
...
class NextaBizClient {
    ...
    async createDraftOrder(intent, correlationId) {
...
class FeedbackServiceClient {
    ...
    async recordOutcome(context, outcome, merchantFeedback) {
...
class AdaptiveAgentClient {
    ...
    async updateModel(context, outcome) {
        // In production, this would be a gRPC call
...
class LoopOrchestrator {
    ...
    constructor(config) {
        ...
        this.feedbackService = new FeedbackServiceClient(config.feedbackServiceUrl, config.feedbackServiceToken);
     ...
     */
    async startLoop(event) {
        const loopId = `loop_${(0, uuid_1.v4)()}`;
     ...
     */
    async processIntent(context) {
        context.state = LoopState.INTENT_PROCESSING;
        this.config.onStateChange(context);
        const intent = await this.intentGraph.processIntent(context.event, context.correlationId);
    ...
    async decideAction(context) {
        ...
        this.config.onStateChange(context);
        const result = await this.actionEngine.decideAction(context.intent, context.tenantId, context.correlationId);
    ...
    async executeAction(context) {
        ...
        if (context.decision === ActionDecision.AUTO_APPROVE) {
            ...
            const result = await this.nextaBiz.createDraftOrder(context.intent, context.correlationId);
            if (!result.success) {
     ...
     */
    async completeWithOutcome(context, outcome, merchantFeedback) {
    ...
    async handleFailure(context, error) {
        ...
        this.metrics.loopsPendingApproval--;
        this.config.onError(context, error instanceof Error ? error : new Error(String(error)));
        this.activeLoops.delete(context.loopId);
    }
    /**
     * Complete a loop that was pending approval (called via webhook)
     */
    async completeLoop(loopId, outcome, merchantFeedback) {
...
exports.LoopOrchestrator = LoopOrchestrator;
// ============================================================================
// Factory Function
// ============================================================================
...
// ============================================================================
function createEventHandler(orchestrator) {
    return async (event) => {

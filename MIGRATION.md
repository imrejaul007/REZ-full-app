...
# Platform Security Migration v1.x to v2.0

## Overview

This section covers breaking security changes introduced in ReZ Platform v2.0.

## Breaking Changes

### 1. INTERNAL_SERVICE_TOKEN is now required

**Before:** Service would start with empty token (security risk)

**After:** Application throws error if `INTERNAL_SERVICE_TOKEN` is missing
...
### 2. WebSocket requires valid JWT token

**Before:** Anonymous WebSocket connections allowed

**After:** All WebSocket connections require valid JWT authentication

**Action Required:**
- Update client code to pass JWT token during connection
...
### 3. CORS requires explicit origin list

**Before:** Wildcard CORS `origin: '*'` allowed any origin

**After:** Only whitelisted origins are allowed
...
|----------|----------|-------------|
| `INTERNAL_SERVICE_TOKEN` | **Yes** | Internal service authentication token |
| `CORS_ORIGIN` | **Yes** | Comma-separated allowed origins (no wildcards) |
| `REDIS_URL` | Yes (WebSocket) | Required for multi-instance WebSocket scaling |
...
// Product collection
db.products.createIndex({ store: 1, 'inventory.isAvailable': 1 })

// TableSession collection
db.tablesessions.createIndex({ storeId: 1, openedAt: -1 })
```

**Action Required:** Run index creation on production database
...
3. Disable new security middleware temporarily
4. Report issues to development team

## Support

For migration assistance:
- GitHub Issues: https://github.com/imrejaul007/rez-issues
